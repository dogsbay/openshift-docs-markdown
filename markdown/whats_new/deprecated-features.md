...
# Deprecated features {id="deprecated-features"}
{% include "./_attributes/common-attributes.md" %}
{%- set context = "deprecated-features" %}

Large changes to the underlying architecture and installation process are
applied in {{ product_title }} v4, and many features from {{ product_title }} v3 are
now deprecated.

## Features deprecated in {{ product_title }} v4 {id="deprecated"}

**Features Deprecated in {{ product_version }}**
...
| `projectatomic/docker-1.13` additional search registries | CRI-O is the default container runtime for {{ product_title }} v4 on Fedora. |
...
| `oc adm ca` | Certificates are managed by Operators internally. |
| `oc adm create-api-client-config` .2+.^ | Functions are managed by Operators internally. |
| `oc adm create-bootstrap-policy-file` | `oc adm policy reconcile-sccs` |
| Functions are managed by `openshift-apiserver` internally. | Web console |