...
## About {{ image_mode_os_lower }} {id="coreos-layering-about_{{ context }}"}
...
:   With {{ image_mode_os_out_lower }}, you create a Containerfile that references an {{ product_title }} image and the RPM that you want to apply, build the layered image in your own environment, and push the image to your repository. Then, in your cluster, create a `MachineConfig` object for the targeted node pool that points to the new image. The Machine Config Operator overrides the base {{ op_system }} image, as specified by the `osImageURL` value in the associated machine config, and boots the new image.


During {{ product_title }} installation
:   You can apply a pre-built custom layered image to specific nodes during {{ product_title }} installation. 
...
## Example Containerfiles {id="coreos-layering-examples_{{ context }}"}

{{ image_mode_os_caps }} allows you to use the following types of images to create custom layered images:

*   **{{ product_title }} Hotfixes**. You can work with Customer Experience and Engagement (CEE) to obtain and apply Hotfix packages on top of your {{ op_system }} image. In some instances, you might want a bug fix or enhancement before it is included in an official {{ product_title }} release. {{ image_mode_os_caps }} allows you to easily add the Hotfix before it is officially released and remove the Hotfix when the underlying {{ op_system }} image incorporates the fix.

    :::important

    Some Hotfixes require a Red Hat Support Exception and are outside of the normal scope of {{ product_title }} support coverage or life cycle policies.
...
1.  Performs cordon and drain operations on the nodes in the pool or pools.
1.  Writes the rest of the machine config parameters onto the nodes.
1.  Applies the custom layered image to the node.
1.  Reboots the node using the new image.
...
{% leveloffset +1 %}{% include "./modules/coreos-layering-configuring-on.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-configuring-on-proc.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-configuring-on-modifying.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-configuring-on-rebuild.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-configuring-on-revert.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-configuring-on-remove.md" %}{% endleveloffset %}

{% leveloffset +1 %}{% include "./modules/coreos-layering-configuring.md" %}{% endleveloffset %}

{% leveloffset +2 %}{% include "./modules/coreos-layering-removing.md" %}{% endleveloffset %}

{% leveloffset +1 %}{% include "./modules/coreos-layering-install-time.md" %}{% endleveloffset %}
...
*   [Pausing the machine config pools](/updating/updating_a_cluster/update-using-custom-machine-config-pools#update-using-custom-machine-config-pools-pause_update-using-custom-machine-config-pools)
*   [Rebuilding an on-cluster custom layered image](/machine_configuration/mco-coreos-layering#coreos-layering-configuring-on-rebuild_mco-coreos-layering)
*   [Reverting an on-cluster custom layered image](/machine_configuration/mco-coreos-layering#coreos-layering-configuring-on-revert_mco-coreos-layering)
*   [Modifying a custom layered image](/machine_configuration/mco-coreos-layering#coreos-layering-configuring-on-modifying_mco-coreos-layering)
...
*   [Updating with a {{ op_system }} custom layered image](/machine_configuration/mco-coreos-layering#coreos-layering-updating_mco-coreos-layering)
