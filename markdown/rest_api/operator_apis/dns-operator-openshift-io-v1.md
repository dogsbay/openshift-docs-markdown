...
# DNS [operator.openshift.io/v1] {id="dns-operator-openshift-io-v1"}
{%- set toc = "macro" -%}
{%- set toc_title = true %}
...
:   caBundle references a ConfigMap that must contain either a single
    ...
    1.  The value must be a PEM encoded CA certificate or CA bundle.
    1.  The administrator must create this configmap in the openshift-config namespace.
    1.  The upstream server certificate must contain a Subject Alternative Name (SAN) that matches ServerName.
...
### /apis/operator.openshift.io/v1/dnses {id="_apisoperatoropenshiftiov1dnses"}
...
### /apis/operator.openshift.io/v1/dnses/{{ name }} {id="_apisoperatoropenshiftiov1dnses_name"}
...
### /apis/operator.openshift.io/v1/dnses/{{ name }}/status {id="_apisoperatoropenshiftiov1dnses_name_status"}
