...
To install the {{ aws_first }} Load Balancer Operator on a cluster that uses the {{ sts_first }}, prepare the cluster by configuring the `CredentialsRequest` object. This ensures the Operator can bootstrap the {{ aws_short }} Load Balancer Controller and access the required secrets. {._abstract}
...
## Additional resources {id="additional-resources-albo-sts-cluster_{{ context }}" ._additional-resources}
