...
## Forwarding and ingestion protocol {id="forwarding-and-ingestion-protocol_{{ context }}"}

Red Hat OpenShift {{ logging_uc }} collects and forwards logs to OpenTelemetry endpoints using [OTLP Specification](https://opentelemetry.io/docs/specs/otlp/). OTLP encodes, transports, and delivers telemetry data. You can also deploy Loki storage, which provides an OTLP endpont to ingest log streams. This document defines the semantic conventions for the logs collected from various OpenShift cluster sources.

## Semantic conventions {id="semantic-conventions_{{ context }}"}
...
### Log entry structure {id="log-entry-structure_{{ context }}"}
...
### Attributes {id="attributes_{{ context }}"}
...
*   `stream label`:
    ...
    *   Can be labeled as `required` if the {{ loki_op }} enforces this attribute in the configuration.
*   `structured metadata`:
