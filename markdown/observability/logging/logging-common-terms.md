...
# Glossary {id="openshift-logging-common-terms"}
{%- set context = "openshift-logging-common-terms" %}
...
Event router
:   The event router is a pod that watches {{ product_title }} events. It collects logs by using the {{ logging }}.


Fluentd
:   Fluentd is a log collector that resides on each {{ product_title }} node. It gathers application, infrastructure, and audit logs and forwards them to different outputs.
...
Elasticsearch
:   Elasticsearch is a distributed search and analytics engine. {{ product_title }} uses Elasticsearch as a default log store for the {{ logging }}.


{{ es_op }}
:   The {{ es_op }} is used to run an Elasticsearch cluster on {{ product_title }}. The {{ es_op }} provides self-service for the Elasticsearch cluster operations and is used by the {{ logging }}.
...
Operators
:   Operators are the preferred method of packaging, deploying, and managing a Kubernetes application in an {{ product_title }} cluster. An Operator takes human operational knowledge and encodes it into software that is packaged and shared with customers.
