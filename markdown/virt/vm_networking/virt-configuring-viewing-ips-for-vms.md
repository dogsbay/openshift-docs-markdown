...
You can configure an IP address when you create a virtual machine (VM). The IP address is provisioned with cloud-init. View the IP address of a VM by using the {{ product_title }} web console or the command line. The network information is collected by the QEMU guest agent.
