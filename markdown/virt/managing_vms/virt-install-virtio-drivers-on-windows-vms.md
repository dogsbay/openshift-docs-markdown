...
VirtIO drivers are paravirtualized device drivers required for Microsoft Windows virtual machines (VMs) to run in {{ VirtProductName }}. The drivers are shipped with the rest of the images. You can also download the `virtio-win` ISO file from the web console to install or update drivers on Windows VMs. {._abstract}
...
*   [Update VirtIO drivers](/virt/managing_vms/virt-update-virtio-drivers#virt-update-virtio-drivers)