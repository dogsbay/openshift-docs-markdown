...
Define probes and watchdogs in the `VirtualMachine` resource to configure virtual machine (VM) health checks. Health checks monitor and report the internal state of a VM. {._abstract}
...
{%- if not (openshift_rosa or openshift_dedicated or openshift_rosa_hcp) %}
## Additional resources {id="additional-resources_monitoring-vm-health" ._additional-resources}
