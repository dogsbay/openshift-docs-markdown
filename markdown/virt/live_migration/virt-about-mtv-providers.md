...
To migrate a virtual machine (VM) across {{ product_title }} clusters, you must configure an {{ product_title }} provider for each cluster that you are including in the migration. If {{ mtv_short }} is already installed on a cluster, a local provider already exists.
