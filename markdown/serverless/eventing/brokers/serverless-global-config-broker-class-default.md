...
# Configuring the default broker class {id="serverless-global-config-broker-class-default"}
{%- set context = "serverless-global-config-broker-class-default" %}
...
*   Modify the `KnativeEventing` custom resource to add configuration details for the `config-br-defaults` config map:
    ...
    1.  The `config-br-defaults` config map specifies the default settings for any broker that does not specify `spec.config` settings or a broker class.
    1.  The cluster-wide default broker class configuration. In this example, the default broker class implementation for the cluster is `Kafka`.
    1.  The `kafka-broker-config` config map specifies default settings for the Kafka broker. See "Configuring Knative broker for Apache Kafka settings" in the "Additional resources" section.
    1.  The namespace where the `kafka-broker-config` config map exists.
    1.  The namespace-scoped default broker class configuration. In this example, the default broker class implementation for the `my-namespace` namespace is `MTChannelBasedBroker`. You can specify default broker class implementations for multiple namespaces.
