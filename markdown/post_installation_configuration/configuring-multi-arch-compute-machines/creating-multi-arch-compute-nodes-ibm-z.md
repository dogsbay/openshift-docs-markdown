...
{%- set context = "creating-multi-arch-compute-nodes-ibm-z" %}
{% include "./_attributes/common-attributes.md" %}
# Creating a cluster with multi-architecture compute machines on {{ ibm_z_title }} and {{ ibm_linuxone_title }} with z/VM {id="creating-multi-arch-compute-nodes-ibm-z"}

To create a cluster with multi-architecture compute machines on {{ ibm_z_name }} and {{ ibm_linuxone_name }} (`s390x`) with z/VM, you must have an existing single-architecture `x86_64` cluster. You can then add `s390x` compute machines to your {{ product_title }} cluster. {._abstract}
