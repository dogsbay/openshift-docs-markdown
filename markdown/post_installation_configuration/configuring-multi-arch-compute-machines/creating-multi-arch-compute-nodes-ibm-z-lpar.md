...
{%- set context = "creating-multi-arch-compute-nodes-ibm-z-lpar" %}
{% include "./_attributes/common-attributes.md" %}
# Creating a cluster with multi-architecture compute machines on {{ ibm_z_title }} and {{ ibm_linuxone_title }} in an LPAR {id="creating-multi-arch-compute-nodes-ibm-z-lpar"}

To create a cluster with multi-architecture compute machines on {{ ibm_z_name }} and {{ ibm_linuxone_name }} (`s390x`) in a logical partition (LPAR), you must have an existing single-architecture `x86_64` cluster. You can then add `s390x` compute machines to your {{ product_title }} cluster. {._abstract}
...
{% leveloffset +1 %}{% include "./modules/machine-user-infra-machines-ibm-z.md" %}{% endleveloffset %}
