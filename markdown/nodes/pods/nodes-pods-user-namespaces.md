...
Not all file systems currently support ID-mapped mounts, such as Network File Systems (NFS) and other network/distributed file systems. Any pod that is using an NFS-backed persistent volume from a vendor that does not support ID-mapped mounts might experience access or permission issues when running in a user namespace. This behavior is not specific to {{ product_title }}. It applies to all Kubernetes distributions from Kubernetes v1.33 onward.

:::


To check user namespaces support for storage options, see [CSI drivers supported by {{ product_title }}](/storage/container_storage_interface/persistent-storage-csi#csi-drivers-supported_persistent-storage-csi).
