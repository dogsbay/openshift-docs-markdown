...
# Scaling policies {id="nodes-pods-autoscaling-policies_{{ context }}"}

To control how the {{ product_title }} horizontal pod autoscaler (HPA) scales pods you can define a _scaling policy_ to restrict the rate at which scaling occurs and a _stabilization window_ to restrict the fluctuation of replicas count when the metrics used for scaling keep fluctuating. {._abstract}
