...
# Configuring CPU and memory requests for the log store {id="cluster-logging-logstore-limits_{{ context }}"}
...
1.  Edit the `ClusterLogging` custom resource (CR) in the `openshift-logging` project:
    ...
    1.  The maximum amount of resources a pod can use.
    1.  The minimum resources required to schedule a pod.
    1.  Specify the CPU and memory requests for the Elasticsearch proxy as needed. If you leave these values blank, the OpenShift Elasticsearch Operator sets default values that are sufficient for most deployments. The default values are `256Mi` for the memory request and `100m` for the CPU request.
