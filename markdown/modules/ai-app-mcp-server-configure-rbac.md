...
# Configure RBAC enforcement {id="ai-app-mcp-server-configure-rbac_{{ context }}"}

To control which cluster resources the Model Context Protocol (MCP) server can access, configure role-based access control (RBAC) so that AI agents are limited to only the resources they need. {._abstract}
    ...
    extraRoleBindings:
      - name: acm-cluster-proxy-discovery
        ...
        roleRef:
          name: acm-cluster-proxy-discovery
    ```
1.  Apply the RBAC configuration by saving the preceding example to a file (for example, `rbac-values.yaml`), and then run the following command:
    ```terminal
