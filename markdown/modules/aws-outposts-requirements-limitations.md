...
This requirement is a workaround in {{ product_title }} version {{ product_version }}.
For more information, see [OCPBUGS-30041](https://issues.redhat.com/browse/OCPBUGS-30041).
*   {{ product_title }} clusters on AWS include the `gp3-csi` and `gp2-csi` storage classes.
These classes correspond to Amazon Elastic Block Store (EBS) gp3 and gp2 volumes.
{{ product_title }} clusters use the `gp3-csi` storage class by default, but AWS Outposts does not support EBS gp3 volumes.
