...
# Example: Multiple interfaces in the same node network configuration policy {id="virt-example-nmstate-multiple-interfaces_{{ context }}"}

You can create multiple interfaces in the same node network configuration policy. These interfaces can reference each other, allowing you to build and deploy a network configuration by using a single policy manifest. {._abstract}
