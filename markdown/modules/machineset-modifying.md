...
By default, the {{ product_title }} router pods are deployed on compute machines.
...
tag::CAPI[]
* Your {{ product_title }} cluster uses the Cluster API.
end::CAPI[]
tag::MAPI[]
* Your {{ product_title }} cluster uses the Machine API.
end::MAPI[]
* You are logged in to the cluster as an administrator by using the {{ oc_first }}.
