...
# Creating a Java application by using a devfile in a disconnected cluster {id="developer-cli-odo-creating-a-java-application-by-using-a-devfile-in-a-disconnected-cluster_{{ context }}"}
...
1.  Push the changes to the cluster:
    ...
    ```
1.  Display the logs to verify that the application has started:
    ```terminal
    $ odo log
    ```
    ```terminal title="Example output"
    ...
    [...]
    ```
1.  Create storage for your application:
    ```terminal
