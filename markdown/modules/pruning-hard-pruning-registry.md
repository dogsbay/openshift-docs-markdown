...
# Hard pruning the registry {id="pruning-hard-pruning-registry_{{ context }}"}

Hard prune the {{ product_registry }} to remove orphaned image blobs that are not referenced in etcd and reclaim registry storage space when standard image pruning is insufficient. {._abstract}

The {{ product_registry }} can accumulate blobs that are not referenced by
the {{ product_title }} cluster’s etcd. The basic pruning images procedure,
...
*   {{ product_title }} refusing an image because of quota restrictions.
...
you are running out of storage space in your {{ product_registry }} and
...
1.  **Add the `system:image-pruner` role.**
    ...
    1.  Get the service account name:
        ...
        ```
    1.  Add the `system:image-pruner` cluster role to the service account:
        ```terminal
