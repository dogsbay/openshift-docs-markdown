...
You can also configure worker latency profiles upon installing an {{ product_title }} cluster.
...
1.  Move to the medium worker latency profile:
    1.  Edit the `node.config` object:
        ...
        ```
    1.  Add `spec.workerLatencyProfile: MediumUpdateAverageReaction`:
...
1.  Optional: Move to the low worker latency profile:
    1.  Edit the `node.config` object:
        ...
        $ oc edit nodes.config/cluster
        ```
    1.  Change the `spec.workerLatencyProfile` value to `LowUpdateSlowReaction`:
        ```yaml title="Example node.config object"
        apiVersion: config.openshift.io/v1
        kind: Node
        metadata:
