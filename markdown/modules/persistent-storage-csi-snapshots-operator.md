...
# About the CSI Snapshot Controller Operator {id="persistent-storage-csi-snapshots-operator_{{ context }}"}

You can manage volume snapshots in {{ product_title }} by using the CSI Snapshot Controller Operator’s custom resource definitions (CRDs) for snapshot requests, storage, and configuration. {._abstract}
...
## Volume snapshot CRDs {id="_volume_snapshot_crds"}
During {{ product_title }} installation, the CSI Snapshot Controller Operator creates the following snapshot custom resource definitions (CRDs) in the `snapshot.storage.k8s.io/v1` API group:
