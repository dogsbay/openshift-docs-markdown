...
# Volume snapshot class csi-gce-pd-vsc-images {id="persistent-storage-csi-gcp-images-snapshot-class-overview_{{ context }}"}

By default, you cannot restore more than six volumes per snapshot per hour. So in Kubevirt environments, you normally cannot create more than six VMs per hour from a "golden image" (templates saved as snapshots). {._abstract}
