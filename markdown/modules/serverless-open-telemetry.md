...
# Using {{ DTProductName }} to enable distributed tracing {id="serverless-open-telemetry_{{ context }}"}
...
*   You have installed {{ DTProductName }} by following the {{ product_title }} "Installing distributed tracing" documentation.
...
1.  Create an `OpenTelemetryCollector` custom resource (CR):
    ...
    ```
1.  Verify that you have two pods running in the namespace where {{ DTProductName }} is installed:
    ```terminal
    ...
    ```
1.  Verify that the following headless services have been created:
    ```terminal
...
1.  Install the {{ ServerlessOperatorName }} by following the "Installing the {{ ServerlessOperatorName }}" documentation.
1.  Install Knative Serving by creating the following `KnativeServing` CR:
...
1.  Install Knative Eventing by creating the following `KnativeEventing` CR:
    ```yaml title="Example KnativeEventing CR"
    apiVersion: operator.knative.dev/v1beta1
        ...
        spec:
          ...
          - image: quay.io/openshift-knative/helloworld:v1.2
            ...
            - name: TARGET
              value: "Go Sample v1"
    ```
1.  Make some requests to the service:
    ```terminal title="Example HTTPS request"
    $ curl https://helloworld-go.example.com
    ```
1.  Get the URL for the Jaeger web console:
