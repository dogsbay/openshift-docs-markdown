...
# Top 3 containers with the most OOM kills in the last day {id="nodes-dashboard-using-identify-critical-top3_{{ context }}"}

You can review details regarding the top three containers that have experienced the most Out-Of-Memory (OOM) kills in the previous day by using the **Top 3 containers with the most OOM kills in the last day** query in the node metrics dashboard. {._abstract}
