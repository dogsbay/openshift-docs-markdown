...
as passwords, {{ product_title }} client configuration files,
...
`stringData.hostname`
:   Specifies the value associated with keys in the `data` map. The value associated with keys in the `stringData` map is made up of
    plain text strings.
...
## Types of secrets {id="nodes-pods-secrets-about-types_{{ context }}"}
...
## Secret data keys {id="nodes-pods-secrets-about-keys_{{ context }}"}
