...
# Installing the ovnkube-trace on local host {id="nw-ovn-kubernetes-install-ovnkube-trace-local_{{ context }}"}

To run `ovnkube-trace` from your local host against a cluster, you can copy the binary from an `ovnkube-control-plane` pod and use familiar Kubernetes namespace and pod arguments. {._abstract}

The `ovnkube-trace` tool traces packet simulations for arbitrary UDP or TCP traffic between points in an OVN-Kubernetes driven {{ product_title }} cluster.
...
1.  Run the following command on your local host to copy the binary from the `ovnkube-control-plane` pods:
    ...
    If you are using {{ op_system_base_full }} 8 to run the `ovnkube-trace` tool, you must copy the file `/usr/lib/rhel8/ovnkube-trace` to your local host.
