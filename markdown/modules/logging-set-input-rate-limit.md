...
# Configuring log forwarder input rate limits {id="logging-set-input-rate-limit_{{ context }}"}
...
1.  Add a `maxRecordsPerSecond` limit value to the `ClusterLogForwarder` CR for a specified input.
    ...
    ```yaml title="Example ClusterLogForwarder CR that sets a per-container limit for containers with certain labels"
