...
# Build the bootc image {id="microshift-install-bootc-build-image_{{ context }}"}

Build your {{ op_system_base_full }} that contains {{ microshift_short }} as a bootable container image by using a Containerfile.  {._abstract}

**Prerequisites**

*   A {{ op_system_base }} {{ op_system_version }} host with an active Red&#160;Hat subscription for building {{ microshift_short }} bootc images and running containers.
*   You logged into the {{ op_system_base }} {{ op_system_version }} host by using the user credentials that have `sudo` permissions.
...
1.  Create a Containerfile that includes the following instructions:
    ...
    RUN dnf config-manager \
            --set-enabled rhocp-${{ USHIFT_VER }}-for-rhel-{{ op_system_version_major }}-$(uname -m)-rpms \
            --set-enabled fast-datapath-for-rhel-{{ op_system_version_major }}-$(uname -m)-rpms
...
1.  Set the `PULL_SECRET` environment variable:
    ...
    ```
1.  Configure the `USER_PASSWD` environment variable:
    ```terminal
...
1.  Verify that the local {{ microshift_short }} bootc image was created by running the following command:
    ```terminal
