...
# Quorum protection with machine lifecycle hooks {id="machine-lifecycle-hook-deletion-etcd_{{ context }}"}

To protect etcd quorum on {{ product_title }} clusters that use the Machine API Operator, the etcd Operator uses lifecycle hooks for the machine deletion phase to implement a quorum protection mechanism. {._abstract}
...
## Control plane deletion with quorum protection processing order {id="machine-lifecycle-hook-deletion-etcd-order_{{ context }}"}
...
1.  To satisfy the `preDrain` lifecycle hook, the etcd Operator takes the following actions:
    ...
    1.  When the new etcd member receives the full database update, the etcd Operator promotes the new etcd member to a voting member and removes the old etcd member from the cluster.
