...
# Restoring a hosted cluster on {{ VirtProductName }} {id="restore-hosted-cluster-virt_{{ context }}"}

After you back up a hosted cluster on {{ VirtProductName }}, you can restore the backup. {._abstract}
...
1.  Delete the following objects from the management cluster:
    *   `HostedCluster`
    *   `NodePool`
    *   PVCs
1.  Create a restoration manifest YAML file that is similar to the following example:
    ```yaml
...
1.  Monitor the restore process by checking the restore status field and the Velero logs.
