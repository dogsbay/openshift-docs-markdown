...
# Operator installation and upgrade workflow in OLM {id="olm-upgrades_{{ context }}"}
...
![olm-catalogsource](/_assets/images/olm-catalogsource.png)

Within a catalog source, Operators are organized into _packages_ and streams of updates called _channels_, which should be a familiar update pattern from {{ product_title }} or other software on a continuous release cycle like web browsers.

**Figure 2. Packages and channels in a Catalog source**

![olm-channels](/_assets/images/olm-channels.png)
...
![olm-replaces](/_assets/images/olm-replaces.png)
...
## Example upgrade path {id="olm-upgrades-example-upgrade-path_{{ context }}"}
...
## Skipping upgrades {id="olm-upgrades-skipping_{{ context }}"}
...
![olm-skipping-updates](/_assets/images/olm-skipping-updates.png)
...
## Replacing multiple Operators {id="olm-upgrades-replacing-multiple_{{ context }}"}
...
1.  The next Operator that replaces the current one, in the source specified by `sourceName`.
1.  Channel head in another source that is visible to the subscription, if the other criteria for skipping are met.
1.  The next Operator that replaces the current one in any source visible to the
subscription.
...
## Z-stream support {id="olm-upgrades-z-stream_{{ context }}"}
...
![olm-z-stream](/_assets/images/olm-z-stream.png)
