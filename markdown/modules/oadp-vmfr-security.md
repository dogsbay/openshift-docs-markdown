...
# {{ oadp_short }} VMFR security considerations {id="oadp-vmfr-security_{{ context }}"}

Review the security considerations for {{ oadp_short }} virtual machine file restore (VMFR) to understand how access control, data protection, and multi-tenancy are managed. This helps you to plan secure file-level restore operations. {._abstract}


Access control
:   VMFR enforces the following access control policies:
    *   File-serving resources are created in a temporary namespace that is separate from the {{ oadp_short }} namespace. This namespace is owned by the `VirtualMachineFileRestore` CR and is cleaned up automatically when you delete the CR.
    ...
    *   Cluster administrators require create, read, update, and delete access to `VirtualMachineBackupsDiscovery` and `VirtualMachineFileRestore` resources in the {{ oadp_short }} namespace.
