...
# Recommended cluster kernel configuration {id="ztp-recommended-cluster-kernel-config_{{ context }}"}

Always use the latest supported real-time kernel version in your cluster. Ensure that you apply the following configurations in the cluster: {._abstract}
...
1.  Ensure that the following `additionalKernelArgs` are set in the cluster performance profile:
    ...
    spec:
      ...
      # ...
    ```
1.  Optional: Set the CPU frequency under the `hardwareTuning` field:
    ...
    ```
1.  Ensure that the `performance-patch` profile in the `Tuned` CR configures the correct CPU isolation set that matches the `isolated` CPU set in the related `PerformanceProfile` CR, for example:
    ```yaml
