...
1.  Set the `kubeconfig` file to access the hosted cluster by running the following command:
    ...
    ```
1.  Get the name of the `NodePool` resource associated to your hosted cluster by running the following command:
    ```terminal
    $ oc get nodepool --namespace <hosted_cluster_namespace>
    ```
1.  Optional: To prevent the pods from draining, add the `nodeDrainTimeout` field in the `NodePool` resource by running the following command:
    ```terminal
...
1.  Scale down the `NodePool` resource associated to your hosted cluster by running the following command:
    ```terminal
...
1.  Optional: Scale up the `NodePool` resource associated to your hosted cluster by running the following command:
    ```terminal
