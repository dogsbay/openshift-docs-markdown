...
# Create the DPUFlavor custom resource {id="nw-dpf-creating-dpuflavor_{{ context }}"}
...
The `DPUFlavor` also supports an optional `configFiles` field for custom DPU configuration files. {._abstract}
...
1.  Create a file named `dpuflavor.yaml` with the content for your MTU configuration.
