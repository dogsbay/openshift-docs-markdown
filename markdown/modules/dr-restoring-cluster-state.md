...
# Restoring to an earlier cluster state for more than one node {id="dr-scenario-2-restoring-cluster-state_{{ context }}"}

To restore your {{ product_title }} cluster with more than one control plane node to an earlier state, use a saved etcd snapshot after quorum loss or critical data deletion. {._abstract}
...
If your cluster uses a control plane machine set, see "Recovering a degraded etcd Operator" in the control plane machine set troubleshooting topic for an etcd recovery procedure. For {{ product_title }} on a single node, follow the procedure to restore to an earlier cluster state for a single node.
...
1.  Use SSH to connect to the recovery host. Restore the cluster from an earlier backup by running the following command:
    ```terminal
    $ sudo -E /usr/local/bin/cluster-restore.sh /home/core/<etcd-backup-directory>
    ```
1.  Exit the SSH session.
1.  When the API responds, turn off the etcd Operator quorum guard by running the following command:
    ```terminal
...
1.  Enable the quorum guard by running the following command:
    ```terminal
