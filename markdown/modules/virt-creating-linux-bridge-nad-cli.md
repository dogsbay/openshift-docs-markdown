...
# Creating a Linux bridge NAD by using the CLI {id="virt-creating-linux-bridge-nad-cli_{{ context }}"}

You can create a network attachment definition (NAD) to provide layer-2 networking to pods and virtual machines (VMs) by using the command line. {._abstract}
...
1.  Add the VM to the `NetworkAttachmentDefinition` configuration, as in the following example:
    ...
    OSA interfaces on {{ ibm_z_name }} do not support VLAN filtering and VLAN-tagged traffic is dropped. Avoid using VLAN-tagged NADs with OSA interfaces.
...
1.  Optional: If you want to connect a VM to the native network, configure the Linux bridge `NetworkAttachmentDefinition` manifest without specifying any VLAN:
    ```yaml
    apiVersion: "k8s.cni.cncf.io/v1"
    kind: NetworkAttachmentDefinition
    metadata:
