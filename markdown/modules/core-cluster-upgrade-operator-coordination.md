...
# Coordinating OLM Operator updates with cluster updates {id="core-cluster-upgrade-operator-coordination_{{ context }}"}

You can coordinate Operator Lifecycle Manager (OLM) Operator updates with {{ product_title }} cluster updates to ensure Operator compatibility.
Incompatible Operators can block cluster updates or cause degraded cluster Operators after updates complete. {._abstract}
...
1.  Check installed Operator versions and subscription channels by running the following commands:
    ...
    ```
1.  Identify Operators that need channel updates before the cluster update:

    Review Operator compatibility in the {{ product_title }} release notes and identify which Operator channels support your target {{ product_title }} version.
    For a cluster updating from 4.20 to 4.21, identify Operators still on 4.20 channels.
    Refer to the [Red&#160;Hat Catalog](https://catalog.redhat.com/en) for compatibility details for each Operator.
1.  Create an OLM Operator update policy.

    In environments where Operator updates are set to manual approval, use a {{ cgu_operator }}-managed policy to coordinate Operator updates. When {{ cgu_operator }} enforces the policy, it automatically approves the Operator `InstallPlan`:
    ```yaml
    ...
    *   `status.state: AtLatestKnown`: Ensures the Operator update completes before {{ cgu_operator }} proceeds to the next step.
...
1.  Monitor Operator updates and verify the new versions by running the following commands:
    ```terminal
    ...
    Succeeded
    ```
1.  After the control plane update completes, check for pending Operator updates by running the following command:
    ```terminal
    $ oc get csv -A
    ```

    Some Operators might not update until after the cluster update completes.
1.  If Operators are configured with manual approval, list and approve pending install plans by running the following commands:
    ```terminal
    ...
    ```
1.  Verify Operator health after cluster update by running the following command:
    ```terminal
    $ oc get csv -A
    ```

    All Operators must show `Succeeded` phase.
1.  Verify CNF-specific Operator pods and custom resources by running the following commands:
    ```terminal
