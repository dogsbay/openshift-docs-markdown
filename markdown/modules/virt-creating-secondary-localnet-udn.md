...
# Creating a user-defined-network for localnet topology by using the CLI {id="virt-creating-secondary-localnet-udn_{{ context }}"}

You can create a secondary cluster-scoped user-defined-network (CUDN) for the localnet network topology by using the CLI. {._abstract}
...
1.  Create a `NodeNetworkConfigurationPolicy` object to map the OVN-Kubernetes secondary network to an Open vSwitch (OVS) bridge.
    ...
    *   `spec.desiredState.ovn.bridge-mappings.state` specifies the state of the mapping. Must be either `present` to add the mapping or `absent` to remove the mapping. The default value is `present`.
        ...
        {{ VirtProductName }} does not support Linux bridge bonding modes 0, 5, and 6. For more information, see [Which bonding modes work when used with a bridge that virtual machine guests or containers connect to?](https://access.redhat.com/solutions/67546).
        
        :::

1.  Apply the `NodeNetworkConfigurationPolicy` manifest by running the following command:
    ```terminal
...
1.  Create a `ClusterUserDefinedNetwork` object to create a localnet secondary network.
...
1.  Apply the `ClusterUserDefinedNetwork` manifest by running the following command:
    ```terminal
    $ oc apply -f <filename>.yaml
    ```
