# Workspaces {id="about-workspaces_{{ context }}"}


:::note

It is recommended that you use workspaces instead of the `PipelineResource` CRs in {{ pipelines_title }}, as `PipelineResource` CRs are difficult to debug, limited in scope, and make tasks less reusable.
...
1.  Definition of tasks used in the pipeline. This snippet defines two tasks, `build-image` and `apply-manifests`, which share a common workspace.
1.  List of workspaces used in the `build-image` task. A task definition can include as many workspaces as it requires. However, it is recommended that a task uses at most one writable workspace.
1.  Name that uniquely identifies the workspace used in the task. This task uses one workspace named `source`.
1.  Name of the pipeline workspace used by the task. Note that the workspace `source` in turn uses the pipeline workspace named `shared-workspace`.
1.  List of workspaces used in the `apply-manifests` task. Note that this task shares the `source` workspace with the `build-image` task.
...
1.  The name of the workspace in the pipeline for which the volume is being provided.
1.  Specifies a volume claim template that creates a persistent volume claim to define the storage volume for the workspace.