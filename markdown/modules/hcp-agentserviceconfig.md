...
# Deploying AgentServiceConfig resources {id="hcp-agentserviceconfig_{{ context }}"}

The `AgentServiceConfig` custom resource (CR) is an essential component of the Assisted Service add-on that is part of {{ mce_short }}. The CR is responsible for bare-metal cluster deployment. When the add-on is enabled, you deploy the `AgentServiceConfig` resource to configure the add-on. {._abstract}

In addition to configuring the `AgentServiceConfig` resource, you need to include additional config maps to ensure that {{ mce_short }} functions properly in a disconnected environment.
...
1.  Configure the Assisted Service by adding the `AssistedServiceConfig` object, as shown in the following example:
    ```yaml
    ...
    *   The `cpuArchitecture` field is added for every {{ product_title }} release that you want to deploy. In this example, `cpuArchitecture` subsections are included for 4.14 and 4.15.
...
1.  Deploy all of the objects by concatenating them into a single file and applying them to the management cluster. To do so, enter the following command:
    ```terminal
    ...
    ```
    *   The `assisted-image-service` pod is responsible for creating the {{ op_system_first }} boot image template, which is customized for each cluster that you deploy.
