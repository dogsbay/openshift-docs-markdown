...
# Sample route advertisements setup {id="nw-route-advertisements-setup_{{ context }}"}

You can implement Border Gateway Protocol (BGP) routing by using a sample configuration to set up route advertisements for your cluster. This configuration demonstrates how to configure route reflection on bare-metal infrastructure to share pod and egress IP routes. {._abstract}
...
1.  Confirm that the `RouteAdvertisements` feature gate is enabled by running the following command:
    ...
    ```
1.  Configure the Cluster Network Operator (CNO) by running the following command:
    ```terminal
...
1.  Get the IP addresses of the nodes by running the following command:
    ```terminal
    $ oc get node -owide
    ```
    ...
    ```
1.  Get the default pod network of each node by running the following command:
    ```terminal
    ...
    ```
1.  On the bare-metal hypervisor, get the IP address for the external FRR container to use by running the following command:
    ```terminal
    $ ip -j -d route get <a cluster node's IP> | jq -r '.[] | .dev' | xargs ip -d -j address show | jq -r '.[] | .addr_info[0].local'
    ```
1.  Create a `frr.conf` file for FRR that includes each node’s IP address, as shown in the following example:
    ...
    router bgp 64512
     ...
     exit-address-family
    ```
1.  Create a file named `daemons` that includes the following content:
    ...
    ```
1.  Save both the `frr.conf` and `daemons` files in the same directory, such as `/tmp/frr`.
1.  Create an external FRR container by running the following command:
    ```terminal
    $ sudo podman run -d --privileged --network host --rm --ulimit core=-1 --name frr --volume /tmp/frr:/etc/frr quay.io/frrouting/frr:9.1.0
    ```
1.  Create the following `FRRConfiguration` and `RouteAdvertisements` configurations:
              ...
              - address: 192.168.111.1
                ...
                toReceive:
                  allowed:
                    mode: all
        ```
    1.  Create a `ra.yaml` file that includes the following content:
        ...
        spec:
          ...
          - "EgressIP"
        ```
1.  Apply the `receive_all.yaml` and `ra.yaml` files by running the following command:
    ```terminal
...
1.  Verify that the configurations were applied:
    1.  Verify that the `FRRConfiguration` configurations were created by running the following command:
        ...
        ```
    1.  Verify that the `RouteAdvertisements` configurations were created by running the following command:
        ```terminal
        $ oc get ra -A
        ```
        ```text title="Example output"
        ...
        default   Accepted
        ```
1.  Get the external FRR container ID by running the following command:
    ```terminal
    ...
    ```
1.  Use the container ID that you obtained in the previous step to check the BGP neighbor and routes in the external FRR container’s `vtysh` session. Run the following command:
    ```terminal
    ...
    ```
1.  Find the `frr-k8s` pod for each cluster node by running the following command:
    ```terminal
    ...
    ```
1.  From the {{ product_title }} cluster, check BGP routes on the cluster node’s `frr-k8s` pod in the FRR container by running the following command:
    ```terminal
    $ oc -n openshift-frr-k8s -c frr rsh frr-k8s-86wmq
    ```
1.  Check the IP routes from the cluster node by running the following command:
    ```terminal
    sh-5.1# vtysh
    ```
    ```text title="Example output"
    ...
    ```
1.  Check the IP routes by running the following command:
    ```terminal
    ...
    ```
1.  From the {{ product_title }} cluster, debug the node by running the following command:
    ```terminal
    ...
    ```
1.  Confirm that the BGP routes are being advertised by running the following command:
    ```terminal
