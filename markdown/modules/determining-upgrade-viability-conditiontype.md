...
# Understanding cluster Operator condition types {id="understanding_clusteroperator_conditiontypes_{{ context }}"}

The status of cluster Operators includes their condition type, which informs you of the current state of your Operator’s health. {._abstract}
...
The Cluster Version Operator (CVO) is responsible for collecting the status conditions from cluster Operators so that cluster administrators can better understand the state of the {{ product_title }} cluster.
