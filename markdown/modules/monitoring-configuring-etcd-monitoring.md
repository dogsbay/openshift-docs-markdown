# Configuring etcd monitoring {id="configuring-etcd-monitoring_{{ context }}"}

If the `etcd` service does not run correctly, successful operation of the whole {{ product_title }} cluster is in danger. Therefore, it is reasonable to configure monitoring of `etcd`.
...
1.  Verify that the monitoring stack is running:
    ...
    ```
1.  Open the configuration file for the cluster monitoring stack:
    ```
    $ oc -n openshift-monitoring edit configmap cluster-monitoring-config
    ```
1.  Under `config.yaml: |+`, add the `etcd` section.
    1.  If you run `etcd` in static pods on your control plane nodes (also known as master nodes), you can specify the `etcd` nodes using the selector:
        ```
        ...
        data:
          config.yaml: |+
        ...
        ```
    1.  If you run `etcd` on separate hosts, you must specify the nodes using IP addresses:
        ```
        ...
        data:
          config.yaml: |+
...
1.  Now you can navigate to the Web interface to see more information about status of `etcd` monitoring:
    1.  To get the URL, run:
        ...
        ```
    1.  Using `https`, navigate to the URL listed for `prometheus-k8s`. Log in.
1.  Ensure the user belongs to the `cluster-monitoring-view` role. This role provides access to viewing cluster monitoring UIs. For example, to add user `developer` to `cluster-monitoring-view`, run:
...
1.  Copy the `/etc/etcd/ca/ca.crt` and `/etc/etcd/ca/ca.key` credentials files from the control plane node to the local machine:
    ```
    $ ssh -i gcp-dev/ssh-privatekey cloud-user@35.237.54.213
    ...
    ```
1.  Create the `openssl.cnf` file with these contents:
    ```
    [ req ]
    ...
    ```
1.  Generate the `etcd.key` private key file:
    ```
    $ openssl genrsa -out etcd.key 2048
    ```
1.  Generate the `etcd.csr` certificate signing request file:
    ```
    $ openssl req -new -key etcd.key -out etcd.csr -subj "/CN=etcd" -config openssl.cnf
    ```
1.  Generate the `etcd.crt` certificate file:
    ```
    ...
    ```
1.  Put the credentials into format used by {{ product_title }}:
    ```
...
1.  Apply the credentials file to the cluster:
