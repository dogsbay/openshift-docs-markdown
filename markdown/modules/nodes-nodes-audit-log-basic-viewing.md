...
In {{ product_title }} deployments, customers who are not using the Customer Cloud Subscription (CCS) model must request a copy of your cluster’s audit logs by contacting Red Hat Support. This is because viewing API server audit logs requires `cluster-admin` privileges.
...
*   View the OpenShift API server audit logs:
    1.  List the OpenShift API server audit logs that are available for each control plane node:
        ...
        ```
    1.  View a specific OpenShift API server audit log by providing the node name and the log name:
        ```terminal
...
*   View the Kubernetes API server audit logs:
    1.  List the Kubernetes API server audit logs that are available for each control plane node:
        ...
        ci-ln-m0wpfjb-f76d1-vnb5x-master-2 audit.log
        ```
    1.  View a specific Kubernetes API server audit log by providing the node name and the log name:
        ```terminal
...
*   View the OpenShift OAuth API server audit logs:
    1.  List the OpenShift OAuth API server audit logs that are available for each control plane node:
        ...
        ci-ln-m0wpfjb-f76d1-vnb5x-master-2 audit.log
        ```
    1.  View a specific OpenShift OAuth API server audit log by providing the node name and the log name:
        ```terminal
...
*   View the OpenShift OAuth server audit logs:
    1.  List the OpenShift OAuth server audit logs that are available for each control plane node:
        ...
        ci-ln-m0wpfjb-f76d1-vnb5x-master-2 audit.log
        ```
    1.  View a specific OpenShift OAuth server audit log by providing the node name and the log name:
        ```terminal
