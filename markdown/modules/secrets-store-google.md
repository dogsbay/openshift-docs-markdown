...
*   Your have installed a cluster on {{ gcp_first }}.
...
*   You have created a service account key named `key.json` from your {{ gcp_full }} service account.
...
1.  Install the Google Secret Manager provider:
    1.  Create a YAML file Create a YAML file named `gcp-provider.yaml` that defines the `ServiceAccount` resource configuration. See the following example configuration:
        ...
        ```
    1.  Grant privileged access to the `csi-secrets-store-provider-gcp` service account by running the following command:
        ```terminal
...
1.  Grant a read permission to the Google Secret Manager secret:
    1.  Create a new project by running the following command:
        ...
        ```
    1.  Label the `my-namespace` namespace for pod security admission by running the following command:
        ```terminal
        ...
        ```
    1.  Create a service account for the pod deployment:
        ```terminal
        $ oc create serviceaccount my-service-account --namespace=my-namespace
        ```
    1.  Create a generic secret from the `key.json` file by running the following command:
        ```terminal
