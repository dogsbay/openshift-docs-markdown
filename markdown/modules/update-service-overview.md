...
# About the OpenShift Update Service {id="update-service-about_{{ context }}"}

The OpenShift Update Service (OSUS) provides update recommendations to {{ product_title }}, including {{ op_system_first }}. It provides a graph, or diagram, that contains the _vertices_ of component Operators and the _edges_ that connect them. {._abstract}
...
The updated release provides support for upgrades from all {{ product_title }} versions starting after 4.8 up to the new release version.
...
If you use {{ op_system_base_full }} machines as workers, the MCO does not update the kubelet because you must update the OpenShift API on the machines first.

With the specification for the new version applied to the old kubelet, the {{ op_system_base }} machine cannot return to the `Ready` state. You cannot complete the update until the machines are available. However, the maximum number of unavailable nodes is set to ensure that normal cluster operations can continue with that number of machines out of service.
