...
# Example: Node network configuration policy for virtual functions {id="virt-example-vf-host-services_{{ context }}"}

You can update host network settings for Single Root I/O Virtualization (SR-IOV) network virtual functions (VF) in an existing cluster by applying a `NodeNetworkConfigurationPolicy` manifest. {._abstract}
