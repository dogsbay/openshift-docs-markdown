...
# How FRR-K8s merges multiple configurations {id="nw-metallb-frr-k8s-merge-multiple-configurations_{{ context }}"}

FRR-K8s uses an additive merge strategy when multiple users configure the same node. By using FRR-K8s, you can extend existing configurations, such as adding neighbors or prefixes, but prevent the removal of components defined by other sources. {._abstract}
