...
# Built-in Data Mover controller and custom resource definitions (CRDs) {id="built-in-data-mover-crds_{{ context }}"}

Review the custom resource definitions (CRDs) that the built-in Data Mover uses to manage volume snapshot backup and restore operations. This helps you understand how Data Mover handles data upload, download, and repository management. {._abstract}
...
*   `BackupRepository`: Represents and manages the lifecycle of the backup repositories. {{ oadp_short }} creates a backup repository per namespace when the first CSI snapshot backup or restore for a namespace is requested.