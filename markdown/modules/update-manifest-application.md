...
# Understanding how manifests are applied during an update {id="update-manifest-application_{{ context }}"}

Some manifests supplied in a release image must be applied in a certain order because of the dependencies between them. {._abstract}
...
Initially reconciling the new release’s manifests is expected to take 60 to 120 minutes in total; see **Understanding {{ product_title }} update duration** for more information about factors that influence update duration.

![A diagram displaying the sequence of Runlevels and the manifests of components within each level](/_assets/images/341_OpenShift_CVO_manifests_0526.png)
