...
# Updating OpenShift Logging to the current version {id="cluster-logging-updating-logging-to-5-1_{{ context }}"}
...
*   The {{ product_title }} version is 4.7 or later.
...
1.  Update the Red Hat OpenShift Logging Operator:
    ...
    1.  Wait for a few seconds, then click **Ecosystem** -> **Installed Operators**.
        ...
        Wait for the **Status** field to report **Succeeded**.
1.  Check the logging components:
    1.  Ensure that all Elasticsearch pods are in the **Ready** status:
        ...
        ```
    1.  Ensure that the Elasticsearch cluster is healthy:
        ```terminal
        ...
        ```
    1.  Ensure that the Elasticsearch cron jobs are created:
        ```terminal
        ...
        ```
    1.  Verify that the log store is updated to 5.x and the indices are `green`:
        ```terminal
    ...
    1.  Verify that the log collector is updated to 5.3:
        ...
        ```
    1.  Verify that the log visualizer is updated to 5.x using the Kibana CRD:
        ```terminal
