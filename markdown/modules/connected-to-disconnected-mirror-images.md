...
# Mirroring the images {id="connected-to-disconnected-mirror-images_{{ context }}"}

After the cluster is properly configured, you can mirror the images from your external repositories to the mirror repository. {._abstract}
...
1.  Mirror the Operator Lifecycle Manager (OLM) images:
    ...
    `product-version`
    :   Specifies the tag that corresponds to the version of {{ product_title }} to install, such as `4.8`.
    ...
    ```
1.  Mirror the content for any other Red Hat-provided Operator:
    ```terminal
    ...
    ```
1.  Mirror the {{ product_title }} image repository:
    ```terminal
    ...
    `product-version`
    :   Specifies the tag that corresponds to the version of {{ product_title }} to install, such as `4.8.15-x86_64`.
    ...
    ```
1.  Mirror any other registries, as needed:
    ```terminal
