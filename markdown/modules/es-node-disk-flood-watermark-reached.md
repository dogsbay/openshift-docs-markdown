...
# Elasticsearch node disk flood watermark reached {id="es-node-disk-flood-watermark-reached_{{ context }}"}
...
1.  If the preceding steps do not fix the issue, delete the old indices.
    ...
    1.  Delete the index by running the following command:
        ...
        ```
1.  Continue freeing up and monitoring the disk space. After the used disk space drops below 90%, unblock writing to this node by running the following command:
    ```terminal
    $ oc exec -n openshift-logging -c elasticsearch $ES_POD_NAME \
