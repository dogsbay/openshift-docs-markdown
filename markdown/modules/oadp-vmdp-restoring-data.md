...
# Restoring data to a virtual machine by using the VMDP CLI {id="oadp-vmdp-restoring-data_{{ context }}"}

Restore data from a backup to your virtual machine (VM) by using the {{ oadp_full }} VM data protection (VMDP) command-line interface (CLI). This helps you to recover files to the original location or to a different directory. {._abstract}
...
*   Depending on your specific use case, use one of the following steps to restore data:
    ...
    *   To restore a specific backup to a custom location, run the following commands:
        1.  Retrieve the source backup ID:
            ...
            ```
        1.  Restore the backup by running the following command:
            ```terminal
    ...
    *   To restore data from a different VM, run the following commands:
        1.  To connect to the existing BSL from the new VM, run the following command:
            ...
            ```
        1.  Restore from the BSL by running the following command:
            ```terminal
