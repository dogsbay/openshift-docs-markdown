...
# Gathering bootstrap node diagnostic data {id="gathering-bootstrap-diagnostic-data_{{ context }}"}

When experiencing bootstrap-related issues, you can gather `bootkube.service` `journald` unit logs and container logs from the bootstrap node. {._abstract}
...
1.  Verify the Ignition file configuration.
    *   If you are hosting Ignition configuration files by using an HTTP server.
        ...
        1.  To verify that the Ignition file was received by the bootstrap node, query the HTTP server logs on the serving host. For example, if you are using an Apache web server to serve Ignition files, enter the following command:
            ```terminal
        ...
        1.  If the Ignition file was not received, check that the Ignition files exist and that they have the appropriate file and web server permissions on the serving host directly.
...
1.  If the bootstrap process fails, verify the following.
    ...
    *   The load balancer proxies port 6443 connections to bootstrap and control plane nodes. Ensure that the proxy configuration meets {{ product_title }} installation requirements.