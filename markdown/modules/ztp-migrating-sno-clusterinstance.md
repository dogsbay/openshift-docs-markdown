...
# Performing the migration from SiteConfig CR to ClusterInstance CR {id="ztp-migrating-sno-clusterinstance_{{ context }}"}
...
*   You have {{ rh_rhacm_first }} version 2.12 or later installed in the hub cluster.
...
1.  Mirror the `site-configs` folder structure to the new `site-configs-v2` directory that will contain the `ClusterInstance` CRs, for example:
    ...
    1.  Mirror the target cluster, in this example `sno1`, to include the required pre-requisite resources such as the image registry pull secret, the baseboard management controller credentials, and so on.
1.  Remove the target cluster from the original Argo CD application by commenting out the resources in the related files in Git:
    1.  Comment out the target cluster from the `site-configs/kustomization.yaml` file, for example:
        ...
        generators:
           ...
           - hub-1/sno3.yaml
        ```
    1.  Comment out the target cluster from the `site-configs/pre-reqs/kustomization.yaml` file. 
...
1.  To ensure that the original Argo CD application no longer manages the cluster resources, you can remove the Argo CD application label from the resources by running the following command:
    ```bash
...
1.  Create the `ClusterInstance` CR for the target cluster by using the `siteconfig-converter` tool packaged with the `ztp-site-generate` container image:
...
1.  Configure the new Argo CD application to manage the target cluster by referencing it in the new pipelines `Kustomization` files, for example:
    ```bash
...
1.  Verify the folder structure and files in the `site-configs-v2/` directory contain the migrated resources for the `sno1` cluster, for example:
    ```text
    site-configs-v2/
    ├── hub-1/
    │   ├── sno1.yaml (1)
    ...
    1.  The tool automatically generates the extra manifests referenced by the `ClusterInstance` CR.
    1.  The tool generates a `kuztomization.yaml` file snippet to create the `ConfigMap` resources that specifies the extra manifests. You can merge the generated `kustomization` snippet with your original `kuztomization.yaml` file.