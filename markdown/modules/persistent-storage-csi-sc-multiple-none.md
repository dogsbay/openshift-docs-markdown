...
# Absent or multiple default storage classes {id="persistent-storage-csi-sc-multiple-none_{{ context }}"}

Absent or multiple default storage classes cause persistent volume claim issues. Multiple default storage classes might result in unpredictable selection and alerts, while absent default storage classes leave claims pending. Resolve by ensuring exactly one storage class is designated as the default. {._abstract}

## Multiple default storage classes {id="_multiple_default_storage_classes"}
...
## Absent default storage class {id="_absent_default_storage_class"}
