...
# {{ gcp_full }} instance types {id="gcp-compute-types_{{ context }}"}

{{ product_title }} offers the following worker node types and sizes on {{ gcp_full }} that are chosen to have a common CPU and memory capacity that are the same as other cloud instance types:
...
`custom`, `c2`, and `n2` instance types are available for clusters created with both CCS and Red&#160;Hat cloud account infrastructure types.

`g2` and `g4` instance types are available for {{ product_title }} version 4.21 and later.
