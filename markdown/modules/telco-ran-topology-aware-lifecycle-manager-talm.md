...
# Topology Aware Lifecycle Manager {id="telco-ran-topology-aware-lifecycle-manager-talm_{{ context }}"}

{{ cgu_operator }} manages how changes like cluster upgrades, Operator upgrades, and cluster configuration are rolled out to the network. {._abstract}
...
:   *   The `PreCachingConfig` CR is optional and does not need to be created if you only need to precache platform-related OpenShift and OLM Operator images.
    ...
    *   Only policies with the `ran.openshift.io/ztp-deploy-wave` annotation are automatically applied by {{ cgu_operator }} during cluster installation.
