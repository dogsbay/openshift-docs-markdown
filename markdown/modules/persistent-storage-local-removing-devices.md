...
# Removing a local volume or local volume set {id="local-removing-device_{{ context }}"}

Remove local volumes (LVs) and local volume sets (LVSs) when no longer needed by releasing bound persistent volumes (PV), deleting the LV or LVS, and cleaning up any PVs with `Retain` reclaim policy. {._abstract}
...
1.  If there are any bound PVs owned by the LV or LVS that is being deleted, delete the corresponding persistent volume claims (PVCs) to release the PVs:
    ...
    1.  Delete corresponding PVCs of bound PVs owned by the LV or LVS being deleted by running the following command:
        ```terminal
        $ oc delete pvc <name>
        ```
...
1.  If any PV owned by the LV or LVS has a `Retain` reclaim policy, back up any important data, and then delete the PV:
    ...
    1.  Back up any important data on this volume.
