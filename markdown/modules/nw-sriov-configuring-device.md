{% if context == "configuring-sriov-device" %}
{%- set ocp_sriov = true -%}
{% endif %}
...
{%- set virt_sriov = true -%}
{% endif %}
{% if context == "virt-post-install-network-config" %}
{%- set virt_sriov = true -%}
{% endif %}
...
1.  Create the `SriovNetworkNodePolicy` object. When running the following command, replace `<name>` with the name for this configuration:
    ```terminal
...
1.  To verify your SR-IOV network device configuration, enter the following command and replace `<node_name>` with the name of the node where you configured the device.
    ```terminal
