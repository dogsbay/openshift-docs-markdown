...
# Overview of API audit filter {id="logging-audit-filtering_{{ context }}"}
...
In logging 5.8 and later, the `ClusterLogForwarder` custom resource (CR) uses the same format as the standard [Kubernetes audit policy](https://kubernetes.io/docs/tasks/debug/debug-cluster/audit/#audit-policy), while providing the following additional functions:
...
1.  The name of your audit policy.