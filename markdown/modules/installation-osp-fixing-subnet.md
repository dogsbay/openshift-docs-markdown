...
# Setting a custom subnet for machines {id="installation-osp-fixing-subnet_{{ context }}"}

The IP range that the installation program uses by default might not match the Neutron subnet that you create when you install {{ product_title }}. If necessary, update the CIDR value for new machines by editing the installation configuration file. {._abstract}
