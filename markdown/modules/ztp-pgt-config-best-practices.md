...
Consider the following best practices when customizing site configuration `{{ policy_gen_cr }}` custom resources (CRs): {._abstract}
...
If you use the `{{ policy_gen_cr }}` CR to change the order of the policies, conflicts might occur and the configuration might not be applied.