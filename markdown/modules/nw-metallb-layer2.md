...
# MetalLB concepts for layer 2 mode {id="nw-metallb-layer2_{{ context }}"}

MetalLB in layer 2 mode announces the external IP for a LoadBalancer service from one node via ARP or NDP. All traffic for the service goes through that node, and failover to another node is automatic when the node becomes unavailable. {._abstract}
