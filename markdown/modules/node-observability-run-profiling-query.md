...
1.  Create a `NodeObservabilityRun` resource file named `nodeobservabilityrun.yaml` that contains the following text:
    ...
    spec:
      nodeObservabilityRef:
        name: cluster
    ```
1.  Trigger the profiling query by running the `NodeObservabilityRun` resource:
    ```terminal
    $ oc apply -f nodeobservabilityrun.yaml
    ```
1.  Review the status of the `NodeObservabilityRun` by running the following command:
    ```terminal
...
1.  Retrieve the profiling data from the container’s `/run/node-observability` path by running the following bash script:
    ```bash
