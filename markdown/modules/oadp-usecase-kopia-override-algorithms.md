...
# Use case for overriding Kopia hashing, encryption, and splitter algorithms {id="oadp-usecase-kopia-override-algorithms_{{ context }}"}

Back up an application by using Kopia environment variables for hashing, encryption, and splitter. Store the backup in an {{ aws_short }} S3 bucket and verify the environment variables by connecting to the Kopia repository. {._abstract}
