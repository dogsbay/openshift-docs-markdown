...
# Compute nodes that need to access an ignition endpoint {id="hcp-proxy-ignition_{{ context }}"}

When compute nodes need a proxy to access the ignition endpoint, you must configure the proxy in the user-data stub that is configured on the compute node when it is created. {._abstract}
