...
# Getting the BareMetalHost resource {id="bmo-getting-the-baremetalhost-resource_{{ context }}"}

You can retrieve the `BareMetalHost` resource to review hardware details, provisioning state, BMC configuration, and power status for a physical host. {._abstract}
...
1.  Get the list of hosts:
    ```terminal
    $ oc get bmh -n openshift-machine-api
    ```
1.  Get the `BareMetalHost` resource for a specific host:
    ```terminal
