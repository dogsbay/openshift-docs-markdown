...
1.  Get the full SHA for your release by running the following command:
    ...
    *   Substitute &lt;release_version> with your release number, for example, `4.14.3`.
        ...
        ---
        ```
1.  Pull down the Red Hat release key by running the following command:
    ```terminal
    $ curl -o pub.key https://access.redhat.com/security/data/fd431d51.txt
    ```
1.  Get the signature file for the specific release that you want to verify by running the following command:
    ```terminal
...
1.  Get the manifest for the release image by running the following command:
    ```terminal
...
1.  Use skopeo to verify the signature:
    ```terminal
