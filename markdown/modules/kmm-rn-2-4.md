...
# Release notes for Kernel Module Management Operator 2.4 {id="kmm-2-4-RN_{{ context }}"}

Review what is new, changed, or fixed in Kernel Module Management Operator 2.4 on {{ product_title }}. {._abstract}

The following new features and enhancements are included in this release:

*   In this release, you now have the option to configure the Kernel Module Management (KMM) module to not load an out-of-tree kernel driver and use the in-tree driver instead, and run only the device plugin. For more information, see [Using in-tree modules with the device plugin](/hardware_enablement/kmm-kernel-module-management#kmm-using-intree-modules_kernel-module-management-operator).
...
*   In this release, KMM configurations are now persistent after cluster and KMM Operator upgrades and redeployments of KMM.
    ...
    For more information, see [Configuring the Kernel Module Management Operator](/hardware_enablement/kmm-kernel-module-management#kmm-configuring-kmmo_kernel-module-management-operator).
...
[Red Hat Catalog](https://catalog.redhat.com/search?searchType=software).
...
*   In this release, the preflight validation resource in the cluster has been modified. You can use the  preflight validation to verify kernel modules to be installed on the nodes after cluster upgrades and possible kernel upgrades. Preflight validation also reports on the status and progress of each module in the cluster that it attempts or has attempted to validate. For more information, see [Preflight validation for Kernel Module Management (KMM) Modules](/updating/preparing_for_updates/kmm-preflight-validation#kmm-validation-kickoff_kmm-preflight-validation).
...
For more information, see [Creating a kmod image](/hardware_enablement/kmm-kernel-module-management#kmm-creating-kmod-image_kernel-module-management-operator).
...
*   Webhook deployment has been renamed from `webhook-server` to `webhook`.
    *   **Cause**: Generating files with `controller-gen` generated a service called `webhook-service` that is not configurable. And, when deploying KMM with {{ olm_first }}, OLM deploys a service for the webhook called `-service`.
...
*   The `PreFflightValidationOCP` custom resource, with a `kernelVersion` flag that is different that the one of the cluster, does not work.
    ...
    *   **Result:** Using the fields `kernelVersion` and `dtkImage` the feature can build installed modules for target {{ product_title }} versions.
...
*   The `ModuleUnloaded` event does not appear when a module is `Unloaded`.
    ...
    *   **Consequence**: The `ModuleLoad` and the `ModuleUnloaded` events might not appear in {{ product_title }}.
