...
# Troubleshooting oc-mirror plugin v2 errors {id="oc-mirror-troubleshooting-v2_{{ context }}"}

oc-mirror plugin v2 now logs all image mirroring errors in a separate file, making it easier to track and diagnose failures. {._abstract}
...
1.  Check for server-related issues:
    ...
    1.  Look for error messages that typically indicate server-side issues, such as `HTTP 500` errors, expired tokens, or timeouts.
    1.  Retry the mirroring process or contact support if the issue persists.
1.  Check for incomplete mirroring of Operators:
    ```terminal title="Example error"
    ...
    1.  Manually mirror the missing image or retry the mirroring process.
1.  Check for errors related to generated cluster resources. Even if some images fail to mirror, oc-mirror v2 will still generate cluster resources such as `IDMS.yaml` and `ITMS.yaml` files for the successfully mirrored images.
    1.  Check the output directory for the generated files.
    1.  If these files are missing for specific images, ensure that no critical errors occurred for those images during the mirroring process.