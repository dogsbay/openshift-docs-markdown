...
# Share a specific IP address {id="share-specific-ip-address_{{ context }}"}

To colocate multiple services on a single IP address, apply the `metallb.io/allow-shared-ip` annotation to your service specifications. Configuring this annotation authorizes MetalLB to assign the same IP to multiple services, enabling efficient address usage while distinguishing traffic by port. {._abstract}
...
:   Specifies the same value for the `metallb.io/allow-shared-ip` annotation. This value is referred to as the _sharing key_.
