...
# Protocol for initiating a remote command from a client {id="nodes-containers-remote-commands-protocol_{{ context }}"}

A client resource in your cluster can initiate the execution of a remote command in a container by issuing a request to the Kubernetes API server. {._abstract}
