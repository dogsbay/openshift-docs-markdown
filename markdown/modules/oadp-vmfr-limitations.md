...
# {{ oadp_short }} VMFR limitations {id="oadp-vmfr-limitations_{{ context }}"}

Review the limitations of {{ oadp_short }} virtual machine file restore (VMFR) to understand which operations are restricted. This helps you plan appropriate file-level restore strategies within the supported functionality. {._abstract}

The following limitations apply to {{ oadp_short }} VMFR:

*   VMFR supports only VM backups created by {{ oadp_short }} with the `kubevirt` Velero plugin. Backups created without this plugin are not supported.
