...
# Copied CSVs {id="olm-operatorgroups-copied-csvs_{{ context }}"}

Operator Lifecycle Manager (OLM) creates copies of all active member CSVs of an Operator group in each of the target namespaces of that Operator group. The purpose of a copied CSV is to tell users of a target namespace that a specific Operator is configured to watch resources created there. {._abstract}
