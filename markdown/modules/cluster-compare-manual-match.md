...
# Configuring manual matching between CRs and templates {id="cluster-compare-manual-match_{{ context }}"}

In some cases, the `cluster-compare` plugin’s default matching might not work as expected. You can manually define how a custom resource (CR) maps to a template by using a user configuration file. {._abstract}
...
1.  Reference the user configuration file in a `cluster-compare` command by running the following command:
    ```terminal
