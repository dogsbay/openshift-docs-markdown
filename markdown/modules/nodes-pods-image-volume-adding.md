...
# Adding an image volume to a pod {id="nodes-pods-image-volume-adding_{{ context }}"}

To mount an Open Container Initiative (OCI)-compliant container image or artifact, use the `volume` parameter in your pod spec to include a path to the image or artifact, along with an optional pull policy.  {._abstract}
