...
# Verifying your serverless application deployment {id="verifying-serverless-app-deployment_{{ context }}"}
...
1.  Find the application URL:
    ...
    ```
1.  Make a request to your cluster and observe the output.
    ```terminal title="Example HTTP request"
    ...
    Hello Serverless!
    ```
1.  Optional. If you receive an error relating to a self-signed certificate in the certificate chain, you can add the `--insecure` flag to the curl command to ignore the error:
    ```terminal
...
1.  Optional. If your {{ product_title }} cluster is configured with a certificate that is signed by a certificate authority (CA) but not yet globally configured for your system, you can specify this with the `curl` command.
