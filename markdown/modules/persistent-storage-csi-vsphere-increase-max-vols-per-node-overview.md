...
# Increasing maximum volumes per node for vSphere {id="persistent-storage-csi-vsphere-increase-max-vols-per-node-overview_{{ context }}"}

To support more storage volumes on individual nodes, you can increase the maximum volumes per node on homogeneous vSphere 8 or later environments. {._abstract}
