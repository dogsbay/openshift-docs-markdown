...
# Accessing a monitoring web service API {id="accessing-a-monitoring-web-service-api_{{ context }}"}
...
1.  Extract an authentication token by running the following command:
    ...
    $ TOKEN=$(oc whoami -t)
    ```
1.  Extract the `alertmanager-main` API route URL by running the following command:
    ```terminal
    $ HOST=$(oc -n openshift-monitoring get route alertmanager-main -ojsonpath='{.status.ingress[].host}')
    ```
1.  Query the service API receivers for Alertmanager by running the following command:
    ```terminal
