...
# Configuring TLS for Redis with autotls enabled {id="gitops-configuring-tls-for-redis-with-autotls-enabled_{{ context }}"}

You can configure TLS encryption for Redis by enabling the `autotls` setting on a new or already existing Argo CD instance. The configuration automatically provisions the `argocd-operator-redis-tls` secret and does not require further steps. Currently, {{ product_title }} is the only supported secret provider.
...
1.  Create an Argo CD instance with `autotls` enabled:
    ...
    1.  Edit or replace the YAML similar to the following example:
        ```yaml title="Example Argo CD CR with autotls enabled"
