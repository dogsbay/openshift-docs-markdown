...
# Verify your AWS account permissions {id="rosa-verify-permissions-sts-deployment-verify-permissions_{{ context }}"}

Run the permissions verification script to check whether your AWS account has the required permissions for a {{ product_title }} STS deployment. The script verifies the permissions for the current account and region. {._abstract}

**Prerequisites**

*   The [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html) is installed.
*   The [ROSA CLI](https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws_classic_architecture/4/html-single/cli_tools/index#rosa-get-started-cli) v1.2.6 or later is installed.
*   The [jq](https://jqlang.github.io/jq/) CLI tool is installed.
*   You have an [AWS role with the required permissions](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_testing-policies.html).
...
1.  To use the script, run the following commands in a `bash` terminal (the `-p`` option defines a prefix for the roles):
    ...
    ```
1.  After the script completes, review each results file to ensure that none of the required API calls are blocked:
    ```terminal
