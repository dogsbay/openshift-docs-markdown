...
# How service accounts assume AWS IAM roles in user-defined projects {id="how-service-accounts-assume-aws-iam-roles-in-user-defined-projects_{{ context }}"}

You can use the pod identity webhook to enable a service account in a user-defined project to assume an AWS Identity and Access Management (IAM) role in a pod in the same project.  {._abstract}
...
## Pod identity webhook workflow in user-defined projects {id="pod-identity-webhook-workflow-in-user-defined-projects_{{ context }}"}
...
{%- if openshift_rosa %}
        ...
        In {{ product_title }} with STS clusters, the OIDC provider is created during install and set as the service account issuer by default. The `sts.amazonaws.com` audience is set by default in the OIDC provider.
{%- endif %}
