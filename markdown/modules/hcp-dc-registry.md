...
# Deploying a registry for {{ hcp }} in a disconnected environment {id="hcp-dc-registry_{{ context }}"}

For development environments, deploy a small, self-hosted registry by using a Podman container. For production environments, deploy an enterprise-hosted registry, such as {{ quay }}, Nexus, or Artifactory. {._abstract}
...
1.  Name the script file `registry.sh` and save it. When you run the script, it pulls in the following information: 
    *   The registry name, based on the hypervisor hostname
    *   The necessary credentials and user access details
1.  Adjust permissions by adding the execution flag as follows: 
    ```terminal
    $ chmod u+x ${HOME}/registry.sh
    ```
1.  To run the script without any parameters, enter the following command:
    ```terminal
    $ ${HOME}/registry.sh
    ```

    The script starts the server. The script uses a `systemd` service for management purposes. 
1.  If you need to manage the script, you can use the following commands.
    1.  To view the status, enter the following command:
        ```terminal
        $ systemctl status
        ```
    1.  To start the script, enter the following command:
        ```terminal
        $ systemctl start
        ```
    1.  To stop the script, enter the following command:
        ```terminal
        $ systemctl stop
        ```
