...
1.  Create a directory to store the install configuration by running the following command:
    ```terminal
    $ mkdir ~/<directory_name>
    ```
1.  Create the `install-config.yaml` file by running the following command:
    ```terminal
...
{% endif %}
    ...
    :   Specifies a configuration that enables provisioning of the {{ op_system_first }} image to a particular device. The installation program examines the devices in the order it discovers them, and compares the discovered values with the hint values. It uses the first discovered device that matches the hint value.

        :::note


        This parameter is mandatory for FCP multipath configurations on {{ ibm_z_title }}.
