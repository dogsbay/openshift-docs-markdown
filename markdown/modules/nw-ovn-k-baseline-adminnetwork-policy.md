...
# BaselineAdminNetworkPolicy {id="BaselineAdminNetworkPolicy_{{ context }}"}

To enforce `Allow` or `Deny` rules for selected namespaces in {{ product_title }}, you can create a single cluster-scoped `BaselineAdminNetworkPolicy` custom resource (CR) named `default` with ingress and egress subject rules. {._abstract}
...
## BaselineAdminNetworkPolicy example {id="baselineddminnetworkpolicy-example_{{ context }}"}
...
## BaselineAdminNetworkPolicy Deny example {id="BaselineAdminNetworkPolicy-default-deny-example"}
