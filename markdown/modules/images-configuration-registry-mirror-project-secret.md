...
# Configuring project-scoped image pull secrets for mirrored registries {id="images-configuration-registry-mirror-project-secret_{{ context }}"}

As a cluster administrator, you can edit the cluster-wide `CRIOCredentialProviderConfig` object, named `cluster`, to enable project-scoped image pull secrets that you can use with mirrored repositories. By using project-scoped secrets, you can maintain security isolation between projects without exposing credentials in a global pull secret. {._abstract}
...
{%- set FeatureName = "Project-scoped image pull secrets for mirrored registries" %}
...
1.  Create a secret in that namespace from an existing authentication file:
    ...
    1.  For Podman clients using `.config/containers/auth.json`, run the following command:
        ```terminal
        $ oc create secret generic <pull_secret_name> \
...
1.  Create a `Role` and a `RoleBinding` object in the namespace to grant service account access to image pull secrets:
    ...
    1.  Create the `Role` and `RoleBinding` objects:
        ```terminal
        $ oc create -f <file_name>.yaml
        ```
    1.  Edit the `CRIOCredentialProviderConfig` object:
        ```terminal
        $ oc edit criocredentialproviderconfig cluster
        ```
    1.  Add the `spec.matchImages` stanza with a list of registries similar to the following example:
        ```terminal
        apiVersion: config.openshift.io/v1alpha1
        kind: CRIOCredentialProviderConfig
        metadata:
        ...
        After you save the changes, {{ product_title }} updates the `CredentialProviderConfig` object on the nodes. 
...
1.  Verify that the `CredentialProviderConfig` object contains the registries from the `matchImages` list:
    ...
    1.  Set `/host` as the root directory within the debug shell:
        ...
        sh-5.1# chroot /host
        ```
    1.  View the `/etc/kubernetes/credential-providers/<platform>-credential-provider.yaml` file:
        ```terminal
        ...
        *   Use `ecr` for an {{ aws_short }} cluster.
        *   Use `gcr` for a {{ gcp_short }} cluster.
        *   Use `acr` for an {{ azure_short }} cluster.
