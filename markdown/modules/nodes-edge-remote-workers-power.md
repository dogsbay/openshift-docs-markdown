...
# Power loss on remote worker nodes {id="nodes-edge-remote-workers-power_{{ context }}"}

You can review the following information to understand how {{ product_title }} responds to power loss on remote nodes and how you can mitigate problems associated with power loss.  {._abstract}

If a remote worker node loses power or restarts ungracefully, {{ product_title }} responds using several default mechanisms.
