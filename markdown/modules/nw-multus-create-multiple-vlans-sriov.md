...
# Creating multiple VLANs on SR-IOV VFs {id="nw-multus-create-multiple-vlans-sriov_{{ context }}"}

You can create multiple VLANs based on SR-IOV VFs. For this configuration, create an SR-IOV network and then define the network attachments for the VLAN interfaces. {._abstract}

The following diagram shows the setup process for creating multiple VLANs on SR-IOV VFs.

![Creating VLANs](/_assets/images/345_OpenShift_config_additional_network_0823.png)
...
1.  Create an SR-IOV node policy.
    ...
    1.  Apply the YAML configuration by running the following command:
        ```terminal
