...
# DNS configuration for {{ hcp }} on {{ ibm_z_title }} {id="hcp-ibm-z-dns_{{ context }}"}

The API server for the hosted cluster is exposed as a `NodePort` service. A DNS entry must exist for the `api.<hosted_cluster_name>.<base_domain>` that points to the destination where the API server is reachable. {._abstract}
...
For {{ ibm_title }} z/VM, add IP addresses that correspond to the IP address of the agent.
