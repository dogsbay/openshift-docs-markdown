...
# Enable KMS encryption {id="kms-configuring_{{ context }}"}

You can enable external Key Management Service (KMS) encryption for etcd data to centralize key management and meet compliance requirements. {._abstract}
...
1.  Create a static pod manifest file:
    ```terminal
    ...
    &lt;vault_namespace>
    :   Optional field.
    The manifest uses the Red Hat certified container image from Quay.io (`quay.io/redhat-isv-containers/698df066f8d1ddf179c15ef9`), which is the recommended image for {{ product_title }}.
...
1.  Deploy the static pod manifest to each control plane node by running the following commands:
    ```terminal
...
1.  Verify the static pods are running by entering the following command:
    ```terminal
...
1.  Verify the socket exists on a control plane node by entering the following command:
    ```terminal
...
1.  Edit the `APIServer` custom resource by entering the following command:
    ...
    ```
1.  Add the KMS configuration to the `spec.encryption` section:
    ```yaml
...
1.  Monitor the `kube-apiserver` rollout progress by entering the following command:
    ```terminal
...
1.  Check the encryption migration status by entering the following command:
    ```terminal
...
1.  Verify secrets are encrypted in etcd:
    ...
    1.  Get an etcd pod name by entering the following command:
        ...
        ```
    1.  Check the secret data in etcd by entering the following command:
        ```terminal
