...
1.  Optional: Set the service account issuer.
    ...
    1.  Edit the `cluster` `Authentication` object:
        ...
        ```
    1.  Set the `spec.serviceAccountIssuer` field to the desired service account issuer value:
        ```yaml
        spec:
    ...
    1.  Optional: Force the holder to request a new bound token either by performing a rolling node restart or by manually restarting all pods in the cluster.
...
1.  Configure a pod to use a bound service account token by using volume projection.
    1.  Create a file called `pod-projected-svc-token.yaml` with the following contents:
        ...
        In order to prevent unexpected failure, {{ product_title }} overrides the `expirationSeconds` value to be one year from the initial token generation with the `--service-account-extend-token-expiration` default of `true`. You cannot change this setting.
