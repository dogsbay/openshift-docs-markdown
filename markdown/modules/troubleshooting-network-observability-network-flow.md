...
# Failing to see network flows from both br-int and br-ex interfaces {id="configure-network-traffic-interfaces_{{ context }}"}

Resolve issues with missing network flows by removing interface restrictions on virtual bridge devices like `br-int` and `br-ex`, ensuring the eBPF agent can attach to the appropriate Layer 3 interfaces. {._abstract}
