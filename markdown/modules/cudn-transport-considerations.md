...
# Considerations for ClusterUserDefinedNetwork transport {id="cudn-transport-considerations_{{ context }}"}

Unlike the `UserDefinedNetwork` (UDN) custom resource (CR), the `ClusterUserDefinedNetwork` CR gives you more control over how pod traffic is carried on the cluster infrastructure and how it relates to networks outside the cluster. {._abstract}
