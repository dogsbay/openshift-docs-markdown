...
# Authentication metrics for Prometheus {id="authentication-prometheus-system-metrics_{{ context }}"}

You can use Prometheus metrics to monitor login activity and troubleshoot authentication failures. {._abstract}

{{ product_title }} captures the following Prometheus metrics that track authentication attempts and outcomes for both the CLI and web console:
