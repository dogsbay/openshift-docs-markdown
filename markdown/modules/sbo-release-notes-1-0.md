# Release notes for {servicebinding-title} 1.0 {id="sbo-release-notes-1-0_{{ context }}"}

{{ servicebinding_title }} is now available on {{ product_title }} 4.7, 4.8 and 4.9.

The custom resource definition (CRD) of the {{ servicebinding_title }} 1.0 supports the following APIs:
...
## Support matrix {id="support-matrix-1-0_{{ context }}"}
...
| Feature | {{ servicebinding_title }} 1.0 |
| --- | --- |
| `binding.operators.coreos.com` API group | GA |
| `servicebinding.io` API group | TP |

## New features {id="new-features-1-0_{{ context }}"}
{{ servicebinding_title }} 1.0 supports OpenShift Container Platform 4.9 and later running on:

*   IBM Power Systems
*   IBM Z and LinuxONE

This section highlights what is new in {{ servicebinding_title }} 1.0:
