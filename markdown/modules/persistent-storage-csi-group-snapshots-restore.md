...
# Restoring a volume group snapshot {id="persistent-storage-csi-group-snapshots-restore_{{ context }}"}

Restore volumes from a volume group snapshot by creating new persistent volume claims (PVCs) from individual `VolumeSnapshot` objects. Each PVC provisions a volume populated with snapshot data. Repeat for each `VolumeSnapshot` in the group to restore all volumes to their previous state. {._abstract}
