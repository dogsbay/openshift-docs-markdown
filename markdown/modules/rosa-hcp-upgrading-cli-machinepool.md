...
# Updating machine pools with the {{ rosa_cli }} {id="rosa-hcp-upgrading-cli-machinepool_{{ context }}"}
...
Your control plane only supports machine pools within two minor y-stream versions. For example, a {{ product_title }} cluster with a control plane using version 4.19.z supports machine pools with version 4.17.z and 4.18.z, but the control plane does not support machine pools using version 4.16.z.
...
1.  Verify the update behavior of the machine pools you intend to update by running the following command:
    ```terminal
...
1.  Update a machine pool by running the following command:
    ```terminal
