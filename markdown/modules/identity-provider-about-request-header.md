...
such as the community-supported [SAML authentication](https://github.com/openshift/request-header-saml-service-provider).
...
{%- set FeatureName = "Using SSPI connection support on Microsoft Windows" %}
...
GSSAPI-enabled proxy to connect an Active Directory server to {{ product_title }},
users can automatically authenticate to {{ product_title }} by using the `oc`  command
