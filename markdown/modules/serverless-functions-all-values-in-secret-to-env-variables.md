...
# Setting environment variables from all values defined in a secret {id="serverless-functions-all-values-in-secret-to-env-variables_{{ context }}"}
...
1.  For every secret for which you want to import all key-value pairs as environment variables, add the following YAML to the `envs` section:
    ```yaml
    name: test
    namespace: ""
    runtime: go
