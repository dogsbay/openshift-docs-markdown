...
# Ways to scale up the storage of clusters {id="lvms-about-scaling-storage-of-cluster_{{ context }}"}
...
{{ product_title }} supports additional worker nodes for clusters on bare metal user-provisioned infrastructure. 

{{ lvms_first }} detects and uses additional worker nodes when the nodes become active.
