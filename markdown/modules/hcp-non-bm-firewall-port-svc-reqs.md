...
# Firewall, port, and service requirements for non-bare-metal agent machines {id="hcp-non-bm-firewall-port-svc-reqs_{{ context }}"}

You must meet the firewall and port requirements so that ports can communicate between the management cluster, the control plane, and hosted clusters. {._abstract}
