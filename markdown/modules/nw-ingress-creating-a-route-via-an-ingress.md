...
*   The private key is not password-encrypted. You must decrypt the key before you create the TLS secret so that {{ product_title }} can read the key material.
...
1.  Define an Ingress object in the {{ product_title }} console or by entering the `oc create` command:
