...
# Identity provider configuration {id="security-identity-prov-config_{{ context }}"}

Configuring an identity provider is the first step in setting up users on the cluster. You can manage groups at the organizational level by using an identity provider.  {._abstract}
