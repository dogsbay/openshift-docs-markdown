...
# Installing Knative Serving by using YAML {id="serverless-install-serving-yaml_{{ context }}"}
...
1.  Check that the Knative Serving resources have been created:
    ```terminal
    ...
    ```
1.  Check that the necessary networking components have been installed to the automatically created `knative-serving-ingress` namespace:
    ```terminal
