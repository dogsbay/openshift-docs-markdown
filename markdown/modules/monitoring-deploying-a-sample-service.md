...
# Deploying a sample service {id="deploying-a-sample-service_{{ context }}"}
...
1.  Add the following deployment and service configuration details to the file:
    ```yaml
    apiVersion: v1
    kind: Namespace
    metadata:
...
1.  You can check that the pod is running:
    ```terminal
    $ oc -n ns1 get pod
    ```
