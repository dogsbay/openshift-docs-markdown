...
# Providing the Jenkins file for pipeline builds {id="builds-strategy-pipeline-providing-jenkinsfile_{{ context }}"}
...
The `jenkinsfile` is run on the Jenkins agent pod, which must have the {{ product_title }} client binaries available if you intend to use the {{ product_title }} DSL.
