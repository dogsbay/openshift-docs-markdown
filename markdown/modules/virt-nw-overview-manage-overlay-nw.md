...
:   You can expose a VM within the cluster or outside the cluster by creating a `Service` object. For on-premise clusters, you can configure a load balancing service by using the MetalLB Operator. You can install the MetalLB Operator by using the {{ product_title }} web console or the CLI.
{% endif %}
...
Add a VM to a {{ SMProductShortName }}
:   {{ VirtProductName }} is integrated with {{ SMProductName }}. You can monitor, visualize, and control traffic between pods and virtual machines on the default pod network with IPv4.