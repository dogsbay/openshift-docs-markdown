...
# Patching Ingress objects to resolve an ingressWithoutClassName alert {id="nw-patch-fields-example_{{ context }}"}

To prevent certain routing issues, you must define define the `ingressClassName` field for each `Ingress` object. {._abstract}
...
1.  List all `Ingress` objects in all namespaces:
    ...
    $ oc get ingress -A
    ```
1.  Patch the `Ingress` object by running the following command. This command patches the `Ingress` object to include the desired ingress class name.
    ```terminal
