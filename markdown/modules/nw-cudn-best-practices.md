...
# Best practices for ClusterUserDefinedNetwork CRs {id="considerations-for-cudn_{{ context }}"}

To create and deploy a successful instance of the `ClusterUserDefinedNetwork` (CUDN) CR, administrators must follow best practices such as avoiding default and openshift-* namespaces, use the proper namespace selector configuration, and ensure physical network parameter matching. {._abstract}
...
*   {{ product_title }} administrators should be aware that all namespaces of a cluster are selected when one of the following conditions are met:
