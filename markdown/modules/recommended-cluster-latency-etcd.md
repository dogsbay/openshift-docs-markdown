...
# Cluster latency requirements for etcd {id="recommended-cluster-latency-etcd_{{ context }}"}

etcd requires low network and disk I/O latency so that Raft consensus can commit changes quickly and recover from leader failures without destabilizing the cluster. {._abstract}
