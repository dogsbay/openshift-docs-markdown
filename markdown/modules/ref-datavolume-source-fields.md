...
# `DataVolume` source fields {id="ref-datavolume-source-fields_{{ context }}"}

You can configure a `DataVolume` object to import a virtual machine (VM) image from several source types.
Each source type has its own set of fields that control where and how the Containerized Data Importer (CDI) retrieves the image. {._abstract}

## Registry source fields {id="ref-datavolume-registry-fields_{{ context }}"}
...
## HTTP source fields {id="ref-datavolume-http-fields_{{ context }}"}
...
## S3 source fields {id="ref-datavolume-s3-fields_{{ context }}"}
...
## ImageIO source fields {id="ref-datavolume-imageio-fields_{{ context }}"}
...
## VDDK source fields {id="ref-datavolume-vddk-fields_{{ context }}"}
...
## Blank source {id="ref-datavolume-blank-fields_{{ context }}"}

The `source.blank` source type creates an empty disk with no data.
This source type has no configurable fields.

## PVC source fields {id="ref-datavolume-pvc-fields_{{ context }}"}
...
## `VolumeSnapshot` source fields {id="ref-datavolume-volumesnapshot-fields_{{ context }}"}
...
## Supported content types {id="ref-datavolume-supported-content-types_{{ context }}"}
