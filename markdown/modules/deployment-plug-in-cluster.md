...
{%- if not openshift_rosa_hcp %}
    ...
    If you are deploying on {{ product_title }} 4.10 and later, it is recommended to exclude configurations related to pod security by adding the parameter `--set plugin.securityContext.enabled=false`.
...
{% if openshift_rosa_hcp %}
    ...
    If you are deploying on {{ product_title }}, it is recommended to exclude configurations related to pod security by adding the parameter `--set plugin.securityContext.enabled=false`.
    
    :::

{% endif %}
1.  Optional: You can specify any additional parameters by using the set of supported parameters in the `charts/openshift-console-plugin/values.yaml` file.
    ```yaml
    plugin:
      name: ""
      description: ""
