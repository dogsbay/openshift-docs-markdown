...
# Reducing permissions while using the GCP PD CSI Driver Operator {id="persistent-storage-csi-gcp-pd-reduce-permissions_{{ context }}"}

By default, the Google Cloud Platform (GCP) persistent disk (PD) Container Storage Interface (CSI) Driver can impersonate any service account in the Google Cloud project. You can reduce the scope of permissions to only the required node service accounts. {._abstract}
