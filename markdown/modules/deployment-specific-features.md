...
# Deployment-specific features {id="deployment-specific-features_{{ context }}"}

You can use `Deployment` objects in {{ product_title }} to roll out multiple sets of rollouts, scale ongoing rollouts during updates, or pause mid-rollout. These capabilities differ from `DeploymentConfig` objects and can make application updates faster and more flexible. {._abstract}

## Rollover {id="deployment-specific-features-rollover_{{ context }}"}
...
## Proportional scaling {id="deployment-specific-features-proportional-scaling_{{ context }}"}
...
## Pausing mid-rollout {id="deployment-specific-features-pausing-mid-rollout_{{ context }}"}
