...
# Enabling TLS authentication for cluster local services {id="serverless-enabling-tls-local-services_{{ context }}"}
...
1.  Generate a root key and certificate:
    ```terminal
    $ openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 \
        ...
        -out ca.crt
    ```
1.  Generate a server key that uses SAN validation:
    ```terminal
...
1.  Create server certificates:
    ...
    ```
1.  Configure a secret for the Kourier local gateway:
    1.  Deploy a secret in `knative-serving-ingress` namespace from the certificates created by the previous steps:
        ...
        ```
    1.  Update the `KnativeServing` custom resource (CR) spec to use the secret that was created by the Kourier gateway:
