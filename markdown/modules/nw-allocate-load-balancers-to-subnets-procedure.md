...
# Specifying AWS subnets for OpenShift API and ingress load balancers at installation {id="nw-allocating-load-balancers-to-subnets-procedure_{{ context }}"}

You can allocate API and ingress load balancers to specific subnets for the purposes of aligning security and networking policies with your organization requirements. {._abstract}
...
*   Pre-configured AWS subnets intended for use by the {{ product_title }} cluster, with the following considerations:
...
1.  Save you changes to the `install-config.yaml` file.
1.  Install the cluster by running the following command:
    ```terminal
