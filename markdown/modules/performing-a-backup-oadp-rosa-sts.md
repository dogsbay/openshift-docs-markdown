...
# Example: Performing a backup with OADP and {{ product_title }} {id="performing-a-backup-oadp-rosa-sts_{{ context }}"}

Perform a backup by using {{ oadp_first }} with {{ product_title }}. The following example `hello-world` application has no persistent volumes (PVs) attached. {._abstract}
