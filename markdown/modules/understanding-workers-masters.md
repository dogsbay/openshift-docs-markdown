# Understanding {{ product_title }} workers and masters {id="understanding-workers-masters_{{ context }}"}

With installation complete, the cluster is now fully in charge of managing itself. Management of worker (compute) and master (control plane) nodes is done from within the cluster. So, before moving on to what the {{ product_title }} cluster does to help you develop and deploy applications, you should explore how an {{ product_title }} cluster manages itself. For that, we focus on three things; workers, masters (the control plane) and Operators.
...
To see more information about internal and external IP addresses, the type of operating system ({{ op_system }}), kernel version, and container runtime (CRI-O), add the `-o wide` option.
