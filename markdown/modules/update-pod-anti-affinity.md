...
# Ensuring that pods do not run on the same cluster node {id="update-pod-anti-affinity_{{ context }}"}
...
In {{ product_title }}, processes can be automatically duplicated in separate pods in a deployment.
You configure anti-affinity in the `Pod` resource to ensure that the pods in a deployment do not run on the same cluster node. {._abstract}
