...
# Installing cert-manager {id="ossm-cert-manager-installation_{{ context }}"}
...
1.  Create the root cluster issuer:
    1.  Create the `cluster-issuer` object as in the following example:
        ...
        The namespace of the `selfsigned-root-issuer` issuer and `root-ca` certificate is `cert-manager` because `root-ca` is a cluster issuer, so the cert-manager looks for a referenced secret in its own namespace. The namespace is called `cert-manager` in the case of the {{ cert_manager_operator }}.

1.  Create the object by using the following command:
    ```terminal
    $ oc apply -f cluster-issuer.yaml
    ```
1.  Create the `istio-ca` object as in the following example:
    ...
    spec:
      ca:
        secretName: istio-ca
    ```
1.  Use the following command to create the object:
    ```terminal
...
1.  Deploy the HTTP and `sleep` apps:
    ...
    ```
1.  Verify that `sleep` can access the `httpbin` service:
    ```terminal
