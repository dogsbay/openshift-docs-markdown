...
*   Do one of the following:
    ...
    *   To create your external authentication provider by entering each argument, run the following command:
        ...
        `<audience_id>`
        :   The audience IDs that this authentication provider issues tokens for. This is a comma-separated list of token audiences.
        ...
        `<client_id_for_app_registration>`
        :   Optional. The application or client ID that your app registration uses for the console.

        `<client_secret>`
        :   The client secret that is used to associate your account with the application. If you do not include the client secret, this command uses a public OIDC OAuthClient.
