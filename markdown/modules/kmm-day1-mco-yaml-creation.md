...
# MCO yaml creation {id="kmm-day1-mco-yaml-creation_{{ context }}"}

Kernel Module Management (KMM) exposes a `ProduceMachineConfig` API that generates Machine Config Operator (MCO) YAML for Day 1 out-of-tree kernel module loading on {{ product_title }}. You apply the returned manifest to target nodes in a specified `MachineConfigPool` object. {._abstract}
