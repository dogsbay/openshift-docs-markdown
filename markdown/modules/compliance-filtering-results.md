...
# Filters for compliance check results {id="filtering-compliance-check-results_{{ context }}"}

You can use the labels in the `ComplianceCheckResult` objects to query the checks and decide on the next steps after the results are generated. {._abstract}
