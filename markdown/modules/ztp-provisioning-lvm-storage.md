...
1.  To configure {{ lvms }} for new managed clusters, add the following YAML to `{{ rangen_yaml_path }}` in the `{{ policy_prefix }}common-ranGen.yaml` file:
...
1.  Commit the `{{ policy_gen_cr }}` changes in Git, and then push the changes to your site configuration repository to deploy {{ lvms }} to new sites using {{ ztp }}.