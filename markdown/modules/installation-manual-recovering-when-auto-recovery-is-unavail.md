...
# Manually recovering from a disruption event when automated recovery is unavailable {id="installation-manual-recovering-when-auto-recovery-is-unavail_{{ context }}"}

You might need to perform manual recovery steps if a disruption event prevents fencing from functioning correctly. In this case, you can run commands directly on the control plane nodes to recover the cluster. There are five main recovery scenarios, which should be attempted in the following order: {._abstract}

1.  Update fencing secrets: Refresh the Baseboard Management Console (BMC) credentials if they are incorrect or outdated.
1.  Recover from a single-node failure: Restore functionality when only one control plane node is down.
1.  Recover from dual node power loss: Restore functionality when both control plane nodes are down and can be restarted.
1.  Restore corosync quorum after dual node power loss: Restore corosync quorum when both control plane nodes lost power but only one node can be restarted.
1.  Replace a control plane node that cannot be recovered: Replace the node to restore cluster functionality.
...
1.  Update the fencing secrets:
    ...
    1.  Edit the username and password for the existing fencing secret for the control plane node by running the following commands:
        ```terminal
    ...
    1.  Verify that the new credentials can reach the BMC by running the following command:
        ```terminal
        $ sudo pcs stonith config <node_name>_redfish
        ```
    1.  Verify that no STONITH resources are blocked by running the following command:
        ```terminal
...
1.  Recover from a single-node failure:
    ...
    1.  Run the following additional diagnostic commands, if necessary:

        Reset the resources on your cluster by running the following command:
        ```terminal
        $ sudo pcs resource cleanup
        ```
    1.  Review all Pacemaker activity on the node by running the following command:
        ```terminal
        $ sudo journalctl -u pacemaker
        ```
    1.  Diagnose etcd resource startup issues by running the following command:
        ```terminal
        $ sudo journalctl -u pacemaker | grep podman-etcd
        ```
    1.  View the fencing configuration for the node by running the following command:
        ```terminal
        $ sudo pcs stonith config <node_name>_redfish
        ```
    ...
    1.  Confirm the node is fenced by running the following command:
        ```terminal
    ...
    1.  If etcd is not starting despite fencing being operational, restore etcd from a backup by running the following commands:
        ```terminal
...
1.  Recover from dual node power loss where both nodes are recoverable:
    ...
    1.  Reset the resources on your cluster and instruct Pacemaker to attempt to start them fresh by running the following command:
        ```terminal
        $ sudo pcs resource cleanup
        ```
    1.  Check resource start order by running the following command:
        ```terminal
        $ sudo pcs status --full
        ```
    1.  Inspect the pacemaker service journal if kubelet fails by running the following commands:
        ```terminal
        $ sudo journalctl -u pacemaker
        ```
        ```terminal
...
1.  Restore corosync quorum after dual node power loss (single node recoverable):
...
1.  Verify that only one node is online by running the following command on the surviving node:
    ...
    ```
1.  Verify that the failed node is permanently inaccessible before proceeding.
...
1.  Confirm to Pacemaker that the failed node is fenced by running the following command:
    ```terminal
...
1.  Verify that quorum is restored by running the following command:
    ```terminal
    ...
    Flags:            2Node Quorate
    ```
1.  Wait 30 seconds for Pacemaker to process the fencing confirmation and begin recovery.
1.  Verify that etcd is running on the surviving node by running the following command:
    ```terminal
