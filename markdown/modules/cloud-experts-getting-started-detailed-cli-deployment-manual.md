...
# Creating a cluster with manual Mode {id="cloud-experts-getting-start-detailed-cli-deployment-manual_{{ context }}"}

If you want to review the roles and policies before applying them to a cluster, use the manual method. This method requires running a few extra commands to create the roles and policies. This section uses the `--interactive` mode.  {._abstract}

**Procedure**

1.  If this is the _first time_ you are deploying {{ product_title }} in this account and you have _not_ yet created the account roles, create the account-wide roles and policies, including the Operator policies. The command creates the needed JSON files for the required roles and policies for your account in the current directory. It also outputs the `aws` CLI commands that you need to run to create these objects.
    ...
    $ cat sts_installer_permission_policy.json
            ...
            "Statement": [
            {
                ...
                "Action": [
                    ...
                    [...]
    ```
1.  Run the `aws` commands listed in step 1. You can copy and paste if you are in the same directory as the JSON files you created.
1.  After the `aws` commands are executed successfully, run the following command to begin {{ product_title }} cluster creation in interactive mode:
    ```terminal
    rosa create cluster --interactive --sts
    ```
1.  For the purpose of this tutorial, copy and then input the following values:
    ```terminal
...
1.  The above step outputs the next commands to run. These roles need to be created _once_ for _each_ cluster. To create the roles run the following command:
    ```terminal
    ...
    **Example output**
    ...
    I: Run the following commands to create the operator roles:
        ...
        [...]
    ```
1.  Run each of the `aws` commands.
...
1.  Run one of the following commands to check the status of your cluster:
    ...
    *   For an abridged view of the status, run:
        ...
        rosa list clusters
        ```
1.  The cluster state will change from “waiting” to “installing” to "ready". This will take about 40 minutes.
1.  Once the state changes to “ready” your cluster is installed.
1.  To obtain the {{ hybrid_console_second }} URL, run the following command:
    ```terminal
