...
# Debugging low latency CNF tuning status {id="cnf-debugging-low-latency-cnf-tuning-status_{{ context }}"}

To report tuning status and debug latency degradation issues, use the status fields in the `PerformanceProfile` custom resource (CR). These fields describe the conditions of the reconciliation functionality of an Operator, helping you verify the state of your configuration. {._abstract}
