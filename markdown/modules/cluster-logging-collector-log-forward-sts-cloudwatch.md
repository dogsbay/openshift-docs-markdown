...
# Forwarding logs to Amazon CloudWatch from STS enabled clusters {id="cluster-logging-collector-log-forward-sts-cloudwatch_{{ context }}"}
...
*   {{ logging_title_uc }}: 5.5 and later

**Procedure**

1.  Create a `CredentialsRequest` custom resource YAML by using the template below:
    ```yaml title="CloudWatch credentials request template"
...
1.  Apply the secret created:
    ```terminal
    $ oc apply -f output/manifests/openshift-logging-<your_role_name>-credentials.yaml
    ```
1.  Create or edit a `ClusterLogForwarder` custom resource:
    ```yaml
    ...
    1.  Specify the `cloudwatch` type.
    1.  Optional: Specify how to group the logs:
    ...
    1.  Optional: Specify a string to replace the default `infrastructureName` prefix in the names of the log groups.
    1.  Specify the AWS region.
    1.  Specify the name of the secret that contains your AWS credentials.
