...
# Configuring linuxptp services as an ordinary clock with dual-port NIC redundancy {id="configuring-linuxptp-services-oc-dual-port_{{ context }}"}

You can configure `linuxptp` services (`ptp4l`, `phc2sys`) as an ordinary clock with dual-port NIC redundancy by creating a `PtpConfig` custom resource (CR) object. {._abstract}
...
{%- set FeatureName = "Configuring linuxptp services as an ordinary clock with dual-port NIC redundancy" %}
