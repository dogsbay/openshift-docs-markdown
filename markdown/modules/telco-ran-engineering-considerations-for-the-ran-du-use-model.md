...
# Engineering considerations for the RAN DU use model {id="telco-ran-engineering-considerations-for-the-ran-du-use-model_{{ context }}"}

The RAN DU use model configures an {{ product_title }} cluster running on commodity hardware for hosting RAN distributed unit (DU) workloads.
Model and system level considerations are described below.
Specific limits, requirements and engineering considerations for individual components are detailed in later sections. {._abstract}


:::note

For details of the telco RAN DU RDS KPI test results, see the [telco RAN DU {{ product_version }} reference design specification KPI test results](https://access.redhat.com/articles/7143490).
...
Workloads
:   1.  DU workloads are described in [Telco RAN DU application workloads](/scalability_and_performance/telco-ran-du-rds#telco-ran-du-application-workloads_telco-ran-du).
    1.  DU worker nodes are Intel 3rd Generation Xeon (Ice Lake) 2.20 GHz or newer with host firmware tuned for maximum performance.

Resources
:   The maximum number of running pods in the system, inclusive of application workload and {{ product_title }} pods, is 160.
...
Resource utilization
    ...
    *   Storage IOPS
    ...
    1.  The cluster is a single host with {{ sno }} installed.
...
:   1.  Uses 75 pods across 5 namespaces with 4 containers per pod for the vRAN application including its management and control functions
    1.  Creates 30 `ConfigMap` CRs and 30 `Secret` CRs for each namespace
