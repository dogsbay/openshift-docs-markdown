...
# Backing up a hosted cluster on {{ VirtProductName }} {id="backup-hosted-cluster-virt_{{ context }}"}

When you back up a hosted cluster on {{ VirtProductName }}, the hosted cluster can remain running. The backup contains the hosted control plane components and the etcd for the hosted cluster. {._abstract}
...
1.  Monitor the backup process in the backup object status and in the Velero logs.
