...
1.  Create a YAML file, for example, `bundle.yaml`, that defines the `Bundle` object as shown in the following example:
    ```yaml
...
1.  Create the `Bundle` custom resource by running the following command:
    ```terminal
