...
# NonAdminBackupStorageLocation management commands {id="oadp-cli-nonadmin-bsl_{{ context }}"}

You can use the {{ oadp_short }} command-line interface (CLI) to create and view `NonAdminBackupStorageLocation` (NABSL) objects in your namespace. NABSLs define where your backup data is stored by using object storage that you own and manage. {._abstract}
...
## Prerequisites {id="oadp-cli-nonadmin-bsl-prereqs_{{ context }}"}

*   The {{ oadp_short }} CLI plugin is installed and configured in non-administrator mode.
*   You are logged in to the {{ product_title }} cluster and your current namespace context is set to the target namespace.
...
## NonAdminBackupStorageLocation creation command {id="oadp-cli-nonadmin-bsl-create_{{ context }}"}
...
## NonAdminBackupStorageLocation listing command {id="oadp-cli-nonadmin-bsl-list_{{ context }}"}
