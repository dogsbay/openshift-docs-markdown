...
# Attaching NVIDIA GPU devices by using the hcp CLI {id="hcp-virt-attach-nvidia-gpus_{{ context }}"}

You can attach one or more NVIDIA graphics processing unit (GPU) devices to node pools by using the `hcp` command-line interface (CLI) in a hosted cluster on {{ VirtProductName }}. {._abstract}

{%- set FeatureName = "Attaching NVIDIA GPU devices to node pools" %}
{% include "./snippets/technology-preview.md" %}

**Prerequisites**

*   You have exposed the NVIDIA GPU device as a resource on the node where the GPU device resides. For more information, see [NVIDIA GPU Operator with {{ VirtProductName }}](https://docs.nvidia.com/datacenter/cloud-native/openshift/latest/openshift-virtualization.html).
