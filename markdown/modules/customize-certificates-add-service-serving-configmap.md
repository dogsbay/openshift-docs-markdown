...
# Add the service CA bundle to a config map {id="add-service-certificate-configmap_{{ context }}"}

To verify TLS connections to services that use serving certificates in {{ product_title }}, you can inject the service Certificate Authority (CA) certificate into a config map. Annotate the config map so that pods can mount the CA bundle from the `service-ca.crt` key. {._abstract}
...
1.  View the config map to ensure that the service CA bundle has been injected:
    ```terminal
    ...
    ...
    ```
1.  Mount the config map as a volume to each container that exists in a pod by configuring your `Deployment` object.
