...
# Converting ImageContentSourcePolicy (ICSP) files for image registry repository mirroring {id="images-configuration-registry-mirror-convert_{{ context }}"}

You can convert existing `ImageContentSourcePolicy` (ICSP) files to `ImageDigestMirrorSet` files to configure image registry repository mirroring. {._abstract}

Using an `ImageContentSourcePolicy` (ICSP) object to configure repository mirroring is a deprecated feature.

This functionality is still included in {{ product_title }} and continues to be supported; however, it will be removed in a future release of this product and is not recommended for new deployments.
...
1.  Create the CR object by running the following command:
    ```terminal
...
1.  Remove the ICSP objects after the IDMS objects are rolled out.