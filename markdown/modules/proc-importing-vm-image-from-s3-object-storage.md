...
# Import a VM image from S3 object storage {id="proc-importing-vm-image-from-s3-object-storage_{{ context }}"}

You can import a virtual machine (VM) image from an S3-compatible object storage bucket by using a `DataVolume` object.
The Containerized Data Importer (CDI) pulls the image from the S3 endpoint and populates a persistent volume claim (PVC) that you can use to create a VM. {._abstract}
...
1.  Apply the `Secret` object:
    ```terminal
    $ oc create -f <secret_manifest>.yaml
    ```
1.  Create a `DataVolume` object that specifies the S3 source URL and secret reference:
    ```yaml
...
1.  Apply the `DataVolume` object by running the following command:
    ```terminal
