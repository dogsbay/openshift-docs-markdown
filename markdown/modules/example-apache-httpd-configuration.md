...
# Example Apache HTTPD configuration for basic identity providers {id="example-apache-httpd-configuration_{{ context }}"}

You can use CGI scripting in Apache HTTPD to configure a remote authentication server that returns JSON responses for basic identity providers in {{ product_title }}. {._abstract}
...
## File requirements {id="_file_requirements"}
