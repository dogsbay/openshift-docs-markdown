...
# Injecting a custom CA certificate for the {{ zero_trust_full }} {id="zero-trust-manager-proxy-support_{{ context }}"}

Inject certificate authority (CA) certificates into the {{ zero_trust_full }} to support proxying HTTPS connections. This configuration helps ensure that the Identity Manager can communicate securely when you enable a cluster-wide proxy. {._abstract}

**Prerequisites**

*   You have access to the cluster as a user with the `cluster-admin` role.
*   You have enabled the cluster-wide proxy for {{ product_title }}.
*   You have installed {{ zero_trust_full }} 1.0.0 or later.
...
1.  Inject the CA bundle that is trusted by {{ product_title }} into the config map by running the following command:
    ...
    ```
1.  Update the subscription for the {{ zero_trust_full }} to use the config map by running the following command:
    ```terminal
