...
# Configuring GNR-D T-BC holdover on a GNR-D platform {id="nw-ptp-gnrd-t-bc-holdover_{{ context }}"}

You can configure an Intel® Granite Rapids-D (GNR-D) platform device as telecom boundary clock (T-BC) with holdover support by using the PTP Operator. {._abstract}

{%- set FeatureName = "Configuring GNR-D T-BC holdover on a GNR-D platform" %}
...
1.  Apply the `MachineConfig` CR by running the following command:
    ```terminal
    $ oc apply -f gnrd-interface-names.yaml
    ```
1.  Create a `PtpConfig` CR with separate TT and TR profiles:
    ```yaml
...
1.  Apply the `PtpConfig` CR by running the following command:
    ...
    ```
1.  Create a `HardwareConfig` CR to configure holdover thresholds:
    ```yaml
...
1.  Apply the `HardwareConfig` CR by running the following command:
    ```terminal
