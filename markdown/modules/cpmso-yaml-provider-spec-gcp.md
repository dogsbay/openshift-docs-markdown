...
# Sample {{ gcp_short }} provider specification {id="cpmso-yaml-provider-spec-gcp_{{ context }}"}
...
The following example YAML illustrates a valid configuration for an {{ gcp_first }} cluster.
...
In the following example, you can obtain some of the values for your cluster by using the {{ oc_first }}.


Infrastructure ID
:   The `<cluster_id>` string is the infrastructure ID.
...
Image path
:   The `<path_to_image>` string is the path to the source image for the disk.
...
```yaml title="Sample {{ gcp_short }} providerSpec values"
...
:   Specifies the path to the source image for the disk.
    ...
    To use a {{ gcp_short }} Marketplace image, specify the offer to use:
...
:   Specifies the {{ gcp_short }} region for the cluster.
