...
# Image registry mirroring for {{ product_title }} {id="images-registry-mirroring_{{ context }}"}

You can configure an existing {{ product_title }} cluster to pull images from a mirrored registry by using the `ImageDigestMirrorSet (IDMS)` object. {._abstract}
...
To configure an existing {{ product_title }} cluster to pull images from a mirrored registry by using the `ImageDigestMirrorSet (IDMS)` object, you must meet the following prerequisites:
...
The benefits of configuring your {{ product_title }} cluster to pull images from a mirrored registry using IDMS include:
