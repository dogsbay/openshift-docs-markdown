...
# Build inputs {id="builds-define-build-inputs_{{ context }}"}

Use different build inputs in {{ product_title }} to provide source content for your builds. Understanding the order of precedence helps ensure your builds use the correct sources. {._abstract}
...
The docker build strategy is not supported in {{ product_title }}. Therefore, inline Dockerfile definitions are not accepted.
...
1.  The build process changes directories into the `contextDir`, if one is defined.

{% if not openshift_online %}
1.  The inline Dockerfile, if any, is written to the current directory.
{% endif %}
1.  The content from the current directory is provided to the build process
