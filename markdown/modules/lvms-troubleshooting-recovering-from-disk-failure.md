...
# Recovering from disk failure {id="recovering-from-disk-failure_{{ context }}"}

Diagnose and resolve disk and volume provisioning failures by inspecting persistent volume claim (PVC) events to identify specific error messages, then connecting to the affected host to fix the underlying disk issue. {._abstract}
