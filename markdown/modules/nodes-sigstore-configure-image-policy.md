...
*   After the nodes in your cluster are updated, you can verify that the image policy has been configured:
    ...
    1.  Set `/host` as the root directory within the debug shell by running the following command:
        ...
        sh-5.1# chroot /host/
        ```
    1.  Examine the `<namespace>.json` file by running the following command:
        ```terminal
    ...
    1.  Check the crio log for sigstore signature verification by running the following command:
        ```terminal
