...
# Preparing to deploy with Virtual Media on the `bare-metal` network {id="preparing-to-deploy-with-virtual-media-on-the-baremetal-network_{{ context }}"}

Configure the provisioning custom resource to enable Virtual Media deployment on the `bare-metal` network when expanding clusters that use a separate provisioning network. {._abstract}
...
1.  If the image URL exists, edit the `machineset` to use the API VIP address. This step only applies to clusters installed in versions 4.9 or earlier.
    ```terminal
    oc edit machineset
    ```
    ```yaml
