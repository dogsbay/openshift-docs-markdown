...
# Prerequisites for installing a cluster on {{ ibm_z_title }} in a disconnected environment {id="prereqs-ibm-z-upi-disconnected_{{ context }}"}

Before you install {{ product_title }} on {{ ibm_z_name }} using user-provisioned infrastructure in a disconnected environment, you must complete prerequisite tasks that prepare your mirrored registry, storage, and network environment. {._abstract}
