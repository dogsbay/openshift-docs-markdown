...
1.  Verify that the settings were applied in CRI-O:
    1.  Open an `oc debug` session to a node in the machine config pool and run `chroot /host`.
        ...
        sh-4.4# chroot /host
        ```
    1.  Verify the changes in the `crio.conf` file:
        ```terminal
        ...
        log_level = "debug"
        ```
    1.  Verify the changes in the `storage.conf` file:
        ```terminal
        ...
        [storage]
          ...
          [storage.options]
            ...
            size = "8G"
        ```
    1.  Verify the changes in the `crio/crio.conf.d/01-ctrcfg-defaultRuntime` file:
        ```terminal
