...
# Applying tolerations to kernel module pods  {id="kmm-applying-tolerations-to-kernel-module-pods_{{ context }}"}

Kernel module pods in {{ product_title }} can tolerate node taints so KMM schedules them on designated nodes. You can configure toleration parameters in the `Module` custom resource to match taint effects, keys, and values on target nodes. {._abstract}
