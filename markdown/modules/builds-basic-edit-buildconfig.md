...
# Editing a BuildConfig {id="builds-basic-edit-buildconfig_{{ context }}"}
...
1.  On the **Builds** page, click the Options menu {{ kebab }} to see the **Edit BuildConfig** option.
1.  Click **Edit BuildConfig** to see the **Form view** option.
1.  In the **Git** section, enter the Git repository URL for the codebase you want to use to create an application. The URL is then validated.
...
1.  In the **Build from** section, select the option that you would like to build from. You can use the following options:
...
1.  Optional: In the **Environment Variables** section, add the environment variables associated with the project by using the **Name** and **Value** fields. To add more environment variables, use **Add Value**, or **Add from ConfigMap** and **Secret** .
1.  Optional: To further customize your application, use the following advanced options:
