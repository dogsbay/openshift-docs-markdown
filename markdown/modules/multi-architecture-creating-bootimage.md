...
# Creating a 64-bit ARM boot image using the Azure image gallery {id="multi-architecture-creating-bootimage_{{ context }}"}

You can generate a 64-bit x86 boot image or a 64-bit ARM boot image by using the Azure image gallery. {._abstract}
...
1.  Log in to your Azure account by running the following command:
    ...
    $ az login
    ```
1.  Create a storage account and upload the `aarch64` virtual hard drive (VHD) to your storage account. The {{ product_title }} installation program creates a resource group, however, the boot image can also be uploaded to a custom named resource group:
    ```terminal
...
1.  Create a storage container using the storage account you generated by entering the following command:
    ...
    ```
1.  You must use the {{ product_title }} installation program JSON file to extract the URL and `aarch64` VHD name:
    ...
    1.  Extract the `aarch64` VHD name and set it to `BLOB_NAME` as the file name by running the following command:
        ```terminal
...
1.  Generate a shared access signature (SAS) token. Use this token to upload the {{ op_system }} VHD to your storage container with the following commands:
    ```terminal
    ...
    ```
1.  Copy the {{ op_system }} VHD into the storage container:
    ```terminal
