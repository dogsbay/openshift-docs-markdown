...
# Destroying a hosted cluster on x86 bare metal with {{ ibm_z_title }} compute nodes {id="destroy-hc-ibmz-cli_{{ context }}"}

To destroy a hosted cluster and its managed cluster on `x86` bare metal with {{ ibm_z_name }} compute nodes, you can use the command-line interface (CLI). {._abstract}
...
1.  Scale the `NodePool` object to `0` nodes by running the following command:
    ...
    After the `NodePool` object is scaled to `0`, the compute nodes are detached from the hosted cluster. In {{ product_title }} version 4.17, this function is applicable only for {{ ibm_z_title }} with KVM. For z/VM and LPAR, you must delete the compute nodes manually.
...
1.  Verify the status of the compute nodes by entering the following command:
    ```terminal
...
1.  Delete the agents from the cluster by running the following command:
    ```terminal
...
1.  Destroy the hosted cluster by running the following command:
    ```terminal
