...
# Inconsistent ComplianceScan {id="compliance-inconsistent_{{ context }}"}

The `ScanSetting` object lists the node roles that the compliance scans generated from the `ScanSetting` or `ScanSettingBinding` objects would scan. Each node role usually maps to a machine config pool. {._abstract}
