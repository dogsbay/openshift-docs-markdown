...
# Specifying multiple host groups for your cluster on vSphere {id="specifying-host-groups-vsphere_{{ context }}"}

You can configure the `infrastructures.config.openshift.io` configuration resource to specify multiple host groups for your {{ product_title }} cluster that runs on a {{ vmw_first }} instance. {._abstract}

This is necessary if your {{ vmw_short }} instance is in a stretched cluster configuration, with your ESXi hosts and storage distributed across multiple physical data centers. Use this procedure if you did not already configure host groups for your {{ product_title }} cluster at installation, or if you need to update your {{ product_title }} cluster with additional host groups.
...
*   `Host.Inventory.EditCluster` privilege is granted on the {{ vmw_short }} vCenter cluster object.

**Procedure**

1.  Edit the infrastructure settings of your {{ product_title }} cluster.
    1.  To copy your existing infrastructure settings to a file, run the following command:
        ...
        ```
    1.  Edit your infrastructure file to include a failure domain for each host group in your {{ vmw_short }} cluster. Refer to the following YAML file for an example of this configuration. Ensure you replace any values wrapped in angle brackets (`< >`) with your values:
        ```yaml
        ...
        # ...
        ```
    1.  To update your cluster with these changes, run the following command:
        ```terminal
        $ oc replace -f <name_of_infrastructure_file>.yaml
        ```
1.  Update your `ControlPlaneMachineSet` custom resource (CR) with the new failure domains by completing the following steps:
    1.  Edit the `ControlPlaneMachineSet` CR by running the following command:
        ...
        ```
    1.  Edit the `failureDomains` parameter as shown in the following example:
        ```yaml
        spec:
          replicas: 3
          selector:
        ...
        # ...
        ```
    1.  Verify that your control plane nodes have finished updating before proceeding further. To do this, run the following command:
        ```terminal
        $ oc get controlplanemachinesets.machine.openshift.io -n openshift-machine-api
        ```
1.  Create new `MachineSet` CRs for your failure domains.
    1.  To retrieve the configuration of an existing `MachineSet` CR for use as a template, run the following command:
        ...
        ```
    1.  Copy the template as needed to create `MachineSet` CR files for each failure domain that you defined in your infrastructure file. Refer to the following example:
        ```yaml
        ...
        # ...
        ```
    1.  For each `MachineSet` CR file, run the following command:
        ```terminal
