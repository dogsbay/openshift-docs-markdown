...
1.  Remove the remediation:
    1.  Set `apply` to false for the remediation object:
        ...
        ```
    1.  Using the `scan-name`, find the `KubeletConfig` object that the remediation was applied to:
        ```terminal
        ...
        ```
    1.  Manually remove the remediation, `imagefs.available: 10%`, from the `KubeletConfig` object:
        ```terminal
