...
# Viewing the image pull source {id="viewing-the-image-pull-source_{{ context }}"}

For clusters with unrestricted network connectivity, you can view the source of your pulled images by using a command on a node, such as `crictl images`. {._abstract}
...
*   Review the CRI-O logs for a master or worker node:
    ...
    If your `ImageContentSourcePolicy` object lists multiple mirrors, {{ product_title }} attempts to pull the images in the order listed in the configuration, for example:
    ```
