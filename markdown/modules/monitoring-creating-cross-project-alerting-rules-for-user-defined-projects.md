...
# Creating cross-project alerting rules for user-defined projects {id="creating-cross-project-alerting-rules-for-user-defined-projects_{{ context }}"}
...
1.  Edit the `user-workload-monitoring-config` config map in the `openshift-user-workload-monitoring` project:
    ...
    ```
1.  Configure projects in which you want to create alerting rules that are not bound to a specific project:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
1.  Create a YAML file for alerting rules. In this example, it is called `example-cross-project-alerting-rule.yaml`.
1.  Add an alerting rule configuration to the YAML file.
The following example creates a new cross-project alerting rule called `example-security`. The alerting rule fires when a user project does not enforce the restricted pod security policy:
    ```yaml title="Example cross-project alerting rule"
    ...
    1.  The PromQL query expression that defines the new rule. You can use label matchers on the `namespace` label to filter which projects are included or excluded from the alerting rule.
