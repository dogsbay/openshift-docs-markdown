...
# Nodes with system reserved memory utilization > 80% {id="nodes-dashboard-using-identify-critical-memory_{{ context }}"}

You can review the percentage of system-reserved memory that is utilized for each node by using the **Nodes with system reserved memory utilization > 80%** query in the node metrics dashboard. {._abstract}
