...
# Guidelines {id="olm-fb-catalogs-guidelines_{{ context }}"}

Follow the guidelines to maintain file-based Operator catalogs. Treat bundle images and metadata as immutable. Store catalog metadata in source control as the source of truth. {._abstract}

## Immutable bundles {id="olm-fb-catalogs-immutable_{{ context }}"}
...
## Source control {id="olm-fb-catalogs-source-control_{{ context }}"}
...
1.  Build and push the catalog image. Use a consistent tagging taxonomy, such as `:latest` or `:<target_cluster_version>`, so that users can receive updates to a catalog as they become available.
