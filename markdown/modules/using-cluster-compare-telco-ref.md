...
# Example: Comparing a cluster with the telco core reference configuration {id="using-cluster-compare-telco_ref_{{ context }}"}

You can use the `cluster-compare` plugin to compare a reference configuration with a configuration from a live cluster or `must-gather` data. {._abstract}
...
1.  Log on to the container image registry with your credentials by running the following command:
    ...
    ```
1.  Extract the content from the `telco-core-rds-rhel9` container image by running the following commands:
    ```terminal
    $ mkdir -p ./out
    ```
    ```terminal
...
1.  Compare the configuration for your cluster to the telco core reference configuration by running the following command:
    ```terminal
