...
# Rolling strategy {id="deployments-rolling-strategy_{{ context }}"}

To update an application with little or no downtime in {{ product_title }}, you can use the rolling deployment strategy. New pods replace previous instances gradually after readiness checks succeed. This strategy is the default when none is specified on a `DeploymentConfig` object. {._abstract}
