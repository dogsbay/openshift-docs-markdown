...
1.  Access the pod and prepare the build environment:
    ...
    1.  Because the default working directory might not be writable by an unprivileged user, change to a writable directory such as `/tmp` by running the following commands:
        ```terminal
        $ cd /tmp
        ```
        ```terminal
...
1.  Create a dockerfile and build an image by using Podman:
    ...
    1.  Create a sample `Dockerfile`:
        ...
        EOF
        ```
    1.  Build the image by running the following command. The `-t` flag tags the image.
        ```terminal
...
1.  Exit the pod and clean up:
    1.  After you are done, exit the shell session in the pod by running the following command:
        ...
        $ exit
        ```
    1.  Delete the pod if it is no longer needed by running the following command:
        ```terminal
        $ oc delete pod fuse-builder-pod
        ```
    1.  Remove the local YAML file by running the following command:
        ```terminal
