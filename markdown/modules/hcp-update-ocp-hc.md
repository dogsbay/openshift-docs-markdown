...
# Updating a control plane in a hosted cluster {id="hcp-update-ocp-hc_{{ context }}"}

In {{ hcp }}, you can upgrade your version of {{ product_title }} by updating the hosted cluster. {._abstract}
...
The `HostedControlPlane` resource orchestrates the rollout of the new version of the control plane components along with the {{ product_title }} component in the data plane through the new version of the Cluster Version Operator (CVO). The `HostedControlPlane` includes the following artifacts:
...
1.  Change the `spec.release.image` value in the hosted cluster by entering the following command:
    ```terminal
