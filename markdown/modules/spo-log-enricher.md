...
# Use the log enricher {id="spo-log-enricher_{{ context }}"}

The Security Profiles Operator contains a log enrichment feature, which is disabled by default. The log enricher container runs with `privileged` permissions in the host PID namespace (`hostPID`) so it can read audit logs from the local node. {._abstract}
...
1.  View the audit logs by running the following command:
    ```terminal
