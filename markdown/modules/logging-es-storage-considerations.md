...
# Storage considerations for Elasticsearch {id="logging-es-storage-considerations_{{ context }}"}

A persistent volume is required for each Elasticsearch deployment configuration. On {{ product_title }} this is achieved using persistent volume claims (PVCs).
