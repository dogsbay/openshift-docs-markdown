...
# Restarting hosted control plane components {id="hosted-restart-hcp-components_{{ context }}"}

If you are an administrator for {{ hcp }}, you can use the `hypershift.openshift.io/restart-date` annotation to restart all control plane components for a particular `HostedCluster` resource. {._abstract}
