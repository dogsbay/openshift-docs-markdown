# Release notes for {{ pipelines_title }} Technology Preview 1.0 {id="op-release-notes-1-0_{{ context }}"}

## New features {id="new-features-1-0_{{ context }}"}
{{ pipelines_title }} Technology Preview (TP) 1.0 is now available on {{ product_title }} 4.4. {{ pipelines_title }} TP 1.0 is updated to support:
...
In addition to the fixes and stability improvements, the following sections highlight what is new in {{ pipelines_title }} 1.0.

### Pipelines {id="pipeline-new-features-1-0_{{ context }}"}
...
### Pipelines CLI {id="cli-new-features-1-0_{{ context }}"}
...
### Triggers {id="triggers-new-features-1-0_{{ context }}"}
...
## Deprecated features {id="deprecated-features-1-0_{{ context }}"}
...
*   If you are upgrading from an older version of {{ pipelines_title }}, you must delete your existing deployments before upgrading to {{ pipelines_title }} version 1.0. To delete an existing deployment, you must first delete Custom Resources and then uninstall the {{ pipelines_title }} Operator. For more details, see the uninstalling {{ pipelines_title }} section.
...
*   The `buildah` cluster task does not work when a new user is added to a container.
    ...
    1.  Login to your cluster as a `cluster-admin`:
        ...
        ```
    1.  Use the `oc edit` command to edit `buildah` cluster task:
        ```
    ...
    1.  Under the `Steps` field, locate the following `command` field:
        ```
        ...
        ```
    1.  Replace the `command` field with the following:
        ```
...
*   All the {{ pipelines_shortname }} images are now based on the Red Hat Universal Base Images (UBI).
...
*   Previously, the {{ pipelines_shortname }} Operator modified the privileged security context constraints (SCCs), which caused an error during cluster upgrade. This error is now fixed.
