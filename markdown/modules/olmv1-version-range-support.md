...
# Support for version ranges {id="olmv1-version-range-support_{{ context }}"}

In {{ olmv1_first }}, you can specify a version range by using a comparison string in an Operator or extension’s custom resource (CR). If you specify a version range in the CR, {{ olmv1 }} installs or updates to the latest version of the Operator that can be resolved within the version range. {._abstract}
