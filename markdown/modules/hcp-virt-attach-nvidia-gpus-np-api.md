...
# Attaching NVIDIA GPU devices by using the NodePool resource {id="hcp-virt-attach-nvidia-gpus-np-api_{{ context }}"}

You can attach one or more NVIDIA graphics processing unit (GPU) devices to node pools by configuring the `nodepool.spec.platform.kubevirt.hostDevices` field in the `NodePool` resource. {._abstract}
