# Enabling the Dex OpenShift OAuth Connector {id="gitops-creating-a-new-client-in-dex_{{ context }}"}
...
1.  The `groups` property allows users of the specified group(s) to log in.
1.  The RBAC policy property assigns the admin role in the Argo CD cluster to users in the OpenShift `cluster-admins` group.