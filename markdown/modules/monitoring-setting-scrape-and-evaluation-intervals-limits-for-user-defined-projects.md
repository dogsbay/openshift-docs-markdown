...
# Setting scrape intervals, evaluation intervals, and enforced limits for user-defined projects {id="setting-scrape-and-evaluation-intervals-limits-for-user-defined-projects_{{ context }}"}
...
1.  Edit the `user-workload-monitoring-config` `ConfigMap` object in the `openshift-user-workload-monitoring` project:
    ...
    ```
1.  Add the enforced limit and time interval configurations to `data/config.yaml`:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
    ...
    1.  Specifies the maximum character length for a label name.
    The default value is `0`, which specifies no limit.
    1.  Specifies the maximum character length for a label value.
    The default value is `0`, which specifies no limit.
    1.  Specifies the interval between consecutive scrapes. The interval must be set between 5 seconds and 5 minutes.
