...
# Selecting the target release {id="update-selecting-the-target-release_{{ context }}"}

Use the [Red Hat {{ product_title }} Update Graph](https://access.redhat.com/labs/ocpupgradegraph/update_path) or the
[cincinnati-graph-data repository](https://github.com/openshift/cincinnati-graph-data/tree/master/channels) to determine what release to update to. {._abstract}

## Determining what z-stream updates are available {id="update-determining-available-z-streams_{{ context }}"}
...
## Changing the channel for a Control Plane Only update {id="update-changing-channel-eus-to-eus_{{ context }}"}
...
## Changing the channel for an early EUS to EUS update {id="update-changing-channel-early-eus-to-eus_{{ context }}"}

The update path to a brand new release of {{ product_title }} is not available in either the EUS channel or the stable channel until 45 to 90 days after the initial GA of a minor release.
...
## Changing the channel for a y-stream update {id="update-updating-y-stream_{{ context }}"}
