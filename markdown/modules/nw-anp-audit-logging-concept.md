...
# AdminNetworkPolicy audit logging {id="nw-anp-audit-logging_{{ context }}"}

You can enable audit logging for individual `AdminNetworkPolicy` custom resources in {{ product_title }} by annotating each policy with the `k8s.ovn.org/acl-logging` key. Use the resulting logs to verify how `Allow`, `Deny`, and `Pass` rules affect traffic between namespaces. {._abstract}
