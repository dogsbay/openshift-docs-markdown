...
# Configure Node Feature Discovery worker and controller pod scheduling {id="configuring-nfd-worker-master-pod-scheduling_{{ context }}"}

You can control where Node Feature Discovery (NFD) worker and controller pods are scheduled by configuring node selectors and tolerations in the `NodeFeatureDiscovery` custom resource. {._abstract}
...
1.  Label the target node with the label you use in the `workerNodeSelector` field by entering the following command:
    ...
    ```
1.  Edit the `NodeFeatureDiscovery` custom resource by entering the following command:
    ```terminal
    $ oc edit NodeFeatureDiscovery nfd-instance -n openshift-nfd
    ```
1.  Configure the scheduling options:
    ```yaml
...
1.  Verify that the NFD worker pods are scheduled on the expected nodes by entering the following command:
    ...
    ```
1.  Verify the node labels by entering the following command:
    ```terminal
    $ oc get nodes --show-labels
    ```
1.  Verify the node taints by entering the following command:
    ```terminal
