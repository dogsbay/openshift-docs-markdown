...
# Configuring Buildah to use `build` user {id="configuring-builah-to-use-build-user_{{ context }}"}
...
1.  Create a copy of the `buildah` cluster task as an ordinary task.
    ...
    ```
1.  Edit the copied `buildah` task.
    ```terminal
    $ oc edit task buildah-as-user
    ```
    ```yaml title="Example: Modified Buildah task with build user"
    ...
    1.  Display the user id to confirm that the process is running as user id `1000`.
    1.  You can change the path for the volume mount as necessary.