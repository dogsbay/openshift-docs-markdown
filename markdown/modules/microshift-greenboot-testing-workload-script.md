...
# Test a workload health check script {id="microshift-greenboot-test-workload-health-check-script_{{ context }}"}

You can look at the output of the greenboot workload health check script to see how it works. While the output varies with the host system type, the example outputs for {{ op_system_base_full }} system types are included for reference only. {._abstract}
...
*   The {{ microshift_short }} service is enabled.
...
1.  To test that greenboot is running a health check script file, reboot the host by running the following command:
    ...
    $ sudo reboot
    ```
1.  Examine the output of greenboot health checks by running the following command:
    ```terminal
    ...
    {{ microshift_short }} core service health checks run before the workload health checks.
    
    :::

    ```terminal title="Example output for an {{ op_system_image }} system"
    ...
    * rhel 19619bd269094510180c845c44d0944fd9aa15925376f249c4d680a3355e51ae.0
        Version: {{ op_system_version }}
        origin refspec: edge:rhel-{{ op_system_version }}-microshift-{{ product_version }}
    #...
    ```
    ```terminal title="Example partial output for an RPM system"
    #...
    GRUB boot variables:
    boot_success=1
    boot_indeterminate=0
