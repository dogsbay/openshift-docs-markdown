...
# Using the `ManagedClusterModule` CRD {id="kmm-hub-using-the-managedclustermodule_{{ context }}"}

To deploy kernel modules on spoke clusters with KMM-Hub on {{ product_title }}, you can configure a cluster-scoped `ManagedClusterModule` custom resource that wraps a `Module` spec and selects target clusters. {._abstract}
