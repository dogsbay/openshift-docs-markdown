...
# OAuth token requests {id="oauth-token-requests_{{ context }}"}

{{ product_title }} automatically creates OAuth clients to handle token requests from different user agents, including browser-based and CLI tools. These clients interact with OAuth endpoints to authenticate users through interactive login flows or WWW-Authenticate challenges. {._abstract}

The following OAuth clients are automatically created when starting the {{ product_title }} API:
...
{{ product_title }} to validate credentials against a backing identity provider.
...
{{ product_title }} supports authenticating using a `WWW-Authenticate`
...
or if {{ product_title }} is configured to use an identity provider that does
