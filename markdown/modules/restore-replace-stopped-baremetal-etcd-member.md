...
1.  Delete the machine of the unhealthy member by running the following command:
    ...
    1.  Edit the machine configuration by running the following command:
        ...
        ```
    1.  Delete the following fields in the `Machine` custom resource, and then save the updated file:
        ```yaml
        finalizers:
...
1.  Create the new `BareMetalHost` object and the secret to store the BMC credentials:
    ...
    Existing control plane `BareMetalHost` objects may have the `externallyProvisioned` flag set to `true` if they were provisioned by the {{ product_title }} installation program.
