...
# Consuming container values using a volume plugin {id="nodes-containers-downward-api-container-values-plugin_{{ context }}"}

Your containers can consume Downward API values by using a volume plugin. {._abstract}
...
1.  Create a new pod spec that contains the environment variables you want the container to consume:
    1.  Create a `volume-pod.yaml` file similar to the following:
        ...
        # ...
        ```
    1.  Create the pod from the `volume-pod.yaml` file by using the following command:
        ```terminal
