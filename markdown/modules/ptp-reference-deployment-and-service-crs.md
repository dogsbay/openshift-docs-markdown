...
# Reference cloud-event-proxy deployment and service CRs {id="ptp-reference-deployment-and-service-crs-{{ ptp_events_rest_api }}_{{ context }}"}
...
```yaml title="Reference cloud-event-proxy deployment with HTTP transport"
apiVersion: apps/v1
kind: Deployment
metadata:
...
```yaml title="Reference cloud-event-proxy subscriber service"
apiVersion: v1
kind: Service
metadata:
  annotations:
