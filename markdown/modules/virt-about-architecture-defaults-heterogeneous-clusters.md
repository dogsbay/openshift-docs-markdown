...
# Default architecture behavior in heterogeneous clusters {id="virt-about-architecture-defaults-heterogeneous-clusters_{{ context }}"}

In a heterogeneous cluster, worker nodes run different CPU architectures, such as amd64 and arm64. A boot source image built for one architecture cannot start on a node with a different architecture. {._abstract}
...
## How data sources for boot source images work in heterogeneous clusters {id="data-sources-boot-source-images-heterogeneous-clusters_{{ context }}"}
...
## Default architecture for standalone VMs {id="default-architecture-standalone-vms_{{ context }}"}
...
## Default architecture for standalone data volumes {id="default-architecture-standalone-data-volumes_{{ context }}"}
