...
# Configuring alert routing for user-defined projects with the Alertmanager secret {id="configuring-alert-routing-user-defined-alerts-secret_{{ context }}"}
...
1.  Print the currently active Alertmanager configuration into the file `alertmanager.yaml`:
    ...
    ```
1.  Edit the configuration in `alertmanager.yaml`:
    ```yaml
    global:
      http_config:
    ...
    1.  Specify the name of the receiver to use for the alerts group.
    1.  Specify the receiver configuration.
