...
# Release notes for Kernel Module Management Operator 2.6 {id="kmm-2-6-RN_{{ context }}"}

Review what is new, changed, or fixed in Kernel Module Management Operator 2.6 on {{ product_title }}. {._abstract}
...
*   In this release, wildcard and glob pattern support for the `filesToSign` field has been added for signing kernel modules in a specific folder. Previously, you had to specify the exact path to each `.ko` file you wanted to sign. Now, you can add the full path to explicit files, as previously required, or any glob patterns supported by the `Ash` shell.
    ...
    For more information, see [Specifying files to sign](/hardware_enablement/kmm-kernel-module-management#kmm-specifying-files-to-sign_kernel-module-management-operator).
