...
# The ClusterVersion object {id="update-cluster-version-object_{{ context }}"}

One of the resources that the Cluster Version Operator (CVO) monitors is the `ClusterVersion` resource. {._abstract}

Administrators and {{ product_title }} components can communicate or interact with the CVO through the `ClusterVersion` object.
...
## Update availability data {id="update-availability-data_{{ context }}"}
