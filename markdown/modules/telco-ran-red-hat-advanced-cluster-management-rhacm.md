...
# Red Hat Advanced Cluster Management {id="telco-ran-red-hat-advanced-cluster-management-rhacm_{{ context }}"}

{{ rh_rhacm }} provides Multi Cluster Engine (MCE) installation and ongoing lifecycle management functionality for deployed clusters. {._abstract}
...
:   {{ rh_rhacm }} provides Multi Cluster Engine (MCE) installation and ongoing lifecycle management functionality for deployed clusters.
    ...
    *   Zero touch provisioning (ZTP) of clusters using the MCE component in {{ rh_rhacm }}.
...
:   *   Use {{ rh_rhacm }} policy hub-side templating to better scale cluster configuration.
    ...
    *   Cluster specific configuration: managed clusters typically have some number of configuration values that are specific to the individual cluster.
    These configurations should be managed using {{ rh_rhacm }} policy hub-side templating with values pulled from `ConfigMap` CRs based on the cluster name.
