...
# Verifying network connectivity for an endpoint {id="nw-pod-network-connectivity-verify_{{ context }}"}
...
1.  Confirm that network diagnostics are enable by entering the following command:
    ...
    ```
1.  List the current `PodNetworkConnectivityCheck` objects by entering the following command:
    ```terminal
...
1.  View the connection test logs:
    ...
    1.  View the object by entering the following command:
        ```terminal
