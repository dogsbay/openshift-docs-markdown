...
# Identifying the image change trigger of a build {id="builds-image-change-trigger-identification_{{ context }}"}
...
1.  Under `imageChangeTriggers`, compare  timestamps to identify the latest
...
*   `lastTriggeredImageID`: This field, which is not shown in the example, is deprecated in {{ product_title }} 4.8 and will be ignored in a future release. It contains the resolved image reference for the `ImageStreamTag` when the last build was triggered from this `BuildConfig`.
