...
# {{ ztp_first }} reference CRs {id="gitops-ztp-crs_{{ context }}"}

The following custom resources (CRs) configure {{ ztp_first }} for the telco hub cluster. {._abstract}
...
| GitOps Operator | `gitopsSubscription.yaml` | Defines a subscription for the {{ product_title }} GitOps Operator, specifying automatic install plan approval and source details. | No |
