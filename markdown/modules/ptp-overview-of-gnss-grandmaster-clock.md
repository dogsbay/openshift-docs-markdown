...
# Overview of GNSS timing for PTP grandmaster clocks {id="ptp-overview-of-gnss-grandmaster-clock_{{ context }}"}

{{ product_title }} supports receiving precision PTP timing from Global Navigation Satellite System (GNSS) sources and grandmaster clocks (T-GM) in the cluster. {._abstract}


:::important

{{ product_title }} supports PTP timing from GNSS sources with Intel E810 Westport Channel NICs only.
...
## Handling leap second events in GNSS-synced PTP grandmaster clocks {id="handling-leap-second-events-in-gnss_{{ context }}"}
