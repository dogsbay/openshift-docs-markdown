...
# Create the DPFOperatorConfig custom resource {id="nw-dpf-creating-dpfoperatorconfig_{{ context }}"}

Create a `DPFOperatorConfig` resource to configure the DPF Operator components after installing the Operator. {._abstract}
...
1.  Verify that the provisioning controller manager deployment has rolled out:
    ...
    ```
1.  Verify that the DPU service controller manager deployment has rolled out:
    ```terminal
    ...
    ```
1.  Verify that all pods in the `dpf-operator-system` namespace are running:
    ```terminal
