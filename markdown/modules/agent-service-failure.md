...
# Resolving agent service failures for {{ hcp }} on {{ ibm_z_title }} {id="agent-service-failure_{{ context }}"}

In some cases, agents might fail to join the cluster after booting the machines with the boot artifacts. {._abstract}
...
1.  Replace the base64-encoded content in the `ignitionConfigOverride` with the required `/etc/containers/policy.json` configuration according to your image registries. See the following example:
    ```json title="Example"
    {
        "default": [
            {
