...
# Understanding pod affinity {id="nodes-scheduler-pod-affinity-about_{{ context }}"}

You can use _pod affinity_ and _pod anti-affinity_ to constrain which nodes your pod is eligible to be scheduled on based on the key/value labels on other pods. {._abstract}
...
:   Specifies the parameters for a _required_ rule. Configure the following `labelSelector.matchExpressions.key` parameters:
...
:   Specifies the parameters for a _preferred_ rule. Configure a weight and the following `podAffinityTerm.labelSelector.matchExpressions` parameters:
