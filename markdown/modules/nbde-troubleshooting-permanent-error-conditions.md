...
server’s configuration as defined in the daemon set.
    ...
    okay
    ```
1.  After you identify the root cause, remedy the underlying situation:
    1.  Delete the non-working daemon set.
    1.  Edit the daemon set definition to fix the underlying issue.  This might include any of the following actions:
...
1.  Locate the name of the container that was created by the daemon set:
    ...
    ```
1.  Print the logs from the container. The following log is from a completed successful rekeying operation:
    ```terminal
