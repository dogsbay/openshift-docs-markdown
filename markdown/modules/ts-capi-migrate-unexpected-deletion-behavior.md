...
# Unexpected resource deletion behavior {id="ts-capi-migrate-unexpected-deletion-behavior_{{ context }}"}

On clusters that support migrating resources between the Machine API and the Cluster API, users might experience unexpected behavior when deleting Cluster API resources on a cluster where the Machine API is authoritative. {._abstract}
