...
# Verifying hosted cluster creation on {{ VirtProductName }} {id="hcp-virt-verify-hc_{{ context }}"}

After you create a hosted cluster on {{ VirtProductName }}, verify that it was successfully created. {._abstract}
...
1.  Verify that the `HostedCluster` resource transitioned to the `completed` state by entering the following command:
    ...
    ```
1.  Verify that all the cluster operators in the hosted cluster are online by entering the following commands:
    ```terminal
