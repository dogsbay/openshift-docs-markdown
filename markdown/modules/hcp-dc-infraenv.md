...
# Creating an InfraEnv resource for the hosted cluster {id="hcp-dc-infraenv_{{ context }}"}

The `InfraEnv` resource is an Assisted Service object that includes essential details that are used to create the {{ op_system_first }} boot image that is customized for the hosted cluster. {._abstract}
...
1.  Create the `InfraEnv` resource by entering the following command:
    ```terminal
