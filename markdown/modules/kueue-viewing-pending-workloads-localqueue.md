...
# Viewing pending workloads in LocalQueue  {id="viewing-pending-workloads-localqueue_{{ context }}"}

To view the pending workloads submitted by a specific tenant within their namespace, users can query the `LocalQueue` resource visibility endpoint of Kueue’s visibility API. This provides an ordered list of their jobs waiting in that queue.  {._abstract}
...
1.  To view only one pending workload starting from position 0 in LocalQueue run the following command:
    ```terminal
