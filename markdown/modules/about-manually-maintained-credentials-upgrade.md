...
# Update requirements for clusters with manually maintained credentials {id="about-manually-maintained-credentials-upgrade_{{ context }}"}

Before you update a cluster that uses manually maintained credentials with the Cloud Credential Operator (CCO), you must update the cloud provider resources for the new release. {._abstract}
...
## Cloud credential configuration options and update requirements by platform type {id="cco-platform-options_{{ context }}"}
