...
# Limitations for layer 2 mode {id="nw-metallb-layer2-limitations_{{ context }}"}

In {{ product_title }}, MetalLB layer 2 mode is limited to single-node bandwidth and failover depends on client ARP handling. Avoid using the same VLAN for MetalLB and an additional network to prevent connection failures. {._abstract}

## Single-node bottleneck {id="nw-metallb-layer2-limitations-bottleneck_{{ context }}"}
...
## Slow failover performance {id="nw-metallb-layer2-limitations-failover_{{ context }}"}
...
## Additional Network and MetalLB cannot use same network {id="additional_network_and_metallb_limitation_{{ context }}"}
