...
# Updating mirror registry for Red Hat OpenShift from a remote host {id="mirror-registry-remote-host-update_{{ context }}"}

You can update the _mirror registry for Red&#160;Hat OpenShift_ from a remote host by using the `upgrade` command. Updating to the latest version ensures bug fixes and security vulnerability fixes. {._abstract}
...
*   To upgrade the _mirror registry for Red&#160;Hat OpenShift_ from a remote host using the existing certificates, enter the following command:
    ```terminal
