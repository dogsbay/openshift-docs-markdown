...
# About dependency resolution {id="olm-dependency-resolution-about_{{ context }}"}

Operator Lifecycle Manager (OLM) manages the dependency resolution and upgrade lifecycle of running Operators. In many ways, the problems OLM faces are similar to other system or language package managers, such as `yum` and `rpm`. {._abstract}
