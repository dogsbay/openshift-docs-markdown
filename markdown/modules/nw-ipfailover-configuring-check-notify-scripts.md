...
1.  Create the `ConfigMap` object by running the following command:
    ...
    ```
1.  Add the script to the pod. The `defaultMode` for the mounted `ConfigMap` object files must able to run by using `oc` commands or by editing the IP failover configuration. 
    ...
    1.  Alternatively, edit the `ipfailover-keepalived` configuration by running the following command:
        ```terminal
    ...
    1.  Save the changes and exit the editor. This restarts the `ipfailover-keepalived` configuration.