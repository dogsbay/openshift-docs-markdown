...
# Deploying a load balancer for an application {id="microshift-deploying-a-load-balancer_{{ context }}"}

To expose an application through a `LoadBalancer` service that uses the node IP as the external IP in {{ microshift_short }}, you can create a namespace, deploy the example workload, and apply the service manifest.  {._abstract}
...
1.  Verify that your pods are running by entering the following command:
    ...
    ```
1.  Create a namespace by running the following commands:
    ```terminal
    ...
    *   Replace _&lt;nginx_lb_test>_ with the application namespace that you want to create.
        ...
        EOF
        ```
1.  You can verify that the three sample replicas started successfully by running the following command:
    ```terminal
    $ oc get pods -n $NAMESPACE
    ```
1.  Create a `LoadBalancer` service for the `nginx` test application by running the following command:
    ```terminal
    ...
    You must ensure that the `port` parameter is a host port that is not occupied by other `LoadBalancer` services or {{ microshift_short }} components.
    
    :::

1.  Verify that the service file exists, that the external IP address is properly assigned, and that the external IP is identical to the node IP by running the following command:
    ```terminal
