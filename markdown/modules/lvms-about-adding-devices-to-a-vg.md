...
# About adding devices to a volume group {id="about-adding-devices-to-a-vg_{{ context }}"}

To add devices to the Logical Volume Manager (LVM) volume group, use the `deviceSelector` field in the `LVMCluster` Custom Resource (CR) to specify the paths to the devices. {._abstract}
...
For more information, see the "{{ op_system_base }} documentation".
...
You can create a RAID array only during an {{ product_title }} installation. For information on creating a RAID array, see:
