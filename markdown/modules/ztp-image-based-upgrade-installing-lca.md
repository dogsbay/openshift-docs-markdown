...
# Installing the {{ lcao }} with {{ ztp }} {id="ztp-image-based-upgrade-installing-lcao-with-gitops_{{ context }}"}

Install the {{ lcao }} with {{ ztp_first }} to do an image-based upgrade. {._abstract}
...
1.  Extract the following CRs from the `ztp-site-generate` container image and push them to the `source-cr` directory:
    ...
    ```
1.  Add the CRs to your common PolicyGenerator:
    ```yaml
