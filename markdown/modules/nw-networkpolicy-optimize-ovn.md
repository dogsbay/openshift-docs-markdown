...
# Optimizations for network policy with OVN-Kubernetes network plugin {id="nw-networkpolicy-optimize-ovn_{{ context }}"}

Learn how to optimize OVN-Kubernetes network policies to reduce flow count and ensure external IP traffic is allowed when needed. {._abstract}
...
## NetworkPolicy CR and external IPs in OVN-Kubernetes {id="nw-networkpolicy-external-ip-ovn_{{ context }}"}
