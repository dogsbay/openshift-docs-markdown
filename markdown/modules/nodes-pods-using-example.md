...
# Example pod configurations {id="nodes-pods-using-example_{{ context }}"}

{{ product_title }} leverages the Kubernetes concept of a _pod_, which is one or more containers deployed together on one host, and the smallest compute unit that can be defined, deployed, and managed. {._abstract}
...
<a name="example-pod-definition_{{ context }}"></a>
...
This pod definition does not include attributes that are filled by {{ product_title }} automatically after the pod is created and its lifecycle begins. The [Kubernetes pod documentation](https://kubernetes.io/docs/concepts/workloads/pods/pod/) has details about the functionality and purpose of pods.
