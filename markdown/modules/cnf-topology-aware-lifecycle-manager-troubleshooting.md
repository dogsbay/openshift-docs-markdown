...
# Troubleshooting the {{ cgu_operator_full }} {id="talo-troubleshooting_{{ context }}"}

The {{ cgu_operator_first }} is an {{ product_title }} Operator that remediates {{ rh_rhacm }} policies. When issues occur, use the `oc adm must-gather` command to gather details and logs and to take steps in debugging the issues. {._abstract}
...
*   Is the configuration that you are applying supported?
    *   Are the {{ rh_rhacm }} and the {{ product_title }} versions compatible?
    *   Are the {{ cgu_operator }} and {{ rh_rhacm }} versions compatible?
*   Which of the following components is causing the problem?
    *   [Cannot modify the ClusterUpgradeGroup CR](#talo-troubleshooting-modify-cgu_{{ context }})
    *   [Managed policies](#talo-troubleshooting-managed-policies_{{ context }})
    *   [Clusters](#talo-troubleshooting-clusters_{{ context }})
    *   [Remediation Strategy](#talo-troubleshooting-remediation-strategy_{{ context }})
    *   [{{ cgu_operator_full }}](#talo-troubleshooting-remediation-talo_{{ context }})
...
:   Restart the procedure by performing the following steps:
    ...
    1.  Check and fix the existing issues with the managed clusters and policies.
        ...
        1.  Ensure that all the policies exist and have the `spec.remediationAction` field set to `inform`.
    1.  Create a new `ClusterGroupUpgrade` CR with the correct configurations.
    ```terminal
...
1.  Alternatively, check the {{ cgu_operator }} manager logs:
    1.  Get the name of the {{ cgu_operator }} manager by running the following command:
        ```terminal
        ...
        ```
    1.  Check the {{ cgu_operator }} manager logs by running the following command:
        ```terminal
...
Issue
:   You want to check the logs of the manager container of {{ cgu_operator }}.
...
Issue
:   The policy compliance status that {{ cgu_operator }} uses to decide if remediation is needed has not yet fully updated for all clusters.
    This may be because:
...
:   1.  To check if pre-caching has failed due to insufficient space, check the log of the pre-caching pod in the node.
        ...
        ```
    1.  If there is no log, check the pod status using the following command:
    ```terminal
...
Issue
:   You want {{ rh_rhacm }} to match policies and managed clusters before the managed clusters become available.

Resolution
:   To ensure that {{ cgu_operator }} correctly applies the {{ rh_rhacm }} policies specified in the `spec.managedPolicies` field of the `ClusterGroupUpgrade` (CGU) CR, {{ cgu_operator }} needs to match these policies to the managed cluster before the managed cluster is available.
    The {{ rh_rhacm }} `PolicyGenerator` uses the generated `Placement` CR to do this automatically.
    By default, this `Placement` CR includes the necessary tolerations to ensure proper {{ cgu_operator }} behavior.
    ...
    If you use a custom `Placement` CR instead of the one generated by the {{ rh_rhacm }} `PolicyGenerator`, include these tolerations in that `Placement` CR.

    For more information on placements in {{ rh_rhacm }}, see [Placement overview](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_management_for_kubernetes/2.13/html-single/clusters/index#placement-overview).

    For more information on tolerations in {{ rh_rhacm }}, see [Placing managed clusters by using taints and tolerations](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_management_for_kubernetes/latest/html-single/clusters/index#taints-tolerations-managed).
...
1.  Wait for the status to be updated and go through the troubleshooting questions.
1.  If everything looks as expected, set the `spec.enable` field to `true` in the `ClusterGroupUpgrade` CR.
