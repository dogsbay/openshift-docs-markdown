...
# Mapping KubeVirt CSI storage classes {id="hcp-virt-map-storage_{{ context }}"}

You can map the infrastructure storage class to the hosted storage class during cluster creation.  {._abstract}
...
| --- | --- | --- | --- |
| RWX: `Block` or `Filesystem` | `ReadWriteOnce` (RWO) `Block` or `Filesystem` RWX `Block` only | Supported | Use `Block` mode because `Filesystem` volume mode results in degraded hosted `Block` mode performance. RWX `Block` volume mode is supported only when the hosted cluster is {{ product_title }} 4.16 or later. |
