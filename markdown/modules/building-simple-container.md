...
# Simple container build process {id="building-simple-container_{{ context }}"}

To containerize an application, you need a build tool, a file to define the image, and a container registry to store and distribute the application. {._abstract}
...
1.  Install container build tools: {{ op_system_base }} contains a set of tools that includes
podman, buildah, and skopeo that you use to build and manage containers.
1.  Create a Dockerfile to combine base image and software: Information about
...
software you want to containerize in a directory on your {{ op_system_base }} system.
1.  Run buildah or docker build: Run the `buildah build-using-dockerfile` or
...
1.  Tag and push to a registry: Add a tag to your new container image that
...
1.  Pull and run the image: From any system that has a container client tool,
