...
# Sample install-config.yaml file for a two-node user-provisioned infrastructure cluster with fencing {id="sample-install-config-two-node-fencing-upi_{{ context }}"}

You can use the following `install-config.yaml` configuration file as a template for deploying a two-node {{ product_title }} cluster with fencing by using the user-provisioned infrastructure method: {._abstract}
