...
1.  Create a standalone `{{ policy_gen_cr }}`{minja} custom resource (CR) that contains the source file
    `validatorCRs/informDuValidator.yaml`. You only need one standalone `{{ policy_gen_cr }}`{minja} CR for each cluster type. For example, this CR applies a validator inform policy for {{ sno }} clusters:
...
{% endif %}
1.  Commit the `{{ policy_gen_cr }}`{minja} CR file in your Git repository and push the changes.