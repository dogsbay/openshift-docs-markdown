...
# Configuring the Git resolver for the authenticated SCM API {id="resolver-git-config-scm_{{ context }}"}
...
1.  In the `TektonConfig` custom resource, edit the `pipeline.git-resolver-config` spec:
    ...
    1.  The SCM provider type.
    1.  The base URL for use with the authenticated SCM API. This setting is not required if you are using `github.com`, `gitlab.com`, or BitBucket Cloud.
    1.  The name of the secret that contains the SCM provider API token.
    1.  The key within the token secret that contains the token.
    1.  The namespace containing the token secret, if not `default`.
