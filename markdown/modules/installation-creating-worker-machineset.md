# Creating worker nodes that the cluster manages {id="installation-creating-worker-machineset_{{ context }}"}
...
1.  Optional: Launch worker nodes that are controlled by the machine API.
1.  View the list of MachineSets in the `openshift-machine-api` namespace:
    ```
...
1.  Edit each worker MachineSet to provide the correct values for your cluster:
    ```
    ...
    status:
      ...
      replicas: 1
    ```
    1.  Specify the {{ op_system_first }} AMI to use for your worker nodes. Use the same
    value that you specified in the parameter values for your control plane and
    bootstrap templates.
    1.  Specify the name of the worker security group that you created in the form
...
1.  View the machines in the `openshift-machine-api` namespace and confirm that
they are launching:
    ```
