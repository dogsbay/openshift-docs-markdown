...
# Configuring resources and scheduling for logging collectors {id="log-collector-resources-scheduling_{{ context }}"}
...
1.  Create a `ClusterLogging` CR that supports your existing `ClusterLogForwarder` CR:
    ```yaml title="Example ClusterLogging CR YAML"
    ...
    1.  The namespace must be the same namespace as the `ClusterLogForwarder` CR.
