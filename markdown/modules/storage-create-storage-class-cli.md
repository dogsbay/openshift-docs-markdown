...
# Creating the {{ StorageClass }} storage class using the CLI {id="storage-create-storage-class-cli_{{ context }}"}

To enable dynamic provisioning of AWS Elastic File System (EFS) volumes by using the command line, create a `StorageClass` object that defines file system parameters, permissions, and access point configuration. {._abstract}
