...
1.  Apply the namespace by running the following command:
    ```bash
    $ oc apply -f namespace.yaml
    ```
1.  Configure host-level LACP bonds:
    ...
    1.  Create a YAML file that defines the `NodeNetworkConfigurationPolicy` resource for the `ens5f1` interface on the `worker-0` node:
        ...
        *   `lacp_rate: 'fast'` sets the LACP rate on the interface. You must also set the rate to `fast` on the switch. The `fast` rate sends LACP packets every second.
        *   `<pf_name>` specifies the PF that you want to include in the bond.
    1.  Apply the resources by running the following commands:
        ```bash
        $ oc apply -f nncpBondF0Worker0.yaml
        $ oc apply -f nncpBondF1Worker0.yaml
        ```
1.  Create SR-IOV network VFs for the bonded interfaces:
    ...
    1.  Create a YAML file that defines the `SriovNetworkNodePolicy` resource for the `ens5f1` interface on the `worker-0` node:
    ...
    1.  Apply the resources by running the following commands:
        ...
        ```
1.  Configure the PF Status Relay Operator:
    ...
    1.  Apply the `PFLACPMonitor` resource by running the following command:
        ```bash
...
1.  Check the logs of the PF Status Relay Operator to verify that it is monitoring the LACP state:
    ...
    ...
    ```
1.  Apply the `SriovNetwork` resources to make the VFs available for use within the `sriov-operator-tests` namespace:
    1.  Create a YAML file that defines the `SriovNetwork` resource for the VFs created on `ens5f0`:
        ...
        ```
    1.  Create a YAML file that defines the `SriovNetwork` resource for the VFs created on `ens5f1`:
    ...
    1.  Apply the resources by running the following commands:
        ...
        ```
1.  Define a high-availability pod that uses the SR-IOV VFs:
    ...
    1.  Apply the `NetworkAttachmentDefinition` resource by running the following command:
        ```bash
        $ oc apply -f nad-bond.yaml
        ```
    1.  Create a YAML file that defines the `Pod` resource that uses the VFs from the bonded interfaces in active-backup mode:
    ...
    1.  Apply the `Pod` resource by running the following command:
        ```bash
        $ oc apply -f client-bond.yaml
        ```
1.  Check that the failover mechanism:
    1.  Log in to the `client-bond` pod by running the following command:
        ```bash
        $ oc rsh -n sriov-operator-tests client-bond
        ```
    1.  Check the initial status of the pod-level bond by running the following command:
        ```bash
    ...
    1.  Simulate an LACP failure on your upstream physical switch. To simulate this scenario, you can filter LACP traffic on the switch port that you want to test the failure on. This ensures that the physical link remains up while the LACP polling fails. The command to do this is vendor-dependent.
    1.  Verify the failover inside the pod by logging back into the `client-bond` pod and checking the bond status again:
        ```bash
        sh-4.4# cat /proc/net/bonding/bond0
        ```
        ```bash title="Example output"
