...
# Migrating a VM to a specific node {id="virt-migrate-vm-to-labeled-node_{{ context }}"}

You can migrate a running virtual machine (VM) to a specific subset of nodes by using the `addedNodeSelector` field on the `VirtualMachineInstanceMigration` object. {._abstract}
...
1.  Apply the manifest to the cluster by running the following command:
    ```terminal
