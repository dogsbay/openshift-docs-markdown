...
# Set namespace exclusions for the default ingress when creating a cluster in {{ cluster_manager_first }} {id="osd-create-cluster-exclude-namespace-selector-day1-ui_{{ context }}"}

Specify a namespace label selector so that namespaces matching those labels are excluded from the default `application ingress` when creating an {{ product_title }} cluster in {{ cluster_manager_url }}. {._abstract}
