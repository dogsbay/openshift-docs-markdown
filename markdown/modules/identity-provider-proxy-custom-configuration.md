...
# Custom proxy configuration {id="identity-provider-proxy-custom-configuration_{{ context }}"}

Review the requirements for a custom authentication proxy used with the request header identity provider. Meeting these requirements prevents header spoofing and ensures OAuth authorization flows work correctly. {._abstract}
