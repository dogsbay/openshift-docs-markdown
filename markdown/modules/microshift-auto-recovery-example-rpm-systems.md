...
1.  Create a directory for the `microshift` systemd service by running the following command:
    ...
    ```
1.  To instruct `systemd` to run `microshift-auto-recovery.service` when the `microshift.service` fails, create the `10-auto-recovery.conf` file by running the following command:
    ```terminal
...
1.  Create the `microshift-auto-recovery.service` file by running the following command:
    ```terminal
    ...
    EOF
    ```
1.  Create the `microshift-auto-recovery` script by running the following command:
    ```terminal
...
1.  Make the script executable by running the following command:
    ...
    ```
1.  Reload the system configuration by running the following command:
    ```terminal
