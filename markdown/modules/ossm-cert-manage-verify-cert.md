...
# Verifying your certificates {id="ossm-cert-manage-verify-cert_{{ context }}"}
...
1.  Verify that the root certificate is the same as the one specified by the administrator. Replace `<path>` with the path to your certificates.
    ```terminal
...
1.  Verify that the CA certificate is the same as the one specified by the administrator. Replace `<path>` with the path to your certificates.
    ```terminal
...
1.  Verify the certificate chain from the root certificate to the workload certificate. Replace `<path>` with the path to your certificates. After you run the command, the expected output shows `./proxy-cert-1.pem: OK`.
    ```terminal
