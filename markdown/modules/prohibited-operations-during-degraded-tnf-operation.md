...
# Prohibited operations during degraded TNF operation {id="prohibited-operations-during-degraded-tnf-operation_{{ context }}"}

You must not perform certain administrative operations while a two-node OpenShift cluster with fencing (TNF) is in a degraded state. Attempting these operations can leave the cluster in a state that is more difficult to recover from than the original degraded state. {._abstract}
