...
# Enabling the cluster-wide proxy {id="nw-proxy-configure-object_{{ context }}"}

To enable the cluster-wide egress proxy for your {{ product_title }} cluster, you can modify the `Proxy` object to configure HTTP and HTTPS proxy settings and specify domains that bypass the proxy. {._abstract}
...
A cluster administrator can configure the proxy for {{ product_title }} by modifying the `cluster` `Proxy` object.
...
*   You installed the {{ product_title }} `oc` CLI tool.
...
1.  Create a config map that contains any additional CA certificates required for proxying HTTPS connections.
    ...
    You can skip this step if the identity certificate of the proxy is signed by an authority from the {{ op_system_first }} trust bundle.
