...
# MCP gateway overview {id="ai-app-mcp-server-mcp-gateway-overview_{{ context }}"}

The primary goal of a Model Context Protocol (MCP) gateway is to standardize and simplify how AI models interact with external data and tools. Without a gateway, an AI model would need a custom integration for every single database, API, or local file system it needs to access. {._abstract}

## Aggregation and orchestration {id="ai-app-mcp-server-mcp-gateway-overview-agg-orch_{{ context }}"}
...
## Security and access control {id="ai-app-mcp-server-mcp-gateway-overview-security-access_{{ context }}"}
...
## Protocol translation {id="ai-app-mcp-server-mcp-gateway-overview-protocol_{{ context }}"}
...
## Scalability {id="ai-app-mcp-server-mcp-gateway-overview-scale_{{ context }}"}
