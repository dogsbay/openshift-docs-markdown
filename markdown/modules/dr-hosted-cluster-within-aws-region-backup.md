...
# Backing up a hosted cluster on {{ aws_short }} {id="dr-hosted-cluster-within-aws-region-backup_{{ context }}"}

To recover your hosted cluster in your target management cluster, you first need to back up all of the relevant data. {._abstract}
...
1.  Create a config map file to declare the source management cluster by entering the following command:
    ...
    ```
1.  Shut down the reconciliation in the hosted cluster and in the node pools by entering the following commands:
    ```terminal
    $ PAUSED_UNTIL="true"
    ```
    ```terminal
    ...
    ```
1.  Back up etcd and upload the data to an S3 bucket by running the following bash script:
...
1.  Back up Kubernetes and {{ product_title }} objects by entering the following commands. You need to back up the following objects:
    ...
    *   `ControlPlane` secrets from the Hosted Control Plane namespace
        1.  Enter the following commands:
            ...
            ```
        1.  Back up the `HostedCluster` objects from the `HostedCluster` namespace by entering the following commands:
            ```terminal
            ...
            $ sed -i '' -e '/^status:$/,$d' \
              ${BACKUP_DIR}/namespaces/${HC_CLUSTER_NS}/hc-${HC_CLUSTER_NAME}.yaml
            ```
        1.  Back up the `NodePool` objects from the `HostedCluster` namespace by entering the following commands:
            ```terminal
            ...
            $ sed -i '' -e '/^status:$/,$ d' \
              ${BACKUP_DIR}/namespaces/${HC_CLUSTER_NS}/np-${NODEPOOLS}.yaml
            ```
        1.  Back up the secrets in the `HostedCluster` namespace by running the following shell script:
            ```terminal
            ...
            ```
        1.  Back up the secrets in the `HostedCluster` control plane namespace by running the following shell script:
            ```terminal
            ...
            ```
        1.  Back up the hosted control plane by entering the following commands:
            ```terminal
            ...
            ```
        1.  Back up the cluster by entering the following commands:
            ```terminal
            $ echo "--> Cluster:"
            ```
            ```terminal
            ...
            ```
        1.  Back up the {{ aws_short }} cluster by entering the following commands:
            ```terminal
            $ echo "--> AWS Cluster:"
            ```
            ```terminal
            ...
            ```
        1.  Back up the {{ aws_short }} `MachineTemplate` objects by entering the following commands:
            ```terminal
            ...
            ```
        1.  Back up the {{ aws_short }} `Machines` objects by running the following shell script:
            ```terminal
            $ echo "--> AWS Machine:"
            ```
            ```terminal
            ...
            ```
        1.  Back up the `MachineDeployments` objects by running the following shell script:
            ```terminal
            ...
            ```
        1.  Back up the `MachineSets` objects by running the following shell script:
            ```terminal
            ...
            ```
        1.  Back up the `Machines` objects from the Hosted Control Plane namespace by running the following shell script:
            ```terminal
            ...
            ```
1.  Clean up the `ControlPlane` routes by entering the following command:
    ```terminal
...
1.  Verify that the Route53 entries are clean by running the following script:
    ```terminal
...
Check all of the {{ product_title }} objects and the S3 bucket to verify that everything looks as expected.
