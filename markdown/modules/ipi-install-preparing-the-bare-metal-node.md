...
# Preparing the bare-metal node {id="preparing-the-bare-metal-node_{{ context }}"}

Configure IP addressing for a new bare-metal node by using either static configuration or DHCP (Dynamic Host Configuration Protocol) reservations to enable network connectivity before adding the node to your cluster. {._abstract}
...
1.  Retrieve the user name and password of the bare-metal node’s baseboard management controller.
1.  Create `base64` strings from the user name and password:
    ```terminal
    $ echo -ne "root" | base64
    $ echo -ne "password" | base64
    ```
1.  Create a configuration file for the bare-metal node. Depending on whether you are using a static configuration or a DHCP server, use one of the following example `bmh.yaml` files, replacing values in the YAML to match your environment:
    ```terminal
    $ vim bmh.yaml
    ```
...
1.  Power on and inspect the bare-metal node:
    ```terminal
    ...
    ```
1.  Add the new machine to the cluster by scaling the machine set.
    1.  To manually scale the machine set, follow the procedure titled _Provisioning the bare-metal node_.
    1.  To automatically scale the machine set, follow the procedure titled _Automatically scaling machines to the number of available bare-metal hosts_ in the _Scalability and Performance_ section.