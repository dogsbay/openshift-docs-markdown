...
# Performing health checks before cluster updates {id="core-cluster-upgrade-health-check_{{ context }}"}
...
These checks validate cluster Operators, etcd health, storage systems, network connectivity, and custom resources specific to your deployment. {._abstract}
...
1.  Verify worker node readiness by running the following command:
    ```terminal
    ...
    If any nodes show `NotReady` or `SchedulingDisabled`, investigate and resolve before updating.
1.  Check cluster Operator status by running the following command:
    ```terminal
    $ oc get co
    ```
...
1.  Verify etcd cluster health by running the following command:
    ```terminal
...
1.  Check your backup storage location for an etcd backup created within the past 24 hours.
...
1.  Verify machine config pool status by running the following command:
    ```terminal
    $ oc get mcp
    ```
...
1.  Verify persistent volume health by running the following command:
    ```terminal
    $ oc get pv
    ```
...
1.  Verify that a default storage class exists by running the following command:
    ```terminal
    $ oc get storageclass
    ```

    At least one storage class must be marked as `(default)`.
1.  If your cluster uses SR-IOV, verify network node policies by running the following command:
    ```terminal
...
1.  Verify OLM Operator health by running the following command:
    ```terminal
    $ oc get csv -A
    ```
...
1.  Verify that no pods are in an unexpected state by running the following command:
    ```terminal
...
1.  Verify that pod disruption budgets (PDBs) are configured for all critical cloud-native network function (CNF) workloads by running the following command:
    ```terminal
    $ oc get pdb -A
    ```
...
1.  Check `PerformanceProfile` status by running the following command:
    ```terminal
    ...
    ```
1.  Verify the `PerformanceProfile` shows `Applied` status by running the following command:
    ```terminal
    $ oc get performanceprofile core-performance -o jsonpath='{.status.conditions[?(@.type=="Applied")].status}'
    ```

    The output must be `True`.
1.  Verify API server responsiveness by running the following command:
    ```terminal
    $ oc version
    ```
...
1.  Verify certificate validity by running the following command:
    ```terminal
...
1.  Check CPU and memory utilization on nodes by running the following command:
    ```terminal
    $ oc adm top nodes
    ```
