...
You can specify the minimum and maximum number of pods and the average memory use that your pods should target. If you do not specify a minimum, the pods are given default values from the {{ product_title }} server.
...
1.  Create a `HorizontalPodAutoscaler` object similar to the following for an existing object:
    ...
    :   Specifies the targeted average memory usage over all the pods,
