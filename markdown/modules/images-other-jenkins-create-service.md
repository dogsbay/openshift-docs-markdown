...
# Creating a Jenkins service from a template {id="images-other-jenkins-create-service_{{ context }}"}

Templates provide parameter fields to define all the environment variables with predefined default values. {{ product_title }} provides templates to make creating a new Jenkins service easy. The Jenkins templates should be registered in the default `openshift` project by your cluster administrator during the initial cluster setup.
...
To use a PV store, the cluster administrator must define a PV pool in the {{ product_title }} deployment.
