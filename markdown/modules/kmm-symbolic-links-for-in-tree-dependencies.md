...
# Symbolic links for in-tree dependencies {id="kmm-symbolic-links-for-in-tree-dependencies_{{ context }}"}

Symbolic links let kmod images reference in-tree kernel module dependencies on {{ product_title }} without copying them. KMM mounts `/usr/lib/modules` so `depmod` and `modprobe` can resolve those dependencies at build and runtime. {._abstract}
