...
# Mirroring catalog contents to airgapped registries {id="olm-mirror-catalog-airgapped_{{ context }}"}

You can mirror catalog contents into airgapped registries. {._abstract}
...
1.  Copy the `v2/` directory to removable media.
1.  Physically remove the media and attach it to a host in the disconnected environment that has access to the mirror registry.
1.  If your mirror registry requires authentication, run the following command on your host in the disconnected environment to log in to the registry:
    ```terminal
    $ podman login <mirror_registry>
    ```
1.  Run the following command from the parent directory containing the `v2/` directory to upload the images from local files to the mirror registry:
    ```terminal
    $ oc adm catalog mirror \
...
1.  Run the `oc adm catalog mirror` command again. Use the newly mirrored index image as the source and the same mirror registry target used in the previous step:
    ```terminal
    $ oc adm catalog mirror \
