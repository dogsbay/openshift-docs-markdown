...
# Sample YAML for a Cluster API machine template resource on {{ azure_full }} {id="capi-yaml-machine-template-azure_{{ context }}"}
...
Default {{ product_title }} cluster names contain hyphens (`-`), which are not compatible with {{ azure_short }} gallery name requirements.
