...
# Configuring linuxptp services as a boundary clock {id="configuring-linuxptp-services-as-boundary-clock_{{ context }}"}

You can configure the `linuxptp` services (`ptp4l`, `phc2sys`) as boundary clock by creating a `PtpConfig` custom resource (CR) object. {._abstract}
