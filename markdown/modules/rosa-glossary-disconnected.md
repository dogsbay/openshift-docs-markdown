...
| Disconnected environment | An environment or network that has some level of isolation from an external network. This isolation could be enabled by physical or logical separation between machines on the internal network and an external network. A cluster in a disconnected environment does not have access to Red&#160;Hat public services and requires additional setup to maintain full cluster functionality. |
