...
# The addition of kernel modules to nodes {id="installation-special-config-kmod_{{ context }}"}

For most common hardware, the Linux kernel includes the device driver modules needed to use that hardware when the computer starts up. For some hardware, however, modules are not available in Linux. You must find a way to provide those modules to each host computer.  {._abstract}

A subsequent procedure describes how to provide modules for nodes in an {{ product_title }} cluster.
