...
# Removing unused components if you do not use the default Elasticsearch log store {id="cluster-logging-removing-unused-components-if-no-elasticsearch_{{ context }}"}
...
1.  Edit the `ClusterLogging` custom resource (CR) in the `openshift-logging` project:
    ...
    ```
1.  If they are present, remove the `logStore` and `visualization` stanzas from the `ClusterLogging` CR.
1.  Preserve the `collection` stanza of the `ClusterLogging` CR. The result should look similar to the following example:
    ```yaml
    ...
    spec:
      ...
      collection:
        ...
        fluentd: {}
    ```
1.  Verify that the collector pods are redeployed:
    ```terminal
