...
# Support policy for unmanaged Operators {id="unmanaged-operators_{{ context }}"}

The _management state_ of an Operator determines whether the Operator actively manages its cluster resources. 
When set to _unmanaged_, the Operator ignores configuration changes and no longer receives updates. {._abstract}
