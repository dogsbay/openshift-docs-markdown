...
# Creating a custom domain Ingress Controller {id="cloud-experts-dynamic-certificate-custom-domain-create-cd-ingress-con_{{ context }}"}

You can create the custom domain Ingress Controller by using the {{ oc_first }} tool. {._abstract}
...
1.  Create and configure a certificate resource to provision a certificate for the custom domain Ingress Controller:
    ...
    EOF
    ```
1.  Verify the certificate has been issued:
...
1.  Verify that your custom domain IngressController has successfully created an external load balancer:
    ...
    **Example output**
    ...
    ```
1.  Prepare a document with the necessary DNS changes to enable DNS resolution for your custom domain Ingress Controller:
    ```terminal
