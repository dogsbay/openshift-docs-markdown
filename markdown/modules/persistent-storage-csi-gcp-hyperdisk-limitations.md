...
# C3 and N4 instance type limitations {id="persistent-storage-csi-gcp-hyperdisk-limitations_{{ context }}"}

Before deploying hyperdisk-balanced disks on C3 bare metal or N4 machine series instances, review the volume size, cloning, resizing, and storage class requirements to ensure successful configuration. {._abstract}
...
*   You must set the volume size to at least 4Gi when you create hyperdisk-balanced disks. {{ product_title }} does not round up to the minimum size, so you must specify the correct size yourself.
