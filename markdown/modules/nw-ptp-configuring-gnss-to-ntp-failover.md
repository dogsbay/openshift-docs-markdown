...
# Creating a PTP Grandmaster configuration with GNSS failover {id="configuring-gnss-to-ntp-failover_{{ context }}"}

Configure a Precision Time Protocol (PTP) Telecom Grandmaster clock with automatic failover from global navigation satellite system (GNSS) to Network Time Protocol (NTP) when satellite signals are unavailable. {._abstract}
...
1.  Check which network interfaces support hardware timestamping by running the following command:
    ```terminal
...
1.  The PTP profile uses node labels for matching. Check your machine config pool (MCP) to find the node labels by running the following command:
    ```terminal
    $ oc get mcp
    ```
...
1.  Create a `PtpConfig` custom resource (CR) that configures the T-GM clock with GNSS to NTP failover. Save the following YAML configuration to a file named `ptp-config-gnss-ntp-failover.yaml`, replacing `<MCP-name>` with the name of your machine config pool from the previous step.
    ```yaml
...
1.  The PTP daemon checks for profile updates every 30 seconds. Wait approximately 30 seconds, then verify by running the following command:
    ...
    grandmaster    2m
    ```
1.  Check the NodePtpDevice to see if the profile is applied by running the following command, replacing `<node_name>` with your node hostname:
    ```terminal
    ...
    $ oc get nodes
    ```
1.  Check if the profile is being loaded by monitoring the daemon logs:
    ```terminal
...
1.  Check `chronyd` status to verify NTP is running as the secondary time source by running the following command:
    ...
    ```
1.  Check GNSS/gpsd by running the following command:
    ```terminal
...
1.  Check `ts2phc` (GNSS synchronization) status by running the following command:
    ```terminal
    $ oc logs -n openshift-ptp <linuxptp-daemon-pod> -c linuxptp-daemon-container | grep ts2phc
    ```
1.  Check `phc2sys` (system clock sync) status by running the following command:
    ```terminal
