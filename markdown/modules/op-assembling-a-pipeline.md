...
# Assembling a pipeline {id="assembling-a-pipeline_{{ context }}"}
...
In this section, you will create a pipeline that takes the source code of the application from GitHub, and then builds and deploys it on {{ product_title }}.
...
*   Pushes the image to the {{ product_registry }} by referring to the `image` parameter.
*   Deploys the new image on {{ product_title }} by using the `apply-manifests` and `update-deployment` tasks.
