...
Replace an unhealthy etcd member when the etcd pod is crashlooping. Restoring the member returns the control plane to a healthy state. {._abstract}
...
1.  Stop the crashlooping etcd pod.
    ...
    1.  Change your root directory to `/host` by running the following command:
        ```terminal
        sh-4.2# chroot /host
        ```
    1.  Create a backup directory by running the following command:
        ```terminal
        sh-4.2# mkdir /var/lib/etcd-backup
        ```
1.  Move the existing etcd pod file out of the kubelet manifest directory by running the following commands:
    ```terminal
...
1.  Remove the unhealthy member.
    1.  Choose a pod that is _not_ on the affected node by running the following command:
        ```terminal
...
1.  Remove the old secrets for the unhealthy etcd member that was removed.
    1.  List the secrets for the unhealthy etcd member that was removed by running the following command:
        ...
        ```
    1.  Delete the peer secret for the unhealthy etcd member that was removed by running the following command:
        ```terminal
    ...
    1.  Delete the metrics secret by running the following command:
        ...
        ```
1.  Force etcd redeployment by running the following command:
    ```terminal
...
1.  Turn the quorum guard back on by running the following command:
    ...
    ```
1.  Verify that the `unsupportedConfigOverrides` section is removed from the object by running the following command:
    ```terminal
