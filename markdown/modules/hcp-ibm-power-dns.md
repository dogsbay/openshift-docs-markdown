...
# DNS configuration for {{ hcp }} on {{ ibm_power_title }} {id="hcp-ibm-power-dns_{{ context }}"}

Clients outside the network can access the API server for the hosted cluster. A DNS entry must exist for the `api.<hosted_cluster_name>.<basedomain>` entry that points to the destination where the API server is reachable. {._abstract}
...
For {{ ibm_power_title }}, add IP addresses that correspond to the IP address of the agent.
