...
# Routing and managing traffic by using a blue-green deployment strategy {id="serverless-blue-green-deploy_{{ context }}"}
...
1.  Find the name of the first revision that was created when you deployed the service, by viewing the output from the following command:
    ```terminal
    ...
    ```
1.  Add the following YAML to the service `spec` to send inbound traffic to the revision:
    ```yaml
    ...
    spec:
      traffic:
...
1.  Update your existing service to create a new, test endpoint for the second revision, while still sending all other traffic to the first revision:
    ```yaml title="Example of updated service spec with test endpoint"
    ...
    spec:
      traffic:
        - revisionName: <first_revision_name>
...
1.  Get the URL of the new service for the second revision, by running the following command:
    ```terminal
...
1.  Update your existing service again, so that 50% of traffic is sent to the first revision, and 50% is sent to the second revision:
    ```yaml title="Example of updated service spec splitting traffic 50/50 between revisions"
    ...
    spec:
      traffic:
        - revisionName: <first_revision_name>
    ...
    ...
    ```
1.  When you are ready to route all traffic to the new version of the app, update the service again to send 100% of traffic to the second revision:
    ```yaml title="Example of updated service spec sending all traffic to the second revision"
    ...
    spec:
      traffic:
        - revisionName: <first_revision_name>
