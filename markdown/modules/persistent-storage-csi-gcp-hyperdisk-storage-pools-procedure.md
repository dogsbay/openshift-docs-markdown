...
# Setting up hyperdisk-balanced disks {id="persistent-storage-csi-gcp-hyperdisk-storage-pools-procedure_{{ context }}"}

To provision high-performance hyperdisk-balanced storage volumes, configure a storage class, create persistent volume claims, and deploy applications that use the hyperdisk storage. {._abstract}
...
{% if not openshift_dedicated %}
    1.  After step 3 in "Incorporating the Cloud Credential Operator utility manifests", copy the following manifests into the manifests directory created by the installation program:
        ...
        *   storageclass.yaml - creates a hyperdisk-specific storage class
            ...
            `spec.storageClassState` specifies disabling creation of the default {{ product_title }} storage classes.
...
1.  Create a deployment that uses the PVC that you just created. Using a deployment helps ensure that your application has access to the persistent storage even after the pod restarts and rescheduling:
    ...
    1.  Use the following example YAML file to create the deployment:
    ...
    1.  Confirm that the deployment was successfully created by running the following command:
        ```terminal
        $ oc get deployment
        ```
    ...
    1.  Confirm that PVC `my-pvc` has been successfully bound to a persistent volume (PV) by running the following command:
        ```terminal
        $ oc get pvc my-pvc
        ```
        ```terminal title="Example output"
        ...
        ```
    1.  Confirm the expected configuration of your hyperdisk-balanced disk:
        ```terminal
    ...
    1.  If using storage pools, check that the volume is provisioned as specified in your storage class and PVC by running the following command:
        ```terminal
