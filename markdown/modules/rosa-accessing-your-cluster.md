...
# Accessing your cluster with an IDP account {id="rosa-accessing-your-cluster_{{ context }}"}

To log in to your cluster, you can configure an identity provider (IDP). This procedure uses GitHub as an example IDP. To view other supported IDPs, run the `rosa create idp --help` command. {._abstract}
...
1.  Add an IDP.
    ...
    1.  Enter the following values:
        ...
        *   GitHub organizations: `rh-test-org` (enter the name of your organization)
        ...
        ...
        ```
    1.  Follow the URL in the output and select **Register application** to register a new OAuth application in your GitHub organization. By registering the application, you enable the OAuth server that is built into ROSA to authenticate members of your GitHub organization into your cluster.

        :::note

        The fields in the **Register a new OAuth application** GitHub form are automatically filled with the required values through the URL that is defined by the {{ product_title }} (ROSA) CLI, `rosa`.
        
        :::

    1.  Use the information from the GitHub application you created and continue the prompts. Enter the following values:
    ...
    1.  Enter the following command to verify that your IDP has been configured correctly:
        ```terminal
...
1.  Log in to your cluster.
    ...
    1.  Copy and paste the `oc` login command into your terminal.
        ...
        For a {{ hcp_title }} cluster, use the port number `443`.
        ```terminal title="Example output"
        ...
        For a {{ hcp_title }} cluster, the port number should be `443`.
    1.  Enter a simple `oc` command to verify everything is setup properly and that you are logged in.
        ```terminal
        $ oc version
        ```
        ```terminal title="Example output"
