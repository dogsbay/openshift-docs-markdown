...
# Consuming container values using environment variables {id="nodes-containers-downward-api-container-values-envars_{{ context }}"}

When using environment variables in a container, you can specify that the variable’s value should come from a `FieldRef` source instead of the literal value specified. {._abstract}
