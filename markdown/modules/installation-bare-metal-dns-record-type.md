...
# Configuring the dnsRecordsType parameter {id="installation-bare-metal-dns-record-type_{{ context }}"}

During cluster installation, you can specify the `dnsRecordsType` parameter in the `install-config.yaml` file to set if the internal DNS service or an external source provides the necessary records for `api`, `api-int`, and `ingress` DNS records. {._abstract}

{%- set FeatureName = "Configuring the dnsRecordsType parameter" %}
