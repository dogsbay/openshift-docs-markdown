...
# Installing Knative broker for Apache Kafka {id="serverless-install-kafka-odc_{{ context }}"}
...
1.  Configure the **KnativeKafka** object in the **Create Knative Kafka** page.
    ...
    ```yaml title="Example KnativeKafka custom resource"
    ...
    1.  A comma-separated list of bootstrap servers from your AMQ Streams cluster.
    1.  Enables developers to use the `KafkaSource` event source type in the cluster.
    1.  Enables developers to use the Knative broker implementation for Apache Kafka in the cluster.
    1.  A comma-separated list of bootstrap servers from your Red Hat AMQ Streams cluster.
    1.  Defines the number of partitions of the Kafka topics, backed by the `Broker` objects. The default is `10`.
