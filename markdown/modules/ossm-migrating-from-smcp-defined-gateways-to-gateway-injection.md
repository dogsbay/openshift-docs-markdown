...
# Migrate from SMCP-Defined gateways to gateway injection {id="ossm-migrating-from-smcp-defined-gateways-to-gateway-injection_{{ context }}"}
...
1.  Create a new ingress gateway that is configured to use gateway injection.
    ...
    1.  Ensure that the labels specified in the pod template include all of the label selectors specified in the `Service` object associated with the existing SMCP-defined gateway.
    1.  Grant access to the new gateway from outside the cluster. This access is required whenever the `spec.security.manageNetworkPolicy` of the `ServiceMeshControlPlane` resource is set to `true`, which is the default setting.
1.  Verify that the new gateway deployment is successfully handling requests.
...
1.  Scale down the old deployment and scale up the new deployment.
    ...
    1.  Increase the number of replicas for the new gateway deployment by running the following command:
        ...
        ```
    1.  Decrease the number of replicas for the old gateway deployment by running the following command:
        ```terminal
        $ oc scale -n istio-system deployment/<old_gateway_deployment> --replicas <new_number_of_replicas>
        ```
    1.  Repeat running the previous two commands. Each time, increase the number of replicas for the new gateway deployment and decrease the number of replicas for the old gateway deployment. Continue repeating until the new gateway deployment handles all traffic to the gateway `Service` object.
1.  Remove the `app.kubernetes.io/managed-by` label from the gateway `Service` object by running the following command:
    ```terminal
...
1.  Remove the `ownerReferences` object from the gateway `Service` object by running the following command:
    ```terminal
...
1.  Disable the old gateway deployment that was managed by the `ServiceMeshControlPlane` resource by running the following command:
    ```terminal
