...
# Creating a PTP Grandmaster configuration with GNSS failover on Single Node OpenShift {id="configuring-gnss-to-ntp-failover-sno_{{ context }}"}

This procedure configures a T-GM (Telecom Grandmaster) clock on {{ sno }} that uses an Intel E810 Westport Channel NIC as the PTP grandmaster clock with GNSS to NTP failover capabilities. {._abstract}
...
1.  The PTP profile uses node labels for matching. Check your machine config pool (MCP) to verify the master MCP by running the following command:
    ```terminal
    $ oc get mcp
    ```
...
1.  Create a `PtpConfig` custom resource (CR) that configures the T-GM clock with GNSS to NTP failover. Save the following YAML configuration to a file named `ptp-config-gnss-ntp-failover-sno.yaml`.
    ```yaml
...
1.  The PTP daemon checks for profile updates every 30 seconds. Wait approximately 30 seconds, then verify by running the following command:
    ...
    grandmaster    2m
    ```
1.  Check the NodePtpDevice to see if the profile is applied. First, get your {{ sno }} node name:
    ```terminal
    $ oc get nodes
    ```
    ...
    ```
1.  Check if the profile is being loaded by monitoring the daemon logs. First, get the daemon pod name:
    ```terminal
