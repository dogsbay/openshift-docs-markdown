...
*   Use a text editor to edit the `install-config.yaml` file before deploying your cluster and add one of the following stanzas:
    1.  To use confidential VMs for only control plane machines:
        ...
        `confidentialCompute`
        :   Enables confidential VMs with AMD Secure Encrypted Virtualization Secure Nested Paging (AMD SEV-SNP). For more information about available options, see "Additional {{ gcp_first }} configuration parameters".
