...
# Capturing flows {id="network-observability-cli-capturing-flows_{{ context }}"}

Capture network flows and apply filters based on resources or zones directly in the CLI. This helps you solve complex use cases, such as visualizing the Round-Trip Time (RTT) between two different zones. {._abstract}
...
1.  Capture flows with filters enabled by running the following command:
    ...
    ```
1.  Add filters to the `live table filter` prompt in the terminal to further refine the incoming flows. For example:
    ```terminal
...
1.  To stop capturing, press <kbd>Ctrl+C</kbd>. The data that was captured is written to two separate files in an `./output` directory located in the same path used to install the CLI.
1.  View the captured data in the `./output/flow/<capture_date_time>.json` JSON file, which contains JSON arrays of the captured data.
    ...
    ```
1.  You can use SQLite to inspect the `./output/flow/<capture_date_time>.db` database file. For example:
    1.  Open the file by running the following command:
        ```terminal
        $ sqlite3 ./output/flow/<capture_date_time>.db
        ```
    1.  Query the data by running a SQLite `SELECT` statement, for example:
        ```terminal
