...
# Configuring linuxptp services as an ordinary clock {id="configuring-linuxptp-services-as-ordinary-clock_{{ context }}"}

You can configure `linuxptp` services (`ptp4l`, `phc2sys`) as ordinary clock by creating a `PtpConfig` custom resource (CR) object. {._abstract}
