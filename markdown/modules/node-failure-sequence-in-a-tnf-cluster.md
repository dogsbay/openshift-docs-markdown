...
# Node failure sequence in a TNF cluster {id="node-failure-sequence-in-a-tnf-cluster_{{ context }}"}

A two-node OpenShift cluster with fencing (TNF) enters a `degraded` state when one of its two control plane nodes becomes unavailable. The active control plane remains hosted on the surviving node, allowing the cluster to remain functional within defined constraints. {._abstract}
