...
# Creating the ExternalSecretsConfig Operator {id="external-secrets-operator-create-externalsecretsconfig_{{ context }}"}

Create the `ExternalSecretsConfig` resource to install and configure the core `external-secrets` component. This setup helps ensure that features like Bitwarden and cert-manager support are correctly enabled. {._abstract}

**Prerequisites**

*   {{ external_secrets_operator }} is installed.
*   {{ cert_manager_operator }} is installed.
...
1.  Create an `externalsecretsconfig` file by defining a YAML file with the following content:
    ...
    spec:
      ...
      plugins: {}
    ```
1.  Create the `ExternalSecretsConfig` object by running the following command:
    ```terminal
...
1.  To verify that the `external-secrets` pods are in a `running` state, run the following command:
    ...
    ```
1.  To verify that the `SecretStore` CR is present, run the following command:
    ```terminal
    ...
    ```
1.  To verify that the `ExternalSecret` CR is present, run the following command:
    ```terminal
    ...
    ```
1.  To verify that the `SecretStore` is `apiVersion: external-secrets.io/v1`, run the following command:
    ```terminal
    ...
    status:
      ...
      - lastTransitionTime: "2025-10-27T11:38:19Z"
        ...
        type: Ready
    ```
1.  To verify that the `ExternalSecret` is `apiVersion: external-secrets.io/v1`, run the following command:
    ```terminal
