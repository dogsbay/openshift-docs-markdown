...
# Configuring a cluster queue {id="configuring-clusterqueues_{{ context }}"}
...
1.  Create a `ClusterQueue` object as a YAML file:
    ...
    1.  Defines the resource types governed by the cluster queue. This example `ClusterQueue` object governs CPU, memory, pod, and GPU resources.
    1.  Defines the resource flavor that is applied to the resource types listed. In this example, the `default-flavor` resource flavor is applied to CPU, memory, pod, and GPU resources.
    1.  Defines the resource requirements for admitting jobs. This example cluster queue only admits jobs if the following conditions are met:
        *   The sum of the CPU requests is less than or equal to 9.
        *   The sum of the memory requests is less than or equal to 36Gi.
        *   The total number of pods is less than or equal to 5.
        *   The sum of the GPU requests is less than or equal to 100.
1.  Apply the `ClusterQueue` object by running the following command:
    ```terminal
