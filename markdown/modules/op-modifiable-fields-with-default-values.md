...
# Modifiable fields with default values {id="op-modifiable-fields-with-default-values_{{ context }}"}
...
*   `await-sidecar-readiness` (default: `true`): Set this field to `false` to stop {{ pipelines_shortname }} from waiting for `TaskRun` sidecar containers to run before it begins to operate. This allows tasks to be run in environments that do not support the `downwardAPI` volume type.
...
*   `enable-api-fields` (default: `stable`): Setting this field determines which features are enabled. Acceptable value is `stable`, `beta`, or `alpha`.
    ...
    {{ pipelines_title }} does not support the `alpha` value.
...
*   `disable-creds-init` (default: `false`): Set this field to `true` to prevent {{ pipelines_shortname }} from scanning attached service accounts and injecting any credentials into your steps.
