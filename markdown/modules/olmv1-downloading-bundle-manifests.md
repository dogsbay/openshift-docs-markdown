...
# Downloading the bundle manifests of an extension {id="olmv1-downloading-bundle-manifests_{{ context }}"}

Use the `opm` CLI tool to download the bundle manifests of the extension that you want to install. Use the CLI tool or text editor of your choice to view the manifests and find the required permissions to install and manage the extension. {._abstract}
...
1.  Inspect the available versions and images of the extension you want to install by running the following command:
    ...
    ```
1.  Make a directory to extract the image of the bundle that you want to install by running the following command:
    ```terminal
    $ mkdir <new_dir>
    ```
1.  Change into the directory by running the following command:
    ```terminal
    $ cd <new_dir>
    ```
1.  Find the image reference of the version that you want to install and run the following command:
    ```terminal
    ...
    ```
1.  Change into the `manifests` directory by running the following command:
    ```terminal
    $ cd manifests
    ```
1.  View the contents of the manifests directory by entering the following command. The output lists the manifests of the resources required to install, manage, and operate your extension.
    ```terminal
    $ tree
    ```
    ```text title="Example output"
