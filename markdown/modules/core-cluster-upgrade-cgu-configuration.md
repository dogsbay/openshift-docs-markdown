...
# ClusterGroupUpgrade custom resource configuration {id="core-cluster-upgrade-cgu-configuration_{{ context }}"}

You can configure `ClusterGroupUpgrade` custom resources (CRs) to orchestrate cluster updates with {{ cgu_operator_full }} ({{ cgu_operator }}).
A `ClusterGroupUpgrade` CR defines which clusters to update, which policies to apply, and how to manage batching and concurrency. {._abstract}


:::important

{{ rh_rhacm }} policies used for updates must be set to `inform` mode. The `ClusterGroupUpgrade` CR temporarily changes these policies to `enforce` mode at the scheduled update time, applying them to the target clusters. When the `ClusterGroupUpgrade` CR completes or times out, the policies automatically revert to `inform` mode. This ensures that update policies are only enforced at the specific time you intend, rather than immediately when the policy is created.

:::


## Basic ClusterGroupUpgrade CR {id="_basic_clustergroupupgrade_cr"}
...
## Timeout guidelines {id="_timeout_guidelines"}
...
## Optional configuration fields {id="_optional_configuration_fields"}
...
## Canary cluster testing {id="_canary_cluster_testing"}
...
*   `<canary_cluster_name>`: Specifies the name of the canary cluster to update first. {{ cgu_operator }} updates the canary cluster first, waits for success, and then proceeds with other clusters.

## Pre-update blocking checks {id="_pre-update_blocking_checks"}
...
## Post-update label management {id="_post-update_label_management"}
...
## Chained ClusterGroupUpgrade CRs for EUS-to-EUS updates {id="_chained_clustergroupupgrade_crs_for_eus-to-eus_updates"}
...
## Applying and monitoring a ClusterGroupUpgrade CR {id="_applying_and_monitoring_a_clustergroupupgrade_cr"}
