...
# Understanding how images are removed through garbage collection {id="nodes-nodes-garbage-collection-images_{{ context }}"}

You can help ensure that your nodes are running efficiently by using image garbage collection to removes images that are not referenced by any running pods. {._abstract}

{{ product_title }} determines which images to remove from a node based on the disk usage that is reported by **cAdvisor**.
...
1.  A list of images available on a host.
