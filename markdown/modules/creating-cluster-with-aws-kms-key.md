...
# Creating a {{ product_title }} cluster using a custom AWS KMS key {id="creating-cluster-with-aws-kms-key_{{ context }}"}

You can create a {{ product_title }} cluster with a customer-provided KMS key that is used to encrypt either node root volumes, the etcd database, or both. A different KMS key ARN can be provided for each option. {._abstract}
...
1.  Verify the KMS key has been created by running the following command:
    ...
    $ echo $KMS_ARN
    ```
1.  Set your AWS account ID to an environment variable.
    ```terminal
    $ AWS_ACCOUNT_ID=<aws_account_id>
    ```
1.  Add the ARN for the account-wide installer role and operator roles that you created in the preceding step to the `Statement.Principal.AWS` section in the file. In the following example, the ARN for the default `ManagedOpenShift-HCP-ROSA-Installer-Role` role is added:
    ...
    ```
1.  Confirm the details of the policy file created by running the following command:
    ```terminal
    $ cat rosa-key-policy.json
    ```
1.  Apply the newly generated key policy to the custom KMS key by running the following command:
...
You can verify that your KMS key works by using {{ cluster_manager_url }}. 

1.  Navigate to {{ cluster_manager_url }} and select **Instances**.
1.  Select your instance.
1.  Click the **Storage** tab.
1.  Copy the **KMS key ID**.
1.  Search and select **Key Management Service**.
1.  Enter your copied _KMS key ID_ in the **Filter** field.