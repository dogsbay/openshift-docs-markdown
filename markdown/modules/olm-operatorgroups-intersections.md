...
# Operator group intersection {id="olm-operatorgroups-intersection_{{ context }}"}

Two Operator groups are said to have _intersecting provided APIs_ if the intersection of their target namespace sets is not an empty set and the intersection of their provided API sets, defined by `olm.providedAPIs` annotations, is not an empty set. {._abstract}
...
## Rules for intersection {id="olm-operatorgroups-intersection-rules_{{ context }}"}
