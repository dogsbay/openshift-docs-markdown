...
To deploy your {{ product_title }} cluster, you initialize installation by running the `openshift-install create cluster` command from the directory that contains the installation program. The installation program provisions the required infrastructure and completes the cluster setup. {._abstract}
...
{%- if gcp %}
1.  Remove any existing {{ gcp_short }} credentials that do not use the service account key for the {{ gcp_short }} account that you configured for your cluster and that are stored in the following locations:
...
{% if gcp %}
    ...
    1.  If you have not configured the service account key for your {{ gcp_short }} account on your host, you must obtain it from {{ gcp_short }} and paste the contents of the file or enter the absolute path to the file.
