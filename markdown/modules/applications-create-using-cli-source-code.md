...
{{ product_title }} automatically detects whether the pipeline, source, or docker build strategy should be used, and in the case of source build, detects an appropriate language builder image.

## Local {id="local_{{ context }}"}
...
If you use a local Git repository, the repository must have a remote named `origin` that points to a URL that is accessible by the {{ product_title }} cluster. If there is no recognized remote,  running the `new-app` command will create a binary build.

:::


## Remote {id="remote_{{ context }}"}
...
*   If a Jenkins file exists in the root or specified context directory of the source repository when creating a new application, {{ product_title }} generates a pipeline build strategy.
...
*   If neither a Jenkins file nor a Dockerfile is detected, {{ product_title }} generates a source build strategy.
...
After a language is detected, `new-app` searches the {{ product_title }} server for image stream tags that have a `supports` annotation matching the detected language, or an image stream that matches the name of the detected language. If a match is not found, `new-app` searches the [Docker Hub registry](https://registry.hub.docker.com) for an image that matches the detected language based on name.
