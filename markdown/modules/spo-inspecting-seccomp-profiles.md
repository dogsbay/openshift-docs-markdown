...
# Inspecting seccomp profiles {id="spo-inspecting-seccomp-profiles_{{ context }}"}

Corrupted `seccomp` profiles can disrupt your workloads. Ensure that the user cannot abuse the system by not allowing other workloads to map any part of the path `/var/lib/kubelet/seccomp/operator`. {._abstract}
...
1.  Confirm that the profile is reconciled by running the following command:
    ...
    ```
1.  Confirm that the `seccomp` profiles are saved into the correct path by running the following command:
    ```terminal
