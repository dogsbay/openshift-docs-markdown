...
# Nodes with Kubelet system reserved memory utilization > 50% {id="nodes-dashboard-using-identify-critical-memory-kubelet_{{ context }}"}

You can review the nodes where the Kubelet’s system-reserved memory utilization exceeds 50% by using the **Nodes with Kubelet system reserved memory utilization > 50%** query in the node metrics dashboard.  {._abstract}
