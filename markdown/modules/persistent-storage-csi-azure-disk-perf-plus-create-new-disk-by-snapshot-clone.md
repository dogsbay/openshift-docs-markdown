...
# Enabling performance plus by snapshot or cloning {id="persistent-storage-csi-azure-disk-perf-plus-create-new-disk-by-snapshot-clone_{{ context }}"}

To work around the limitation that performance plus applies only to new disks, snapshot or clone an existing volume to provision a new disk with performance plus enabled. {._abstract}
