...
# Customizing {{ pac }} configuration {id="customizing-pipelines-as-code-configuration_{{ context }}"}

To customize {{ pac }}, cluster administrators can configure the following parameters in the `TektonConfig` custom resource, in the `pipelinesAsCode.settings` spec: {._abstract}
...
| `hub-url` | The base URL for the [Tekton Hub API](https://api.hub.tekton.dev/v1). | `https://hub.tekton.dev/` |
