...
# Understanding init containers {id="nodes-containers-init-about_{{ context }}"}

You can use an init container to perform tasks before the rest of a pod is deployed. You can use init containers to perform setup tasks that should be completed before the main application logic begins, such as environment preparation, dependency checks, or configuration generation.  {._abstract}
