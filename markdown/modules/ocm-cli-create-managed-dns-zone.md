...
*   To create a managed DNS zone, run the following command:
    ...
    `--domain-prefix`
    :   Optional. Specifies a domain prefix for the DNS zone. If the {{ product_title }} cluster name is longer than 15 characters, the domain prefix is used to ensure the generated domain name does not exceed DNS length limits.


`--project-id`
:   Specifies the ID of the {{ GCP }} project where the DNS zone will be created.
