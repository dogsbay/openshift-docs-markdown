...
# Required {{ gcp_short }} permissions for shared VPC installations {id="minimum-required-permissions-ipi-gcp-xpn_{{ context }}"}

When you are installing a cluster to a [shared VPC](https://cloud.google.com/vpc/docs/shared-vpc), you must configure the service account for both the host project and the service project. {._abstract}
