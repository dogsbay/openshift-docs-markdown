...
# Creating a storage pool with a PVC template {id="virt-creating-storage-pool-pvc-template_{{ context }}"}

You can create a storage pool for multiple hostpath provisioner (HPP) volumes by specifying a persistent volume claim (PVC) template in the HPP custom resource (CR). {._abstract}
