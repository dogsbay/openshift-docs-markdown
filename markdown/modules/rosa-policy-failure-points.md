...
# Potential points of failure {id="rosa-policy-failure-points_{{ context }}"}

{{ product_title }} (ROSA) provides many features and options for protecting your workloads against downtime, but applications must be architected appropriately to take advantage of these features.
...
## Container or pod failure {id="rosa-policy-container-pod-failure_{{ context }}"}
...
## Worker node failure {id="rosa-policy-worker-node-failure_{{ context }}"}
...
## Cluster failure {id="rosa-policy-container-cluster-failure_{{ context }}"}
...
Multi-AZ ROSA clusters have three control plane nodes and three infrastructure nodes that are preconfigured for high availability, one in each AZ. Control plane and infrastructure nodes have the same resiliency as worker nodes, with the added benefit of being managed completely by Red&#160;Hat.
...
## Zone failure {id="rosa-policy-container-zone-failure_{{ context }}"}
...
## Storage failure {id="rosa-policy-container-storage-failure_{{ context }}"}
