...
# Understanding custom metadata on rules and check results {id="compliance-operator-checkresult-custom-metadata-overview_{{ context }}"}

When the Compliance Operator creates or updates a `ComplianceCheckResult` object for a check that Open Security Content Automation Protocol (OpenSCAP) evaluates, it copies custom labels and annotations from the corresponding `Rule` object in the same namespace. {._abstract}
