...
# Logging in to the OpenShift CLI {id="cli-logging-in_{{ context }}"}

You can log in to the OpenShift CLI (`oc`) to access and manage your cluster. {._abstract}
...
*   You must have access to a {{ product_title }} cluster.
...
1.  When prompted, enter the required information:
    ...
    ```
    *   For the `Server` prompt, enter the {{ product_title }} server URL.
