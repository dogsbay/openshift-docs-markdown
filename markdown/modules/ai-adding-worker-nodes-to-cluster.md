...
# Adding worker nodes using the Assisted Installer REST API {id="ai-adding-worker-nodes-to-cluster_{{ context }}"}
...
*   Log in to {{ cluster_manager_url }} as a user with cluster creation privileges.
*   Install `jq`.
...
1.  Import the {{ sno }} cluster by running the following commands:
    1.  Set the `$OPENSHIFT_CLUSTER_ID` variable. Log in to the cluster and run the following command:
        ...
        ```
    1.  Set the `$CLUSTER_REQUEST` variable that is used to import the cluster:
        ```terminal
    ...
    1.  Import the cluster and set the `$CLUSTER_ID` variable. Run the following command:
        ```terminal
...
1.  Generate the `InfraEnv` resource for the cluster and set the `$INFRA_ENV_ID` variable by running the following commands:
    1.  Download the {{ cluster_manager_url_pull }}.
    1.  Set the `$INFRA_ENV_REQUEST` variable:
        ```terminal
        ...
        :   Specifies the path to the local file containing the downloaded {{ cluster_manager_url_pull }}.
    ...
    1.  Post the `$INFRA_ENV_REQUEST` to the [/v2/infra-envs](https://api.openshift.com/?urls.primaryName=assisted-service%20service#/installer/RegisterInfraEnv) API and set the `$INFRA_ENV_ID` variable:
        ```terminal
...
1.  Check that the host is ready to install by running the following command:
    ...
    ```
1.  When the previous command shows that the host is ready, start the installation using the [/v2/infra-envs/{{ infra_env_id }}/hosts/{{ host_id }}/actions/install](https://api.openshift.com/?urls.primaryName=assisted-service%20service#/installer/v2InstallHost) API by running the following command:
    ```terminal
