...
# Configuring AWS Elastic IP (EIP) addresses for a Network Load Balancer (NLB) {id="nw-ingress-aws-static-eip-nlb-configuration_{{ context }}"}

You can specify static IPs, otherwise known as elastic IPs, for your network load balancer (NLB) in the Ingress Controller. This is useful in situations where you want to configure appropriate firewall rules for your cluster network. {._abstract}

**Prerequisites**

*   You must have an installed {{ aws_full }} cluster.
