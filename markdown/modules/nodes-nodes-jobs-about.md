...
# Understanding jobs and cron jobs {id="nodes-nodes-jobs-about_{{ context }}"}

Jobs run one-time tasks and track completion status, while cron jobs schedule recurring tasks using cron expressions, supporting non-parallel, parallel with fixed completion count, and work queue execution patterns. {._abstract}
...
There are two possible resource types that allow creating run-once objects in {{ product_title }}:
...
:   A regular job is a run-once object that creates a task and ensures the job finishes.
    ...
    *   Parallel jobs with a work queue:
        ...
        *   {{ product_title }} coordinates pods to determine what each should work on or use an external queue service.
...
## Understanding how to set a maximum duration for jobs {id="jobs-set-max_{{ context }}"}
...
an execution. After reaching the specified timeout, the job is terminated by {{ product_title }}.

## Understanding how to set a job back off policy for pod failure {id="jobs-set-backoff_{{ context }}"}
...
## Understanding how to configure a cron job to remove artifacts {id="jobs-artifacts_{{ context }}"}
...
## Understanding how to automatically remove completed jobs {id="jobs-ttl_{{ context }}"}

You can specify that {{ product_title }} should delete jobs when they are finished, either `Complete` or `Failed`, in order to limit the number of objects in your cluster.

By default, for jobs that are not associated with a cron job or other controlling object, {{ product_title }} does not delete the job or its related pods when the job is finished. If you keep these artifacts in your cluster, you can view the status of finished jobs or view job logs for errors, warnings, or other diagnostic output.
...
You can configure a time-to-live (TTL) duration for jobs without a controlling object by setting the `ttlSecondsAfterFinished` parameter in the job spec. When set, {{ product_title }} deletes the jobs and any related pods immediately after the job is finished or after a specified period of time in seconds.

## Known limitations {id="jobs-limits_{{ context }}"}
