...
# Configure MetalLB address pool for VLAN {id="nw-metallb-configure-address-pool-vlan_{{ context }}"}

To precisely manage external access across a specific VLAN, configure MetalLB address pools for your cluster. Defining these pools ensures that load balancer services receive authorized IP addresses from designated network ranges for secure and consistent routing. {._abstract}
...
1.  To ensure this configuration applies to the VLAN, you need to set the `spec` `gatewayConfig.ipForwarding` to `Global`. 
    1.  Run the following command to edit the network configuration custom resource (CR):
        ...
        ```
    1.  Update the `spec.defaultNetwork.ovnKubernetesConfig` section to include the `gatewayConfig.ipForwarding` set to `Global`. The following example demonstrates this configuration:
        ```yaml
