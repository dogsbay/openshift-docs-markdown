...
# Verifying a Workload Identity Federation configuration {id="ocm-cli-verify-wif-commands_{{ context }}"}

You can verify that the configuration of resources associated with a WIF configuration are correct by running the `ocm gcp verify wif-config` command. If a misconfiguration is found, the output provides details about the misconfiguration and recommends that you update the WIF configuration. {._abstract}
