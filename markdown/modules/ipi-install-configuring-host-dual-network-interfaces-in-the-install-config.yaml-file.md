...
# Configuring host network interfaces for dual-port NIC {id="configuring-host-dual-network-interfaces-in-the-install-config-yaml-file_{{ context }}"}

Before installation, you can set the `networkConfig` configuration setting in the `install-config.yaml` file to configure host network interfaces by using NMState to support dual-port network interface controller (NIC). {._abstract}
