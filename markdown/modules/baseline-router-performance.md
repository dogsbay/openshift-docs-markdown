...
# Baseline Ingress Controller (router) performance {id="baseline-router-performance_{{ context }}"}

You can use the {{ product_title }} Ingress Controller, or router, as the ingress point for ingress traffic for applications and services that are configured by using routes and ingresses. {._abstract}
