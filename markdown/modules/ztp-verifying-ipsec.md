...
# Verifying the IPsec encryption {id="ztp-verifying-ipsec_{{ context }}"}

You can verify that the IPsec encryption is successfully applied in a managed {{ product_title }} cluster. {._abstract}
...
1.  Start a debug pod for the managed cluster by running the following command:
    ...
    ```
1.  Check that the IPsec policy is applied in the cluster node by running the following command:
    ```terminal
    ...
    src 10.1.232.10/32 dst 172.16.123.0/24
      ...
      tmpl src 10.1.232.10 dst 10.1.28.190
        proto esp reqid 16393 mode tunnel
    ```
1.  Check that the IPsec tunnel is up and connected by running the following command:
    ```terminal
    ...
    src 10.1.28.190 dst 10.1.232.10
      ...
      anti-replay esn context:
       ...
       00000000 00000000 00000000 00000000
    ```
1.  Ping a known IP in the external host subnet by running the following command:
