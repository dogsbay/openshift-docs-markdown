...
1.  Replace the default values in the `network` section of the {{ microshift_short }} YAML with your valid values.
    ```yaml title="Example single-stack IPv6 networking configuration"
    apiServer:
    # ...
    network:
      clusterNetwork:
    ...
    `node.nodeIP`
    :   Specifies a node IP address. Valid values are IP addresses in the IPv6 address family. You must only specify an IPv6 address when an IPv4 network is also present. If an IPv4 network is not present, the {{ microshift_short }} service automatically fills in this value upon restart.
1.  Complete any other configurations you require, then start {{ microshift_short }} by running the following command:
    ```terminal
...
1.  Retrieve the status of the pods by running the following command:
    ...
    ```
1.  Retrieve the status of services by running the following command:
    ```terminal
    $ oc get svc -A
    ```
    ```text title="Example output"
