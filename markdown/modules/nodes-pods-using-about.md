...
# Understanding pods {id="nodes-pods-using-about_{{ context }}"}

To run and manage your application containers in {{ product_title }}, you must use pods. Pods allow multiple containers to share the same network and storage resources on a single host. {._abstract}
...
{{ product_title }} treats pods as largely immutable; changes cannot be made to
a pod definition while it is running. {{ product_title }} implements changes by
...
For the maximum number of pods per {{ product_title }} node host, see the Cluster Limits.
