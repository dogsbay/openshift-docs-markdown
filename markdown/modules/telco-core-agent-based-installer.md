...
# Agent-based Installer {id="telco-core-agent-based-installer_{{ context }}"}

The Agent-based Installer provides an installation method for {{ product_title }} in environments without existing deployment infrastructure. {._abstract}
...
:   The recommended method for Telco Core cluster installation is using Red Hat Advanced Cluster Management.
    ...
    Use the ABI to install {{ product_title }} on bare-metal servers without requiring additional servers or VMs for managing the installation, but does not provide ongoing lifecycle management, monitoring or automations.
