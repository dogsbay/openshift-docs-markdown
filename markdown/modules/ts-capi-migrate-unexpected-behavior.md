...
# Unexpected behavior when changing resource configurations {id="ts-capi-migrate-unexpected-behavior_{{ context }}"}

On clusters that support migrating resources between the Machine API and the Cluster API, users might experience unexpected resource behavior when updating the authoritative API. {._abstract}
...
:   Do not change other values when you update the value of the `spec.authoritativeAPI` field.
    For more information, see [OCPBUGS-74638](https://issues.redhat.com/browse/OCPBUGS-74638).