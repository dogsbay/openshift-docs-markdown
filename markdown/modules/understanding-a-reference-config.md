...
# Understanding a reference configuration {id="understanding-a-reference-config_{{ context }}"}

The `cluster-compare` plugin uses a reference configuration to validate a configuration from a live cluster.
The reference configuration consists of a YAML file called `metadata.yaml`, which references a set of templates that represent the baseline configuration. {._abstract}
