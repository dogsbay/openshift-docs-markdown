...
# Running a storage checkup by using the CLI {id="virt-checking-storage-configuration_{{ context }}"}

Use a predefined checkup to verify that the {{ product_title }} cluster storage is configured optimally to run {{ VirtProductName }} workloads. {._abstract}
...
1.  Create a `ServiceAccount`, `Role`, and `RoleBinding` manifest file for the storage checkup.
    ...
    roleRef:
      ...
      name: storage-checkup-role
    ```
1.  Apply the `ServiceAccount`, `Role`, and `RoleBinding` manifest in the target namespace:
    ```terminal
    $ oc apply -n <target_namespace> -f <storage_sa_roles_rolebinding>.yaml
    ```
1.  Create a `ConfigMap` and `Job` manifest file. The config map contains the input parameters for the checkup job.
    ...
    ```
1.  Apply the `ConfigMap` and `Job` manifest file in the target namespace to run the checkup:
    ```terminal
...
1.  Wait for the job to complete:
    ...
    ```
1.  Review the results of the checkup by running the following command:
    ```terminal
...
*   `data.status.result.ocpVersion` defines the {{ product_title }} version.
...
*   `data.status.result.vmsWithUnsetEfsStorageClass` defines the list of virtual machines that use an Elastic File Store (EFS) storage class where the GID and UID are not set in the storage class.
    1.  Delete the job and config map that you previously created by running the following commands:
        ...
        ```
    1.  Optional: If you do not plan to run another checkup, delete the `ServiceAccount`, `Role`, and `RoleBinding` manifest:
        ```terminal
