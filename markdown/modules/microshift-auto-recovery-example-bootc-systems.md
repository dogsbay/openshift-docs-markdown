...
# Use automatic recovery in image mode for {{ op_system_base }} systems {id="microshift-auto-recovery-example-bootc-systems_{{ context }}"}

To use automatic recovery for {{ product_title }} on {{ op_system_base }} image-based systems, you can embed the `10-auto-recovery.conf` and `microshift-auto-recovery.service` files in your Containerfile and rebuild the bootc image. {._abstract}


:::important

You must include the entire `auto-recovery` process for {{ op_system_image }} systems that use `systemd` in the container file.
...
*   You created a Containerfile as instructed in [Building the bootc image](https://docs.redhat.com/en/documentation/red_hat_build_of_microshift/4.18/html/installing_with_rhel_image_mode/installing-with-rhel-image-mode#microshift-rhel-image-mode-build-image_microshift-install-rhel-image-mode).
...
1.  Use the following example snippet to update the container file that you use to prepare the {{ op_system_image }} image.
    ```text
...
1.  Rebuild your local bootc image by running the following image build command:
    ```terminal
