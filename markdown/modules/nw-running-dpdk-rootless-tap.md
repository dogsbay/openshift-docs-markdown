...
1.  Create the new `Namespace` object by running the following command:
    ...
    ```
1.  Create a file, such as `sriov-node-network-policy.yaml`, with content such as the following example:
    ```yaml
...
1.  Create the `SriovNetwork` object by running the following command:
    ...
    ```
1.  Create a file, such as `tap-example.yaml`, that defines a network attachment definition, with content such as the following example:
    ```yaml
...
1.  Create the `NetworkAttachmentDefinition` object by running the following command:
    ...
    ```
1.  Create a file, such as `dpdk-pod-rootless.yaml`, with content such as the following example:
    ```yaml
    apiVersion: v1
    kind: Pod
    metadata:
