...
1.  Check BGP session status by running the following command:
    ...
    ```
1.  Check `FRRNodeState` to see the BGP configuration on each node by running the following command:
    ```terminal
    ...
    ```
1.  View the detailed BGP configuration and running state by running the following command for a specific node:
    ```terminal
...
1.  Verify that the system is advertising routes by checking the `FRRNodeState` resource for the relevant node with this command:
    ```terminal
