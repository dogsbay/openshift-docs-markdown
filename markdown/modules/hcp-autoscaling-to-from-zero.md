...
## Terminology {id="hcp-autoscaling-terminology_{{ context }}"}


`AutoscalingEnabled` condition
:   A `NodePool` status condition that reports whether autoscaling is correctly configured and active on the pool.
...
## Use cases for scale-to-zero autoscaling {id="hcp-scale-to-zero-use-cases_{{ context }}"}
...
## Best practices for scale-to-zero autoscaling {id="hcp-scale-to-zero-best-practices_{{ context }}"}
...
:   Configure scale-to-zero only on {{ aws_short }} or {{ azure_short }} hosted clusters running {{ product_title }} 4.18 or later with a HyperShift Operator release that includes the backport.
