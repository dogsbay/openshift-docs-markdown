# Release notes for {{ gitops_title }} 1.3 {id="gitops-release-notes-1-3_{{ context }}"}

{{ gitops_title }} 1.3 is now available on {{ product_title }} 4.7, 4.8, 4.9, and 4.6 with limited GA support.

## New features {id="new-features-1-3_{{ context }}"}
In addition to the fixes and stability improvements, the following sections highlight what is new in {{ gitops_title }} 1.3.0:
...
*   While migrating from the Dex authentication provider to the Keycloak provider, you may experience login issues with Keycloak. [GITOPS-1450](https://issues.redhat.com/browse/GITOPS-1450)
