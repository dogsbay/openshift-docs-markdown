...
# Specifying multiple regions and zones for your cluster on vSphere {id="specifying-regions-zones-infrastructure-vsphere_{{ context }}"}

You can configure the `infrastructures.config.openshift.io` configuration resource to specify multiple regions and zones for your {{ product_title }} cluster that runs on a VMware vSphere instance. {._abstract}

Topology-aware features for the cloud controller manager and the vSphere Container Storage Interface (CSI) Operator Driver require information about the vSphere topology where you host your {{ product_title }} cluster. This topology information exists in the `infrastructures.config.openshift.io` configuration resource.
...
1.  Save the resource file to apply the changes.