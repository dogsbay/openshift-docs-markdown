...
{% if user_managed_lb %}
# Example load balancer configuration for clusters that are deployed with user-managed load balancers {id="_example_load_balancer_configuration_for_clusters_that_are_deployed_with_user-managed_load_balancers"}
...
:   The bootstrap entries must be in place before the {{ product_title }} cluster installation and they must be removed after the bootstrap process is complete.
