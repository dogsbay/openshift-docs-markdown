...
# status parameter {id="ossm-cr-status_{{ context }}"}
...
| `conditions` | Represents the latest available observations of the object’s current state. `Reconciled` indicates that the Operator has finished reconciling the actual state of deployed components with the configuration in the `ServiceMeshControlPlane` resource. `Installed` indicates that the {{ SMProductShortName }} control plane has been installed. `Ready` indicates that all {{ SMProductShortName }} control plane components are ready. | string |
