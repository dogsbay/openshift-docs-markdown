...
# Filter network flows at ingestion {id="network-observability-filter-network-flows-at-ingestion_{{ context }}"}

Create filters to reduce the number of generated network flows. Filtering network flows can reduce the resource usage of the network observability components. {._abstract}
...
## eBPF agent filters {id="ebpf-agent-filters_{{ context }}"}
...
## Flowlogs-pipeline filters {id="flowlogs-pipeline-filters_{{ context }}"}
