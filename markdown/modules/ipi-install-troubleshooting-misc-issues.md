...
# Miscellaneous issues {id="ipi-install-troubleshooting-misc-issues_{{ context }}"}

## Addressing the `runtime network not ready` error {id="_addressing_the_runtime_network_not_ready_error"}
...
1.  Inspect the pods in the `openshift-network-operator` namespace:
    ...
    ```
1.  On the `provisioner` node, determine that the network configuration exists:
    ```terminal
    ...
    ```
1.  Check that the `network-operator` is running:
    ```terminal
...
## Addressing the "No disk found with matching rootDeviceHints" error message {id="_addressing_the_no_disk_found_with_matching_rootdevicehints_error_message"}
...
## Cluster nodes not getting the correct IPv6 address over DHCP {id="_cluster_nodes_not_getting_the_correct_ipv6_address_over_dhcp"}
...
1.  In the IP address reservation on the DHCP server, ensure the reservation specifies the correct DHCP Unique Identifier (DUID). For example:
    ```terminal
...
## Cluster nodes not getting the correct hostname over DHCP {id="_cluster_nodes_not_getting_the_correct_hostname_over_dhcp"}
...
1.  Force the cluster node to renew the DHCP lease:
    ```terminal
...
1.  Check `hostname` again:
    ```terminal
    [core@master-X ~]$ hostname
    ```
1.  If the hostname is still `localhost.localdomain`, restart `NetworkManager`:
    ```terminal
    [core@master-X ~]$ sudo systemctl restart NetworkManager
    ```
1.  If the hostname is still `localhost.localdomain`, wait a few minutes and check again. If the hostname remains  `localhost.localdomain`, repeat the previous steps.
1.  Restart the `nodeip-configuration` service:
    ```terminal
    [core@master-X ~]$ sudo systemctl restart nodeip-configuration.service
    ```

    This service will reconfigure the `kubelet` service with the correct hostname references.
1.  Reload the unit files definition since the kubelet changed in the previous step:
    ```terminal
...
1.  Restart the `kubelet` service:
    ...
    ```
1.  Ensure `kubelet` booted with the correct hostname:
    ```terminal
...
1.  Get CSRs on the cluster:
    ...
    $ oc get csr
    ```
1.  Verify if a pending `csr` contains `Subject Name: localhost.localdomain`:
    ```terminal
    $ oc get csr <pending_csr> -o jsonpath='{.spec.request}' | base64 --decode | openssl req -noout -text
    ```
1.  Remove any `csr` that contains `Subject Name: localhost.localdomain`:
    ```terminal
    $ oc delete csr <wrong_csr>
    ```

## Routes do not reach endpoints {id="_routes_do_not_reach_endpoints"}

During the installation process, it is possible to encounter a Virtual Router Redundancy Protocol (VRRP) conflict. This conflict might occur if a previously used {{ product_title }} node that was once part of a cluster deployment using a specific cluster name is still running but not part of the current {{ product_title }} cluster deployment using that same cluster name. For example, a cluster was deployed using the cluster name `openshift`, deploying three control plane (master) nodes and three worker nodes. Later, a separate install uses the same cluster name `openshift`, but this redeployment only installed three control plane (master) nodes, leaving the three worker nodes from a previous deployment in an `ON` state. This might cause a Virtual Router Identifier (VRID) conflict and a VRRP conflict.
...
1.  Check the service endpoint:
    ...
    ```
1.  Attempt to reach the service from a control plane (master) node:
    ```terminal
    ...
    ```terminal
    {
      ...
      "code": 403
    ```
1.  Identify the `authentication-operator` errors from the `provisioner` node:
    ```terminal
...
1.  Turn off all the rogue nodes which are not part of the cluster deployment that are using the same cluster name. Otherwise, the authentication pod of the  {{ product_title }} cluster might never start successfully.