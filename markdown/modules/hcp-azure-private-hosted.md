...
# Creating a private {{ azure_short }} hosted cluster {id="hcp-azure-private-hosted_{{ context }}"}

Create a private hosted cluster to ensure that the communication between compute nodes and the hosted control plane occurs over {{ azure_short }} Private Link. {._abstract}

**Prerequisites**

*   You configured a subnet, as described in "Preparing a subnet for a private hosted cluster on {{ azure_short }}".
*   You installed the HyperShift Operator on an {{ product_title }} management cluster on {{ azure_short }}, as described in "Installing the HyperShift Operator with private platform support".
...
1.  Create the private hosted cluster by entering the following command:
    ...
    *   `--endpoint-access` accepts 3 values:
        ...
        *   `Private`, where the API server is accessible only through Private Link.
            ...
            If you need to allow `Private` endpoint connections from {{ azure_short }} subscriptions other than the hosted cluster’s subscription, use the following flag: `--endpoint-access-private-additional-allowed-subscriptions "sub-id-1, sub-id-2"`.
...
1.  Check the Private Link resources by entering the following command:
    ...
    ```
1.  Check the status and connections by entering the following command:
    ```terminal
...
1.  Check the overall cluster status by entering the following command:
    ```terminal
    $ oc get hostedcluster ${CLUSTER_NAME} -n clusters
    ```
1.  Wait for the status by entering the following command:
    ```terminal
