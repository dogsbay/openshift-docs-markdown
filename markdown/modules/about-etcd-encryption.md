...
# etcd encryption {id="about-etcd_{{ context }}"}

You can encrypt sensitive resource data in etcd to provide an additional layer of protection if an etcd backup or storage data is exposed. {._abstract}

By default, etcd data is not encrypted in {{ product_title }}. You can enable etcd encryption for your cluster to provide an additional layer of data security. For example, it can help protect the loss of sensitive data if an etcd backup is exposed to the incorrect parties.
