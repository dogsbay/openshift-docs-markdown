...
# Enable the Network Observability Operator FlowCollectorSlice {id="network-observability-per-tenant-flowcollector-slice-enable_{{ context }}"}

Enabling the `FlowCollectorSlice` feature in the `FlowCollector` resource allows cluster administrators to delegate flow collection and data enrichment management to specific namespaces. {._abstract}
...
1.  Edit the `FlowCollector` custom resource by running the following command:
    ...
    ```
1.  Configure the `spec.processor.slicesConfig` field to define which namespaces are permitted to use slices:
    ```yaml
