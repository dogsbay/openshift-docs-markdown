...
# Kernel module deployment {id="kmm-deploy-kernel-modules_{{ context }}"}

Kernel Module Management (KMM) monitors `Node` and `Module` resources on {{ product_title }} to load or unload kernel modules on eligible nodes. KMM creates worker pods on target nodes to reconcile the desired module state. {._abstract}
