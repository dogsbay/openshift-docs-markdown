...
# Listing the OVN-Kubernetes southbound database contents {id="nw-ovn-kubernetes-list-southbound-database-contents_{{ context }}"}

To understand OVN-Kubernetes (OVN-K) logical networking entities, examine the southbound database running as a container inside the `ovnkube-node` pod. Each node is controlled by the `ovnkube-controller` container running in that pod on the node. {._abstract}
...
1.  Optional: To list the pods with node information, enter the following command:
    ...
    ```
1.  Open a remote shell into the `nbdb` or `sbdb` containers on the relevant node to examine the southbound database.
1.  Show all the objects in the southbound database by entering the following command:
    ```terminal
    $ ovn-sbctl show
    ```
...
1.  Display the options available with the `ovn-sbctl` command by entering the following command:
    ```terminal
