...
1.  Ensure that `etcd` is running on each of the control plane nodes by running the following command:
    ...
    ```
1.  If the previous command fails, ensure that Kubelet created the `etcd` pods by running the following command:
    ```terminal
    $ sudo crictl pods --name=etcd-member
    ```

    If there are no pods, investigate `etcd`.
1.  Check the cluster nodes to ensure they have a fully qualified domain name, and not just `localhost.localdomain`, by using the following command:
    ```terminal
    $ hostname
    ```
