...
# Switching the Ingress Controller from using a Network Load Balancer to a Classic Load Balancer {id="nw-aws-switching-nlb-with-clb_{{ context }}"}

To support specific networking configurations in {{ product_title }} on {{ aws_full }}, switch an Ingress Controller using a Network Load Balancer (NLB) to one that uses a Classic Load Balancer (CLB).  {._abstract}
