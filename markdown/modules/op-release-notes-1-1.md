# Release notes for {{ pipelines_title }} Technology Preview 1.1 {id="op-release-notes-1-1_{{ context }}"}

## New features {id="new-features-1-1_{{ context }}"}
{{ pipelines_title }} Technology Preview (TP) 1.1 is now available on {{ product_title }} 4.5. {{ pipelines_title }} TP 1.1 is updated to support:
...
In addition to the fixes and stability improvements, the following sections highlight what is new in {{ pipelines_title }} 1.1.

### Pipelines {id="pipeline-new-features-1-1_{{ context }}"}

*   Workspaces can now be used instead of pipeline resources. It is recommended that you use workspaces in {{ pipelines_shortname }}, as pipeline resources are difficult to debug, limited in scope, and make tasks less reusable. For more details on workspaces, see the Understanding {{ pipelines_shortname }} section.
...
*   An Affinity Assistant is now available to support node affinity in task runs that share workspace volume. By default, this is disabled on {{ pipelines_shortname }}.
...
### Pipelines CLI {id="cli-new-features-1-1_{{ context }}"}
...
### Triggers {id="triggers-new-features-1-1_{{ context }}"}
...
## Deprecated features {id="deprecated-features-1-1_{{ context }}"}
