...
# Scheduling workloads on edge and cloud-based AWS compute resources {id="aws-outposts-network-scheduling-workloads_{{ context }}"}

When you extend an AWS VPC cluster into an Outpost, the Outpost uses edge compute nodes and the VPC uses cloud-based compute nodes. {._abstract}
