...
# Completing the cluster configuration {id="virt-install-ibm-cloud-complete-cluster-config_{{ context }}"}

Complete the cluster configuration by installing software on the control plane and compute nodes and configuring DNS for external access. {._abstract}
...
1.  For each bare-metal server, perform the following tasks:
    ...
    1.  Mount the Assisted Installer ISO file with the following attributes:
...
1.  Wait for the **Assisted Installer** interface to indicate each node is ready.
1.  Click **Next**.
1.  Select **Cluster Managed Network**.
1.  Select the **API VIP** and **Ingress VIP** checkboxes to obtain them from DHCP or leave them unchecked to enter static values.
1.  Click **Install**.
1.  For each bare-metal server, perform the following tasks:
    1.  Access the server using the IPMI console.
...
1.  Perform the following tasks in the **Cluster Credentials** section:
    ...
    1.  Save the `kubeadmin` password.
1.  Install `haproxy` on the Bastion virtual server instance.
1.  Configure `haproxy` for your environment. The following is an example configuration:
    ```text
    #---------------------------------------------------------------------
...
1.  Save the `haproxy` configuration.
1.  Configure two DNS Address records (A records) for the subdomain that are externally available over the Internet:
    ```text
...
1.  Perform the following tasks to verify cluster access using the web console:
    ...
    1.  Log in to the cluster with the username `kubeadmin` and the `kubeadmin` password provided in the **Cluster Credentials** section.