...
(ROSA)
{%- endif %}
clusters is controlled through several layers of required authentication, all of which are managed by strict company policy. All authentication attempts to access a cluster and changes made within a cluster are recorded within audit logs, along with the specific account identity of the SRE responsible for those actions. These audit logs help ensure that all changes made by SREs to a customer’s cluster adhere to the strict policies and procedures that make up Red&#160;Hat’s managed services guidelines. {._abstract}
...
## SRE access to AWS accounts {id="rosa-policy-sre-aws-infra-access_{{ context }}"}
Red&#160;Hat personnel do not access AWS accounts in the course of routine {{ product_title }} operations. For emergency troubleshooting purposes, the SREs have well-defined and auditable procedures to access cloud infrastructure accounts.
...
For a full list of permissions, see `sts_support_permission_policy.json` in the [About IAM resources](https://docs.openshift.com/rosa/rosa_architecture/rosa-sts-about-iam-resources.html) user guide.

## SRE access through PrivateLink VPC endpoint service {id="rosa-sre-access-privatelink-vpc_{{ context }}"}
