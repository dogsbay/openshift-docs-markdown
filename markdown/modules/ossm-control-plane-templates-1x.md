...
# Creating control plane templates {id="ossm-control-plane-templates-1x_{{ context }}"}
...
1.  From the CLI, run this command to create the ConfigMap named `smcp-templates` in the `openshift-operators` project and replace `<templates-directory>` with the location of the `ServiceMeshControlPlane` files on your local disk:
    ```terminal
    $ oc create configmap --from-file=<templates-directory> smcp-templates -n openshift-operators
    ```
1.  Locate the Operator ClusterServiceVersion name.
    ```terminal
    ...
    ```
1.  Edit the Operator cluster service version to instruct the Operator to use the `smcp-templates` ConfigMap.
    ```terminal
    $ oc edit clusterserviceversion -n openshift-operators maistra.v1.0.0
    ```
1.  Add a volume mount and volume to the Operator deployment.
    ```yaml
    deployments:
      - name: istio-operator
        spec:
...
1.  You can now use the `template` parameter in the `ServiceMeshControlPlane` to specify a template.
    ```yaml
