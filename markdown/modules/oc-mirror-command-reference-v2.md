...
# Command reference for oc-mirror plugin v2 {id="oc-mirror-command-reference-v2_{{ context }}"}

You can use the `oc mirror` subcommands and flags to modify the behavior of the oc-mirror plugin v2 and control your mirroring workflow. {._abstract}
...
| Flag | Description |
| --- | --- |
| `--authfile` | Displays the string path of the authentication file. Default is `${{ XDG_RUNTIME_DIR }}/containers/auth.json`{minja}. |
