...
# Minimizing worker node deployment time {id="minimizing-worker-node-deployment-time_{{ context }}"}
...
1.  Optional: If you set the `paused` parameter to `true` to apply additional configurations, complete the following steps:
    ...
    1.  Unpause the MCPs to start the configuration phase and reboot if needed. Clusters must be deployed to access the API and run `oc` commands:
        ```terminal
