...
{% endif %}
{% if post_aws_zones %}
# Wavelength Zones only: Creating a VPC carrier gateway {id="_wavelength_zones_only_creating_a_vpc_carrier_gateway"}

{% endif %}

To use public subnets in your {{ product_title }} cluster that runs on Wavelength Zones, you must create the carrier gateway and associate the carrier gateway to the VPC. Subnets are useful for deploying load balancers or edge compute nodes. {._abstract}

To create edge nodes or internet-facing load balancers in Wavelength Zones locations for your {{ product_title }} cluster, you must create the following required network components:
