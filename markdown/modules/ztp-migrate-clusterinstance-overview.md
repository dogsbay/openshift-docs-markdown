...
# Overview of migrating from SiteConfig CRs to ClusterInstance CRs {id="ztp-migrate-clusterinstance-overview_{{ context }}"}
...
The SiteConfig Operator, which manages the `ClusterInstance` custom resource (CR), is a fully developed controller shipped as an add-on within {{ rh_rhacm_first }}.
...
1.  To migrate the clusters incrementally, first remove the associated `SiteConfig` CR from the old pipeline. Then, add a corresponding `ClusterInstance` CR to the new pipeline.
...
1.  When you complete the cluster migration, delete the original Argo project and application and clean up any related resources.
