...
# Excluding the SR-IOV network topology for NUMA-aware scheduling {id="nw-sriov-configure-exclude-topology-manager_{{ context }}"}

To exclude advertising the SR-IOV network resource’s Non-Uniform Memory Access (NUMA) node to the Topology Manager, you can configure the `excludeTopology` specification in the `SriovNetworkNodePolicy` custom resource. Use this configuration for more flexible SR-IOV network deployments during NUMA-aware pod scheduling. {._abstract}
...
1.  Create the `SriovNetworkNodePolicy` CR:
    ...
    1.  Create the `SriovNetworkNodePolicy` resource by running the following command. Successful output lists the name of the `SriovNetworkNodePolicy` resource and the `created` status.
        ```terminal
...
1.  Create the `SriovNetwork` CR:
    ...
    1.  Create the `SriovNetwork` resource by running the following command. Successful output lists the name of the `SriovNetwork` resource and the `created` status.
        ```terminal
...
1.  Create a pod and assign the SR-IOV network resource from the previous step:
    ...
    1.  Create the `Pod` resource by running the following command. The expected output shows the name of the `Pod` resource and the `created` status.
        ```terminal
