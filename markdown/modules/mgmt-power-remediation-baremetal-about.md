...
# About power-based remediation of bare metal {id="mgmt-power-remediation-baremetal-about_{{ context }}"}

You can configure and deploy a machine health check to detect and repair unhealthy bare-metal nodes, which is critical to ensuring the overall health of the cluster.  {._abstract}
...
{{ product_title }} uses the `MachineHealthCheck` controller to detect faulty bare metal nodes. Power-based remediation is fast and reboots faulty nodes instead of removing them from the cluster.
...
## About machine health checks on bare metal {id="mgmt-power-remediation-baremetal-about-health-checks_{{ context }}"}
