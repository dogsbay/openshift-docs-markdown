...
# Adding triggers to a pipeline {id="adding-triggers_{{ context }}"}
...
1.  Copy the content of the following sample `TriggerBinding` YAML file and save it:
    ...
    ```
1.  Create the `TriggerBinding` resource:
    ```terminal
...
1.  Create the `TriggerTemplate` resource:
    ```terminal
...
1.  Create the `EventListener` resource by performing the following steps:
    *   To create an `EventListener` resource using a secure HTTPS connection:
        ...
        1.  Create a route with the re-encrypt TLS termination:
            ...
            ```yaml title="Example Re-encrypt TLS Termination YAML of the Secured Route"
    ...
    *   To create an `EventListener` resource using an insecure HTTP connection:
        1.  Create the `EventListener` resource.
        1.  Expose the `EventListener` service as an {{ product_title }} route to make it publicly accessible:
            ```terminal
