...
# Using generic webhooks {id="builds-using-generic-webhooks_{{ context }}"}
...
1.  To call the webhook manually, enter the following `curl` command:
    ```terminal
...
1.  To pass this payload using `curl`, define it in a file named `payload_file.yaml` and run the following command:
    ```terminal
