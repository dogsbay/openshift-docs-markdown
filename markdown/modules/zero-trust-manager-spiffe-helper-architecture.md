...
# SPIFFE Helper architecture {id="zero-trust-manager-spiffe-helper-architecture_{{ context }}"}

The SPIFFE Helper working pattern demonstrates how to apply the same certificate and identity flow to your own workloads. {._abstract}
...
Each namespace defines a `ClusterSPIFFEID` so {{ spire_full }} issues X.509 {{ svid_full }}s with the correct DNS names: the server identity includes `postgresql-spiffe.postgresql-spiffe.svc`; the client identity includes `postgresql_spiffe`, which must match the PostgreSQL user created during database initialization.
