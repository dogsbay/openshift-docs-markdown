...
# Immutable and certified containers {id="security-registries-immutable_{{ context }}"}

Immutable containers are containers that will never be changed while running. You do not step into the running immutable container to replace one or more binaries. From an operational standpoint, you rebuild and redeploy an updated container image to replace a container instead of changing it. {._abstract}
...
*   Compatible across the {{ op_system_base }} platforms, from bare metal to cloud
