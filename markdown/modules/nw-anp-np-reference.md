...
# Key differences between AdminNetworkPolicy and NetworkPolicy custom resources {id="nw-anp-differences-networkpolicy_{{ context }}"}

To choose the right network policy API for your cluster or namespace in {{ product_title }}, you can compare `AdminNetworkPolicy` and `NetworkPolicy` CRs across scope, rule actions, precedence, and peer options.  {._abstract}
