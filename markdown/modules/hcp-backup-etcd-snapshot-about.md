...
# Overview of the etcd snapshot method {id="hcp-backup-etcd-snapshot-about_{{ context }}"}

You can use the etcd snapshot method as an alternative to the default volume snapshot approach to back up etcd data for {{ hcp }}. The etcd snapshot method results in smaller backup artifacts. {._abstract}
...
The etcd snapshot approach is driven by the `HCPEtcdBackup` custom resource and is orchestrated through the {{ oadp_first }} HyperShift plugin during Velero backup operations.
...
| Encryption | Depends on the storage provider. | Optional Key Management Service (KMS) encryption for {{ hcp }} on {{ aws_short }}. |