...
# API compatibility exceptions {id="api-compatibility-exceptions_{{ context }}"}

The following are exceptions to compatibility in {{ product_title }}:

{% if not microshift %}

## RHEL CoreOS file system modifications not made with a supported Operator {id="OS-file-system-modifications-not-made_{{ context }}"}
...
## Modifications to cluster infrastructure in cloud or virtualized environments {id="modifications-to-cluster-infrastructure-in-cloud_{{ context }}"}
...
## Functional defaults between an upgraded cluster and a new installation {id="Functional-defaults-between-upgraded-cluster-new-installation_{{ context }}"}
...
## Usage of API fields that have the prefix "unsupported” or undocumented annotations {id="API-fields-that-have-the-prefix-unsupported-annotations_{{ context }}"}
...
## API availability per product installation topology {id="API-availability-per-product-installation-topology_{{ context }}"}
