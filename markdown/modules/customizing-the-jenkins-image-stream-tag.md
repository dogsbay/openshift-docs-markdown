...
# Customizing the Jenkins image stream tag {id="customizing-the-jenkins-image-stream-tag_{{ context }}"}
...
*   `ocp-upgrade-redeploy`, the default value, updates your Jenkins image when you upgrade {{ product_title }}.
*   `user-maintained-upgrade-redeploy` requires you to manually redeploy Jenkins by running `$ oc import-image jenkins:user-maintained-upgrade-redeploy -n openshift` after upgrading {{ product_title }}.
...
*   You are running OpenShift Jenkins on {{ product_title }} {{ product_version }}.
