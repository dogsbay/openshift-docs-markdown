...
# Go runtime metrics {id="serverless-go-metrics_{{ context }}"}
...
| `go_last_gc` | The time that the last garbage collection was completed in [_Epoch_ or Unix time](https://en.wikipedia.org/wiki/Unix_time). | Gauge | `name` | Nanoseconds |
| `go_total_gc_pause_ns` | The cumulative time in garbage collection _stop-the-world_ pauses since the program started. | Gauge | `name` | Nanoseconds |
