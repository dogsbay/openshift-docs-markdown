...
# Re-enabling the etcd quorum guard {id="enabling-etcd-quorum-guard_{{ context }}"}

Re-enable the etcd quorum guard after completing the control plane node replacement to restore normal cluster operation and verify that the cluster has returned to a healthy three-member etcd configuration. {._abstract}
...
1.  Turn the quorum guard back on by running the following command:
    ...
    ```
1.  Verify that the `unsupportedConfigOverrides` section is removed by running the following command:
    ```terminal
    $ oc get etcd/cluster -oyaml
    ```
1.  If using {{ sno }}, restart the node. Otherwise, you might encounter the following error in the etcd cluster Operator:
...
1.  Verify that all etcd members are healthy by running the following command:
    ...
    ```
1.  Exit the rsh session by running the following command:
    ```terminal
    sh-4.2# exit
    ```
1.  Verify that all nodes are at the latest revision by running the following command:
    ```terminal
