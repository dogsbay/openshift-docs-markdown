...
# Configure a Google identity provider {id="config-google-idp_{{ context }}"}

Configure a Google identity provider to allow users to authenticate with their Google credentials. {._abstract}
...
{% endif %}
        ...
        ```
1.  Configure a Google identity provider using [Google OpenID Connect integration](https://developers.google.com/identity/protocols/OpenIDConnect).
