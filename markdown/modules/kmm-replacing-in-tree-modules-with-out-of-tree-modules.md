...
# Replacing in-tree modules with out-of-tree modules {id="kmm-replacing-in-tree-modules-with-out-of-tree-modules_{{ context }}"}

You can use Kernel Module Management (KMM) to build kernel modules that can be loaded or unloaded into the kernel on demand. These modules extend the functionality of the kernel without the need to reboot the system. Modules can be configured as built-in or dynamically loaded. {._abstract}
