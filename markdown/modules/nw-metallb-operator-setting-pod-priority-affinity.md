...
# Configuring pod priority and pod affinity in a MetalLB deployment {id="nw-metallb-operator-setting-pod-priority-affinity_{{ context }}"}

To control scheduling of MetalLB controller and `speaker` pods in {{ product_title }}, you can assign pod priority and pod affinity in the `MetalLB` custom resource. You create a `PriorityClass` and set `priorityClassName` and affinity in the `MetalLB` spec, then apply the configuration. {._abstract}
...
1.  Apply the `MetalLB` custom resource configuration by running the following command:
    ```terminal
