...
# Deploying a virtualized control plane with the Agent-based installer {id="proc_virt-installing-vcp-abi_{{ context }}"}

Use the Agent-based Installer to deploy a virtualized control plane cluster.
This method generates a bootable ISO containing all required configuration. You must mount the ISO to both the virtualized control plane nodes, by using KubeVirt Redfish, and the baremetal worker nodes, by using the Redfish API. {._abstract}
...
1.  Create an `agent-config.yaml` YAML file similar to the following example:
    ```yaml
    apiVersion: v1alpha1
    kind: AgentConfig
    metadata:
...
1.  Generate the agent ISO by running the following command:
    ```terminal
    $ openshift-install agent create image --dir <installation_directory>
    ```
1.  Host the generated `agent.x86_64.iso` on an HTTP server accessible from your hosting cluster.
1.  Boot each node from the agent ISO:
    1.  For virtualized control plane nodes, use KubeVirt Redfish to mount the ISO and power on the VMs.
    1.  For baremetal worker nodes, use the server’s BMC interface to mount the ISO and boot.
