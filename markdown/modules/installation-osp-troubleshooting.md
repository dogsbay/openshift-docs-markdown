# Troubleshooting {{ product_title }} on OpenStack installations {id="installation-osp-customizing_{{ context }}"}

Unfortunately, there will always be some cases where {{ product_title }} fails to install properly. In these events, it is helpful to understand the likely failure modes as well as how to troubleshoot the failure.

This document discusses some troubleshooting options for {{ rh_openstack }}-based
...
## View instance logs {id="_view_instance_logs"}

{{ rh_openstack }} CLI tools must be installed, then:

```
$ openstack console log show <instance>
```

## Connect to instances via SSH {id="_connect_to_instances_via_ssh"}
