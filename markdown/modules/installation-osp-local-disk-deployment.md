...
# Deploying OpenStack on local disk {id="installation-osp-local-disk-deployment_{{ context }}"}

If you have an existing {{ rh_openstack }} cloud, you can move etcd from that cloud to a dedicated ephemeral local disk. {._abstract}

**Prerequisites**

*   You have an {{ rh_openstack }} cloud with a working Cinder.
*   Your {{ rh_openstack }} cloud has at least 75 GB of available storage to accommodate 3 root volumes for the OpenShift control plane.
*   The {{ rh_openstack }} cloud is deployed with Nova ephemeral storage that uses a local storage backend and not `rbd`.
...
1.  Create a Nova flavor for the control plane with at least 10 GB of ephemeral disk. Replace the values for `--ram`, `--disk`, and `<flavor_name>` based on your environment.
    ...
    ```
1.  Deploy a cluster with root volumes for the control plane; for example:
    ...
    # ...
    ```
1.  Deploy the cluster you created by running the following command:
    ```terminal
...
1.  Verify that the cluster you deployed is healthy before proceeding to the next step by running the following command:
    ```terminal
...
1.  Create a file named `98-var-lib-etcd.yaml` by using the following YAML file:
    ```yaml
...
1.  Verify that the etcd databases has been transferred to the local disk of each control plane by running the following commands:
    1.  Verify that the cluster is still updating by running the following command:
        ...
        ```
    1.  Verify that the cluster is ready by running the following command:
        ```terminal
        $ oc wait node --selector='node-role.kubernetes.io/master' --for condition=Ready --timeout=30s
        ```
    1.  Verify that the cluster Operators are running in the cluster by running the following command:
        ```terminal
