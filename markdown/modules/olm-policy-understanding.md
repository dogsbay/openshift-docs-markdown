...
# Understanding Operator installation policy {id="olm-policy-understanding_{{ context }}"}

By default, Operator Lifecycle Manager (OLM) runs with `cluster-admin` privileges and grants any permissions that an Operator author specifies in its cluster service version (CSV). {._abstract}
