{% if context == "deploying-egress-router-ovn-redirection" %}
{%- set redirect = true -%}
{%- set router_type = "redirect" -%}
{% endif %}
{%- set router_name = "egress-router-{{ router_type }}" -%}

{%- set _mod_docs_content_type = "REFERENCE" %}
# Egress router custom resource {id="nw-egress-router-ovn-cr_{{ context }}"}

You can define the configuration for an egress router pod in an egress router custom resource.  {._abstract}

The following YAML describes the fields for the configuration of an egress router in {{ router_type }} mode:
