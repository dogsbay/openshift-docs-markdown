...
# ClusterObjectSets deployment mechanism {id="olmv1-clusterobjectsets-deployment-mechanism_{{ context }}"}

`ClusterObjectSets` deploy cluster extensions through ordered phases, enabling safe upgrades by maintaining both old and new revisions until the new version succeeds. {._abstract}

{%- set FeatureName = "{{ olmv1 }} ClusterObjectSets" %}
...
## Benefits {id="olmv1-clusterobjectsets-benefits_{{ context }}"}
...
## Relationship to the deploymentConfig API {id="olmv1-clusterobjectsets-relationship_{{ context }}"}
...
## Deployment phases {id="olmv1-clusterobjectsets-phases_{{ context }}"}
