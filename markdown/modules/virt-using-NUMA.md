...
# Using NUMA topology with {{ VirtProductName }} {id="virt-using-NUMA_{{ context }}"}

You must enable the NUMA functionality for {{ VirtProductName }} VMs to prevent performance degradation on nodes with multiple NUMA zones. This feature is vital for high-performance and latency-sensitive workloads. {._abstract}
