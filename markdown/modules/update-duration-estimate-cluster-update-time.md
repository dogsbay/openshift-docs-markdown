...
# How to estimate cluster update time {id="estimating-cluster-update-time_{{ context }}"}

Historical update duration of similar clusters provides you the best estimate for the future cluster updates. If you do not have historical data, you can calculate an estimate of the update duration. {._abstract}
...
For example, to estimate the update time, consider an {{ product_title }} cluster with three control plane nodes and six compute nodes, where each host takes about 5 minutes to reboot.
...
The default setting for `maxUnavailable` is `1` for all the MCPs in {{ product_title }}. It is recommended that you do not change the `maxUnavailable` in the control plane MCP.
