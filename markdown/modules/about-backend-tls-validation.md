...
# About backend TLS validation {id="about-backend-tls-validation_{{ context }}"}

To ensure secure end-to-end encryption, you can use a `BackendTLSPolicy` custom resource (CR) to validate your backend certificates. This validation ensures that your `Gateway` CR securely connects to the correct destination pod. {._abstract}
