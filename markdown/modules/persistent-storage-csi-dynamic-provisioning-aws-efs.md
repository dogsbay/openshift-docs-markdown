...
# Dynamic provisioning for Amazon Elastic File Storage {id="csi-dynamic-provisioning-aws-efs_{{ context }}"}

To dynamically provision persistent volumes (PVs) as subdirectories of an existing Elastic File System (EFS) volume, create persistent volume claims (PVCs) referencing your EFS storage class to enable multiple independent volumes sharing the same resource. {._abstract}
