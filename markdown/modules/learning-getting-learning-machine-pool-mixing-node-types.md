...
# Mixing node types {id="learning-getting-started-learning-machine-pool-mixing-node-types_{{ context }}"}
...
1.  For example, to change the database nodes to a different node type, run the following command:
    ...
    **For example**:
    ...
    ```
1.  To see all the [instance types available](https://docs.openshift.com/rosa/rosa_architecture/rosa_policy_service_definition/rosa-instance-types.html), run the following command:
    ```terminal
