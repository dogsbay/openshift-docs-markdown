...
# Introduction to {{ hcp }} {id="hosted-control-planes-overview_{{ context }}"}

{{ hcp_capital }} is available by using a supported version of {{ mce }} on several platforms. {._abstract}
...
*   {{ ibm_z_title }}
*   {{ ibm_power_title }}
*   {{ rh_openstack_first }} 17.1, as a Technology Preview feature
...
The {{ mce_short }} is an integral part of {{ rh_rhacm_first }} and is enabled by default with {{ rh_rhacm }}. However, you do not need {{ rh_rhacm }} to use {{ hcp }}.

:::


## Architecture of {{ hcp }} {id="hosted-control-planes-architecture_{{ context }}"}

{{ product_title }} is often deployed in a coupled, or standalone, model, where a cluster consists of a control plane and a data plane. The control plane includes an API endpoint, a storage endpoint, a workload scheduler, and an actuator that ensures state. The data plane includes compute, storage, and networking where workloads and applications run.
...
*   Because the control planes consist of pods that are launched on {{ product_title }}, control planes start quickly. The same principles apply to control planes and workloads, such as monitoring, logging, and auto-scaling.
