# Providing secure connection with OpenShift routes {id="op-providing-secure-connection_{{ context }}"}
...
```yaml title="Example re-encrypt TLS termination YAML to create a secure route"
...
1.  This is required for re-encryption. The `destinationCACertificate` field specifies a CA certificate to validate the endpoint certificate, thus securing the connection from the router to the destination pods. You can omit this field in either of the following scenarios:
