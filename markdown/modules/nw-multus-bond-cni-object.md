...
# Configuration for a Bond CNI secondary network {id="nw-multus-bond-cni-object_{{ context }}"}

The Bond Container Network Interface (Bond CNI) enables the aggregation of multiple network interfaces into a single logical bonded interface within a container, which enhanches network redundancy and fault tolerance. Only SR-IOV Virtual Functions (VFs) are supported for bonding with this plugin. {._abstract}
...
{%- set FeatureName = "xmitHashPolicy" %}
{% include "./snippets/technology-preview.md" %}

## Bond CNI plugin configuration example {id="nw-multus-bond-cni-config-example_{{ context }}"}
