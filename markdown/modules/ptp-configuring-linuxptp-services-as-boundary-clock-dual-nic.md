...
# Configuring linuxptp services as boundary clocks for dual-NIC hardware {id="ptp-configuring-linuxptp-services-as-bc-for-dual-nic_{{ context }}"}

You can configure the `linuxptp` services (`ptp4l`, `phc2sys`) as boundary clocks for dual-NIC hardware by creating a `PtpConfig` custom resource (CR) object for each NIC. {._abstract}
