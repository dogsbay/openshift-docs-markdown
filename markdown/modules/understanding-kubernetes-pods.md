...
# Kubernetes pods and services {id="understanding-kubernetes-pods_{{ context }}"}

While the container image is the basic unit with Docker, the basic units that Kubernetes works with are called pods.
Pods represent the next step in building out your application.  {._abstract}
...
name and the {{ product_title }} cluster resolves that name into the IP addresses
