...
In {{ product_title }} {{ product_version }}, you can deploy a cluster into existing subnets in an existing Virtual Private Cloud (VPC) in {{ gcp_first }} to avoid limit constraints in new accounts or to comply with operational constraints set by your organization. {._abstract}

By deploying {{ product_title }} into an existing {{ gcp_short }} VPC, you might be able to avoid limit constraints in new accounts or more easily abide by the operational constraints that your company’s guidelines set. If you cannot obtain the infrastructure creation permissions that are required to create the VPC yourself, use this installation option. You must configure networking for the subnets.
