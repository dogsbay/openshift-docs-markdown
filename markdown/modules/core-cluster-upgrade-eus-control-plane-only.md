...
# Updating with EUS-to-EUS control-plane-only updates {id="core-cluster-upgrade-eus-control-plane-only_{{ context }}"}

You can update clusters from one Extended Update Support (EUS) release to another using control-plane-only updates.
This approach updates the control plane while leaving worker nodes at the previous EUS version, enabling staged rollouts that minimize workload disruption. {._abstract}
...
1.  Validate that your cloud-native network function (CNF) workloads tolerate control plane and worker version skew:
            ...
            - complianceType: musthave
              objectDefinition:
                ...
                spec:
                  paused: true
    ```
1.  Verify the current cluster channel before creating the update policy by running the following command:
    ```terminal
...
1.  Get the release image reference for the target version.
...
1.  Create a policy for the EUS update.
...
1.  Apply the policies to {{ rh_rhacm }} by running the following commands:
    ...
    ```
1.  Create a `ClusterGroupUpgrade` custom resource (CR) for the EUS update:
    ```yaml
...
1.  Apply and enable the `ClusterGroupUpgrade` CR to start the EUS update by running the following commands:
    ```terminal
    ...
    $ oc patch cgu core-eus-upgrade-4.20-to-4.22 \
      ...
      -p '{"spec":{"enable":true}}'
    ```
1.  Monitor the EUS control plane update progress by running the following commands:
    ```terminal
...
1.  After the control plane update completes, verify control plane and worker node status by running the following commands:
    ```terminal
...
1.  Update OLM Operators for the new EUS version:
    ...
    ```
1.  When ready to update worker nodes, create and apply an unpause policy:
    ...
    Follow the procedure in "Pausing and unpausing worker nodes by using {{ cgu_operator }}" to unpause and update workers.
1.  Monitor worker node updates to completion by running the following command:
    ```terminal
...
1.  Verify all cluster Operators are healthy by running the following command:
    ...
    $ oc --context=spoke1 get co
    ```
1.  Validate CNF-specific resources by running the following commands:
    ```terminal
    ...
    ```
1.  Run comprehensive smoke tests for all CNF workloads.
