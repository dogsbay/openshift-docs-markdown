...
# Create control plane VMs {id="proc_virt-creating-vcp-vms_{{ context }}"}

Create VMs on the hosting cluster that will become the control plane nodes for your virtualized control plane cluster. {._abstract}
...
1.  Enable the `declarativeHotplugVolumes` feature gate on the hosting cluster by running the following command:
    ```terminal
...
1.  Create a `VirtualMachine` CR for each control plane node by creating a YAML file with content such as the following example:
    ```yaml
