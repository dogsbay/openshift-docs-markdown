...
# Accepting risks with the oc adm upgrade recommend command {id="oc-adm-upgrade-recommend-accept_{{ context }}"}

You can use a command flag to explicitly accept update risks that are shown in the output of the `oc adm upgrade recommend` command. {._abstract}
...
1.  Accept update risks by running the following command:
    ```terminal
