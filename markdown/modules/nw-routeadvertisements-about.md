...
# Advertise cluster network routes with Border Gateway Protocol {id="nw-routeadvertisements-about_{{ context }}"}

To simplify routing and improve failover visibility without manual route management, you can enable route advertisements. Route advertisements allow you to advertise default and user-defined network routes, including EgressIPs, between your cluster and the provider network. {._abstract}
...
## Infrastructure requirements {id="infrastructure-requirements_{{ context }}"}
...
## Compatibility with other networking features {id="compatibility-with-other-networking-features_{{ context }}"}

Route advertisements support the following {{ product_title }} Networking features:
...
## Considerations for use with the MetalLB Operator {id="considerations-for-use-with-the-metallb-operator_{{ context }}"}
...
## Considerations for naming cluster user-defined networks (CUDNs) {id="considerations-for-naming-cluster-user-defined-networks_{{ context }}"}
...
## BGP routing custom resources {id="bgp-routing-custom-resources_{{ context }}"}
...
## OVN-Kubernetes controller generation of `FRRConfiguration` objects {id="ovn-kubernetes-controller-generation-of-frrconfiguration-objects_{{ context }}"}
...
## Cluster Network Operator configuration {id="cluster-network-operator_{{ context }}"}
