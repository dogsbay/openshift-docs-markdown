...
# Creating an ARM or AMD NodePool object on {{ aws_short }} hosted clusters {id="hcp-create-np-arm64-aws_{{ context }}"}

You can schedule application workloads that are the `NodePool` objects on 64-bit ARM and AMD from the same hosted control plane. To set the required processor architecture for the `NodePool` object, you define the `arch` field in the `NodePool` specification.  {._abstract}
...
*   Add an ARM or AMD `NodePool` object to the hosted cluster on {{ aws_short }} by running the following command:
    ```terminal
