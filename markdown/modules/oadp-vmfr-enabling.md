...
# Enable {{ oadp_short }} VMFR {id="oadp-vmfr-enabling_{{ context }}"}

Enable {{ oadp_short }} virtual machine file restore (VMFR) by configuring the `DataProtectionApplication` (DPA) custom resource (CR) with the `vmFileRestore` section. You can allow file-level restore operations for VM backups. {._abstract}
...
1.  To verify that the `oadp-vm-file-restore-controller-manager` pod is running, run the following command:
    ```terminal
