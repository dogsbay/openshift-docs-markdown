...
# Restoring a cluster manually from an etcd backup {id="manually-restoring-cluster-etcd-backup_{{ context }}"}

Manually restore your cluster from an etcd backup by starting a three-member etcd cluster on existing control plane nodes, without re-creating nodes as required by the standard restore procedure. {._abstract}
...
When you restore your cluster, you must use an etcd backup that was taken from the same z-stream release. For example, an {{ product_title }} 4.7.2 cluster must use an etcd backup that was taken from 4.7.2.
...
1.  Copy the etcd backup directory to each control plane host.
...
1.  Stop the static pods on all the control plane nodes; one host at a time.
    1.  Move the existing Kubernetes API Server static pod manifest out of the kubelet manifest directory.
        ...
        ```
    1.  Verify that the Kubernetes API Server containers have stopped with the command:
        ```terminal
        $ crictl ps | grep kube-apiserver | grep -E -v "operator|guard"
        ```

        The output of this command should be empty. If it is not empty, wait a few minutes and check again.
    1.  If the Kubernetes API Server containers are still running, terminate them manually with the following command:
        ```terminal
    ...
    1.  Repeat the same steps for `kube-controller-manager-pod.yaml`, `kube-scheduler-pod.yaml` and **finally** `etcd-pod.yaml`.
        1.  Stop the `kube-controller-manager` pod with the following command:
            ...
            ```
        1.  Check if the containers are stopped using the following command:
            ```terminal
            $ crictl ps | grep kube-controller-manager | grep -E -v "operator|guard"
            ```
        1.  Stop the `kube-scheduler` pod using the following command:
            ```terminal
        ...
        1.  Check if the containers are stopped using the following command:
            ...
            ```
        1.  Stop the `etcd` pod using the following command:
            ```terminal
        ...
        1.  Check if the containers are stopped using the following command:
            ...
            ```
1.  On each control plane host, save the current `etcd` data, by moving it into the `backup` folder:
    ```terminal
...
1.  Find the correct etcd parameters for each control plane host.
    1.  The value for `<ETCD_NAME>` is unique for the each control plane host, and it is equal to the value of the `ETCD_NAME` variable in the manifest `/etc/kubernetes/static-pod-resources/etcd-certs/configmaps/restore-etcd-pod/pod.yaml` file in the specific control plane host. It can be found with the command:
        ...
        cat $RESTORE_ETCD_POD_YAML | \
          ...
          grep -Po '(?<=value: ").+(?=")'
        ```
    1.  The value for `<UUID>` can be generated in a control plane host with the command:
        ```terminal
        $ uuidgen
        ```
    ...
    1.  The value for `ETCD_NODE_PEER_URL` should be set like the following example:
        ```terminal
        ...
        $ echo <ETCD_NAME> | \
          ...
          grep "IP" | grep -Po '(?<=").+(?=")'
        ```
    1.  The value for `<ETCD_INITIAL_CLUSTER>` should be set like the following, where `<ETCD_NAME_n>` is the `<ETCD_NAME>` of each control plane host. 
...
1.  Regenerate the etcd database from the backup. 
    ...
    1.  Copy the `etcd` backup to `/var/lib/etcd` directory with the command:
        ...
        ```
    1.  Identify the correct `etcdctl` image before proceeding. Use the following command to retrieve the image from the backup of the pod manifest:
        ```terminal
        ...
        ```
    1.  Check that the version of the `etcdctl` tool is the version of the `etcd` server where the backup was created:
        ```terminal
        $ etcdctl version
        ```
    1.  Run the following command to regenerate the `etcd` database, using the correct values for the current host: 
        ```terminal
...
1.  Record the values printed in the `added member` logs; for example:
    ...
    1.  Repeat these steps on the other control plane hosts, checking that the values printed in the `added member` logs are the same for all control plane hosts.
1.  Move the regenerated `etcd` database to the default location. 
    ...
    1.  Move the regenerated database (the `member` folder created by the previous `etcdctl snapshot restore` command) to the default etcd location `/var/lib/etcd`:
        ...
        ```
    1.  Restore the SELinux context for `/var/lib/etcd/member` folder on `/var/lib/etcd` directory:
        ```terminal
        $ restorecon -vR /var/lib/etcd/
        ```
    1.  Remove the leftover files and directories:
        ```terminal
...
1.  Restart the etcd cluster.
    ...
    1.  Move the `etcd` static pod manifest back to the kubelet manifest directory, in order to make kubelet start the related containers :
        ...
        ```
    1.  Verify that all the `etcd` containers have started:
        ```terminal
...
1.  Check the status of the `etcd` cluster.
    1.  On any of the control plane hosts, check the status of the `etcd` cluster with the following command:
        ...
        +--------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
        ```
1.  Restart the other static pods.
    ...
    1.  Move the Kubernetes API Server static pod manifest back to the kubelet manifest directory to make kubelet start the related containers with the command:
        ...
        ```
    1.  Verify that all the Kubernetes API Server containers have started:
        ```terminal
    ...
    1.  Repeat the same steps for `kube-controller-manager-pod.yaml` and `kube-scheduler-pod.yaml` files.
        1.  Restart the kubelets in all nodes using the following command:
            ```terminal
            $ systemctl restart kubelet
            ```
        1.  Start the remaining control plane pods using the following command:
            ```terminal
            $ mv /root/manifests-backup/kube-* /etc/kubernetes/manifests/
            ```
        1.  Check if the `kube-apiserver`, `kube-scheduler` and `kube-controller-manager` pods start correctly:
            ```terminal
            $ crictl ps | grep -E 'kube-(apiserver|scheduler|controller-manager)' | grep -v -E 'operator|guard'
            ```
        1.  Wipe the OVN databases using the following commands:
            ```terminal
