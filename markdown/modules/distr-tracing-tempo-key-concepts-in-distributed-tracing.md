...
# Key concepts in distributed tracing {id="distr-tracing-tempo-key-concepts-in-distributed-tracing_{{ context }}"}
...
You can combine {{ DTShortName }} with other relevant components of the {{ product_title }}:

*   {{ OTELName }} for forwarding traces to a TempoStack instance
*   Distributed tracing UI plugin of the {{ coo_first }}