...
# Deploying GPU workloads with the Dynamic Accelerator Slicer Operator {id="das-operator-deploying-workloads_{{ context }}"}
...
1.  Create a namespace by running the following command:
    ...
    ```
1.  Create a deployment that requests GPU resources using the NVIDIA MIG resource:
    ```yaml
    apiVersion: apps/v1
    kind: Deployment
    metadata:
    ...
    ```
1.  Apply the deployment configuration by running the following command:
    ```terminal
    $ oc apply -f cuda-vectoradd-deployment.yaml
    ```
1.  Verify that the deployment is created and pods are scheduled by running the following command:
    ```terminal
...
1.  Check that `AllocationClaim` resources were created for your deployment pods by running the following command:
    ...
    ```
1.  Verify that the GPU slices are properly allocated by checking one of the pod’s resource allocation by running the following command:
    ```terminal
    $ oc describe pod -l app=cuda-vectoradd
    ```
1.  Check the logs to verify the CUDA sample application runs successfully by running the following command:
    ```terminal
    ...
    Test PASSED
    ```
1.  Check the environment variables to verify that the GPU devices are properly exposed to the container by running the following command:
    ```terminal
