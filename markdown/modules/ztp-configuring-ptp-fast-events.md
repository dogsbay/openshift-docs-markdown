...
1.  Apply the following `{{ policy_gen_cr }}`{minja} changes to `{{ policy_prefix }}group-du-3node-ranGen.yaml`{minja}, `{{ policy_prefix }}group-du-sno-ranGen.yaml`{minja}, or `{{ policy_prefix }}group-du-standard-ranGen.yaml`{minja} files according to your requirements:
    1.  In `{{ rangen_yaml_path }}`{minja}, add the `PtpOperatorConfig` CR file that configures the transport host:
        ```yaml {minja}
    ...
    1.  Configure the `linuxptp` and `phc2sys` for the PTP clock type and interface. For example, add the following YAML into `{{ rangen_yaml_path }}`{minja}:
{% if policy-gen-cr == "PolicyGenTemplate" %}
