...
# Deploying a leader worker set {id="lws-config_{{ context }}"}

You can use the {{ lws_operator }} to deploy a leader worker set to assist with managing distributed workloads across nodes. {._abstract}
...
1.  Create a new project by running the following command:
    ...
    ```
1.  Create a file named `leader-worker-set.yaml`
    ```yaml
...
1.  Review the stateful sets by running the following command:
    ```terminal
    $ oc get statefulsets
    ```
    ```terminal title="Example output"
