...
:   You can manage the link state of a primary or secondary VM network interface by using the {{ product_title }} web console or the command line. By specifying the link state, you can logically connect or disconnect the virtual network interface controller (vNIC) from a network.
...
:   You can configure the IP address of a secondary network interface when you create a VM. The IP address is provisioned with cloud-init. You can view the IP address of a VM by using the {{ product_title }} web console or the command line. The network information is collected by the QEMU guest agent.
