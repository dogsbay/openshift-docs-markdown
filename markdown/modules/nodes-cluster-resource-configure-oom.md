...
# Understanding OOM kill policy {id="nodes-cluster-resource-configure-oom_{{ context }}"}

{{ product_title }} can kill a process in a container if the total memory usage of all the processes in the container exceeds the memory limit, or in serious cases of node memory exhaustion. {._abstract}
