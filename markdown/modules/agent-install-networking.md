...
# About networking {id="agent-install-networking_{{ context }}"}

The **rendezvous IP** must be known at the time of generating the agent ISO, so that during the initial boot all the hosts can check in to the assisted service. {._abstract}
...
## Port requirements for the rendezvous host {id="agent-install-networking-ports_{{ context }}"}
...
This specific port requirement is in addition to the standard {{ product_title }} networking requirements for installation.

## DHCP {id="agent-install-networking-DHCP_{{ context }}"}
...
## Static networking {id="agent-install-networking-static_{{ context }}"}
