...
# AdminNetworkPolicy {id="adminnetworkpolicy_{{ context }}"}

An `AdminNetworkPolicy` (ANP) is a cluster-scoped custom resource definition (CRD) that you can define to establish ingress and egress rules before namespaces are created. You can use ANPs to ensure tenant `NetworkPolicy` objects cannot override your administrative controls. {._abstract}
...
## AdminNetworkPolicy example {id="adminnetworkpolicy-example_{{ context }}"}
