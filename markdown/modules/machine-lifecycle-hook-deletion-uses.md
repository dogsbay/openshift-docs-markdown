...
# Machine deletion lifecycle hook examples for Operator developers {id="machine-lifecycle-hook-deletion-uses_{{ context }}"}

Operators can use lifecycle hooks for the machine deletion phase to modify the machine deletion process. {._abstract}

The following examples demonstrate possible ways that an Operator can use this functionality.

## Example use cases for `preDrain` lifecycle hooks {id="machine-lifecycle-hook-deletion-uses-predrain_{{ context }}"}
...
## Example use cases for `preTerminate` lifecycle hooks {id="machine-lifecycle-hook-deletion-uses-preterminate_{{ context }}"}
