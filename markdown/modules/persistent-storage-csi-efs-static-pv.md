...
# Creating static PVs with Amazon Elastic File Storage {id="efs-create-static-pv_{{ context }}"}

To mount an entire Amazon Elastic File Storage (EFS) volume as a single persistent volume without dynamic provisioning, create a static persistent volume (PV) that allows pods to access the full volume. {._abstract}
