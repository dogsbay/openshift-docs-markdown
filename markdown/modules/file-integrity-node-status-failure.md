...
# FileIntegrityNodeStatus CR failure status example {id="file-integrity-node-status-failure_{{ context }}"}

To simulate a failure condition, modify one of the files AIDE tracks. For example, modify `/etc/resolv.conf` on one of the worker nodes: {._abstract}
