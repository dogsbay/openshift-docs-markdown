...
*   You have set up authentication credentials in a `Secret` object for the remote write endpoint. You must create the secret in the `{{ namespace_name }}`{minja} namespace.
...
1.  Edit the `{{ configmap_name }}`{minja} config map in the `{{ namespace_name }}`{minja} project:
    ...
    ```
1.  Add a `remoteWrite:` section under `data/config.yaml/{{ component }}`{minja}, as shown in the following example:
    ```yaml {minja}
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
1.  Add write relabel configuration values after the authentication credentials:
    ...
    ```yaml title="Example of forwarding a single metric called my_metric" {minja}
    apiVersion: v1
    kind: ConfigMap
    metadata:
      name: {{ configmap_name }}
        ...
        {{ component }}:
          ...
          - url: "https://remote-write-endpoint.example.com"
            ...
            - sourceLabels: [__name__]
              ...
              action: keep
    ```
    ```yaml title="Example of forwarding metrics called my_metric_1 and my_metric_2 in my_namespace namespace" {minja}
    apiVersion: v1
    kind: ConfigMap
    metadata:
      name: {{ configmap_name }}
