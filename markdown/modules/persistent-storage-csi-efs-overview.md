...
# Overview {id="efs-driver-overview_{{ context }}"}

{{ product_title }} is capable of provisioning persistent volumes (PVs) using the Container Storage Interface (CSI) driver for AWS Elastic File Service (EFS). {._abstract}
...
After installing the AWS EFS CSI Driver Operator, {{ product_title }} installs the AWS EFS CSI Operator and the AWS EFS CSI driver by default in the `openshift-cluster-csi-drivers` namespace. This allows the AWS EFS CSI Driver Operator to create CSI-provisioned PVs that mount to AWS EFS assets.
