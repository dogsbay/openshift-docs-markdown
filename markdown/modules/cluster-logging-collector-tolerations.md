...
# Using tolerations to control log collector pod placement {id="cluster-logging-collector-tolerations_{{ context }}"}
...
*   You have installed the {{ clo }} and {{ oc_first }}.
...
1.  Edit the `collection` stanza of the `ClusterLogging` custom resource (CR) to configure a toleration for the logging collector pods:
    ...
    1.  Specify the `Exists` operator to require the `key`/`value`/`effect` parameters to match.
    1.  Specify the `NoExecute` effect.
    1.  Optionally, specify the `tolerationSeconds` parameter to set how long a pod can remain bound to a node before being evicted.
