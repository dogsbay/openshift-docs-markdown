...
# Storage considerations for cluster logging and {{ product_title }} {id="efk-logging-deploy-storage-considerations_{{ context }}"}
...
unless you specify otherwise the Cluster Logging Custom Resource. The initial set of {{ product_title }} nodes might not be large enough
to support the Elasticsearch cluster. You must add additional nodes to the  {{ product_title }} cluster to run with the recommended
...
A persistent volume is required for each Elasticsearch deployment to have one data volume per data node. On {{ product_title }} this is achieved using
...
Below are capacity planning guidelines for {{ product_title }} aggregate logging.
...
Calculating total logging throughput and disk space required for your {{ product_title }} cluster requires knowledge of your applications. For example, if one of your
...
is necessary to
[keep absolute storage consumption around 50% and below 70% at all times](https://signalfx.com/blog/how-we-monitor-and-run-elasticsearch-at-scale/). This
