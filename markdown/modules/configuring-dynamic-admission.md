...
The webhook server is also configured as an aggregated API server. This allows other {{ product_title }} components to communicate with the webhook that uses internal credentials and facilitates testing that uses the `oc` command. Additionally, this enables role-based access control (RBAC) into the webhook and prevents token information from other API servers from being disclosed to the webhook.

**Prerequisites**

*   An {{ product_title }} account with cluster administrator access.
