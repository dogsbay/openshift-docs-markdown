...
# Preflight permissions check for cluster extensions (Technology Preview) {id="olmv1-troubleshooting-rbac-errors-with-preflight-check_{{ context }}"}

Before installing an extension, the Operator Controller performs a dry run to verify service account access. This process confirms that the service account can create all required Kubernetes resources and RBAC rules defined by the extension bundle. {._abstract}

{%- set FeatureName = "The preflight permissions check for cluster extensions" %}
...
## Example report from the preflight permissions check {id="olmv1-preflight-permissions-check-output_{{ context }}"}
...
`Verbs`
:   Specifies the actions, or _verbs_, that the service account needs permission to perform. If the report lists several verbs, all of the listed verbs require RBAC rules.

## Common permission errors {id="olmv1-common-rbac-errors_{{ context }}"}
