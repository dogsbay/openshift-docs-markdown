...
*   A hosted cluster on {{ aws_short }} or {{ azure_short }} running {{ product_title }} 4.18 or later
...
*   For {{ aws_short }} and {{ azure_short }}, scale-from-zero provider credentials

**Procedure**

1.  Confirm that your hosted cluster platform supports `spec.autoScaling.min: 0`.

    On {{ aws_short }} and {{ azure_short }}, the `NodePool` API accepts `min: 0`. On other platforms, `min` must be at least `1`.
1.  Confirm that the HyperShift Operator enables scale-from-zero.

    On {{ aws_short }} and {{ azure_short }}, confirm that `--scale-from-zero-provider` and `--scale-from-zero-creds` are configured so instance-type metadata is available.
1.  Enable autoscaling on the target `NodePool` object by removing fixed replicas and setting autoscaling bounds.
...
1.  Optional: Configure hosted cluster-level autoscaling behavior on the `HostedCluster` resource.
...
1.  Verify that the `AutoscalingEnabled` condition on the `NodePool` is `True`.
    ```terminal
...
1.  Verify scale-down to zero by removing schedulable workloads from the pool.
...
1.  Verify scale-up from zero by deploying a workload that creates pending pods on the pool.
...
1.  Extract the hosted cluster kubeconfig and verify node count changes.
    ```terminal
