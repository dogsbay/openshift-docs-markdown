...
# Verifying that the PTP events REST API v1 consumer application is receiving events {id="ptp-verifying-events-consumer-app-is-receiving-events-{{ ptp_events_rest_api }}_{{ context }}"}
...
Run the following command:
    ...
    ```
1.  Access the metrics for the required consumer-side `cloud-event-proxy` container by running the following command:
    ```terminal
