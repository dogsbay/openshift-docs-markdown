...
# Creating a VM from a template {id="virt-creating-vm-from-template_{{ context }}"}

You can create a virtual machine (VM) from a template with an available boot source by using the {{ product_title }} web console. You can customize template or VM parameters, such as data sources, Cloud-init, or SSH keys, before you start the VM. {._abstract}
...
1.  Click **All templates** to view the available templates for your filters.
...
1.  Click a template tile to view its details.
    ...
    *   If you need to customize the template or VM parameters, do the following:
        ...
        1.  Click the **Scripts** tab to edit the parameters that must be set before the VM boots, such as `Cloud-init`, `SSH key`, or `Sysprep` (Windows VM only).
        1.  Optional: Click the **Start this virtualmachine after creation (Always)** checkbox.
