...
# Installing FlexVolume drivers {id="flexvolume-installing_{{ context }}"}

You can implement FlexVolumes by using a list of operations to call and the installation path are all that is required. FlexVolume drivers that are used to extend {{ product_title }} are executed only on the node.  {._abstract}
