...
1.  On the command line, run the following command to display information about a required worker:
    ...
    Events:                       <none>
    ```
1.  Run the `oc describe` command again:
    ```terminal
    $ oc describe machineconfigpool/worker
    ```
...
1.  Confirm that the `policy.json` file exists with the following command:
    ```terminal
    ...
    ```
1.  Confirm that the `registry.redhat.io.yaml` file exists with the following command:
    ```terminal
    ...
    ```
1.  Confirm that the `registry.access.redhat.com.yaml` file exists with the following command:
    ```terminal
