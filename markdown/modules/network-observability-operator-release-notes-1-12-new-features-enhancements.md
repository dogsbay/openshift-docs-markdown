...
# Network Observability Operator 1.12 new features and enhancements {id="network-observability-operator-release-notes-1-12-new-features-enhancements_{{ context }}"}

The Network Observability Operator 1.12 release introduces non-decrypting TLS metadata tracking, Kafka message compression options, automated secondary network indexing, and expanded web console compatibility for {{ product_title }} clusters. {._abstract}
...
{{ product_title }} web console compatibility
:   The Network Observability web console plugin is updated to support {{ product_title }} 4.22 and later. Backward compatibility is maintained for {{ product_title }} versions 4.14 through 4.21.