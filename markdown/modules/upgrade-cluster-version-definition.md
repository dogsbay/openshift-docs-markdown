# ClusterVersion definition {id="upgrade-cluster-version-definition_{{ context }}"}
...
1.  A list of overrides for components that the CVO manages. Mark
...
1.  The status of available updates and any in-progress updates. These values display
...
cannot modify them.
1.  The list of appropriate updates for the cluster. This list is empty if no
updates are recommended, the update service is unavailable, or you specified
an invalid channel.
1.  The condition of the CVO. This section contains both the reason that the
