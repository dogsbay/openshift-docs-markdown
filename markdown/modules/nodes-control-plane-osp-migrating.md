...
# Migrating control plane nodes from one RHOSP host to another manually {id="nodes-control-plane-osp-migrating_{{ context }}"}

If control plane machine sets are not enabled on your cluster, you can run a script that moves a control plane node from one {{ rh_openstack_first }} node to another. {._abstract}
...
*   The environment variable `KUBECONFIG` refers to a configuration that contains administrative {{ product_title }} credentials.
...
*   From a command line, run the following script:
    ...
    If the script completes, the control plane machine is migrated to a new {{ rh_openstack }} node.