...
# Adding a control plane node to your cluster {id="creating-control-plane-node_{{ context }}"}

Add a control plane node to recover from a degraded state, perform deep-level debugging, or ensure stability and security of the control plane in complex bare metal scenarios.  {._abstract}
...
1.  Create the `BareMetalHost` and `Machine` CRs and link them to the `Node` CR of the control plane node.  
    1.  Create the `BareMetalHost` CR with a unique `.metadata.name` value as demonstrated in the following example:
        ...
        # ...
        ```
    1.  Apply the `BareMetalHost` CR by entering the following command:
        ```terminal
        $ oc apply -f <filename>
        ```

        where `<filename>` specifies the name of the `BareMetalHost` CR.
    1.  Create the `Machine` CR by using the unique `.metadata.name` value as demonstrated in the following example:
        ```yaml
    ...
    1.  Get the cluster name by running the following command:
        ...
        ```
    1.  Apply the `Machine` CR by entering the following command:
        ```terminal
        $ oc apply -f <filename>
        ```

        where `<filename>` specifies the name of the `Machine` CR.
    1.  Link `BareMetalHost`, `Machine`, and `Node` objects by running the `link-machine-and-node.sh` script:
        1.  Copy the following `link-machine-and-node.sh` script to a local machine: 
            ...
            ```
        1.  Make the script executable by entering the following command:
            ```terminal
...
1.  Confirm etcd health by running the following commands:
    1.  Open a remote shell session to the control plane node:
        ...
        $ oc rsh -n openshift-etcd etcd-node-0
        ```
    1.  Check endpoint health. Expected output shows `is healthy` for the endpoint.
        ```terminal
