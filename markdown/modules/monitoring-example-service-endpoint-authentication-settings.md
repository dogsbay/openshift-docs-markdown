...
# Example service endpoint authentication settings {id="example-service-endpoint-authentication-settings_{{ context }}"}
...
## Sample YAML authentication with a bearer token {id="_sample_yaml_authentication_with_a_bearer_token"}

The following sample shows bearer token settings for a `Secret` object named `example-bearer-auth` in the `ns1` namespace:

```yaml title="Example bearer token secret"
apiVersion: v1
kind: Secret
metadata:
...
<a name="sample-yaml-bearer-token_{{ context }}"></a>
```yaml title="Example bearer token authentication settings"
...
## Sample YAML for Basic authentication {id="sample-yaml-basic-auth_{{ context }}"}
...
```yaml title="Example Basic authentication secret"
apiVersion: v1
kind: Secret
metadata:
...
```yaml title="Example Basic authentication settings"
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  name: prometheus-example-monitor
...
1.  The name of the `Secret` object that contains the Basic authentication.
1.  The key that contains the password in the specified `Secret` object.

## Sample YAML authentication with OAuth 2.0 {id="sample-yaml-oauth-20_{{ context }}"}

The following sample shows OAuth 2.0 settings for a `Secret` object named `example-oauth2` in the `ns1` namespace:

```yaml title="Example OAuth 2.0 secret"
apiVersion: v1
kind: Secret
metadata:
  name: example-oauth2
...
```yaml title="Example OAuth 2.0 authentication settings"
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  name: prometheus-example-monitor
...
1.  The name of the `Secret` object that contains the OAuth 2.0 credentials.
1.  The key that contains the OAuth 2.0 secret in the specified `Secret` object.
