...
# Building in the cluster {id="kmm-building-in-cluster_{{ context }}"}

Kernel Module Management (KMM) can build kmod container images in the cluster on {{ product_title }} when the image does not already exist in the registry. You configure in-cluster builds through the `build` section of a kernel mapping in the `Module` CR. {._abstract}
