...
1.  Verify the raw data location by using the name and namespace of one of the results:
    ```terminal
    ...
    ```
1.  Fetch the raw results by spawning a pod that mounts the volume and copying the results:
    ```terminal
    ...
    ```
1.  After the pod is running, download the results:
    ```terminal
...
1.  After the extraction is complete, the pod can be deleted:
    ```terminal
