...
1.  Use the `virsh console` command to find the IP address of the bootstrap VM:
    ...
    When deploying an {{ product_title }} cluster without the `provisioning` network, you must use a public IP address and not a private IP address like `172.22.0.2`.
