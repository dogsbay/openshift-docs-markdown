{%- set image_prefix = "ose" -%}

{% if openshift_origin %}
{%- set image_prefix = "origin" -%}
{% endif %}

{%- set _mod_docs_content_type = "PROCEDURE" %}
# Configuring SR-IOV {id="nw-multinetwork-sriov_{{ context }}"}

{{ product_title }} includes the capability to use SR-IOV hardware on
{{ product_title }} nodes, which enables you to attach SR-IOV virtual function (VF)
...
## Supported Devices {id="_supported_devices"}
...
## Creating SR-IOV plug-ins and daemonsets {id="_creating_sr-iov_plug-ins_and_daemonsets"}
...
1.  Create a YAML file for the `openshift-sriov` namespace with the following
contents:
    ```yaml
    apiVersion: v1
    kind: Namespace
    ...
    ```
1.  Run the following command to create the `openshift-sriov` namespace:
    ```
    $ oc create -f openshift-sriov.yaml
    ```
1.  Create a YAML file for the `sriov-device-plugin` service account with the
following contents:
    ```yaml
    apiVersion: v1
    kind: ServiceAccount
    ...
    ```
1.  Run the following command to create the `sriov-device-plugin` service account:
    ```
    $ oc create -f sriov-device-plugin.yaml
    ```
1.  Create a YAML file for the `sriov-cni` service account with the following
contents:
    ```yaml
    apiVersion: v1
    kind: ServiceAccount
    metadata:
      ...
      namespace: openshift-sriov
    ```
1.  Run the following command to create the `sriov-cni` service account:
    ```
    $ oc create -f sriov-cni.yaml
    ```
1.  Create a YAML file for the `sriov-device-plugin` DaemonSet with the following
contents:
    ...
    `oc describe node <node-name>` command. The resource name for the
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: sriov-device-plugin
            image: quay.io/openshift/{{ image_prefix }}-sriov-network-device-plugin:v4.0.0
            args:
            - --log-level=10
            securityContext:
              privileged: true
          ...
          volumes:
            ...
            - name: net
              hostPath:
                path: /sys/class/net
    ```
1.  Run the following command to create the `sriov-device-plugin` DaemonSet:
    ```
    oc create -f sriov-device-plugin.yaml
    ```
1.  Create a YAML file for the `sriov-cni` DaemonSet with the following contents:
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: sriov-cni
            image: quay.io/openshift/{{ image_prefix }}-sriov-cni:v4.0.0
            securityContext:
              privileged: true
            volumeMounts:
            - name: cnibin
    ...
    ```
1.  Run the following command to create the `sriov-cni` DaemonSet:
    ```
    $ oc create -f sriov-cni.yaml
    ```

## Configuring additional interfaces using SR-IOV {id="_configuring_additional_interfaces_using_sr-iov"}

1.  Create a YAML file for the Custom Resource (CR) with SR-IOV configuration. The
...
1.  Run the following command to create the `sriov-conf` CR:
    ```
    $ oc create -f sriov-conf.yaml
    ```
1.  Create a YAML file for a Pod which references the name of the
      ...
      - name: sriovsamplepod
        ...
        resources:
          ...
          limits:
            openshift.io/sriov: '1'
    ```
1.  Run the following command to create the `sriovsamplepod` Pod:
    ```
    $ oc create -f sriovsamplepod.yaml
    ```
1.  View the additional interface by executing the `ip` command:
    ```
