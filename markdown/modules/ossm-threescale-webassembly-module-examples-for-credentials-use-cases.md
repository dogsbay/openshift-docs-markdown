# The 3scale WebAssembly module examples for credentials use cases {id="ossm-threescale-webassembly-module-examples-for-credentials-use-cases_{{ context }}"}
...
## API key (user_key) in query string parameters {id="api-key-in-query-string-parameters_{{ context }}"}
...
## Application ID and key {id="application-id-and-key_{{ context }}"}
...
## Authorization header {id="authorization-header_{{ context }}"}
...
1.  It then decodes the second value containing the data it needs, and splits it by using a colon (:) character to have an operations stack including first the `app_id`, then the `app_key`, if it exists.
...
1.  To add extra conditions to `credentials`, allow `Basic` authorizations, where `app_id` is either `aladdin` or `admin`, or any `app_id` being at least 8 characters in length.
1.  `app_key` must contain a value and have a minimum of 64 characters as shown in the following example:
    ```yaml
    apiVersion: extensions.istio.io/v1alpha1
    kind: WasmPlugin
    metadata:
    ...
    # ...
    ```
1.  After picking up the `authorization` header value, you get a `Basic` `credential`-type by reversing the stack so that the type is placed on top.
1.  Run a glob match on it. When it validates, and the credential is decoded and split, you get the `app_id` at the bottom of the stack, and potentially the `app_key` at the top.
1.  Run a `test:` if there are two values in the stack, meaning an `app_key` was acquired.
...
1.  Use `and` to preserve the contents of the stack across tests.
...
## OpenID Connect (OIDC) use case {id="openid-connect-use-case_{{ context }}"}
...
## Picking up the JWT token from a header {id="picking-up-the-jwt-token-from-a-header_{{ context }}"}
