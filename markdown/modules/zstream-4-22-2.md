...
# RHSA-2026:27009 - {{ product_title }} {{ product_version }}.2 bug fix and security update {id="zstream-4-22-2_{{ context }}"}

Issued: 23 June 2026

{{ product_title }} release {{ product_version }}.2 is now available. The list of fixed issues that are included in the update is documented in the [RHSA-2026:27009](https://access.redhat.com/errata/RHSA-2026:27009) advisory. The RPM packages that are included in the update are provided by the [RHBA-2026:27007](https://access.redhat.com/errata/RHBA-2026:27007) advisory. {._abstract}
...
## New features {id="zstream-4-22-2-new-features_{{ context }}"}


The Helm CLI v4 for Red&#160;Hat OpenShift is now available as a standalone download
:   The Helm CLI v4.1.4 for Red&#160;Hat OpenShift is the first supported downstream build of Helm v4, available as a standalone binary from the {{ product_title }} mirror repository. This release provides access to Helm v4 capabilities, enabling you to evaluate and use Helm v4 workflows on {{ product_title }}.

    Note that all integrated {{ product_title }} console flows and software catalogs still run on Helm v3 only. If you use new Helm chart features or Helm v4-exclusive features through the command line interface (CLI), they will not map to or render in the current {{ product_title }} web console.

    Red&#160;Hat does not distribute Helm CLI v4 as a traditional RPM.

    For more information, see [Helm CLI for Red&#160;Hat {{ product_title }}](https://access.redhat.com/solutions/7144038)(Red&#160;Hat Knowledgebase).

## Enhancements {id="zstream-4-22-2-enhancements_{{ context }}"}
...
## Updating {id="zstream-4-22-2-updating_{{ context }}"}
