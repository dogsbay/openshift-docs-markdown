...
# Configuring KubeVirt VM root volume {id="hcp-virt-root-volume_{{ context }}"}

At cluster creation time, you can configure the storage class that is used to host the KubeVirt virtual machine (VM) root volumes by using the `--root-volume-storage-class` argument. {._abstract}
