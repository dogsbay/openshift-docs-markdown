...
# Migrating {{ SMProductName }} from version 1.1 to version 2.0 {id="ossm-migrating-to-20_{{ context }}"}
...
*   You must upgrade to {{ product_title }} 4.7. before you upgrade to {{ SMProductName }} 2.0.
...
## Upgrading {{ SMProductName }} {id="ossm-migrating_{{ context }}"}
...
1.  Check your v1 `ServiceMeshControlPlane` resource configuration to make sure it is valid.
    1.  Run the following command to view your `ServiceMeshControlPlane` resource as a v2 resource.
        ...
        $ oc get smcp -o yaml
        ```
    1.  Check the `spec.techPreview.errored.message` field in the output for information about any invalid fields.
    1.  If there are invalid fields in your v1 resource, the resource is not reconciled and cannot be edited as a v2 resource. All updates to v2 fields will be overridden by the original v1 settings. To fix the invalid fields, you can replace, patch, or edit the v1 version of the resource. You can also delete the resource without fixing it. After the resource has been fixed, it can be reconciled, and you can to modify or view the v2 version of the resource.
    1.  To fix the resource by editing a file, use `oc get` to retrieve the resource, edit the text file locally, and replace the resource with the file you edited.
        ```terminal
        ...
        ```
    1.  To fix the resource using patching, use `oc patch`.
        ```terminal
        $ oc patch smcp.v1.maistra.io <smcp_name> --type json --patch '[{"op": "replace","path":"/spec/path/to/bad/setting","value":"corrected-value"}]'
        ```
    1.  To fix the resource by editing with command-line tools, use `oc edit`.
        ```terminal
        $ oc edit smcp.v1.maistra.io <smcp_name>
        ```
1.  Back up your {{ SMProductShortName }} control plane configuration. Switch to the project that contains your `ServiceMeshControlPlane` resource.  In this example, `istio-system` is the name of the {{ SMProductShortName }} control plane project.
    ```terminal
    $ oc project istio-system
    ```
1.  Enter the following command to retrieve the current configuration. Your &lt;smcp_name> is specified in the metadata of your `ServiceMeshControlPlane` resource, for example `basic-install` or `full-install`.
    ```terminal
    $ oc get servicemeshcontrolplanes.v1.maistra.io <smcp_name> -o yaml > <smcp_name>.v1.yaml
    ```
1.  Convert your `ServiceMeshControlPlane` to a v2 control plane version that contains information about your configuration as a starting point.
    ```terminal
    $ oc get smcp <smcp_name> -o yaml > <smcp_name>.v2.yaml
    ```
1.  Create a project. In the {{ product_title }} console Project menu, click `New Project` and enter a name for your project, `istio-system-upgrade`, for example. Or, you can run this command from the CLI.
    ```terminal
    $ oc new-project istio-system-upgrade
    ```
1.  Update the `metadata.namespace` field in your v2 `ServiceMeshControlPlane` with your new project name. In this example, use `istio-system-upgrade`.
1.  Update the `version` field from 1.1 to 2.0 or remove it in your v2 `ServiceMeshControlPlane`.
1.  Create a `ServiceMeshControlPlane` in the new namespace. On the command line, run the following command to deploy the control plane with the v2 version of the `ServiceMeshControlPlane` that you retrieved. In this example, replace `&lt;smcp_name.v2> `with the path to your file.
    ```terminal
...
## Configuring the 2.0 ServiceMeshControlPlane {id="ossm-migrating-smcp_{{ context }}"}
...
### Architecture changes {id="ossm-migrating-differences-arch_{{ context }}"}
...
### Annotation changes {id="ossm-migrating-differences-annotation_{{ context }}"}
...
### Behavioral changes {id="ossm-migrating-differences-behavior_{{ context }}"}
...
### Migration details for unsupported resources {id="ossm-migrating-unsupported-resources_{{ context }}"}
...
### Mixer plugins {id="ossm-migrating-mixer_{{ context }}"}
...
### Mutual TLS changes {id="ossm-migrating-mtls_{{ context }}"}
...
For information about mTLS, see [Enabling mutual Transport Layer Security (mTLS)](/service_mesh/v2x/ossm-security#ossm-security-mtls_ossm-security)

#### Other mTLS Examples {id="_other_mtls_examples"}
...
## Configuration recipes {id="ossm-migrating-config-recipes_{{ context }}"}

You can configure the following items with these configuration recipes.

### Mutual TLS in a data plane {id="ossm-migrating-config-mtls_{{ context }}"}
...
### Visualization {id="ossm-migrating-config-vis_{{ context }}"}
...
### Resource utilization and scheduling {id="_resource_utilization_and_scheduling"}
...
| `tracing.jaeger` | general Jaeger container - not all settings may be applied. Complete customization of Jaeger installation is supported by specifying an existing Jaeger resource in the {{ SMProductShortName }} control plane configuration. | v1.0/1.1/2.0 |
...
| kiali | Kiali container - complete customization of Kiali installation is supported by specifying an existing Kiali resource in the {{ SMProductShortName }} control plane configuration. | v1.0/1.1/2.0 |
...
Some components support resource limiting and scheduling. For more information, see [Performance and scalability](/service_mesh/v2x/ossm-performance-scalability#ossm-performance-scalability).

## Next steps for migrating your applications and workloads {id="ossm-migrrating-apps_{{ context }}"}
