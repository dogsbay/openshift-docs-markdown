...
# Example of a FlowCollector resource {id="network-observability-flowcollector-example_{{ context }}"}

Review a comprehensive, annotated example of the `FlowCollector` custom resource that demonstrates configurations for `eBPF` sampling, conversation tracking, Loki integration, and console quick filters. {._abstract}

## Sample `FlowCollector` resource {id="network-observability-flowcollector-configuring-about-sample_{{ context }}"}
```yaml
...
`spec.agent.type`
:   Must be `eBPF` as eBPF is the only {{ product_title }} supported option.
