...
# Configuring Role Based Access Control (RBAC) {id="ossm-vs-istio_{{ context }}"}
    ...
    spec:
      ...
      - from:
        - source:
            ipBlocks: ["1.2.3.4"]
    ```
1.  Run the following command after you write your resource to create your resource in your namespace. The namespace must match your `metadata.namespace` field in your `AuthorizationPolicy` resource.
    ```terminal
...
## Configure intra-project communication {id="_configure_intra-project_communication"}
...
### Restrict access to services outside a namespace {id="_restrict_access_to_services_outside_a_namespace"}
...
### Creating allow-all and default deny-all authorization policies {id="_creating_allow-all_and_default_deny-all_authorization_policies"}
...
## Allow or deny access to the ingress gateway {id="_allow_or_deny_access_to_the_ingress_gateway"}
...
## Restrict access with JSON Web Token {id="_restrict_access_with_json_web_token"}
