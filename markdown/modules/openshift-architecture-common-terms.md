...
# Glossary of common terms for {{ product_title }} architecture {id="openshift-architecture-common-terms_{{ context }}"}
{% endif %}
{% if openshift_rosa %}
# Glossary of common terms for {{ product_title }} {id="openshift-architecture-common-terms_{{ context }}"}
{% endif %}

Review the architecture terms and concepts to plan and deploy the cluster infrastructure. {._abstract}
...
node
:   A worker machine in the {{ product_title }} cluster. A node is either a virtual machine (VM) or a physical machine.
...
pod
:   One or more containers with shared resources, such as volume and IP addresses, running in your {{ product_title }} cluster. A pod is the smallest compute unit defined, deployed, and managed.


private registry
:   {{ product_title }} can use any server implementing the container image registry API as a source of the image which allows the developers to push and pull their private container images.
...
route
:   Routes expose a service to allow for network access to pods from users and applications outside the {{ product_title }} instance.
