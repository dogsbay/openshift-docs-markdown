...
# Forwarding logs to Amazon CloudWatch {id="cluster-logging-collector-log-forward-cloudwatch_{{ context }}"}
...
1.  Create the secret. For example:
    ...
    ```
1.  Create or edit a YAML file that defines the `ClusterLogForwarder` CR object. In the file, specify the name of the secret. For example:
    ```yaml
