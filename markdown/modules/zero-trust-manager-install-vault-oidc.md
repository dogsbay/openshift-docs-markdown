...
1.  Run the `helm install` command:
    ...
    ```
1.  Expose the Vault service by running the following command:
    ```terminal
    $ oc expose service vault -n vault
    ```
1.  Set the `VAULT_ADDR` environment variable to retrieve the hostname from the new route and then export it by running the following command:
    ```terminal
