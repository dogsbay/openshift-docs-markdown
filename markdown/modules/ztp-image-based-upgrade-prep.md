...
# Moving to the Prep stage of the image-based upgrade with {{ lcao }} and {{ ztp }} {id="ztp-image-based-upgrade-prep_{{ context }}"}
...
```yaml title="ImageBasedUpgrade CR in the ztp-site-generate container"
...
1.  Add policies for the `Prep`, `Upgrade`, and `Idle` stages to your existing group `PolicyGenTemplate` called `ibu-upgrade-ranGen.yaml`:
    ...
    1.  Define the seed image, {{ product_title }} version, and pull secret for the upgrade in the Prep stage.
    1.  Define the {{ oadp_short }} `ConfigMap` resources required for backup and restore.
1.  Verify that the policies required for an image-based upgrade are created by running the following command:
    ```terminal
    ...
    ```
1.  Update the `du-profile` cluster label to the target platform version or the corresponding policy-binding label in the `ClusterInstance` CR.
    ```yaml
...
1.  Commit and push the updated `ClusterInstance` CR to the Git repository.
1.  When you are ready to move to the `Prep` stage, create the `ClusterGroupUpgrade` CR on the target hub cluster with the `Prep` and {{ oadp_short }} `ConfigMap` policies:
    ```yaml
    ...
    spec:
      ...
      remediationStrategy:
        ...
        timeout: 240
    ```
1.  Apply the `Prep` policy by running the following command:
    ```terminal
    $ oc apply -f cgu-ibu-prep.yml
    ```

    If you provide `ConfigMap` objects for {{ oadp_short }} resources and extra manifests, {{ lcao }} validates the specified `ConfigMap` objects during the `Prep` stage.
    ...
    *   Validation errors if the {{ lcao }} detects any issues with `oadpContent`
    ...
    ```yaml title="Example validation warning"
    [...]
    metadata:
    annotations:
...
1.  Monitor the status and wait for the `cgu-ibu-prep` `ClusterGroupUpgrade` to report `Completed` by running the following command:
    ```terminal
