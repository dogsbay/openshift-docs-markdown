...
# Workload max duration or timeout script example {id="microshift-greenboot-workload-health-check-script-ex_{{ context }}"}

You can use the {{ microshift_short }} core services health check script as a template to write your own health check scripts for your applications. {._abstract}

## Basic prerequisites for creating a health check script {id="microshift-greenboot-app-health-check-basic-prereqs_{{ context }}"}

*   The workload must be installed.
*   You must have root access.

## Example and functional requirements {id="microshift-greenboot-app-health-check-ex_{{ context }}"}
...
Choose a name prefix for your application that ensures it runs after the `40_microshift_running_check.sh` script, which implements the {{ microshift_short }} health check procedure for its core services.

:::


```bash title="Example greenboot health check script"
#!/bin/bash
set -e
