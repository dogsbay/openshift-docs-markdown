...
# Provisioning storage by using {{ lvms }} {id="lvms-provisioning-storage-using-lvms_{{ context }}"}

After you have created the LVM volume groups by using the `LVMCluster` custom resource (CR), you can provision storage for your workloads by creating persistent volume claims (PVCs) that dynamically allocate local storage from the volume groups. {._abstract}
