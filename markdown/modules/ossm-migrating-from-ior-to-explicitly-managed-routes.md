...
# Migrating from Istio OpenShift Routing to explicitly-managed routes {id="ossm-migrating-from-ior-to-explicitly-managed-routes_{{ context }}"}
...
*   Modify the `ServiceMeshControlPlane` resource to disable IOR:
    ...
    1.  Use the name of ingress gateway `Service` object that is the target.