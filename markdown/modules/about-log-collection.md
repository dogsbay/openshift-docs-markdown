...
# Log collection {id="about-log-collection_{{ context }}"}

The log collector is a daemon set that deploys pods to each {{ product_title }} node to collect container and node logs.

By default, the log collector uses the following sources:

*   System and infrastructure logs generated by journald log messages from the operating system, the container runtime, and {{ product_title }}.
...
## Log collector types {id="about-log-collectors-types_{{ context }}"}

[Vector](https://vector.dev/docs/about/what-is-vector/) is a log collector offered as an alternative to Fluentd for the {{ logging }}.
...
```yaml title="Example ClusterLogging CR that configures Vector as the collector"
...
## Log collection limitations {id="about-log-collectors-limitations_{{ context }}"}
