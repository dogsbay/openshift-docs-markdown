...
1.  Verify that the `noderesourcetopologies` CRD is deployed in the cluster by running the following command:
    ...
    ```
1.  Check that the NUMA-aware scheduler name matches the name specified in your NUMA-aware workloads by running the following command:
    ```terminal
    ...
    ```
1.  Verify that NUMA-aware schedulable nodes have the `noderesourcetopologies` CR applied to them. Run the following command:
    ```terminal
...
1.  Verify the NUMA zone granularity for all schedulable nodes by running the following command:
    ```terminal
