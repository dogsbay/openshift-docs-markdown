...
1.  If you have not started {{ microshift_short }}, replace the default values in the `network` section of the {{ microshift_short }} YAML with your valid values.
    ```yaml title="Example dual-stack IPv6 networking configuration with network assignments"
    apiServer:
    # ...
    apiServer:
      subjectAltNames:
...
1.  Complete any other {{ microshift_short }} configurations you require, then start {{ microshift_short }} by running the following command:
    ```terminal
...
1.  You can verify that all of the system services and pods to have two IP addresses, one for each family, by using the following steps:
    1.  Retrieve the networks defined in the node resource by running the following command:
        ...
        ```
    1.  Retrieve the networks defined by the host network pods by running the following command:
        ```terminal
