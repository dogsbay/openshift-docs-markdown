...
# Create an AWS KMS encryption key {id="aws-encryption-key_{{ context }}"}

Using your AWS account and the `aws` CLI tool, you can create an AWS KMS encryption key to encrypt your resources. {._abstract}
...
1.  Verify the KMS key has been created by running the following command:
    ...
    $ echo $KMS_ARN
    ```
1.  Set your AWS account ID to an environment variable by running the following command:
    ```terminal
    $ AWS_ACCOUNT=$(aws sts get-caller-identity --query Account --output text)
    ```
1.  Create your AWS key policy by running the following command.
