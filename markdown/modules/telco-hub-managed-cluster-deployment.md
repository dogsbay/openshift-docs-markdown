...
# Managed cluster deployment {id="telco-hub-managed-cluster-deployment_{{ context }}"}


Description
:   As of {{ rh_rhacm_first }} 2.12, using the SiteConfig Operator is the recommended method for deploying managed clusters.
    ...
    The `ClusterInstance` CR can be used to initiate cluster installation by using either Assisted Installer, or the image-based installation available in multicluster engine. {._abstract}


Limits and requirements
:   *   The `SiteConfig` ArgoCD plugin which handles `SiteConfig` CRs is removed from {{ product_title }} 4.21.
    ...
    *   An HTTP server hosting the root filesystem and RHCOS live ISO images is required for cluster deployment. 
    ...
    ISO images for each {{ product_title }} release to be deployed must be reachable by the hub cluster and each deployed spoke cluster.
    Only include ISO images which exist on the HTTP server in the `AgentServiceConfig` CR.
    *   A container registry hosting all {{ product_title }} and day-2 OLM operator images must be reachable from all deployed spoke clusters.
