...
# Configuring content filters to prune log records {id="logging-content-filter-prune-records_{{ context }}"}
...
1.  Add a configuration for a filter to the `prune` spec in the `ClusterLogForwarder` CR.
    ...
    1.  Optional: Any fields that are specified in this array are removed from the log record.
