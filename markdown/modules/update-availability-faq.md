...
# Common questions about update availability {id="update-availability_{{ context }}"}

There are several factors that affect if and when an update is made available to an {{ product_title }} cluster. {._abstract}

The following list provides common questions regarding the availability of an update:

<a name="channel-differences_{{ context }}"></a>
...
<a name="channel-safety_{{ context }}"></a>
...
<a name="supported-updates_{{ context }}"></a>
...
<a name="removed-recommendation_{{ context }}"></a>
...
<a name="z-stream-release-cadence_{{ context }}"></a>
