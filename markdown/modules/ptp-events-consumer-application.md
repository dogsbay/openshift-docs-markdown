...
# PTP events consumer application reference {id="ptp-events-consumer-application-{{ ptp_events_rest_api }}_{{ context }}"}
...
func createSubscription(resourceAddress string) (sub pubsub.PubSub, err error) {
  var status int
      apiPath:= "/api/ocloudNotifications/{{ ptp_events_rest_api }}/"
...
func getCurrentState(resource string) {
  ...
  url := &types.URI{URL: url.URL{Scheme: "http",
    Host: localhost:8989,
    Path: fmt.SPrintf("/api/ocloudNotifications/{{ ptp_events_rest_api }}/%s/CurrentState",resource}}
  rc := restclient.New()
