...
# Specifying group and site configurations in group PolicyGenerator or PolicyGentemplate CRs {id="ztp-specifying-nics-in-pgt-crs-with-hub-cluster-templates_{{ context }}"}
...
Using hub templates in site `PolicyGenerator` or `PolicyGentemplate` CRs means that you do not need to create a policy CR for each site. {._abstract}
...
There is a [1 MiB size limit](https://kubernetes.io/docs/concepts/configuration/configmap/#motivation) (Kubernetes documentation) for `ConfigMap` CRs.
...
1.  Commit the `ConfigMap` CRs in Git, and then push to the Git repository being monitored by the Argo CD application.
1.  Apply the hardware type and region labels to the clusters. 
