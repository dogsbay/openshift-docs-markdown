...
# Creating compute machines on {{ rh_openstack }} {id="installation-osp-creating-compute-machines_{{ context }}"}

After standing up the control plane, create compute machines. Red Hat provides an Ansible playbook that you run to simplify this process. {._abstract}
...
1.  On a command line, run the playbook:
    ```terminal
