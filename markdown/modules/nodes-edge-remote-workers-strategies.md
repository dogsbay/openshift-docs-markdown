...
# Remote worker node strategies {id="nodes-edge-remote-workers-strategies_{{ context }}"}

You can review the following information to understand how to mitigate problems associated with power or connection loss with remote worker nodes. {._abstract}
...
:   Daemon sets are the best approach to managing pods on remote worker nodes for the following reasons:
    *   Daemon sets do not typically need rescheduling behavior. If a node disconnects from the cluster, pods on the node can continue to run. {{ product_title }} does not change the state of daemon set pods, and leaves the pods in the state they last reported. For example, if a daemon set pod is in the `Running` state, when a node stops communicating, the pod keeps running and is assumed to be running by {{ product_title }}.
    ...
    *   You can use an {{ product_title }} service endpoint to load balance daemon set pods.

    :::note


    Daemon sets do not schedule pods after a reboot of the node if {{ product_title }} cannot reach the node.
...
Other types of {{ product_title }} objects
...
<a name="nodes-edge-remote-workers-strategies-statefulset_{{ context }}"></a>
    ...
    To avoid scheduling a to a node that does not have access to the same type of persistent storage, {{ product_title }} cannot migrate pods that require persistent volumes to other zones in the case of network separation.