...
1.  Add an annotation to a `Pod` YAML file. Only one of the following annotation formats can be used:
    1.  To attach an additional network without any customization, add an annotation with the following format. Replace `_<network>_` with the name of the additional network to associate with the pod:
        ```yaml
        apiVersion: v1
        kind: Pod
        metadata:
        ...
        # ...
        ```
    1.  To attach an additional network with customizations, add an annotation with the following format:
        ```yaml
        apiVersion: v1
        kind: Pod
        metadata:
...
1.  To create a `Pod` YAML file and add the  `NetworkAttachmentDefinition` annotation for an additional network, run the following command and use the example YAML:
    ```terminal
    ...
    ```
    ```yaml title="Example test_bridge pod YAML"
    apiVersion: v1
    kind: Pod
    metadata:
      name: test_bridge
      ...
      - name: hello-microshift
        ...
        securityContext:
          ...
          seccompProfile:
            type: RuntimeDefault
    ```
1.  Make sure that the `NetworkAttachmentDefinition` annotation is correct:
    ...
    # ...
    ```
1.  Optional: To confirm that the `NetworkAttachmentDefinition` annotation exists in a `Pod` YAML, run the following command, replacing `_<name>_` with the name of the pod.
    ```terminal
