...
# Create backups using the auto-recovery feature {id="microshift-creating-backups-auto-recovery_{{ context }}"}

To create backups for {{ product_title }} automatic recovery, you can run `microshift backup --auto-recovery` with a directory path. The command then stores each backup in that directory so that the latest is available when you restore. {._abstract}


:::note

Creating backups requires stopping {{ microshift_short }}. You must decide on the best time to stop {{ microshift_short }}.
