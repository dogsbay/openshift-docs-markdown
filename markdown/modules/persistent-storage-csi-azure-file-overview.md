...
# Overview {id="persistent-storage-csi-azure-file-overview_{{ context }}"}

{{ product_title }} is capable of provisioning persistent volumes (PVs) by using the Container Storage Interface (CSI) driver for Microsoft Azure File Storage. {._abstract}
...
To create CSI-provisioned PVs that mount to Azure File storage assets, {{ product_title }} installs the Azure File CSI Driver Operator and the Azure File CSI driver by default in the `openshift-cluster-csi-drivers` namespace.
