...
# Troubleshooting the Dynamic Accelerator Slicer Operator {id="das-operator-troubleshooting_{{ context }}"}
...
## Debugging DAS Operator components {id="_debugging_das_operator_components"}
...
1.  Check the status of all DAS Operator components by running the following command:
    ...
    ```
1.  Inspect the logs of the DAS Operator controller by running the following command:
    ```terminal
    $ oc logs -n das-operator deployment/das-operator
    ```
1.  Check the logs of the webhook server by running the following command:
    ```terminal
    $ oc logs -n das-operator deployment/das-operator-webhook
    ```
1.  Check the logs of the scheduler plugin by running the following command:
    ```terminal
    $ oc logs -n das-operator deployment/das-scheduler
    ```
1.  Check the logs of the device plugin daemonset by running the following command:
    ```terminal
    $ oc logs -n das-operator daemonset/das-daemonset
    ```

## Monitoring AllocationClaims {id="_monitoring_allocationclaims"}
...
1.  Inspect active `AllocationClaim` resources by running the following command:
    ...
    ```
1.  View detailed information about a specific `AllocationClaim` by running the following command:
    ```terminal
    $ oc get allocationclaims -n das-operator -o yaml
    ```
    ```yaml title="Example output (truncated)"
    ...
    status:
      ...
      state: inUse
    ```
1.  Check for claims in different states by running the following command:
    ```terminal
    ...
    ```
1.  View events related to `AllocationClaim` resources by running the following command:
    ```terminal
    $ oc get events -n das-operator --field-selector involvedObject.kind=AllocationClaim
    ```
1.  Check `NodeAccelerator` resources to verify GPU hardware detection by running the following command:
    ```terminal
...
## Verifying GPU device availability {id="_verifying_gpu_device_availability"}
...
1.  On a node with GPU hardware, verify that CDI devices were created by running the following command:
    ...
    ```
1.  Check the NVIDIA GPU Operator status by running the following command:
    ```terminal
...
## Increasing log verbosity {id="_increasing_log_verbosity"}
...
1.  Edit the `DASOperator` resource to increase log verbosity by running the following command:
    ...
    ```
1.  Set the `operatorLogLevel` field to `Debug` or `Trace`:
    ```yaml
    spec:
      operatorLogLevel: Debug
    ```
1.  Save the changes and verify that the operator pods restart with increased verbosity.

## Common issues and solutions {id="_common_issues_and_solutions"}


:::note[Pods stuck in UnexpectedAdmissionError state]
