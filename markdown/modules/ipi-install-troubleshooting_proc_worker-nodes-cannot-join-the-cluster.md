...
1.  Add a DNS A/AAAA or CNAME record to internally identify the API load balancer. For example, when using dnsmasq, modify the `dnsmasq.conf` configuration file:
    ...
    ```
1.  Add a DNS PTR record to internally identify the API load balancer. For example, when using dnsmasq, modify the `dnsmasq.conf` configuration file:
    ```terminal
    $ sudo nano /etc/dnsmasq.conf
    ```
    ```terminal
    ...
    ```
1.  Restart the DNS server. For example, when using dnsmasq, execute the following command:
    ```terminal
