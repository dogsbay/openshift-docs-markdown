...
# Customizing extra installation manifests in the {{ ztp }} pipeline {id="ztp-customizing-the-install-extra-manifests_{{ context }}"}

You can define a set of extra manifests for inclusion in the installation phase of the {{ ztp_first }} pipeline. These manifests are linked to the `ClusterInstance` custom resources (CRs) and are applied to the cluster during installation. Including `MachineConfig` CRs at install time makes the installation process more efficient. {._abstract}
...
1.  In your `/clusterinstance` directory, create a subdirectory with your extra manifests. The following example illustrates a sample folder structure:
    ```text
    clusterinstance/
    ├── site1-sno-du.yaml
    ├── extra-manifest/
    ...
    └── kustomization.yaml
    ```
1.  Create or update the `kustomization.yaml` file to use `configMapGenerator` to package your extra manifests into a `ConfigMap`:
    ```yaml
...
1.  Commit the `ClusterInstance` CR, extra manifest files, and `kustomization.yaml` to your Git repository and push the changes.
