...
# Forwarding logs to CloudWatch {id="cloud-experts-deploying-application-logging-forward-to-cloudwatch_{{ context }}"}

Install the logging add-on service to forward the logs to AWS CloudWatch. {._abstract}

**Procedure**

1.  Run the following script to configure your {{ product_title }} cluster to forward logs to CloudWatch:
    ```terminal
    ...
    Configuring {{ product_title }} to send logs to CloudWatch goes beyond the scope of this tutorial. Integrating with AWS and enabling CloudWatch logging are important aspects of {{ product_title }}, so a script is included to simplify the configuration process. The script will automatically set up AWS CloudWatch. You can examine the script to understand the steps involved.
    ...
    ***Example output***
    ...
    Complete.
    ```
1.  After a few minutes, you should begin to see log groups inside of AWS CloudWatch. Run the following command to see the log groups:
    ```terminal
