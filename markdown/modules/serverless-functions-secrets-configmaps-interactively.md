...
# Modifying function access to secrets and config maps interactively {id="serverless-functions-secrets-configmaps-interactively_{{ context }}"}
...
1.  Use the interactive interface to perform the necessary operation. For example, using the utility to list configured volumes produces an output similar to this:
    ```terminal
    $ kn func config
    ...
    ```
1.  Optional. Deploy the function to make the changes take effect:
    ```terminal
