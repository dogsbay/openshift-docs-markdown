...
*   Log into an {{ product_title }} cluster as a user configured with the `restricted-v3` or `nested-container` SCC, or as a user from a user group configured with either SCC. Alternatively, you can set the `restricted-v3` or `nested-container` SCC directly in the workload object.

**Procedure**

1.  Edit the default user ID (UID) and group ID (GID) range of the {{ product_title }} namespace where your pod is deployed by running the following command:
    ```terminal
...
1.  Check the user and group IDs being used by the container in the pod you created. The pod is inside the Linux user namespace.
    1.  Start a shell session with the container in your pod:
        ...
        ```
    1.  Display the user and group IDs being used inside the container:
        ```terminal
        sh-5.1$ id
        ```
    ...
    1.  Display the user ID being used in the container user namespace:
        ```terminal
        sh-5.1$ lsns -t user
        ```
        ```terminal title="Example output"
    ...
    1.  Exit the container shell session by using the following command:
        ```terminal
        sh-5.1$ exit
        ```
1.  Check the UID being used by the node. The node is outside of the Linux user namespace. This user ID should be different from the UID being used in the container.
    ...
    1.  Set `/host` as the root directory within the debug shell:
        ...
        sh-5.1# chroot /host
        ```
    1.  Display the UID being used by the node:
        ```terminal
        sh-5.1#  lsns -t user
        ```
        ```terminal title="Example command"
    ...
    1.  Exit the debug session by using the following commands:
        ```terminal
        sh-5.1#  exit
        ```
        ```terminal
