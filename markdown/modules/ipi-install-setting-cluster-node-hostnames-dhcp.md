...
# Setting the cluster node hostnames through DHCP {id="ipi-install-setting-cluster-node-hostnames-dhcp_{{ context }}"}

On {{ op_system_first }} machines, `NetworkManager` sets the hostnames. By default, DHCP provides the hostnames to `NetworkManager`, which is the recommended method.  {._abstract}
