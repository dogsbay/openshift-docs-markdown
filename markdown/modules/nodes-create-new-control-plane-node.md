...
# Creating the new control plane node {id="create-new-machine_{{ context }}"}
...
1.  Edit the `bmh_affected.yaml` file that you previously saved:
    1.  Remove the following metadata items from the file:
        ...
        *   `resourceVersion`
        *   `uid`
    1.  Remove the `status` section of the file.
...
1.  Create the customized {{ op_system_first }} live ISO by running the following command:
    ```terminal
...
1.  Boot the new control plane node with the customized {{ op_system }} live ISO.
