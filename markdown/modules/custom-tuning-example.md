...
# Custom tuning examples {id="custom-tuning-example_{{ context }}"}

The following examples demonstrate custom tuning configurations for {{ product_title }} nodes using the Node Tuning Operator. {._abstract}

**Using TuneD profiles from the default CR**

The following CR applies custom node-level tuning for
{{ product_title }} nodes with label
...
Various kernel parameters can be changed at runtime by using `/run/sysctl.d/`, `/etc/sysctl.d/`, and `/etc/sysctl.conf` host configuration files. {{ product_title }} adds several host configuration files which set kernel parameters at runtime; for example, `net.ipv[4-6].`, `fs.inotify.`, and `vm.max_map_count`. These runtime parameters provide basic functional tuning for the system prior to the kubelet and the Operator start.
