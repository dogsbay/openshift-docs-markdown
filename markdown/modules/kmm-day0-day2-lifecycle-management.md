...
# Lifecycle management {id="kmm-day0-day2-lifecycle-management_{{ context }}"}

KMM lifecycle management on {{ product_title }} lets you upgrade kmods from Day 0 through Day 2 without rebooting nodes when the driver supports it. {._abstract}
...
## Treat the kmod as an in-tree driver {id="kmm-kmod-in-tree-driver_{{ context }}"}
...
## Use ordered upgrade {id="kmm-ordered-upgrade_{{ context }}"}
