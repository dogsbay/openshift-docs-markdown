...
# About the cluster MTU {id="nw-cluster-mtu-change-about_{{ context }}"}

During installation, the cluster network maximum transmission unit (MTU) is set automatically based on the primary network interface MTU of cluster nodes. Typically, you do not need to override the detected MTU, but in some instances you must override it. {._abstract}
...
## Service interruption considerations {id="service-interruption-considerations_{{ context }}"}
...
## MTU value selection {id="mtu-value-selection_{{ context }}"}
...
## How the migration process works {id="how-the-migration-process-works_{{ context }}"}
