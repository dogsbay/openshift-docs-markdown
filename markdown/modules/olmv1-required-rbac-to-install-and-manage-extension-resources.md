...
# Required permissions to install and manage a cluster extension {id="olmv1-required-rbac-to-install-and-manage-extension-resources_{{ context }}"}

To assign the necessary permissions for a cluster extension, inspect the manifests in its bundle image and grant the service account sufficient role-based access control (RBAC). {._abstract}
...
Admission plugins
:   Because {{ product_title }} clusters use the `OwnerReferencesPermissionEnforcement` admission plugin, cluster extensions must have permissions to update the `blockOwnerDeletion` and `ownerReferences` finalizers.
