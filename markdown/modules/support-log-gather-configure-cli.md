...
# Configuring a {{ support_log_gather }} instance {id="support-log-gather-config-cli_{{ context }}"}

You must create a `MustGather` custom resource (CR) from the command-line interface (CLI) to automate the collection of diagnostic data from your cluster. This process also automatically uploads the data to a Red&#160;Hat Support case. {._abstract}
...
*   You have installed the {{ oc_first }} tool.
*   You have installed {{ support_log_gather }} in your cluster.
...
1.  Create the `MustGather` object by running the following command:
    ```terminal
...
1.  To monitor the progress of the file upload, view the logs of the upload container in the job pod by running the following command:
    ```terminal
