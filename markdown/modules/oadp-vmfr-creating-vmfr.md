...
# Create a VirtualMachineFileRestore CR {id="oadp-vmfr-creating-vmfr_{{ context }}"}

Create a `VirtualMachineFileRestore` (VMFR) custom resource (CR) to make files from discovered virtual machine (VM) backups accessible for browsing and downloading. You can recover individual files without restoring the entire VM. {._abstract}
...
1.  Create a `VirtualMachineFileRestore` CR YAML manifest file with the following configuration.
    ...
    `namespace`
    :   Specifies the namespace where the VMFR CR is created. This must be the {{ oadp_short }} protected namespace, typically `openshift-adp`.
...
1.  To apply the VMFR CR configuration, run the following command:
    ```terminal
