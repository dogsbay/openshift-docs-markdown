...
# Deploying a managed {{ sno }} cluster using the IBI Operator {id="ibi-create-config-iso_{{ context }}"}

Create the site-specific configuration resources in the hub cluster to initiate the image-based deployment of a preinstalled host. {._abstract}
...
*   You deployed a {{ rh_rhacm_first }} hub cluster or you deployed the multicluster engine for Kubernetes operator (MCE).
...
1.  Create the `ibi-ns` namespace by running the following command:
    ...
    ```
1.  Create the `Secret` resource for your image registry:
    ...
    1.  Create the `Secret` resource for your image registry by running the following command:
        ```terminal
        $ oc create -f secret-image-registry.yaml
        ```
1.  Optional: Configure static networking for the host:
...
1.  Create the `BareMetalHost` and `Secret` resources:
    ...
    1.  Create the `BareMetalHost` and `Secret` resources by running the following command:
        ```terminal
        $ oc create -f ibi-bmh.yaml
        ```
1.  Create the `ClusterImageSet` resource:
...
1.  Create the `ImageClusterInstall` resource:
    ...
    1.  Create the `ImageClusterInstall` resource by running the following command:
        ```terminal
        $ oc create -f ibi-image-cluster-install.yaml
        ```
1.  Create the `ClusterDeployment` resource:
    ...
    1.  Create the `ClusterDeployment` resource by running the following command:
        ```terminal
...
1.  Create the `ManagedCluster` resource:
    1.  Create a YAML file that defines the `ManagedCluster` resource:
        ...
        `<hub_accepts_client>`
        :   Specifies whether {{ rh_rhacm }} manages the cluster. Set to `true` to enable management.

    1.  Create the `ManagedCluster` resource by running the following command:
        ```terminal
...
1.  When the installation completes, you can retrieve the `kubeconfig` secret to log in to the managed cluster by running the following command:
    ```terminal
