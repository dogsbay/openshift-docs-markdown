...
# Disabling copied CSVs {id="olm-disabling-copied-csvs_{{ context }}"}

To reduce memory, etcd, and network usage on large {{ product_title }} clusters, you can disable copied cluster service versions (CSVs) for Operators installed in `AllNamespaces` mode through the cluster `OLMConfig`. {._abstract}
