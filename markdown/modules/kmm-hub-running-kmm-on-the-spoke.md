...
# Running KMM on the spoke {id="kmm-hub-running-kmm-on-the-spoke_{{ context }}"}

To run Kernel Module Management (KMM) on spoke clusters in {{ product_title }}, you can install it with a {{ rh_rhacm }} `Policy` object. After installation, create a `ManagedClusterModule` object from the hub to deploy kernel modules. {._abstract}
