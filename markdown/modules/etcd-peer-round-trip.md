...
# How etcd peer round-trip time affects performance {id="etcd-peer-round-trip_{{ context }}"}

The etcd peer round-trip time (RTT) metric shows replication latency across members, beyond raw network RTT. Monitor it to detect control plane slowdowns before they affect cluster operations. {._abstract}
...
You can monitor various etcd metrics on dashboards in the {{ product_title }} console. In the console, click **Observe** → **Dashboards** and from the dropdown list, select **etcd**.
...
These etcd metrics are collected by the {{ product_title }} metrics system in Prometheus. You can access them from the CLI by using a Prometheus query. For more information, see the Red&#160;Hat Knowledgebase solution, "How to query from the command line Prometheus statistics".
