...
# Test the VM file restore workflow {id="oadp-vmfr-end-to-end-workflow_{{ context }}"}

Complete an end-to-end workflow that creates a VM, backs it up, and restores individual files through SSH to test the VM file restore feature or verify your configuration. {._abstract}
...
1.  Create a `DataProtectionApplication` (DPA) CR with the VMFR feature enabled and the `kubevirt` plugin:
    ```yaml
...
1.  Apply the DPA configuration by running the following command:
    ```terminal
    $ oc apply -f <dpa_cr_filename>
    ```
1.  Verify that the DPA is reconciled and the VMFR feature is enabled by running the following command:
    ```terminal
...
1.  Verify that the `velero`, `nodeAgent`, and `oadp-vm-file-restore-controller-manager` pods are running by running the following command:
    ```terminal
    $ oc get pod -n openshift-adp
    ```
1.  Generate an SSH key pair for accessing the VM:
    ```terminal
    $ ssh-keygen -t ed25519 -f ~/.ssh/<vm_key_name> -C "<vm_key_name>"
    ```

    Replace `<vm_key_name>` with a name for the SSH folder and the key file.
1.  Create a namespace for the VM by running the following command:
    ```terminal
    $ oc create ns <vm_namespace>
    ```
1.  Create a Kubernetes secret with the SSH public key in the VM namespace:
    ```terminal
...
1.  Create a `VirtualMachine` CR by using the built-in VM template `fedora-server-small`:
    ```terminal
...
1.  Wait for the VM to be ready:
    ...
    ```
1.  Patch the VM configuration `accessCredentials` object with the SSH public key:
    ```terminal
...
1.  SSH to the VM and create a test file:
    ```terminal
...
1.  Create a `Backup` CR to back up the VM namespace:
    ```yaml
    ...
    ```
1.  Apply the `Backup` CR by running the following command:
    ```terminal
    $ oc apply -f <backup_cr_filename>
    ```
1.  Verify that the backup is complete by running the following command:
    ```terminal
    $ oc get backup.velero <backup_name> -n openshift-adp -o jsonpath='{.status.phase}'
    ```

    The output should display `Completed`.
1.  Create a `VirtualMachineBackupsDiscovery` CR to identify which backups contain the target VM:
    ```yaml
    apiVersion: oadp.openshift.io/v1alpha1
    ...
    ```
1.  Apply the VMBD CR by running the following command:
    ```terminal
    $ oc apply -f <vmbd_cr_filename>
    ```
1.  Verify that the discovery is complete by running the following command:
    ```terminal
    $ oc get vmbd <vmbd_name> -n openshift-adp
    ```

    The `PHASE` column should display `Completed`.
1.  Create a `VirtualMachineFileRestore` CR with SSH access to restore files from the discovered backups:
    ```yaml
    apiVersion: oadp.openshift.io/v1alpha1
...
1.  Apply the VMFR CR by running the following command:
    ```terminal
    $ oc apply -f <vmfr_cr_filename>
    ```
1.  Wait for the VMFR phase to complete:
    ```terminal
    $ oc get vmfr <vmfr_name> -n openshift-adp
    ```

    The `PHASE` column should display `Completed`.
1.  Retrieve the SSH access information by running the following command:
    ```terminal
...
1.  Retrieve the private key from the auto generated secret and save it to a file:
    ```terminal
...
1.  Copy the private key to the VM so that you can use it for `scp` from within the VM:
    ```terminal
    $ virtctl scp id-rsa \
      ...
      --identity-file=$HOME/.ssh/<vm_key_name>
    ```
1.  Update the file permissions on the private key inside the VM:
    ```terminal
    $ virtctl ssh <vm_user>@vmi/<vm_name> \
      -n <vm_namespace> \
      --identity-file=$HOME/.ssh/<vm_key_name> \
...
1.  Restore the file by using `scp` from within the VM. The remote path format is `/restores/<date>/<backup_name>/<vm_name>/<path_to_file>`:
    ```terminal
    $ virtctl ssh <vm_user>@vmi/<vm_name> \
      -n <vm_namespace> \
      --identity-file=$HOME/.ssh/<vm_key_name> \
...
1.  Verify that the restored file is intact by comparing MD5 checksums:
    ```terminal
    $ virtctl ssh <vm_user>@vmi/<vm_name> \
      -n <vm_namespace> \
      --identity-file=$HOME/.ssh/<vm_key_name> \
    ...
    ```
1.  After you have recovered the files, delete the VMFR CR to clean up resources:
    ```terminal
