...
# Install Grafana for DTS metrics visualization {id="nw-dpf-installing-grafana-for-dts_{{ context }}"}

You can install the Grafana Operator and a Grafana instance to provide enhanced visualization for DPU telemetry metrics, including per-DPU filtering and customizable dashboards. You can also deploy a dashboard ConfigMap that adds DTS metrics to the {{ product_title }} web console. {._abstract}

**Prerequisites**

*   You have enabled user workload monitoring in {{ product_title }}.
...
1.  Create Grafana RBAC for Prometheus access:
    ...
    ```
1.  Deploy the Grafana instance with control-plane scheduling:
    ```yaml
...
1.  Configure the Prometheus datasource:
    ...
    ```
1.  Deploy the DTS console dashboard for {{ product_title }} web console integration:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
    ...
    ```
1.  Deploy the complete DTS Grafana dashboard:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
1.  Verify that the Grafana Operator is running:
    ...
    ```
1.  Verify that the Grafana instance is running:
    ```terminal
    ...
    dpf-grafana   3m15s
    ```
1.  Verify that the Grafana pod is running on a control plane node:
    ```terminal
    ...
    ```
1.  Verify the Grafana route exists:
    ```terminal
    ...
    ```
1.  Verify that the console dashboard ConfigMap exists:
    ```terminal
...
1.  Open the DTS dashboard:
    ...
    1.  Optional: In the {{ product_title }} web console, go to **Observe** → **Dashboards** and open **DOCA DPU Telemetry (DTS)** to view the console-integrated dashboard.
