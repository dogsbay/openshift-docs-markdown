...
# Forwarding logs to Amazon CloudWatch from STS enabled clusters {id="rosa-cluster-logging-collector-log-forward-sts-cloudwatch_{{ context }}"}
...
1.  Prepare the AWS account:
    ...
    1.  Create an IAM trust JSON file with the following content:
        ...
        1.  Specify your AWS account ID and the OpenShift OIDC provider endpoint. Obtain the endpoint by running the following command:
            ...
            ```
        1.  Specify the OpenShift OIDC endpoint again.
...
1.  Create a `Secret` YAML file for the {{ clo }}:
    ```yaml
    apiVersion: v1
    kind: Secret
    metadata:
...
1.  Create or edit a `ClusterLogForwarder` custom resource:
    ...
    1.  Specify the name of the secret you created previously.
