...
# Configuring the container runtime {id="config-container-runtime_{{ context }}"}

You can configure the container runtime used with new workloads on your nodes, based on which runtime you or your organization prefers. You can use either crun, which is considered a faster and more lightweight runtime, or runC, which is more widely used than crun.  {._abstract}
...
*   If your cluster is updated from {{ product_title }} 4.17, you can use the crun container runtime by deleting the following `MachineConfig` objects:
    ...
    *   Migrate your control plane nodes to crun by running the following command:
        ...
        ```
*   For any {{ product_title }} 4.18 or greater cluster, you can configure crun or runC as the container runtime for specific nodes by creating a container runtime configuration:
...
1.  Check the container runtime by using the following command:
    ...
    The `default_runtime` parameter specifies the container runtime that {{ product_title }} uses for new workloads on this node, either `crun` or `runc`, depending on what you have configured.