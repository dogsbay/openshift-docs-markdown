...
# {{ cert_manager_operator }} 1.20.0 {id="cert-manager-operator-release-notes-1-20-0_{{ context }}"}

Review the release notes for the {{ cert_manager_operator }} 1.20.0 to learn what is new and updated with this release. {._abstract}

Issued: 2 July 2026

The following advisories are available for the {{ cert_manager_operator }} for {{ product_title }} 1.20.0:
...
:   With this release, the {{ cert_manager_operator }} no longer requires the `featuregates.config.openshift.io/cluster` object to use a preview `FeatureSet`, such as `TechPreviewNoUpgrade`, in order to enable the TrustManager Technology Preview operand.
    ...
    *   The Operator subscription must opt in to TrustManager by setting `UNSUPPORTED_ADDON_FEATURES=TrustManager=true`.
    ...
    For more information, see [Enabling the TrustManager Operand](/security/cert_manager_operator/cert-manager-trust-manager#cert-manager-trust-manager-install_cert-manager-trust-manager).
