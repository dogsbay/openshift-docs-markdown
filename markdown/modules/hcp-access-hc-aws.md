...
# Accessing a hosted cluster on {{ aws_short }} {id="hcp-access-hc-aws_{{ context }}"}

After you create a hosted cluster on {{ aws_short }}, you can access it by using your `kubeconfig` file, access secrets, and `kubeadmin` credentials. {._abstract}
...
1.  Generate the `kubeconfig` file by entering the following command:
    ...
    ```
1.  Use your `<hosted_cluster_name>.kubeconfig` file that contains the decoded `kubeconfig` configuration to access the hosted cluster. Enter the following command:
    ```terminal
