...
# Routes {id="cloud-experts-getting-started-openshift-concepts-routes_{{ context }}"}

An OpenShift route exposes a service at a hostname so that external clients can reach it by name. When a `Route` object is created on OpenShift, it gets picked up by the built-in HAProxy load balancer to expose the requested service and make it externally available with the given configuration. {._abstract}
