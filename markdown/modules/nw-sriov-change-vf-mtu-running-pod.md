...
# Change the MTU value of a virtual function for a running pod {id="nw-sriov-change-vf-mtu-running-pod_{{ context }}"}

You can change the maximum transmission unit (MTU) of a virtual function (VF) for a running pod by omitting the `mtu` field from the `SriovNetworkNodePolicy` custom resource (CR) and configuring the physical function (PF) MTU by using the Kubernetes NMState Operator. {._abstract}
...
1.  Verify that the VFs are created with the default MTU by running the following commands:
    ```terminal
...
1.  Create a `NodeNetworkConfigurationPolicy` CR to set the MTU of the PF:
    ...
    1.  Apply the CR by running the following command:
        ...
        ```
1.  Verify that the NMState policy has been applied successfully by running the following command:
    ```terminal
...
1.  Verify that the PF MTU has been updated on the node by running the following commands:
    ```terminal
    $ oc debug node/<node_name>
    ```
    ```terminal
...
1.  Deploy or update the application pod to set the VF MTU at container startup:
    ...
    1.  Apply the pod spec by running the following command:
        ```terminal
