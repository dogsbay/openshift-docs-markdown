...
# Listing all resources in the OVN-Kubernetes project {id="nw-ovn-kubernetes-list-resources_{{ context }}"}

List the resources and containers running within the OVN-Kubernetes project to identify network workload states and verify component placement across the infrastructure. {._abstract}
...
1.  List all pods and endpoints by entering the following command:
    ...
    The `ovnkube-config` config map has the {{ product_title }} OVN-Kubernetes configurations.
1.  Display the container layout within the node pod by entering the following command:
    ```terminal
...
1.  List all the containers in the `ovnkube-control-plane` pods by entering the following command:
    ```terminal
    ...
    The `ovnkube-control-plane` pod has a container that runs on each {{ product_title }} node, the `ovnkube-cluster-manager`. The `ovnkube-cluster-manager` container allocates pod subnet, transit-switch subnet IP addresses, and join-switch subnet IP addresses to each node in the cluster. The `kube-rbac-proxy` container monitors metrics for the `ovnkube-cluster-manager` container.