...
# Scaling the NodePool object for a hosted cluster on {{ ibm_power_title }} {id="hcp-ibm-power-scale-np_{{ context }}"}

The `NodePool` object is created when you create a hosted cluster. By scaling the `NodePool` object, you can add more compute nodes to {{ hcp }}. {._abstract}
