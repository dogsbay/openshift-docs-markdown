...
# Deploying PostgreSQL with SPIFFE Helper {id="zero-trust-manager-deploy-spiffe-helper_{{ context }}"}

Deploy a PostgreSQL server and client that use SPIFFE Helper to fetch X.509 {{ svid_full }} from the SPIFFE Workload API and write them to disk for mutual Transport Layer Security (mTLS). {._abstract}

The manifests in this procedure use the {{ zero_trust_full }} SPIFFE Helper image.
...
The stock SPIFFE Helper image does not include the `psql` client required for that reload command. Build a custom image that extends the {{ zero_trust_full }} image and adds the PostgreSQL client package.
...
1.  Build and push a custom SPIFFE Helper image for the PostgreSQL server by running the following command.
    ```terminal
...
1.  Create the PostgreSQL server resources by using the following example:
    ```yaml
    $ oc apply -f - << 'EOF'
    ---
    kind: Namespace
...
1.  Confirm that the SPIFFE Helper configuration matches the PostgreSQL TLS settings by running the following command:
    ```terminal
    ...
    svid_bundle_file_name = "svid_bundle.pem"
    ```
1.  Create the PostgreSQL client by using the following example. The client uses the stock SPIFFE Helper image from the {{ zero_trust_full }}. Only the server deployment requires the custom image with the `psql` client.
    ```yaml
    $ oc apply -f - << 'EOF'
    ---
    kind: Namespace
    ...
    Replace `<version>` with the tag that matches your {{ zero_trust_full }} installation.
1.  Confirm that pods are running in both namespaces by running the following commands:
    ```terminal
...
1.  Inspect the client certificate by running the following command:
    ```terminal
...
1.  Connect to PostgreSQL from the client pod by running the following commands:
    ```terminal
    $ oc rsh -n postgresql-spiffe-client -c postgresql-spiffe-client \
      deployment/postgresql-spiffe-client
    ```
