...
# Incomplete synchronization of labels and annotations {id="ts-capi-migrate-sync-label-annotation_{{ context }}"}

The label and annotation synchronization behavior differs between the Machine API and the Cluster API.
In some cases, these differences cause the two-way synchronization controller to overwrite labels on a Cluster API machine during migration. {._abstract}
