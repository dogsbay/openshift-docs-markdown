...
# Modifying the retention time for Thanos Ruler metrics data {id="modifying-the-retention-time-for-thanos-ruler-metrics-data_{{ context }}"}
...
1.  Edit the `user-workload-monitoring-config` `ConfigMap` object in the `openshift-user-workload-monitoring` project:
    ...
    ```
1.  Add the retention time configuration under `data/config.yaml`:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
