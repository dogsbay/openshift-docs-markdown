...
# Instance types {id="instance-types_{{ context }}"}
...
All worker nodes within a single {{ product_title }} machine pool must be of the same type and size. However, worker nodes across multiple machine pools within an {{ product_title }} cluster can be of different types and sizes.
...
For more information about Red&#160;Hat workloads that must be deployed on worker nodes, see the _Red&#160;Hat Operator support_ section below.
