...
# Deploying a virtualized control plane with {{ ztp }} {id="proc_virt-installing-vcp-ztp_{{ context }}"}

Use {{ ztp_first }} to deploy virtualized control plane clusters at scale.
{{ ztp_first }} uses GitOps to manage cluster deployments declaratively through {{ rh_rhacm_first }}. {._abstract}

**Prerequisites**

*   {{ rh_rhacm }} is installed on a hub cluster.
...
1.  Create a `ClusterInstance` custom resource (CR) that defines the cluster. Specify BMC addresses pointing to KubeVirt Redfish for the virtualized control plane nodes and real Redfish endpoints for the baremetal worker nodes:
    ```yaml
...
1.  Apply the configuration through ArgoCD or your GitOps tooling.
