...
# Telco core common baseline model {id="telco-core-common-baseline-model_{{ context }}"}

The following configurations and use models are applicable to all telco core use cases.
The telco core use cases build on this common baseline of features. {._abstract}
...
:   Telco core use cases require highly available persistent storage as provided by an external storage solution.
    {{ rh_storage }} might be used to manage access to the external storage.
