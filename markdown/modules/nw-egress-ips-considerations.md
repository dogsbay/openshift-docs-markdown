...
# Assignment of egress IPs to a namespace, nodes, and pods {id="nw-egress-ips-considerations_{{ context }}"}

To assign one or more egress IPs to a namespace or specific pods in a namespace, you must meet specific conditions. {._abstract}
...
If you create `EgressIP` objects prior to labeling any nodes in your cluster for egress IP assignment, {{ product_title }} might assign every egress IP address to the first node with the `k8s.ovn.org/egress-assignable: ""` label.
