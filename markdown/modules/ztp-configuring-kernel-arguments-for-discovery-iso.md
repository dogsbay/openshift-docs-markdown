...
# Configuring Discovery ISO kernel arguments for installations using {{ ztp }} {id="setting-managed-bare-metal-host-kernel-arguments_{{ context }}"}

The {{ ztp_first }} workflow uses the Discovery ISO as part of the {{ product_title }} installation process on managed bare-metal hosts. You can edit the `InfraEnv` resource to specify kernel arguments for the Discovery ISO. This is useful for cluster installations with specific environmental requirements.  {._abstract}
...
In {{ product_title }} {{ product_version }}, you can only add kernel arguments. You can not replace or delete kernel arguments.
...
1.  Commit the `InfraEnv-example.yaml` file to your Git repository and push your changes. The following example shows a sample Git repository structure:
    ```text
    ~/example-ztp/install
              └── site-install
                   ...
                   └── kustomization.yaml
    ```
1.  Update the `kustomization.yaml` file to use the `configMapGenerator` field to package the `InfraEnv` CR into a `ConfigMap`:
    ```yaml
...
1.  In your `ClusterInstance` CR, reference the `ConfigMap` in the `spec.templateRefs` field:
    ```yaml
...
1.  Commit the `ClusterInstance` CR and `kustomization.yaml` to your Git repository and push your changes.
