...
# Mapping API tiers to API groups {id="api-support-tiers-mapping_{{ context }}"}
...
## Support for Kubernetes API groups {id="mapping-support-tiers-to-kubernetes-api-groups_{{ context }}"}
...
{% if not microshift %}
## Support for OpenShift API groups {id="mapping-support-tiers-to-openshift-api-groups_{{ context }}"}

API groups that end with the suffix `*.openshift.io` are governed by the {{ product_title }} deprecation policy and follow a general mapping between API version exposed and corresponding compatibility level unless otherwise specified.
...
{% if microshift %}
## Support for OpenShift API groups {id="microshift-mapping-support-tiers-to-openshift-api-groups_{{ context }}"}
API groups that end with the suffix `*.openshift.io` are governed by the {{ product_title }} deprecation policy and follow a general mapping between API version exposed and corresponding compatibility level unless otherwise specified.
...
{% if not microshift %}
## Support for Monitoring API groups {id="mapping-support-tiers-to-monitoring-api-groups_{{ context }}"}
...
{% if not microshift %}
## Support for Operator Lifecycle Manager API groups {id="mapping-support-tiers-to-olm-api-groups_{{ context }}"}
