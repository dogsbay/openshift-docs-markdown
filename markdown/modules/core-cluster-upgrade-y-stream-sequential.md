...
# Updating through sequential y-stream releases {id="core-cluster-upgrade-y-stream-sequential_{{ context }}"}
...
You must update through each intermediate y-stream version because skipping versions is not supported. {._abstract}
...
1.  Verify the first y-stream update completed successfully by running the following command:
    ```terminal
...
1.  Wait at least 24 hours, and up to 48 hours, between y-stream updates:
...
1.  Update OLM Operator subscriptions if needed:
...
1.  Create {{ rh_rhacm }} policies for the second y-stream update:
              ...
              objectDefinition:
                ...
                spec:
                  ...
                  desiredUpdate:
                    version: 4.22.0
    ```
1.  Apply the updated policy by running the following command:
    ```terminal
    $ oc apply -f core-upgrade-4.22-policy.yaml
    ```
1.  Create a new `ClusterGroupUpgrade` custom resource (CR) for the second y-stream:
    ```yaml
    ...
    spec:
      ...
      remediationStrategy:
        ...
        timeout: 120
    ```
1.  Apply and enable the `ClusterGroupUpgrade` CR by running the following commands:
    ```terminal
    ...
    $ oc patch cgu core-ystream-upgrade-4.22 \
      ...
      -p '{"spec":{"enable":true}}'
    ```
1.  Monitor the second y-stream update and verify completion by running the following commands:
    ```terminal
    $ oc get cgu core-ystream-upgrade-4.22 -n openshift-upgrade-policies -w
    $ oc --context=spoke1 get clusterversion
    $ oc --context=spoke1 get co
    ```
1.  Update Operators for the second y-stream by running the following command:
    ```terminal
    $ oc --context=spoke1 get csv -A | grep -v Succeeded
    ```

    Update Operator channels as needed for the new cluster version.
1.  Unpause worker nodes and complete the second y-stream update:

    Follow the worker node unpause procedure from "Pausing and unpausing worker nodes by using {{ cgu_operator }}".
1.  Repeat steps 2-12 for any additional intermediate y-stream versions:
...
1.  Verify workload functionality after all updates:
