...
1.  Create a pod and assign the pod to the secondary network that includes the VRF plugin configuration.
    ...
    1.  Create the `Pod` resource by running the following command. The expected output shows the name of the `Pod` resource and the creation age in minutes.
        ```terminal
        $ oc create -f pod-additional-net.yaml
        ```
1.  Verify that the pod network attachment connects to the VRF secondary network. Start a remote session with the pod and run the following command. The expected output shows the name of the VRF interface and its unique ID in the routing table.
    ```terminal
    $ ip vrf show
    ```
1.  Confirm that the VRF interface is the controller for the secondary interface by entering the following command:
    ```terminal
    $ ip link
    ```
    ```terminal
