...
# Deploying user-defined firmware to cluster hosts with {{ ztp }} {id="ztp-deploying-user-defined-firmware-configuration-with-gitops-ztp_{{ context }}"}
...
You can configure hardware profiles to apply to hosts in the following scenarios: {._abstract}
...
*   You have installed {{ rh_rhacm_first }} version 2.12 or later and logged in to the hub cluster as a user with `cluster-admin` privileges.
...
1.  Create the `HostFirmwareSettings` CR that contains the firmware settings you want to apply.
For example, create the following YAML file:
    ...
    ```
1.  Save the `HostFirmwareSettings` CR file relative to the `kustomization.yaml` file that you use to provision the cluster.
For example:
    ```terminal
    site-configs/
      └── site1-sno-du/
    ...
    ```
1.  Create a `ConfigMap` to store the `HostFirmwareSettings` CR.
...
1.  To apply a hardware profile to all hosts in the cluster, reference the `ConfigMap` in the `spec.templateRefs` field of your `ClusterInstance` CR.
For example:
    ```yaml
...
1.  Optional: To apply a hardware profile to a specific host in the cluster, reference the `ConfigMap` in the `spec.nodes[].templateRefs` field.
For example:
    ```yaml
    apiVersion: siteconfig.open-cluster-management.io/v1alpha1
    kind: ClusterInstance
...
1.  Commit the `ClusterInstance` CR, `ConfigMap`, and associated `kustomization.yaml` changes in your Git repository and push the changes.
