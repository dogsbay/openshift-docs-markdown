...
# Adding a multi-architecture compute machine set to your Google Cloud cluster {id="multi-architecture-modify-machine-set-google-cloud_{{ context }}"}
...
1.  Create a YAML file and add the configuration to create a compute machine set to control the 64-bit ARM or 64-bit x86 compute nodes in your cluster.
    ```yaml title="Example MachineSet object for a {{ gcp_short }} 64-bit ARM or 64-bit x86 compute node"
    ...
    `machineType`
    :   Specifies a machine type that aligns with the CPU architecture of the chosen OS image. For more information, see "Tested instance types for {{ gcp_short }} on 64-bit ARM infrastructures".
