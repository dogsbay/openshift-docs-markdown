...
# Defining cluster size requirements {id="serverless-cluster-sizing-req_{{ context }}"}

To install and use {{ ServerlessProductName }}, the {{ product_title }} cluster must be sized correctly.


:::note

The following requirements relate only to the pool of worker machines of the {{ product_title }} cluster. Control plane nodes are not used for general scheduling and are omitted from the requirements.
