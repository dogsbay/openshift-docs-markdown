...
# Optimization strategies for large-cluster NUMA-aware scheduling {id="cnf-optimizing-topology-aware-scheduler-large-clusters_{{ context }}"}
...
These optimization points help you balance scheduling accuracy, API server load, and control plane resource consumption for your specific cluster size and workload profile. {._abstract}
...
## NUMAResourcesOperator settings {id="_numaresourcesoperator_settings"}
...
## NUMAResourcesScheduler settings {id="_numaresourcesscheduler_settings"}
...
*   `Burstable` QoS (default): The recommended setting from {{ product_title }} 4.22. Minimizes baseline resource usage while allowing the scheduler to scale up during peak loads.
*   `Guaranteed` QoS (opt-in): Ensures the scheduler pod is protected from eviction. Use this profile for clusters where control plane memory is frequently overcommitted or under pressure.
    ...
    This option is expected to be removed in a later {{ product_title }} release.
    
    :::


## Best practices for large clusters {id="_best_practices_for_large_clusters"}
