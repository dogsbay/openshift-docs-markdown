...
1.  The type of Kubernetes object, `Task`.
1.  The unique name of this task.
1.  The list of parameters and steps in the task and the workspace used by the task.
...
Starting with {{ pipelines_shortname }} 1.6, the following defaults from the step YAML file are removed:
...
As a temporary measure, to maintain backward compatibility with the older {{ pipelines_shortname }} versions, you can set the following fields in the `TektonConfig` custom resource definition to `false`:
```
spec:
  pipeline:
