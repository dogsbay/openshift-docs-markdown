...
# Collecting a host network trace {id="support-collecting-host-network-trace_{{ context }}"}

Sometimes, troubleshooting a network-related issue is simplified by tracing network communication and capturing packets on multiple nodes at the same time. {._abstract}
...
1.  Perform the action, such as accessing a web application, that triggers the network communication issue while the network trace captures packets.

{% if not openshift_origin %}
1.  Review the packet capture files that `oc adm must-gather` transferred from the pods to your client machine:
    ```text
    tmp/captures
    ├── event-filter.html
