...
# Running the network verification manually using the CLI {id="running-network-verification-manually-cli_{{ context }}"}

You can manually run the network verification checks for an existing {{ product_title }} cluster by using the ROSA CLI (`rosa`). {._abstract}
