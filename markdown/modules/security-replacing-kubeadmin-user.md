...
# Replacing the kubeadmin user with a cluster-admin user {id="security-replacing-kubeadmin-user_{{ context }}"}

The `kubeadmin` user with the `cluster-admin` privileges is created on every cluster by default. To enhance the cluster security, you can replace the `kubeadmin` user with a `cluster-admin` user and then disable or remove the `kubeadmin` user. {._abstract}
...
1.  Verify the emergency user access:
    ...
    1.  Confirm that the user has `cluster-admin` privileges by running the following command:
        ```terminal
        $ oc whoami
        ```
