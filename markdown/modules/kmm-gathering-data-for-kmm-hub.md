...
# Gathering data for KMM-Hub {id="kmm-gathering-data-for-kmm-hub_{{ context }}"}

To collect diagnostic data for the KMM-Hub Operator on {{ product_title }}, you can run the `must-gather` tool with the hub controller image and review Operator logs. {._abstract}
...
1.  Gather the data for the KMM Operator hub controller manager:
    ...
    1.  Run the `must-gather` tool:
        ```terminal
        $ oc adm must-gather --image="${MUST_GATHER_IMAGE}" -- /usr/bin/gather -u
        ```
1.  View the Operator logs:
    ```terminal
