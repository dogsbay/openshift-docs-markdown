...
# Platform "none" Load balancing requirements {id="agent-install-load-balancing-none_{{ context }}"}
...
1.  **Application Ingress load balancer**: Provides an ingress point for application traffic flowing in from outside the cluster. A working configuration for the Ingress router is required for an {{ product_title }} cluster.
...
## Example load balancer configuration for platform "none" clusters {id="agent-install-load-balancing-none-example_{{ context }}"}
