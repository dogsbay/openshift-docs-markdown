...
# Static provisioning for Azure File {id="persistent-storage-csi-azure-file-static-provisioning-procedure_{{ context }}"}

Use static provisioning to manually create persistent volumes (PVs) for existing Azure File shares. Create a secret with storage credentials, define a PV that references the share, and create a persistent volume claim (PVC) to consume the storage. {._abstract}

**Prerequisites**

*   Access to an {{ product_title }} cluster with administrator rights
