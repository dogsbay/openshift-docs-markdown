...
# Go function return values {id="serverless-go-function-return-values_{{ context }}"}

Functions triggered by HTTP requests can set the response directly. You can configure the function to do this by using the Go [http.ResponseWriter](https://golang.org/pkg/net/http/#ResponseWriter).
...
```go title="Example CloudEvent response"
