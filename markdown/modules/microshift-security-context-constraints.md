...
# Security context constraint synchronization with pod security standards {id="microshift-security-context-constraints_{{ context }}"}

{{ microshift_short }} includes Kubernetes pod security admission (PSA) and a controller that applies PSA labels to namespaces based on the security context constraint (SCC) permissions of service accounts in that namespace. {._abstract}
