...
# About short-circuiting machine health check remediation {id="machine-health-checks-short-circuiting_{{ context }}"}

You can use machine health check short-circuiting to ensure that machine health checks remediate machines only when the cluster is healthy, by configuring the `maxUnhealthy` field in the `MachineHealthCheck` resource. {._abstract}
