...
# Enabling feature sets by using feature gates {id="hcp-enable-feature-sets_{{ context }}"}

You can enable the `TechPreviewNoUpgrade` feature set in a hosted cluster by editing the `HostedCluster` custom resource (CR) with the OpenShift CLI. {._abstract}
...
1.  Open the `HostedCluster` CR for editing on the hosting cluster by running the following command:
    ...
    ```
1.  Define the feature set by entering a value in the `featureSet` field. For example:
    ```yaml
