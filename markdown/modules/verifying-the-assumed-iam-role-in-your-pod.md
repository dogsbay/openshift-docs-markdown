...
# Verify the assumed IAM role in your pod {id="verifying-the-assumed-iam-role-in-your-pod_{{ context }}"}

After deploying an `awsboto3sdk` pod in your project, verify that the pod identity webhook has mutated the pod and check that the required AWS environment variables, volume mount, and OpenID Connect (OIDC) token volume are present within the pod. {._abstract}
...
1.  Verify that the AWS environment variables, the volume mount, and the OIDC token volume are listed in the description of the deployed `awsboto3sdk` pod:
    ...
    `Mounts`
    :   Lists the volume mounts for the pod. The `aws-iam-token` volume mount that was injected into the pod by the pod identity webhook is listed here.
...
1.  In the pod, verify that AWS Boto3 SDK operations run successfully:
    1.  In the interactive terminal for the pod, start a Python 3 shell:
        ...
        $ python3
        ```
    1.  In the Python 3 shell, import the `boto3` module:
        ```python
        >>> import boto3
        ```
    1.  Create a variable that includes the Boto3 `s3` service resource:
        ```python
        >>> s3 = boto3.resource('s3')
        ```
    1.  Print the names of all of the S3 buckets in your AWS account:
        ```python
