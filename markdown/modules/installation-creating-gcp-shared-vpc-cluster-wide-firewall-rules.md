...
*   You created the networking and load balancing components in {{ gcp_short }} that your cluster requires.

**Procedure**

1.  Add a single firewall rule to allow the {{ gcp_full }} Engine health checks to access all of the services. This rule enables the ingress load balancers to determine the health status of their instances.
    ```terminal
    ...
    ```
1.  Add a single firewall rule to allow access to all cluster services:
