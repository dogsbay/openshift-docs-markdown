...
# Migrating consumer applications to use HTTP transport for PTP {id="cnf-migrating-from-amqp-to-http-transport_{{ context }}"}
...
1.  Deploy the `consumer-events-subscription-service` service alongside the events consumer application.
For example:
    ```yaml
    apiVersion: v1
    kind: Service
