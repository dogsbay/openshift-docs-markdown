...
# Release notes for NVIDIA DPF Operator {{ dpf_version }} {id="nw-dpf-release-notes_{{ context }}"}
...
:   Added comprehensive DPU telemetry monitoring through the DOCA Telemetry Service (DTS) with built-in {{ product_title }} Console dashboard integration. DTS metrics are now accessible directly through the {{ product_title }} web console without requiring additional tools.
