...
# Overview of CSI volume group snapshots {id="persistent-storage-csi-group-snapshots-overview_{{ context }}"}

Volume group snapshots capture point-in-time copies of multiple persistent volume claims using label selectors. Three API objects manage snapshots: VolumeGroupSnapshot, VolumeGroupSnapshotContent, and VolumeGroupSnapshotClass.  {._abstract}
...
These three API kinds are defined as `CustomResourceDefinitions` (CRDs). These CRDs must be installed in a {{ product_title }} cluster for a CSI driver to support volume group snapshots.