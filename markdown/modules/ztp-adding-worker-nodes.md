...
# Adding an additional worker node {{ sno }} clusters with {{ ztp }} {id="ztp-additional-worker-sno-proc_{{ context }}"}

You can add an additional worker node to existing {{ sno }} clusters to increase available CPU resources in the cluster. {._abstract}

**Prerequisites**

*   Install and configure {{ rh_rhacm }} 2.12 or later in an {{ product_title }} 4.11 or later bare-metal hub cluster
*   Install {{ cgu_operator_full }} in the hub cluster
*   Install {{ gitops_title }} in the hub cluster
*   Use the {{ ztp }} `ztp-site-generate` container image version 4.12 or later
*   Deploy a managed {{ sno }} cluster with {{ ztp }}
*   Configure the Central Infrastructure Management as described in the {{ rh_rhacm }} documentation
              ...
              config:
                ...
                routes:
                  ...
                  - destination: ::/0
                    ...
                    table-id: 254
    ```
1.  Create a BMC authentication secret for the new host, as referenced by the `bmcCredentialsName` field in the `spec.nodes` section of your `ClusterInstance` CR:
    ```yaml
    apiVersion: v1
    data:
      password: "password"
    ...
    type: Opaque
    ```
1.  Commit the changes in Git, and then push to the Git repository that is being monitored by the {{ ztp }} ArgoCD application.
...
*   Continuously monitor the installation process:
    1.  Watch the agent install process by running the following command:
        ...
        ```
    1.  When the worker node installation is finished, the worker node certificates are approved automatically. At this point, the worker appears in the `ManagedClusterInfo` status. Run the following command to see the status:
        ```terminal
