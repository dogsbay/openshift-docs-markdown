...
kernel includes a preemptive scheduler that provides the operating system with real-time characteristics. For {{ product_title }}, {{ product_version }} you can make the switch to real-time kernel by using a `MachineConfig` object. 
...
*   Real-time support in {{ product_title }} is limited to specific subscriptions.
*   The following procedure is also supported for use with {{ gcp_full }}.

**Prerequisites**

*   Have a running {{ product_title }} cluster (version 4.4 or later).
