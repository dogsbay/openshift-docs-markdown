...
# Adding input secrets and config maps {id="builds-adding-input-secrets-configmaps_{{ context }}"}
...
1.  If the `Secret` object does not exist, create it by entering the following command:
    ```terminal
...
1.  Add the config map and secret to the `source` section in the existing
`BuildConfig` object:
    ```yaml
    source:
      git:
      ...
      secrets:
        - secret:
            name: secret-mvn
    ```
1.  To include the secret and config map in a new `BuildConfig` object, enter the following command:
    ```terminal
    $ oc new-build \
    ...
    During the build, the build process copies the `settings.xml` and `id_rsa` files into the directory where the source code is located. In {{ product_title }} S2I builder images, this is the image working directory, which is set using the `WORKDIR` instruction in the `Dockerfile`. If you want to specify another directory, add a `destinationDir` to the definition:
    ```yaml
    source:
      git:
        uri: https://github.com/wildfly/quickstart.git
