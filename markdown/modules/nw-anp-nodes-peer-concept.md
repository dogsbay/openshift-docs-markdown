...
# Northbound traffic controls for AdminNetworkPolicy and BaselineAdminNetworkPolicy {id="nw-anp-egress_{{ context }}"}

You can use `AdminNetworkPolicy` and `BaselineAdminNetworkPolicy` resources in {{ product_title }} to control your northbound traffic leaving the cluster or traffic leaving the node to other nodes in the cluster. {._abstract}

`AdminNetworkPolicy and BaselineAdminNetworkPolicy` resources support the following egress traffic control scenarios in {{ product_title }}:
...
## Using nodes peer to control egress traffic to cluster nodes {id="egress-traffic-control-towards-cluster-nodes_{{ context }}"}
...
## Using networks peer to control egress traffic towards external destinations {id="egress-traffic-control-networks-peer-external-destinations_{{ context }}"}
...
## Using nodes peer and networks peer together {id="combined-nodes-peer-networks-peer-anp"}
