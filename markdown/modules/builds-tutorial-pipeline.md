...
# Pipeline build tutorial {id="builds-tutorial-pipeline_{{ context }}"}
...
This example demonstrates how to create an {{ product_title }} Pipeline that will build, deploy, and verify a `Node.js/MongoDB` application using the `nodejs-mongodb.json` template.
...
1.  Create a file named `nodejs-sample-pipeline.yaml` with the following content:
    ...
    ```
1.  After you create a `BuildConfig` object with a `jenkinsPipelineStrategy`, tell the
pipeline what to do by using an inline `jenkinsfile`:
    ...
    The following `jenkinsfile` content is written in Groovy using the {{ product_title }} DSL. For this example, include inline content in the `BuildConfig` object using the YAML Literal Style, though including a `jenkinsfile` in your source repository is the preferred method.
    ...
    1.  Name of the template that will be created.
    1.  Spin up a `node.js` agent pod on which to run this build.
    1.  Set a timeout of 20 minutes for this pipeline.
    1.  Delete everything with this template label.
    1.  Delete any secrets with this template label.
