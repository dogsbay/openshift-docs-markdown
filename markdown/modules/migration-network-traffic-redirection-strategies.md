...
# Network traffic redirection strategies {id="migration-network-traffic-redirection-strategies_{{ context }}"}
...
*   Redirecting network traffic gradually for individual applications
    ...
    1.  Create a DNS record for each application with the source cluster hostname pointing to the proxy.
    1.  Configure the proxy entry for the application to route a percentage of the traffic to the target cluster router’s VIP and the rest of the traffic to the source cluster router’s VIP.
    1.  Gradually increase the percentage of traffic that you route to the target cluster router’s VIP until all the network traffic is redirected.
*   User-based redirection of traffic for individual applications
    ...
    1.  Create a proxy that can direct traffic to both the source cluster router’s VIP and the target cluster router’s VIP, for each application.
    1.  Create a DNS record for each application with the source cluster hostname pointing to the proxy.
    1.  Configure the proxy entry for the application to route traffic matching a given header pattern, such as `test customers`, to the target cluster router’s VIP and the rest of the traffic to the source cluster router’s VIP.
    1.  Redirect traffic to the target cluster router’s VIP in stages until all the traffic is on the target cluster router’s VIP.