...
# Cluster autoscaling settings using {{ cluster_manager }} {id="rosa-cluster-autoscale-settings_{{ context }}"}

The tables explain all the configurable UI settings when using cluster autoscaling with {{ cluster_manager }}. {._abstract}

**Configurable general settings for cluster autoscaling when using {{ cluster_manager }}**
...
| `balancing-ignored-labels` | This option specifies labels that the cluster autoscaler should ignore when considering node group similarity. This option cannot contain spaces. | `array (string)` | Format should be a comma-separated list of labels. |

**Configurable resource limit settings for cluster autoscaling when using {{ cluster_manager }}**
...
**Configurable scale down settings for cluster autoscaling when using {{ cluster_manager }}**
