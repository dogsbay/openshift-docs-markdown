...
# Modify node tuning configurations {id="rosa-modifying-node-tuning_{{ context }}"}

View and update node tuning configurations to adjust performance settings for your cluster by using the {{ rosa_cli_first }}. {._abstract}
...
1.  View the tuning configurations with the `rosa describe` command:
    ```terminal
    ...
    `<cluster_id>`
    :   Specifies the cluster ID for the cluster that you own that you want to apply a node tuning configuration.

    `<name_of_tuning>`
    :   Specifies the name of your tuning configuration.

    `-o json`
    :   Specifies the output type. This parameter is optional. If you do not specify any outputs, you see only the ID and name of the tuning configuration.
    ...
    **Example output specifying JSON output**
    ...
    ```
1.  After verifying the tuning configuration, edit the existing configuration with the `rosa edit` command:
    ```terminal
