...
# Starting pipelines from Pipelines view {id="op-starting-pipelines_from_pipelines_view_{{ context }}"}
...
1.  The **Start Pipeline** dialog box displays the **Git Resources** and the **Image Resources** based on the pipeline definition.
...
1.  Optional: Modify the **Advanced Options** to add the credentials that authenticate the specified private Git server or the image registry.
    1.  Under **Advanced Options**, click **Show Credentials Options** and select **Add Secret**.
    1.  In the **Create Source Secret** section, specify the following:
        1.  A unique **Secret Name** for the secret.
        1.  In the **Designated provider to be authenticated** section, specify the provider to be authenticated in the **Access to** field, and the base **Server URL**.
        1.  Select the **Authentication Type** and provide the credentials:
        ...
        1.  Select the check mark to add the secret.


            You can add multiple secrets based upon the number of resources in your pipeline.
1.  Click **Start** to start the pipeline.
1.  The **PipelineRun details** page displays the pipeline being executed. After the pipeline starts, the tasks and steps within each task are executed.
You can:
