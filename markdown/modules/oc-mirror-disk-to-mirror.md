...
# Mirroring from disk to mirror {id="oc-mirror-disk-to-mirror_{{ context }}"}

To mirror the contents of a generated image set to a target mirror registry, use the `oc-mirror` plugin. This action ensures that the required images are successfully transferred to your target environment {._abstract}

**Prerequisites**

*   You have installed the {{ oc_first }} in the disconnected environment.
