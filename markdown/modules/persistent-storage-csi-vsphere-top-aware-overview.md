...
# vSphere CSI topology overview {id="persistent-storage-csi-vsphere-top-aware-overview_{{ context }}"}

You can improve cluster resilience and avoid single points of failure by deploying {{ product_title }} across multiple vSphere zones and regions using topology-aware storage provisioning. {._abstract}
...
## vSphere CSI topology requirements {id="persistent-storage-csi-vsphere-top-aware-overview-top-requirements_{{ context }}"}
