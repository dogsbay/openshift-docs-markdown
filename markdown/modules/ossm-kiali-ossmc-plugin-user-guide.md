...
# About the OpenShift Service Mesh Console plugin {id="ossm-kiali-ossmc-plugin-user-guide_{{ context }}"}

The {{ SMPlugin }} is an extension to the {{ product_title }} web console that provides visibility into your Service Mesh.
...
You can install the OSSMC plugin in only one of two ways: using the {{ product_title }} web console, or through the CLI.
...
Installing the {{ SMPluginShort }} creates a new category, **Service Mesh**, in the main {{ product_title }} web console navigation. Click **Service Mesh** to see:
