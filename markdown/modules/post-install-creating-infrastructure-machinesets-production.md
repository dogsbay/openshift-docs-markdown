...
# Creating infrastructure machine sets for production environments {id="post-install-creating-infrastructure-machinesets-production_{{ context }}"}

Create infrastructure machine sets to host cluster infrastructure components without counting those machines toward subscription limits. {._abstract}
...
Applying a specific node selector to all infrastructure components causes {{ product_title }} to schedule those workloads on nodes with that label; see "Schedule infrastructure workloads using node selectors".