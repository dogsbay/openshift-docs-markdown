...
# Enabling 64k pages on the {{ op_system_first }} kernel {id="multi-architecture-enabling-64k-pages_{{ context }}"}

You can enable the 64k memory page in the {{ op_system_first }} kernel on the 64-bit ARM compute machines in your cluster. The 64k page size kernel specification can be used for large GPU or high memory workloads.  {._abstract}
