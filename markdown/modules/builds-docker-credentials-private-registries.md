...
# Using docker credentials for private registries {id="builds-docker-credentials-private-registries_{{ context }}"}
...
For the {{ product_title }} container image registry, this is not required because secrets are generated automatically for you by {{ product_title }}.
...
1.  Email address for the login.
1.  URL and credentials for a specific image in a namespace.
1.  URL and credentials for a registry namespace.
...
1.  Add a `pushSecret` field into the `output` section of the `BuildConfig` and set it to the name of the `secret` that you created, which in the previous example is `dockerhub`:
    ```yaml
    spec:
      output:
        to:
    ...
    ```
1.  Pull the builder container image from a private container image registry by specifying the `pullSecret` field, which is part of the build strategy definition:
    ```yaml
    strategy:
      sourceStrategy:
        from:
