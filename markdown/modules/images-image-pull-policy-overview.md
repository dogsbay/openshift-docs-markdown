...
# About the imagePullPolicy parameter {id="images-image-pull-policy-overview_{{ context }}"}

To control when {{ product_title }} pulls container images from registries or uses locally cached copies when starting containers, you can configure the `imagePullPolicy` parameter. This policy helps you manage image updates and optimize pod startup performance. {._abstract}
