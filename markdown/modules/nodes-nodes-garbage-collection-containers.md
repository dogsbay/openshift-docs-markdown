...
# Understanding how terminated containers are removed through garbage collection {id="nodes-nodes-garbage-collection-containers_{{ context }}"}

You can help ensure that your nodes are running efficiently by using container garbage collection to remove terminated containers. {._abstract}
...
*   **eviction-hard** - A hard eviction threshold has no grace period, and if observed, {{ product_title }} takes immediate action.
...
To protect against this oscillation, use the `evictionpressure-transition-period` flag to control how long {{ product_title }} must wait before transitioning out of a pressure condition. {{ product_title }} will not set an eviction threshold as being met for the specified pressure condition for the period specified before toggling the condition back to false.
