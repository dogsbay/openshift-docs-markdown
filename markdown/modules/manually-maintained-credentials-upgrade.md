...
# Manually updating cloud provider resources {id="manually-maintained-credentials-upgrade_{{ context }}"}

Meet the requirements of the target release by manually updating cloud provider credentials. Update these credentials by creating secrets for new components and by adjusting the permissions for existing components. {._abstract}
...
1.  Create YAML files with secrets for any `CredentialsRequest` custom resources that the new release image adds. The secrets must be stored using the namespace and secret name defined in the `spec.secretRef` for each `CredentialsRequest` object.
    ...
    ```yaml title="Sample {{ gcp_short }} CredentialsRequest object with secrets"
    apiVersion: cloudcredential.openshift.io/v1
    kind: CredentialsRequest
    metadata:
      name: <component_credentials_request>
    ...
    ```yaml title="Sample {{ gcp_short }} Secret object"
    apiVersion: v1
    kind: Secret
    metadata:
      name: <component_secret>
