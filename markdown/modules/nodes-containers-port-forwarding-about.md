...
The {{ oc_first }} listens on each local port specified by the user, forwarding using the protocol described below.
...
{{ product_title }} handles port-forward requests from clients. Upon receiving a request, {{ product_title }} upgrades the response and waits for the client
to create port-forwarding streams. When {{ product_title }} receives a new stream, it copies data between the stream and the pod’s port.

Architecturally, there are options for forwarding to a pod’s port. The supported {{ product_title }} implementation invokes `nsenter` directly on the node host
