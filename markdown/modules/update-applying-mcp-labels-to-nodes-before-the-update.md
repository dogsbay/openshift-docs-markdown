...
# Applying MachineConfigPool labels to nodes before the update {id="update-applying-mcp-labels-to-nodes-before-the-update_{{ context }}"}

Prepare `MachineConfigPool` (MCP) node labels to group nodes together in groups of roughly 8 to 10 nodes.
With MCP groups, you can reboot groups of nodes independently from the rest of the cluster. {._abstract}
