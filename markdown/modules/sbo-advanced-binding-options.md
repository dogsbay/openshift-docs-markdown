...
# Advanced binding options {id="sbo-advanced-binding-options_{{ context }}"}
...
## Changing the binding names before projecting them into the workload {id="changing-binding-names_{{ context }}"}
...
```yaml title="Example: Spring PetClinic sample application in the ServiceBinding CR"
# ...
    application:
...
```yaml title="Example: PostgreSQL database service in the ServiceBinding CR"
# ...
    services:
...
## Composing custom binding data {id="composing-custom-binding-data_{{ context }}"}
...
1.  The JSON name that the {{ servicebinding_title }} generates. The {{ servicebinding_title }} projects this JSON name as the name of a file or environment variable.
1.  The JSON value that the {{ servicebinding_title }} generates. The {{ servicebinding_title }} projects this JSON value as a file or environment variable. The JSON value contains the attributes from your specified field of the backing service custom resource.

## Binding workloads using a label selector {id="binding-workloads-using-a-label-selector_{{ context }}"}
You can use a label selector to specify the workload to bind. If you declare a service binding using the label selectors to pick up workloads, the {{ servicebinding_title }} periodically attempts to find and bind new workloads that match the given label selector.

For example, as a cluster administrator, you can bind a service to every `Deployment` in a namespace with the `environment: production` label by setting an appropriate `labelSelector` field in the `ServiceBinding` CR. This enables the {{ servicebinding_title }} to bind each of these workloads with one `ServiceBinding` CR.

```yaml title="Example ServiceBinding CR in the binding.operators.coreos.com/v1alpha1 API"
apiVersion: binding.operators.coreos.com/v1alpha1
kind: ServiceBinding
metadata:
...
```yaml title="Example ServiceBinding CR in the servicebinding.io API"
...
If you define the following pairs of fields, {{ servicebinding_title }} refuses the binding operation and generates an error:
