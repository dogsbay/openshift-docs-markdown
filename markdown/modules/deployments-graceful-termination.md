...
# Graceful termination {id="deployments-graceful-termination_{{ context }}"}

To avoid dropping user connections when pods shut down in {{ product_title }}, you can rely on graceful termination. The platform sends a `TERM` signal so your application can stop accepting new traffic, close open connections, and exit before the grace period ends. {._abstract}

On shutdown, {{ product_title }} sends a `TERM` signal to the processes in the container. Application code, on receiving `SIGTERM`, stop accepting new connections. This ensures that load balancers route traffic to other active instances. The application code then waits until all open connections are closed, or gracefully terminate individual connections at the next opportunity, before exiting.
