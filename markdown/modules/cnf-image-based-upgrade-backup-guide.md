...
{{ lvms }}
:   You must create the `Backup` and `Restore` CRs for {{ lvms }} artifacts. You must include the `persistentVolumes` resource in the application `Backup` CR.
...
## lca.openshift.io/apply-wave guidelines {id="ztp-image-based-upgrade-apply-wave-guide_{{ context }}"}
...
The `lca.openshift.io/apply-wave` annotation must be numerically lower in your platform `Restore` CRs, for example {{ rh_rhacm }} and {{ lvms }} artifacts, than that of the application.
...
## lca.openshift.io/apply-label guidelines {id="ztp-image-based-upgrade-apply-label-guide_{{ context }}"}
