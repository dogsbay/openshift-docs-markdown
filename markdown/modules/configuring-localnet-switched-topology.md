...
# Configuration for a localnet switched topology {id="configuration-localnet-switched-topology_{{ context }}"}

The switched `localnet` topology interconnects the workloads created as Network Attachment Definitions (NADs) through a cluster-wide logical switch to a physical network. {._abstract}
