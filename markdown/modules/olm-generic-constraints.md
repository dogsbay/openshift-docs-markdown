...
# Generic constraints {id="olm-generic-constraints_{{ context }}"}

An `olm.constraint` property declares a dependency constraint of a particular type. The `value` field is an object containing a `failureMessage` field holding a string-representation of the constraint message. If the constraint cannot be satisfied at runtime, OLM prints the message to users. {._abstract}
...
## Common Expression Language (CEL) constraints {id="olm-cel_{{ context }}"}
...
## Compound constraints (all, any, not) {id="olm-compound-constraints_{{ context }}"}
...
## Nested compound constraints {id="olm-nested-compound_{{ context }}"}
