...
*   Have a running {{ product_title }} cluster (version 4.6 or later).
    ...
    systemd:
      units:
        - name: usbguard.service
          enabled: true
    ```
1.  Convert the Butane configuration into a `MachineConfig` manifest by entering the following command:
    ```terminal
    $ butane 80-worker-usbguard.bu -o 80-worker-usbguard.yaml
    ```
1.  Apply the `MachineConfig` to the cluster by entering the following command: 
    ```terminal
...
1.  Check that the new machine config was successfully created by entering the following command:
    ...
    ```
1.  Check that the machine config is now applied and that the nodes are not in a degraded state. This operation might take a few minutes. The worker pool will show the updates in progress, as each machine successfully has the new machine config applied:
    ```terminal
    ...
    ```
1.  After the pool reports `UPDATED` as `True`, verify that the extension package was installed and that the service is running normally by debugging a compute node. You can complete these tasks by running the following commands:
    ```terminal
