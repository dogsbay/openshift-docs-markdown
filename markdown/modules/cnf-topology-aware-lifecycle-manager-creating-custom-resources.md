...
# Creating the custom resources for precaching {id="talm-prechache-user-specified-images-preparing-crs_{{ context }}"}

You must create the `PreCachingConfig` CR before or concurrently with the `ClusterGroupUpgrade` CR. {._abstract}
...
1.  Create a `ClusterGroupUpgrade` CR with the `preCaching` field set to `true` and specify the `PreCachingConfig` CR created in the previous step:
    ```yaml
    apiVersion: ran.openshift.io/v1alpha1
...
1.  When you want to start precaching the images, apply the `ClusterGroupUpgrade` CR by running the following command:
    ```terminal
    $ oc apply -f cgu.yaml
    ```

    {{ cgu_operator }} verifies the `ClusterGroupUpgrade` CR.
    From this point, you can continue with the {{ cgu_operator }} precaching workflow.
...
1.  Check the precaching status on the hub cluster where the `ClusterGroupUpgrade` CR is applied by running the following command:
    ...
    ```
1.  You can find the precaching job by running the following command on the managed cluster:
    ```terminal
    ...
    pre-cache   0/1               1s            1s
    ```
1.  You can check the status of the pod created for the precaching job by running the following command:
    ```terminal
    ...
    ```
1.  You can get live updates on the status of the job by running the following command:
    ```terminal
    $ oc logs -f pre-cache-abcd1 -n openshift-talo-pre-cache
    ```
1.  To verify the precache job is successfully completed, run the following command:
    ```terminal
    $ oc describe pod pre-cache -n openshift-talo-pre-cache
    ```
    ...
    ```
1.  To verify that the images are successfully precached on the {{ sno }}, do the following:
    1.  Enter into the node in debug mode:
        ```terminal
        $ oc debug node/cnfdf00.example.lab
        ```
    1.  Change root to `host`:
        ```terminal
        $ chroot /host/
        ```
    1.  Search for the required images:
        ```terminal
