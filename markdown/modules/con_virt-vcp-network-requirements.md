...
# Network requirements {id="con_virt-vcp-network-requirements_{{ context }}"}

Virtualized control plane deployments require specific network connectivity between VMs, worker nodes, and external services. {._abstract}

Configure networking to meet the following requirements:

*   L2 network connectivity between control plane VMs. The {{ product_title }} installation program requires all control plane nodes to share the same L2 network segment.
