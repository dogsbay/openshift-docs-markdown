...
# Using the cluster-compare plugin with must-gather data {id="using-cluster-compare-must-gather_{{ context }}"}

You can use the `cluster-compare` plugin to compare a reference configuration with configuration custom resources (CRs) from `must-gather` data. {._abstract}
