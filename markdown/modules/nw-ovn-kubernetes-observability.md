...
# Checking OVN-Kubernetes network traffic with OVS sampling using the CLI {id="nw-ovn-kubernetes-observability_{{ context }}"}

To trace OVN-Kubernetes network traffic with OVS sampling in {{ product_title }}, you can enable the `OVNObservability` feature gate and run `ovnkube-observ` from an `ovnkube-node` pod. {._abstract}

{%- set FeatureName = "Checking OVN-Kubernetes network traffic with OVS sampling" %}
