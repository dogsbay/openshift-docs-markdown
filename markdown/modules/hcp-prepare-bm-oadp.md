...
# Preparing bare metal to use {{ oadp_short }} for disaster recovery {id="hcp-prepare-bm-oadp_{{ context }}"}

Before you can perform disaster recovery for {{ hcp }} on bare metal, you need to meet a few prerequisites and configure the {{ oadp_first }} with Multicloud Object Gateway.  {._abstract}

**Prerequisites**

*   You installed the {{ oadp_short }} Operator on the management cluster. For more information, see "About installing {{ oadp_short }}".
...
*   In a disconnected environment, you have access to a self-hosted storage provider that is compatible with {{ oadp_short }}, such as [{{ odf_full }}](https://docs.redhat.com/en/documentation/red_hat_openshift_data_foundation/) or [MinIO](https://min.io/).
...
*   To prepare bare metal to use {{ oadp_short }}, complete the steps in "Configuring the {{ oadp_full }} with Multicloud Object Gateway".
