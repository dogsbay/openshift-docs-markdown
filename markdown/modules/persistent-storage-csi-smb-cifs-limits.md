...
# Limitations of the CIFS/SMB Driver Operator {id="persistent-storage-csi-smb-cifs-limits_{{ context }}"}

Before you deploy Common Internet File System (CIFS)/Server Message Block (SMB) storage, review the CSI driver limitations, including no support for FIPS mode, HTTP proxy, DFS, or Kerberos authentication. {._abstract}
