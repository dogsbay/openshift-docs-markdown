# Configuring node affinity rules {id="nodes-scheduler-node-affinity-configuring_{{ context }}"}

You can configure two types of node affinity rules: required and preferred.

## Configuring a required node affinity rule {id="_configuring_a_required_node_affinity_rule"}
...
1.  In the pod specification, use the `nodeAffinity` stanza to configure the `requiredDuringSchedulingIgnoredDuringExecution` parameter:
...
## Configuring a Preferred Node Affinity Rule {id="_configuring_a_preferred_node_affinity_rule"}
...
1.  Add a label to a node using the `oc label node` command:
    ...
    ```
1.  In the pod specification, use the `nodeAffinity` stanza to configure the `preferredDuringSchedulingIgnoredDuringExecution` parameter:
