...
# Remote writing to an external Prometheus instance {id="monitoring-configuring-shiftstack-remotewrite_{{ context }}"}

Use remote write with both {{ rhoso_first }} and {{ product_title }} to push their metrics to an external Prometheus instance.
...
1.  Configure your {{ rhoso }} management cluster to send metrics to Prometheus:
    1.  Create a secret that is named `mtls-bundle` in the `openstack` namespace that contains HTTPS client certificates for authentication to Prometheus by entering the following command:
        ...
        ```
    1.  Open the `controlplane` configuration for editing by running the following command:
        ```shell
        $ oc -n openstack edit openstackcontrolplane/controlplane
        ```
    1.  With the configuration open, replace the `.spec.telemetry.template.metricStorage` section so that {{ rhoso }} sends metrics to Prometheus. As an example:
        ```yaml
              metricStorage:
...
1.  Configure the tenant cluster on which your workloads run to send metrics to Prometheus:
    1.  Create a cluster monitoring config map as a YAML file. The map must include a remote write configuration and cluster identifiers. As an example:
        ...
        ```
        1.  Set a retention period. Optionally, you can reduce retention for local metrics because of external collection.
    1.  Save the config map as a file called `cluster-monitoring-config.yaml`.
    1.  Create a secret that is named `mtls-bundle` in the `openshift-monitoring` namespace that contains HTTPS client certificates for authentication to Prometheus by entering the following command:
        ```terminal
        ...
        ```
    1.  Apply the cluster monitoring configuration by running the following command:
        ```terminal
