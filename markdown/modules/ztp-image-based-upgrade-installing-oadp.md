...
# Installing and configuring the {{ oadp_short }} Operator with {{ ztp }} {id="ztp-image-based-upgrade-installing-oadp_{{ context }}"}

Install and configure the {{ oadp_short }} Operator with {{ ztp }} before starting the upgrade. {._abstract}
...
1.  Extract the following CRs from the `ztp-site-generate` container image and push them to the `source-cr` directory:
    ...
    ```
1.  Add the CRs to your common `PolicyGenTemplate`:
    ```yaml
    ...
    [...]
    ```
1.  Create the `DataProtectionApplication` CR and the S3 secret only for the target cluster:
    1.  Extract the following CRs from the `ztp-site-generate` container image and push them to the `source-cr` directory:
    ...
    1.  Add the CRs to your site `PolicyGenTemplate` with overrides:
        ```yaml
        apiVersion: ran.openshift.io/v1
        kind: PolicyGenTemplate
        metadata:
