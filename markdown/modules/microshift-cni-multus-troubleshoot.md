...
## Pod networking cannot be configured {id="Pod-networking-cannot-be-configured_{{ context }}"}
...
*   If you are a device administrator, you can inspect the `crio.service` or `microshift.service` logs, paying special attention to those that are generated by the `kubelet`.
    ...
    ```terminal title="Example kubelet-generated error"
...
## Missing configuration file {id="missing-nad_{{ context }}"}
...
```
```terminal title="Example error output"
