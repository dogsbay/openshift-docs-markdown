...
# Install KubeVirt Redfish {id="proc_virt-installing-kubevirt-redfish_{{ context }}"}

Install KubeVirt Redfish on your {{ VirtProductName }} cluster by applying a series of custom resources (CRs).
These CRs create the namespace, permissions, configuration, and deployment required to expose VMs through the Redfish API. {._abstract}

**Prerequisites**

*   You have a {{ product_title }} cluster with {{ VirtProductName }} installed.
*   You installed the OpenShift CLI (`oc`).
*   You logged in to {{ product_title }} as a user with `cluster-admin` privileges.
...
1.  Create the `Namespace` CR for KubeVirt Redfish by creating a YAML file with content such as the following example:
    ...
    ```
1.  Apply the resource by running the following command:
    ```terminal
    $ oc apply -f namespace.yaml
    ```
1.  Create the `ServiceAccount` CR by creating a YAML file with content such as the following example:
    ```yaml
    apiVersion: v1
    kind: ServiceAccount
    metadata:
...
1.  Apply the resource by running the following command:
    ...
    ```
1.  Create the `ClusterRole` CR with required permissions by creating a YAML file with content such as the following example:
    ```yaml
...
1.  Apply the resource by running the following command:
    ...
    ```
1.  Create the `ClusterRoleBinding` CR by creating a YAML file with content such as the following example:
    ```yaml
    apiVersion: rbac.authorization.k8s.io/v1
...
1.  Apply the resource by running the following command:
    ...
    ```
1.  Create a `Secret` CR to enable the Redfish API to manage specific VMs:
    ...
    1.  Create the `Secret` CR containing the configuration with content such as the following example.
    Edit the `config.yaml` section to match your environment:
        ...
        `datavolume`
        :   Specifies storage for VirtualMedia operations. Replace `<storage_class>` with a storage class available on your cluster, such as `lvms-vg1` or `ocs-storagecluster-ceph-rbd-virtualization`. For more information about storage options, see _Storage requirements_ in "Prerequisites for virtualized control planes".
...
1.  Apply the resource by running the following command:
    ...
    The credentials defined in this `Secret` CR enable full management control over the VMs exposed through KubeVirt Redfish, independently of any {{ product_title }} privileges.
...
1.  Apply the resource by running the following command:
    ...
    ```
1.  Create the `Service` CR by creating a YAML file with content such as the following example:
    ```yaml
    apiVersion: v1
    kind: Service
    metadata:
...
1.  Apply the resource by running the following command:
    ...
    ```
1.  Create the `Route` CR to expose the Redfish API externally by creating a YAML file with content such as the following example:
    ```yaml
...
1.  Verify that the pods are running by running the following command:
    ...
    ```
1.  Get the route hostname by running the following command:
    ```terminal
    $ oc get route kubevirt-redfish -n kubevirt-redfish -o jsonpath='{.spec.host}'
    ```
1.  Test the Redfish endpoint by running the following command:
    ```terminal
