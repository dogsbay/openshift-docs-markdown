...
# Configuring the NMState Operator {id="rdma-configuring-the-nmstate-operator_{{ context }}"}
...
1.  Validate that the Operator is installed and running by looking at the pods in the `openshift-nmstate` namespace:
    ```terminal
    ...
    ```
1.  Create a custom resource file for the required nmstate instance:
    ```yaml
    ...
    metadata:
      name: nmstate
    ```
1.  Create the instance on the cluster:
    ```terminal
    ...
    ```
1.  Validate the instance is running:
    ```terminal
    $ oc get pods -n openshift-nmstate
    ```
    ```terminal title="Example output"
    ...
    ```
1.  Build a `NodeNetworkConfigurationPolicy`. The example below configures a static ipaddress on the `ens8f0np0` interface on `vd-srv-32`.
    ```yaml
    apiVersion: nmstate.io/v1
      ...
      desiredState:
        ...
        - name: ens8f0np0 
          ...
          ipv4:
            ...
            enabled: true
    ```
1.  Create the custom resource file on the cluster:
    ```terminal
    ...
    ```
1.  Validate the policy is successfully configured:
    ```terminal
    $ oc get nncp -A
    ```
    ```terminal title="Example output"
    ...
    ```
1.  Validate the ipaddress is set by viewing inside the node at the interface:
    ```terminal
