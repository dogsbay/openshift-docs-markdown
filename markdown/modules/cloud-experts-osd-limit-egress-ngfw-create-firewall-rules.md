...
# Creating the firewall rules {id="cloud-experts-osd-limit-egress-ngfw-create-firewall-rules_{{ context }}"}

Create firewall rules for egress to private IP ranges and to the {{ product_title }} domains listed in this procedure. Egress to other external destinations does not match these rules and is not permitted. {._abstract}
...
1.  Create a blanket allow rule for private IP (Request for Comments (RFC) 1918) address space by running the following command:
    ...
    ```
1.  Create an allow rule for HTTPS (tcp/443) domains required for {{ product_title }} by running the following command:
