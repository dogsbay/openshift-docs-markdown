...
# Configuring multiple network interface controllers by using machine sets {id="machineset-vsphere-multiple-nics_{{ context }}"}

By configuring multiple network interface controllers (NICs), you can provide dedicated network links in the node virtual machines (VMs) for uses such as storage or databases. {{ product_title }} clusters on {{ vmw_first }} support connecting up to 10 network NICs to a node. {._abstract}

You can use machine sets to manage this configuration.

*   If you want to use multiple NICs in a {{ vmw_short }} cluster that was not configured to do so during installation, you can use machine sets to implement this configuration.
...
1.  For a cluster that already uses multiple NICs, obtain the following values from the `Infrastructure` resource by running the following command:
    ...
    **Required network interface controller values**
    ...
    | `failureDomain.topology.networks[<n-1>]` | `<vm_network_name_n>` | The name of the __n__th NIC to use. Collect the name of each NIC in the `Infrastructure` resource. |
    | `failureDomain.topology.template` | `<vm_template_name>` | The {{ vmw_short }} VM template to use. |
    ...
    | `failureDomain.topology.folder` | `<vcenter_vm_folder_path>` | The path to the {{ vmw_short }} VM folder in vCenter, such as `/dc1/vm/user-inst-5ddjd`. |
    | `failureDomain.topology.computeCluster` + `/Resources` | `<vsphere_resource_pool>` | The {{ vmw_short }} resource pool for your VMs. |
...
1.  Use a machine set configuration formatted like the following example.
    ...
    *   For a cluster that is not using multiple NICs, populate the values you want to use in the machine set custom resource.
    ...
    :   Specifies the {{ vmw_short }} VM template to use, such as `user-5ddjd-rhcos`.
