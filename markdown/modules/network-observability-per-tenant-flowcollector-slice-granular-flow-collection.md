...
# FlowCollectorSlice resource for granular flow collection {id="network-observability-per-tenant-flowcollector-slice-granular-flow-collection_{{ context }}"}

The `FlowCollectorSlice` is a custom resource definition (CRD) that enables granular, multi-tenant network flow collection. By defining logical slices based on namespaces or subnets, you can selectively collect traffic and apply custom sampling to specific workloads rather than the entire cluster. {._abstract}
...
## Benefits of FlowCollectorSlice {id="benefits-of-flowcollector-slice_{{ context }}"}
...
## Relationship between FlowCollector and FlowCollectorSlice {id="relationship-flowcollector-flowcollector-slice_{{ context }}"}
...
## Collection modes {id="collection-modes_{{ context }}"}
...
## FlowCollectorSlice status {id="flowcollector-slice-status_{{ context }}"}
