...
# A/B deployments {id="deployments-ab-testing_{{ context }}"}

To test a new application version with a limited share of production traffic in {{ product_title }}, you can use an A/B deployment. You send most requests to the stable version, route a fraction to the new version, and increase that fraction as testing progresses. {._abstract}
...
{{ product_title }} supports N-1 compatibility through the web console as well as
the CLI.