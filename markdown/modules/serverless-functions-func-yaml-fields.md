...
# Configurable fields in func.yaml {id="serverless-functions-func-yaml_{{ context }}"}
...
## buildEnvs {id="serverless-functions-func-yaml-buildenvs_{{ context }}"}
...
## envs {id="serverless-functions-func-yaml-envs_{{ context }}"}
...
1.  From a value assigned to a local environment variable. See the section "Referencing local environment variables from func.yaml fields" for more information.
1.  From a key-value pair stored in a secret or config map.
1.  You can also import all key-value pairs stored in a secret or config map, with keys used as names of the created environment variables.
...
1.  An environment variable set from a value assigned to a local environment variable.
1.  An environment variable assigned from a key-value pair stored in a secret.
1.  An environment variable assigned from a key-value pair stored in a config map.
1.  A set of environment variables imported from key-value pairs of a secret.
1.  A set of environment variables imported from key-value pairs of a config map.

## builder {id="serverless-functions-func-yaml-builder_{{ context }}"}

The `builder` field specifies the strategy used by the function to build the image. It accepts values of `pack` or `s2i`.

## build {id="serverless-functions-func-yaml-build_{{ context }}"}
...
## volumes {id="serverless-functions-func-yaml-volumes_{{ context }}"}
...
1.  The `myconfigmap` config map is mounted as a volume residing at `/workspace/configmap`.

## options {id="serverless-functions-func-yaml-options_{{ context }}"}
...
## image {id="serverless-functions-func-yaml-image_{{ context }}"}
...
## imageDigest {id="serverless-functions-func-yaml-imagedigest_{{ context }}"}
...
## labels {id="serverless-functions-func-yaml-labels_{{ context }}"}
...
## name {id="serverless-functions-func-yaml-name_{{ context }}"}
...
## namespace {id="serverless-functions-func-yaml-namespace_{{ context }}"}

The `namespace` field specifies the namespace in which your function is deployed.

## runtime {id="serverless-functions-func-yaml-runtime_{{ context }}"}
