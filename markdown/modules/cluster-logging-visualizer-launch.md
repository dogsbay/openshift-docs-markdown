...
# Launching the log visualizer {id="cluster-logging-visualizer-launch_{{ context }}"}

{{ product_title }} uses Kibana as the log visualizer. Kibana is a browser-based console to query, discover, and visualize your logs through histograms, line graphs,
...
*   To list the **infra** and **audit** indices in Kibana, a user must have the `cluster-admin` role, the `cluster-reader` role, or both roles. The default `kubeadmin` user has proper permissions to list these indices.
    ...
    The audit logs are not stored in the internal {{ product_title }} Elasticsearch instance by default. To view the audit logs in Kibana, you must use the Log Forward API to configure a pipeline that uses the `default` output for audit logs.
...
1.  Log in using the same credentials you use to log in to the {{ product_title }} console.
