...
# Gathering logs from a failed installation {id="installation-bootstrap-gather_{{ context }}"}

If you provided an SSH key to your installation program, you can gather data about your failed installation. {._abstract}
...
*   Your {{ product_title }} installation failed before the bootstrap process finished. The bootstrap node is running and accessible through SSH.
...
1.  Generate the commands that are required to obtain the installation logs from the bootstrap and control plane machines:
    *   If you used installer-provisioned infrastructure, change to the directory that contains the installation program and run the following command:
        ...
        The `installation_directory` placeholder is for the directory you specified when you ran `./openshift-install create cluster`. This directory contains the {{ product_title }} definition files that the installation program creates.
    ...
    *   If you used infrastructure that you provisioned yourself, change to the directory that contains the installation program and run the following command:
        ...
        where:
        *   `installation_directory`:: Specifies the same directory you specified when you ran `./openshift-install create cluster`. This directory contains the {{ product_title }} definition files that the installation program creates.
