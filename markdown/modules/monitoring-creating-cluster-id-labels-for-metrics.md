...
You can create cluster ID labels for metrics by adding the `write_relabel` settings for remote write storage in the `{{ configmap_name }}` config map in the `{{ namespace_name }}` namespace.
...
1.  Edit the `{{ configmap_name }}` config map in the `{{ namespace_name }}` project:
    ...
    ```
1.  In the `writeRelabelConfigs:` section under `data/config.yaml/{{ component }}/remoteWrite`, add cluster ID relabel configuration values:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
