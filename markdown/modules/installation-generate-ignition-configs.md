...
# Creating the Ignition config files {id="installation-generate-ignition-configs_{{ context }}"}

Because you must manually start the cluster machines, you must generate the
Ignition config files that the cluster needs to make its machines. {._abstract}
...
*   Obtain the Ignition config files:
    ...
    when copying installation files from an earlier {{ product_title }} version.
