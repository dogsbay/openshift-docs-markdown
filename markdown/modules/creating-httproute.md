...
# Creating a basic HTTPRoute custom resource {id="creating-httproute_{{ context }}"}

To direct incoming network traffic from a gateway to your backend applications, you must create an HTTPRoute custom resource (CR). The resource specifies the hostnames the route handles and binds them to a parent `Gateway` CR. {._abstract}
...
1.  Apply the `HTTPRoute` CR file to your cluster:
    ```terminal
