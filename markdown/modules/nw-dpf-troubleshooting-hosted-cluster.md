...
# Troubleshoot hosted cluster issues {id="nw-dpf-troubleshooting-hosted-cluster_{{ context }}"}

You can diagnose and resolve DPU hosted cluster issues, including CSR approval failures, kubeconfig access problems, and worker node join failures. {._abstract}
...
1.  Switch to the management cluster context and verify that the hosted control plane components are running:
    ```terminal
...
1.  Check the DPF HCP Provisioner status for any error conditions:
    ```terminal
...
1.  Return to the hosted cluster context and check for pending CSRs:
    ```terminal
    $ export KUBECONFIG=/path/to/hosted-cluster.kubeconfig
    ```
    ```terminal
    ...
    ```
1.  Approve any pending CSRs. To approve a single CSR, run the following command, replacing `<csr-name>` with the CSR name:
    ```terminal
    ...
    ```
1.  Verify that the DPU workers are joining the hosted cluster:
    ```terminal
    $ oc get nodes
    ```
    ```terminal title="Example output"
    ...
    ```
1.  If nodes are not joining, check whether the bootstrap token is still valid by running the following command on the hosted cluster:
    ```terminal
...
1.  Check the kubelet logs on the DPU for authentication or certificate errors. Switch to the management cluster context and open a debug shell on the DPU-enabled worker node:
    ```terminal
    $ export KUBECONFIG=/path/to/management-cluster.kubeconfig
    ```
    ```terminal
...
1.  Verify that OVN-Kubernetes is running correctly on the hosted cluster. Switch to the hosted cluster context and run the following command:
    ```terminal
    $ export KUBECONFIG=/path/to/hosted-cluster.kubeconfig
    ```
    ```terminal
