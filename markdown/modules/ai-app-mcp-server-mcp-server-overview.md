...
# MCP server overview {id="ai-app-mcp-server-mcp-server-overview_{{ context }}"}

A Model Context Protocol (MCP) server exposes cluster data and operations to AI models in a structured, secure way, so that AI agents can diagnose and act on your cluster without needing direct access to APIs or databases. {._abstract}

## The Goals of an MCP server {id="ai-app-mcp-server-mcp-server-overview-goals_{{ context }}"}
