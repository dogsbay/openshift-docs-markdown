...
When deploying a `UserDefinedNetwork` custom resource (CR) on {{ ibm_power_name }} Virtual Server with installer-provisioned infrastructure (IPI), you must set the MTU size to `1300` or `1250`.
...
1.  Optional: For a `UserDefinedNetwork` CR that uses a primary network, create a namespace with the `k8s.ovn.org/primary-user-defined-network` label by entering the following command:
    ...
    EOF
    ```
1.  Create a user-defined network for either a `Layer2` or `Layer3` topology type:
