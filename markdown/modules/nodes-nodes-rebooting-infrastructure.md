...
# About rebooting nodes running critical infrastructure {id="nodes-nodes-rebooting-infrastructure_{{ context }}"}

When rebooting nodes that host critical {{ product_title }} infrastructure components, such as router pods, registry pods, and monitoring pods, ensure that there are at least three nodes available to run these components. {._abstract}

The following scenario demonstrates how service interruptions can occur with applications running on {{ product_title }} when only two nodes are available:
