...
# Setting the body size limit for metrics scraping {id="setting-the-body-size-limit-for-metrics-scraping_{{ context }}"}
...
1.  Edit the `cluster-monitoring-config` `ConfigMap` object in the `openshift-monitoring` namespace:
    ...
    ```
1.  Add a value for `enforcedBodySizeLimit` to `data/config.yaml/prometheusK8s` to limit the body size that can be accepted per target scrape:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
