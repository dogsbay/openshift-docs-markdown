...
1.  Uninstall and reinstall the Network Observability Operator, ensuring that the installation is in a clean state.
...
1.  Verify that the old `FlowCollector` CRD version is still referenced in the `storedVersion`:
    ...
    ```
1.  If `v1alpha1` appears in the list of results, proceed with **Step a** to use the Kubernetes Storage Version Migrator or **Step b** to uninstall and reinstall the CRD and the Operator.
    1.  **Option 1: Kubernetes Storage Version Migrator**: Create a YAML to define the `StorageVersionMigration` object, for example `migrate-flowcollector-v1alpha1.yaml`:
        ...
        ```
        1.  Save the file.
        1.  Apply the `StorageVersionMigration` by running the following command:
            ```terminal
            $ oc apply -f migrate-flowcollector-v1alpha1.yaml
            ```
        1.  Update the `FlowCollector` CRD to manually remove `v1alpha1` from the `storedVersion`:
            ```terminal
            $ oc edit crd flowcollectors.flows.netobserv.io
            ```
    1.  **Option 2: Reinstall**: Save the Network Observability Operator 1.5 version of the `FlowCollector` CR to a file, for example `flowcollector-1.5.yaml`.
        ```terminal
        ...
        1.  Install the Network Observability Operator latest version, 1.6.0.
        1.  Create the `FlowCollector` using backup that was saved in Step b.
