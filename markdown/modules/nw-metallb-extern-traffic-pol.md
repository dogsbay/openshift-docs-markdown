...
# MetalLB and external traffic policy {id="nw-metallb-extern-traffic-pol_{{ context }}"}

External traffic policy for MetalLB LoadBalancer services determines how the service proxy distributes traffic to pods. Set the policy to `cluster` for uniform distribution or to `local` to preserve client IP addresses. {._abstract}
