...
# Running low latency applications on {{ product_title }} {id="ztp-low-latency_{{ context }}"}

{{ product_title }} enables low latency processing for applications running on commercial off-the-shelf (COTS) hardware by using several technologies and specialized hardware devices: {._abstract}
