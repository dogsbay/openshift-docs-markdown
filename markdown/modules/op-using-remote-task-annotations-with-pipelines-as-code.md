...
# Using remote task annotations with {{ pac }} {id="using-remote-task-annotations-with-pipelines-as-code_{{ context }}"}

{{ pac }} supports fetching remote tasks or pipelines by using annotations in a pipeline run. If you reference a remote task in a pipeline run, or a pipeline in a `PipelineRun` or a `PipelineSpec` object, the {{ pac }} resolver automatically includes it. If there is any error while fetching the remote tasks or parsing them, {{ pac }} stops processing the tasks. {._abstract}

To include remote tasks, refer to the following examples of annotation:

**Reference remote tasks in {{ tekton_hub }}**

*   Reference a single remote task in {{ tekton_hub }}.
    ...
    ...
    ```
    1.  {{ pac }} includes the latest version of the task from the {{ tekton_hub }}.
*   Reference multiple remote tasks from {{ tekton_hub }}
    ...
    ...
    ```
*   Reference multiple remote tasks from {{ tekton_hub }} using the `-<NUMBER>` suffix.
    ...
    ...
    ```
    1.  By default, {{ pac }} interprets the string as the latest task to fetch from {{ tekton_hub }}.
*   Reference a specific version of a remote task from {{ tekton_hub }}.
