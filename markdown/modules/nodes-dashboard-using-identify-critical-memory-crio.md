...
# Nodes with CRI-O system reserved memory utilization > 50% {id="nodes-dashboard-using-identify-critical-memory-crio_{{ context }}"}

You can identify the nodes where the percentage of used memory reserved for the CRI-O system is greater than or equal to 50% by using the **Nodes with CRI-O system reserved memory utilization > 50%** query in the node metrics dashboard.  {._abstract}
