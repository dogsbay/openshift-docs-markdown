...
# Creating a log forwarder {id="logging-create-clf_{{ context }}"}
...
```yaml title="ClusterLogForwarder resource example"
...
1.  The type of output that you want to forward logs to. The value of this field can be `default`, `loki`, `kafka`, `elasticsearch`, `fluentdForward`, `syslog`, or `cloudwatch`.
...
1.  A name for the output that you want to forward logs to.
1.  The URL of the output that you want to forward logs to.