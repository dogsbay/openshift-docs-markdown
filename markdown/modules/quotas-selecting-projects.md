...
# Selecting multiple projects during quota creation {id="quotas-setting-projects_{{ context }}"}

To aggregate resource usage and enforce consistent limits across multiple namespaces, you can select target projects by using annotation or label selectors when creating a `ClusterResourceQuota` object. {._abstract}
