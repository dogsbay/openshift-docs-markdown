...
# {{ zero_trust_full }} 1.0.0 (General Availability) {id="zero-trust-manager-ga-1-0-0-rns_{{ context }}"}

Issued: 2025-12-17

This release introduces capabilities for enterprise readiness, security, and operational flexibility. It includes SPIRE federation for cross-cluster identity, PostgreSQL support for production persistence, and enhanced security through stricter constraints and API validation. {._abstract}
...
{{ zero_trust_full }} supports the following components and versions:git
...
## Status and observability improvements {id="stats-obs-improvements_{{ context }}"}
