...
# Configuring Roles for Logging {id="log6x-config-roles_{{ context }}"}
...
## Cluster wide access {id="cluster-wide-access_{{ context }}"}
Cluster role binding resources reference cluster roles, and set permissions cluster wide.

```yaml title="Example ClusterRoleBinding"
...
1.  Specifies the users or groups this object applies to.

## Namespaced access {id="namespaced-access_{{ context }}"}
...
```yaml title="Example RoleBinding"
kind: RoleBinding
apiVersion: rbac.authorization.k8s.io/v1
metadata:
...
## Custom admin group access {id="custom-admin-group-access_{{ context }}"}
...
1.  Overrides the default groups (`system:cluster-admins`, `cluster-admin`, `dedicated-admin`)