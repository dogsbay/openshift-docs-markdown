...
The Machine Config Operator (MCO) is responsible for mounting a secondary disk for {{ product_title }} {{ product_version }} container storage.
...
1.  Create the following script and name it `etcd-find-secondary-device.sh`:
    ```bash
    #!/bin/bash
    set -uo pipefail
...
1.  Create a base64-encoded string from the `etcd-find-secondary-device.sh` script and note its contents:
    ```terminal
    $ base64 -w0 etcd-find-secondary-device.sh
    ```
1.  Create a `MachineConfig` YAML file named `etcd-mc.yml` with contents such as the following example:
    ```yaml
...
1.  Apply the created `MachineConfig` YAML file:
    ```terminal
