...
# Configuring Amazon SQS and Amazon EventBridge to receive Amazon EC2 interruption events {id="hcp-aws-config-sqs-eventbridge_{{ context }}"}

Before you enable Amazon Spot Instances, you must set up Amazon Simple Queue Service (SQS) and Amazon EventBridge to receive Amazon EC2 interruption events.  {._abstract}
...
1.  Create an Amazon SQS queue to receive Spot Instance notifications by entering the following command:
    ...
    $ aws sqs create-queue \
      ...
      --region "${AWS_REGION}"
    ```
    *   Be sure to replace `my-cluster` and `us-east-1` with your cluster name and {{ aws_short }} region. 
    *   Take note of the URL from the output. You need the URL when you create the hosted cluster.
1.  Create Amazon EventBridge rules to route Amazon EC2 Spot interruption warnings and rebalance recommendations to the SQS queue.
    1.  Set the attributes by entering the following command:
        ...
        ```
    1.  Set the interruption warning by entering the following command:
        ```terminal
        $ aws events put-rule \
          ...
          --region "${AWS_REGION}"
        ```
    1.  Set the target for the interruption warning by entering the following command:
        ```terminal
        $ aws events put-targets \
          ...
          --region "${AWS_REGION}"
        ```
    1.  Set the recommendations by entering the following command:
        ```terminal
        $ aws events put-rule \
          ...
          --region "${AWS_REGION}"
        ```
    1.  Set the target for the recommendations by entering the following command:
        ```terminal
        $ aws events put-targets \
          ...
          --region "${AWS_REGION}"
        ```
1.  Configure the Amazon SQS queue policy to allow EventBridge to send messages to the queue by entering the following command:
    ```terminal
    ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
    QUEUE_URL="https://sqs.${AWS_REGION}.amazonaws.com/${ACCOUNT_ID}/${CLUSTER_NAME}-spot-interruption-queue"
