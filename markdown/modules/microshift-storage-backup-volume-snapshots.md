...
1.  Get the unique identity of the volume created on the storage backend by using the following command and inserting the name retrieved in the previous step:
    ```terminal
...
1.  Display the snapshots by using the unique identity of the volume you retrieved in the previous step to determine which one you want to backup by running the following command:
    ```terminal
    ...
    *   Replace `_<retrieved_volume_identity>_` with the volume identity.
        ...
        Block device           253:11
        ```
1.  Create a directory to use for mounting the LV by running the following command:
    ```terminal
    $ sudo mkdir /mnt/snapshot
    ```
1.  Mount the LV using the device name for the retrieved snapshot handle by running the following command:
    ```terminal
    $ sudo mount /dev/_<retrieved_snapshot_handle>_ /mnt/snapshot
    ```
    *   Replace `_<retrieved_snapshot_handle>_` with the device name.
1.  Copy the files from the mounted location and store them in a secure location by running the following command:
    ```terminal
