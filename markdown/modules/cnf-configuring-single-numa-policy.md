...
# Configuring a single NUMA node policy {id="cnf-configuring-single-numa-policy_{{ context }}"}

To enable the NUMA Resources Operator, configure a single NUMA node policy on your cluster. You can implement this policy by creating a performance profile or by configuring a `KubeletConfig` custom resource (CR). {._abstract}
