...
# Creating a volume group snapshot {id="persistent-storage-csi-group-snapshots-create_{{ context }}"}

Create a VolumeGroupSnapshot object to capture point-in-time copies of multiple persistent volume claims (PVCs). Label the PVCs, specify the `VolumeGroupSnapshotClass`, and individual `VolumeSnapshot` objects are created automatically. {._abstract}

When you create a `VolumeGroupSnapshot` object, {{ product_title }} creates a volume group snapshot.
...
1.  Label the PVCs to belong to a snapshot group:
    1.  Label PVC pvc-0 by running the following command:
        ...
        ```
    1.  Label PVC pvc-1 by running the following command:
        ```terminal title="Example output"
...
1.  Create a `VolumeGroupSnapshot` object to specify your volume group snapshot:
    ...
    1.  Create the `VolumeGroupSnapshot` object by running the following command:
        ```terminal
