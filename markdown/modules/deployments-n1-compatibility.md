...
# N-1 compatibility {id="deployments-n1-compatibility_{{ context }}"}

To run old and new application code side by side during a rollout in {{ product_title }}, you need N-1 compatibility. Design your data and schemas so that values written by the new version can be read or safely ignored by the old version. {._abstract}
