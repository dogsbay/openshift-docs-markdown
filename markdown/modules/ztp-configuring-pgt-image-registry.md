...
*   You have created a Git repository where you manage your custom site configuration data for use with {{ ztp_first }}.
...
*   Verify successful login to the registry while logged in to the managed cluster. Run the following commands:
    ...
    1.  Download and export the cluster `kubeconfig`:
        ...
        ```
    1.  Verify access to the image registry from the managed cluster. See "Accessing the registry".
...
*   Check that the disk partition on the managed cluster is correct:
    1.  Open a debug shell to the managed cluster:
        ...
        ```
    1.  Run `lsblk` to check the host disk partitions:
        ```terminal
        sh-4.4# lsblk
