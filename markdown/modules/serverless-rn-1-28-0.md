...
# Release notes for Red Hat {{ ServerlessProductName }} 1.28 {id="serverless-rn-1-28-0_{{ context }}"}

{{ ServerlessProductName }} 1.28 is now available. New features, changes, and known issues that pertain to {{ ServerlessProductName }} on {{ product_title }} are included in this topic.

## New features {id="new-features-1-28-0_{{ context }}"}
...
## Known issues {id="known-issues-1-28-0_{{ context }}"}

*   Currently, runtimes for Python are not supported for {{ ServerlessProductName }} Functions on {{ ibm_power_name }}, {{ ibm_z_name }} Systems, and {{ ibm_linuxone_name }}.
