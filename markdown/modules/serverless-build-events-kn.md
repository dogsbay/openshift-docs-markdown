...
# Building events by using the kn-event plugin {id="serverless-build-events-kn_{{ context }}"}
...
*   Build an event:
    ...
    *   You can use the `json` or `yaml` arguments with the `--output` flag to change the output format of the event.
        ...
        ```terminal title="Building a simple event"
        $ kn event build -o yaml
        ```
        ```yaml title="Resultant event in the YAML format"
        data: {}
        ...
        ```
        ```terminal title="Building a sample transaction event"
        $ kn event build \
            ...
            --output json
        ```
        ```json title="Resultant event in the JSON format"
        {
          "specversion": "1.0",
          "id": "RjtL8UH66X+UJg==",
          "source": "kn-event/v0.4.0",
