...
# Replacing a bare-metal control plane node {id="replacing-a-bare-metal-control-plane-node_{{ context }}"}

Replace a failed or unhealthy control plane node by removing the old `BareMetalHost` and `Machine` objects, then creating new ones to maintain cluster high availability. {._abstract}
...
Existing control plane `BareMetalHost` objects might have the `externallyProvisioned` flag set to `true` if they were provisioned by the {{ product_title }} installation program.
...
1.  Ensure that the Bare Metal Operator is available:
    ...
    ```
1.  Remove the old `BareMetalHost` and `Machine` objects:
    ```terminal
...
1.  Create the new `BareMetalHost` object and the secret to store the BMC credentials. Set the following parameters:
...
1.  View available `BareMetalHost` objects:
    ```terminal
...
1.  To view the `BareMetalHost` objects, run the following command:
    ```terminal
    $ oc get bmh -A
    ```
    ```terminal title="Example output"
    ...
    ```
1.  After the RHCOS installation, verify that the `BareMetalHost` is added to the cluster:
    ```terminal
    $ oc get nodes
    ```
    ```terminal title="Example output"
