...
# Creating an API server source by using the Knative CLI {id="apiserversource-kn_{{ context }}"}
...
1.  Create an API server source that has an event sink. In the following example, the sink is a broker:
    ```terminal
    $ kn source apiserver create <event_source_name> --sink broker:<broker_name> --resource "event:v1" --service-account <service_account_name> --mode Resource
    ```
1.  To check that the API server source is set up correctly, create a Knative service that dumps incoming messages to its log:
    ```terminal
    $ kn service create <service_name> --image quay.io/openshift-knative/knative-eventing-sources-event-display:latest
    ```
1.  If you used a broker as an event sink, create a trigger to filter events from the `default` broker to the service:
    ```terminal
...
1.  Create events by launching a pod in the default namespace:
    ...
    ```
1.  Check that the controller is mapped correctly by inspecting the output generated by the following command:
    ```terminal
...
1.  Get the pods:
    ...
    $ oc get pods
    ```
1.  View the message dumper function logs for the pods:
    ```terminal
