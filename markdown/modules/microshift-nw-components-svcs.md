...
# {{ microshift_short }} networking components and services {id="microshift-nw-components-svcs_{{ context }}"}

Understand networking components and services and their operation in {{ microshift_short }}. {._abstract}
...
NetworkManager
:   NetworkManager is required to set up the initial gateway bridge on the {{ microshift_short }} node. The NetworkManager and `NetworkManager-ovs` RPM packages are installed as dependencies to the `microshift-networking` RPM package, which contains the necessary configuration files. NetworkManager in {{ microshift_short }} uses the `keyfile` plugin and is restarted after installation of the `microshift-networking` RPM package.
...
OVN containers
:   Two OVN-Kubernetes daemon sets are rendered and applied by {{ microshift_short }}.
