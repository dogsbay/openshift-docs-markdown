...
# Example security context constraints {id="security-context-constraints-example_{{ context }}"}

The following examples show how to define and use security context constraints (SCCs) in your cluster. {._abstract}
...
{{ product_title }} only when a service account or a user is granted access to a SCC that allows such a user ID. The SCC can allow arbitrary IDs, an ID that falls into a range, or the exact user ID specific to the request.
