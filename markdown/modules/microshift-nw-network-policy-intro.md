...
By default, all pods in a {{ microshift_short }} node are accessible from other pods and network endpoints. To isolate one or more pods in a node, you can create `NetworkPolicy` objects to indicate allowed incoming connections. You can create and delete `NetworkPolicy` objects.
...
*   Deny all traffic:
    ...
    spec:
      ...
      ingress: []
    ```
*   Allow connections from the default router, which is the ingress in {{ microshift_short }}:

    To allow connections from the {{ microshift_short }} default router, add the following `NetworkPolicy` object:
    ```yaml
    apiVersion: networking.k8s.io/v1
    kind: NetworkPolicy
    metadata:
