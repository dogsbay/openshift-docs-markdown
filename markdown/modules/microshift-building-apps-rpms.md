...
# Build the RPM package for the application manifests {id="microshift-building-apps-rpms_{{ context }}"}

To build your own RPMs, you must create a spec file that adds the application manifests to the RPM package. If the application RPMs and other elements needed for image building are accessible to image builder, you can use the method that you prefer. {._abstract}
...
1.  In the `~/rpmbuild/SPECS` directory, create a file such as `_<application_workload_manifests.spec>_` using the following template:
    ```terminal title="Example spec file"
...
1.  Build your RPM package in the `~/rpmbuild/RPMS` directory by running the following command:
    ```terminal
