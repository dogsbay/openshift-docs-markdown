...
# Overview of the CIFS/SMB Driver Operator {id="persistent-storage-csi-smb-cifs-overview_{{ context }}"}

You can provision persistent volumes (PVs) that mount to network file shares by using the CIFS/SMB CSI Driver Operator, which supports dynamic volume provisioning for on-demand storage. {._abstract}
...
After installing the CIFS/SMB CSI Driver Operator, {{ product_title }} installs corresponding pods for the Operator and driver in the `openshift-cluster-csi-drivers` namespace by default. This allows the CIFS/SMB CSI Driver to create CSI-provisioned persistent volumes (PVs) that mount to CIFS/SMB shares.
