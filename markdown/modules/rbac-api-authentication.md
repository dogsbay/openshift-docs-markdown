...
# API authentication {id="rbac-api-authentication_{{ context }}"}

Requests to the {{ product_title }} API are authenticated using OAuth access tokens or X.509 client certificates, with invalid credentials rejected and anonymous requests assigned virtual user and group identities for authorization processing. {._abstract}


OAuth access tokens
:   *   Obtained from the {{ product_title }} OAuth server using the
