...
# Cluster operator stability during degraded operation {id="cluster-operator-stability-during-degraded-operation_{{ context }}"}

{{ product_title }} cluster Operators maintain control plane stability and API availability when a two-node OpenShift cluster with fencing (TNF) enters a degraded state. {._abstract}
