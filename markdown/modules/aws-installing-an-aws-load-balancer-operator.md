...
# Installing an AWS Load Balancer Operator {id="aws-installing-an-aws-load-balancer-operator_{{ context }}"}
You can install an AWS Load Balancer Operator and an AWS Load Balancer Controller if you meet certain requirements.

**Prerequisites**

*   You have an existing {{ product_title }} (ROSA) cluster with bring-your-own-VPC (BYO-VPC) configuration across multiple Availability Zones (AZs) installed in Hosted Control Plane (HCP) mode.
...
1.  Identify the cluster infrastructure ID and the cluster OpenID Connect (OIDC) DNS by running the following commands:
    1.  Identify the ROSA cluster ID:
        ...
        ```
    1.  Identify the ROSA cluster OIDC DNS by using the following `rosa` CLI command:
        ```terminal
...
1.  Create the AWS IAM policy required for the AWS Load Balancer Operator by using the AWS CLI.
    1.  Log in to the ROSA cluster as a user with the `dedicated-admin` role and create a new project using the following command:
        ...
        ```
    1.  Assign the following trust policy to the newly-created AWS IAM role:
        ```terminal
    ...
    1.  Create and verify the role by using the generated trust policy:
        ```terminal
    ...
    1.  Attach the operator’s permission policy to the role:
        ```terminal
        ...
        ```
1.  Create the AWS IAM policy required for the AWS Load Balancer Controller by using the AWS CLI:
    ...
    1.  Attach the controller’s permission policy to the role:
        ```terminal
        ...
        ```
1.  For a ROSA with HCP cluster, add the tags necessary for subnet discovery:
    1.  Add the following `{Key: Value}` tag to the VPC hosting the ROSA cluster and to all its subnets. Replace `{Cluster Infra ID}` with the Infra ID specified previously:
        ...
        ```
    1.  Add the following ELBv2 `{Key: Value}` tags to the private subnets and, optionally, to the public subnets:
...
1.  Create the AWS Load Balancer Operator by completing the following steps:
    ...
    1.  Create a `Subscription` object by running the following command:
        ...
        EOF
        ```
        1.  Specifies the ARN role for the {{ aws_short }} Load Balancer Operator. The `CredentialsRequest` object uses this ARN role to provision the {{ aws_short }} credentials. An example of `<operator_role_arn>` is `arn:aws:iam::<aws_account_number>:role/albo-operator`.
1.  Create the AWS Load Balancer Controller:
    ```yaml
...
1.  Confirm a successful installation by running the following commands:
    1.  Gather information about pods within the project:
        ...
        ```
    1.  View the logs within the project:
        ```terminal
