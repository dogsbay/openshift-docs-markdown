...
# Adding worker nodes to {{ sno }} clusters manually {id="sno-adding-worker-nodes-to-single-node-clusters-manually_{{ context }}"}

You can add a worker node to a {{ sno }} cluster manually by booting the worker node from {{ op_system_first }} ISO and by using the cluster `worker.ign` file to join the new worker node to the cluster. {._abstract}
...
1.  Download the {{ product_title }} installer and make it available for use by running the following commands:
    ```terminal
    ...
    ```
1.  Retrieve the {{ op_system }} ISO URL:
    ```terminal
...
1.  Download the {{ op_system }} ISO:
    ...
    ```
1.  Use the {{ op_system }} ISO and the hosted `worker.ign` file to install the worker node:
    1.  Boot the target host with the {{ op_system }} ISO and your preferred method of installation.
    1.  When the target host has booted from the {{ op_system }} ISO, open a console on the target host.
    1.  If your local network does not have DHCP enabled, you need to create an ignition file with the new hostname and configure the worker node static IP address before running the {{ op_system }} installation. Perform the following steps:
        ...
        1.  Activate the modified network interface:
            ```terminal
            $ nmcli con up <network_interface>
            ```
        1.  Create a new ignition file `new-worker.ign` that includes a reference to the original `worker.ign` and an additional instruction that the `coreos-installer` program uses to populate the `/etc/hostname` file on the new worker host. For example:
            ```json
            {
              "ignition":{
                "version":"3.2.0",
        ...
        1.  Host the `new-worker.ign` file on a web server accessible from your network.
        1.  Run the following `coreos-installer` command, passing in the `ignition-url` and hard disk details:
            ```terminal
            ...
            `<hard_disk>`
            :   Specifies the hard disk where you install {{ op_system }}, for example, `/dev/sda`.
    1.  For networks that have DHCP enabled, you do not need to set a static IP. Run the following `coreos-installer` command from the target host console to install the system:
        ```terminal
        $ coreos-installer install --ignition-url=<hosted_worker_ign_file> <hard_disk>
        ```
    1.  To manually enable DHCP, apply the following `NMStateConfig` CR to the {{ sno }} cluster:
        ```yaml
