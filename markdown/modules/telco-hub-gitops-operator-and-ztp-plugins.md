...
# GitOps Operator and {{ ztp }} {id="telco-hub-gitops-operator-and-ztp-plugins_{{ context }}"}
...
:   *   To avoid confusion or unintentional overwrite when updating content, use unique and distinguishable names for custom CRs in the `source-crs` directory and extra manifests.
    ...
    *   To help with multiple versions, keep all source CRs and policy creation CRs in versioned Git repositories to ensure consistent generation of policies for each {{ product_title }} version.