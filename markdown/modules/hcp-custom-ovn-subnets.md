...
# Configuring internal OVN IPv4 subnets for hosted clusters {id="hcp-custom-ovn-subnets_{{ context }}"}

In hosted clusters, you can configure internal OVN subnets to avoid routing conflicts, customize network architecture, or enable virtual private cloud (VPC) peering. {._abstract}


Avoid CIDR conflicts
:   Connect VPCs that host {{ product_rosa }} clusters with other VPCs that use the default OVN internal subnets of 100.88.0.0/16 and 100.64.0.0/16. To avoid a conflict on a `kubevirt` hosted cluster, the HyperShift Operator sets the OVN join subnet to `100.66.0.0/16` instead of `100.64.0.0/16` or `100.65.0.0/16`. The default classless inter-domain routing (CIDR) for the OVN gateway router is `100.64.0.0/16`. The default CIDR for the user-defined network (UDN) join subnet is `100.65.0.0/16`.
...
*   Your hosted cluster version must be {{ product_title }} 4.20 or later.
...
1.  Verify that the hosted configuration is correct by entering the following command:
    ...
    ```
1.  Check the Network Operator configuration in the hosted cluster:
    1.  Extract the hosted cluster kubeconfig file by entering the following command:
        ...
        ```
    1.  Verify the Network Operator configuration by entering the following command:
        ```terminal
        ...
        ```
1.  Create 2 test pods by completing the following steps:
    1.  In node 1, create pod 1, as shown in the following example:
        ...
        apiVersion: v1
          ...
          spec:
            ...
            nodeName: "${NODE1}"
        ```
    1.  In node 2, create pod 2, as shown in the following example:
        ```yaml
        kind: Pod
        apiVersion: v1
          metadata:
          ...
          spec:
            ...
            nodeName: "${NODE2}"
        ```
1.  Create a test service that backs up both pods, as shown in the following example:
    ```yaml
    kind: Service
    apiVersion: v1
      metadata:
      ...
      spec:
        ...
        type: "ClusterIP"
    ```
1.  Verify that the OVN pods are running: 
...
1.  Make sure that the changes synchronized to the Network Operator by entering the following command:
    ```terminal
    $ oc get network.operator.openshift.io/cluster \
      -ojsonpath='{.spec.defaultNetwork.ovnKubernetesConfig.ipv4}' \
      --kubeconfig=<clusters-hostedclustername> | jq .
    ```
1.  Get the IP address of pod 2 and transfer it from pod 1:
    1.  Enter the following command:
        ```terminal
    ...
    1.  Enter the following command:
        ...
        ```
1.  Get the service IP address and verify that the pod can be visited externally from a service:
    1.  Enter the following command:
        ```terminal
