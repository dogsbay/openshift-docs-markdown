...
# Sample GitHub CR {id="identity-provider-github-CR_{{ context }}"}

Review the custom resource fields and acceptable values for configuring a GitHub identity provider in {{ product_title }}. Use these definitions to set client credentials and access restrictions before applying the configuration to the cluster. {._abstract}
...
:   Specifies an optional reference to an {{ product_title }} `ConfigMap` object containing the PEM-encoded certificate authority bundle to use in validating server
...
:   Specifies a reference to an {{ product_title }} `Secret` object containing the client secret issued by GitHub.
