...
# View DTS metrics and dashboards {id="nw-dpf-viewing-dts-metrics_{{ context }}"}

After you configure the DTS `ServiceMonitor`, you can view DPU telemetry metrics by using the {{ product_title }} web console, `PromQL` queries, or Grafana dashboards. {._abstract}
...
1.  Verify that the DTS `DPUService` is ready on the management cluster.
...
1.  Deploy the DTS console dashboard for the {{ product_title }} web console:
    ...
    ```
1.  View metrics in the {{ product_title }} web console.

    :::note

    The {{ product_title }} web console reads from the cluster Prometheus instance through Thanos and user workload monitoring.
...
1.  Optional: View metrics in Grafana.
...
1.  Optional: Review DPF framework dashboards in Grafana.
