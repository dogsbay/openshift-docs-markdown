...
# Running builds using SharedSecret objects {id="builds-running-entitled-builds-with-sharedsecret-objects_{{ context }}"}
...
The Shared Resource CSI Driver feature is now generally available in [{{ builds_v2title }} 1.1](https://docs.redhat.com/en/documentation/builds_for_red_hat_openshift/1.1). This feature is now removed in {{ product_title }} 4.18 and later. To use this feature, ensure that you are using {{ builds_v2title }} 1.1 or later.
...
*   You have enabled the `TechPreviewNoUpgrade` feature set by using the feature gates. For more information, see [Enabling features using feature gates](/nodes/clusters/nodes-cluster-enabling-features#nodes-cluster-enabling).
...
1.  Use `oc apply` to create a `SharedSecret` object instance with the cluster’s entitlement secret.
    ...
    EOF
    ```
1.  Create a role to grant the `builder` service account permission to access the `SharedSecret` object:
    ...
    EOF
    ```
1.  Create a `RoleBinding` object that grants the `builder` service account permission to access the `SharedSecret` object by running the following command:
    ...
    ```
1.  Add the entitlement secret to your `BuildConfig` object by using a CSI volume mount:
    ...
    1.  Reference the name of the `SharedSecret` object to include it in the build. 
1.  Start a build from the `BuildConfig` object and follow the logs using the `oc` command.
    ```terminal
