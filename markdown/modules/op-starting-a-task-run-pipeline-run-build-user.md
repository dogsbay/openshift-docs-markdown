...
# Starting a task run with custom config map, or a pipeline run {id="starting-a-task-run-with-custom-config-map-or-a-pipeline-run_{{ context }}"}
...
1.  Create a `TaskRun` object with a custom `ConfigMap` and `Dockerfile` objects.
    ```yaml title="Example: A task run that runs Buildah as user id 1000"
    apiVersion: v1
    data:
      Dockerfile: |
    ...
    1.  The name of the service account that you created.
    1.  Mount a config map as the source workspace for the `buildah-as-user` task.
1.  (Optional) Create a pipeline and a corresponding pipeline run.
    ```yaml title="Example: A pipeline and corresponding pipeline run"
    apiVersion: tekton.dev/v1beta1
    kind: Pipeline
    metadata:
    ...
    1.  Refer to the modified Buildah task.
    1.  Use the service account that you created for the Buildah task.
    1.  Share data between the `git-clone` task and the modified Buildah task using a persistent volume claim (PVC) created automatically by the controller.
