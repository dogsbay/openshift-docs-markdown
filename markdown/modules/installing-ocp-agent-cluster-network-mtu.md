...
# Configuring cluster network MTU at installation time {id="installing-ocp-agent-cluster-network-mtu_{{ context }}"}

You can explicitly set the cluster network maximum transmission unit (MTU) during installation by placing a `Network` custom resource (CR) as an additional manifest in the `openshift` directory of your Agent-based Installer configuration. {._abstract}
...
1.  Place the `set-cluster-mtu.yaml` manifest file in the `openshift` subdirectory of your installation directory:
    ```text
...
1.  Create the agent image and boot your servers as described in "Creating and booting the agent image".
