...
# Specific application storage recommendations {id="specific-application-storage-recommendations_{{ context }}"}

Review the specific storage recommendations for registries, scaled registries, metrics, logs, and applications to better understand the storage requirements for each of these entities. {._abstract}


:::important

Testing shows issues with using the NFS server on {{ op_system_base_full }} as a storage backend for core services. This includes the OpenShift Container Registry and Quay, Prometheus for monitoring storage, and Elasticsearch for logging storage. Therefore, using {{ op_system_base }} NFS to back PVs used by core services is not recommended.

Other NFS implementations in the marketplace might not have these issues. Contact the individual NFS implementation vendor for more information on any testing that was possibly completed against these {{ product_title }} core components.
...
:   In a non-scaled/high-availability (HA) {{ product_registry }} cluster deployment:

    *   The storage technology does not have to support RWX access mode.
    *   The storage technology must ensure read-after-write consistency.
    *   The preferred storage technology is object storage followed by block storage.
    *   File storage is not recommended for {{ product_registry }} cluster deployment with production workloads.
...
:   In an {{ product_title }} hosted logging cluster deployment:
