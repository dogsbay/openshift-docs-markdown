...
# Configuring node pool capacity blocks on {{ aws_short }} {id="hcp-np-capacity-blocks_{{ context }}"}

After you create a hosted cluster, you can configure node pool capacity blocks for graphics processing unit (GPU) reservations on {{ aws_first }}. {._abstract}

**Procedure**

1.  Create GPU reservations on {{ aws_short }} by running a command similar to the following example:
    ...
    $ aws ec2 describe-capacity-block-offerings \
          ...
          --output json
    ```
    *   `--instance-type` defines the type of your {{ aws_short }} instance.
...
1.  Purchase the minimum fee capacity block by running the following command:
    ```terminal
...
1.  Create an environment variable to set the capacity reservation ID by running the following command:
    ```terminal
...
1.  Add a node pool to use the GPU reservation by running the following command:
    ```terminal
...
1.  Add the `capacityReservation` setting in your `NodePool` resource by using the following example configuration:
    ```yaml
    # ...
    spec:
      arch: amd64
    ...
    # ...
    ```
1.  Apply the node pool configuration by running the following command:
    ```terminal
...
1.  Verify that your new node pool is created successfully by running the following command:
    ...
    ```
1.  Verify that your new compute nodes are created in the hosted cluster by running the following command:
    ```terminal
    $ oc get nodes
    ```
    ```terminal title="Example output"
