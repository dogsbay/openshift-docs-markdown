...
# Add a service certificate {id="add-service-certificate_{{ context }}"}

To secure internal communication to a service in {{ product_title }}, you can annotate the service to generate a signed serving certificate and key pair into a secret in the same namespace. {._abstract}
...
1.  Annotate the service with `service.beta.openshift.io/serving-cert-secret-name`:
    ...
    *   `<secret_name>` will be the name of the generated secret containing the certificate and key pair.
        ...
        ```
1.  Examine the service to confirm that the annotations are present:
    ```terminal
    ...
    ...
    ```
1.  After the cluster generates a secret for your service, your `Pod` spec can
