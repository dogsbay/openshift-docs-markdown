...
# Managing weights using the CLI {id="deployments-ab-testing-lb-cli_{{ context }}"}

To manage service traffic weights for A/B testing in {{ product_title }}, you can use the `oc set route-backends` command. Set or adjust weights on a route so the router sends a proportional share of requests to each service. {._abstract}
...
1.  Run the command with no flags to verify the current configuration:
    ...
    ```
1.  To override the default values for the load balancing algorithm, adjust the annotation on the route by setting the algorithm to `roundrobin`. For a route on {{ product_title }}, the default load balancing algorithm is set to `random` or `source` values.
