...
# Mounting secrets from HashiCorp Vault {id="secrets-store-vault_{{ context }}"}

You can use the {{ secrets_store_operator }} to mount secrets from HashiCorp Vault to a Container Storage Interface (CSI) volume in {{ product_title }}. Using an external secret store protects information that you do not want developers to have and can be more secure than `secret` objects. {._abstract}


:::important

Mounting secrets from HashiCorp Vault by using the {{ secrets_store_operator }} has been tested with the following cloud providers:
...
*   You have installed the {{ secrets_store_operator }}. See "Installing the {{ secrets_store_driver }}" for instructions.
...
1.  Install the HashiCorp Vault provider:
    1.  Create a new project for Vault by running the following command:
        ...
        ```
    1.  Label the `vault` namespace for pod security admission by running the following command:
        ```terminal
        ...
        ```
    1.  Grant privileged access to the `vault` service account by running the following command:
        ```terminal
        $ oc adm policy add-scc-to-user privileged -z vault -n vault
        ```
    1.  Grant privileged access to the `vault-csi-provider` service account by running the following command:
        ```terminal
        $ oc adm policy add-scc-to-user privileged -z vault-csi-provider -n vault
        ```
    1.  If you are using {{ ibm_z_title }}, create a `vault-license` secret for HashiCorp Vault Enterprise:
        ...
        1.  Create the secret by running the following command:
            ...
            ```
    1.  Deploy HashiCorp Vault by running the following command:
        ```terminal
        ...
        If you are using {{ ibm_z_title }}, you must use Vault Enterprise images and the Vault license secret. Run the following command instead:
        ```terminal
        ...
        ```
    1.  Patch the `vault-csi-driver` daemon set to set the `securityContext` to `privileged` by running the following command:
        ```terminal
        ...
        ```
    1.  Verify that the `vault-csi-provider` pods have started properly by running the following command:
        ```terminal
        $ oc get pods -n vault
        ```
...
1.  Configure HashiCorp Vault to store the required secrets:
    1.  Create a secret by running the following command:
        ...
        ```
    1.  Verify that the secret is readable at the path `secret/example1` by running the following command:
        ```terminal
...
1.  Configure Vault to use Kubernetes authentication:
    ...
    1.  Configure the Kubernetes auth method:
        1.  Set the token reviewer as an environment variable by running the following command:
            ...
            ```
        1.  Set the Kubernetes service IP address as an environment variable by running the following command:
            ```terminal
            $ KUBERNETES_SERVICE_IP="$(oc get svc kubernetes --namespace=default -o go-template="{{ .spec.clusterIP }}")"
            ```
        1.  Update the Kubernetes auth method by running the following command:
            ```terminal
            ...
            ```
    1.  Create a policy for the application by running the following command:
        ```terminal
        ...
        ```
    1.  Create an authentication role to access the application by running the following command:
        ```terminal
