...
# Group IDs {id="storage-persistent-storage-nfs-group-ids_{{ context }}"}

You can use supplemental groups to manage NFS access in {{ product_title }} when you cannot change permissions on the NFS export. Supplemental groups manage shared storage such as NFS, while block storage such as iSCSI uses the `fsGroup` SCC strategy and `fsGroup` value in the pod `securityContext`. {._abstract}
