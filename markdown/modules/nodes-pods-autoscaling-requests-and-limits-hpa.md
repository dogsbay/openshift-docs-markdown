...
# About requests and limits {id="nodes-pods-autoscaling-requests-and-limits-hpa_{{ context }}"}

To use resource metrics, you must specify the resource requests, such as CPU and memory, in a pod specification. The HPA uses this specification to determine the resource utilization and then scales the target up or down. {._abstract}
