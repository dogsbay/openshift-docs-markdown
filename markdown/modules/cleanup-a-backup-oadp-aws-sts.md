...
# Cleaning up a cluster after a backup with OADP and AWS STS {id="cleanup-a-backup-oadp-aws-sts_{{ context }}"}

Uninstall the {{ oadp_first }} Operator together with the backups and the S3 bucket from the `hello-world` example. {._abstract}
...
1.  If the backup and restore resources are no longer required, remove them from the cluster by running the following command:
    ...
    ```
1.  To delete backup, restore and remote objects in {{ aws_short }} S3, run the following command:
    ```terminal
...
1.  If you no longer need the Custom Resource Definitions (CRD), remove them from the cluster by running the following command:
    ...
    ```
1.  Delete the {{ aws_short }} S3 bucket by running the following commands:
    ```terminal
