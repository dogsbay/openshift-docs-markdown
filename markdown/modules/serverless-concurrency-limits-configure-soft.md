...
# Configuring a soft concurrency target {id="serverless-concurrency-limits-configure-soft_{{ context }}"}
...
*   Optional: Use the `kn service` command to specify the `--concurrency-target` flag:
    ...
    ```
    ```terminal title="Example command to create a service with a concurrency target of 50 requests"
