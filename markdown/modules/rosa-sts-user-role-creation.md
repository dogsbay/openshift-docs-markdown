...
# Creating a user-role IAM role {id="rosa-sts-user-role-iam-basic-role_{{ context }}"}

You can create your `user-role` IAM roles by using the {{ rosa_cli_first }}. {._abstract}
...
*   To create a `user-role` IAM role with basic privileges, run the following command:
    ...
    :   The `auto` method asks if you want to create a specific `user-role` by using your prefix.
