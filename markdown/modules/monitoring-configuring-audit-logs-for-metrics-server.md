...
# Configuring audit logs for Metrics Server {id="configuring-audit-logs-for-metrics-server_{{ context }}"}
...
1.  Edit the `cluster-monitoring-config` config map in the `openshift-monitoring` project:
    ...
    ```
1.  Add audit log configuration for Metrics Server under `data/config.yaml`:
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
1.  Verify that the audit profile is applied:
    ```terminal
