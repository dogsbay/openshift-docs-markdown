...
# Setting the default maximum container root partition size for Overlay with CRI-O {id="set-the-default-max-container-root-partition-size-for-overlay-with-crio_{{ context }}"}

You can use a `ContainerRuntimeConfig` custom resource (CR) to set a maximum partition size for the root disk of all containers. {._abstract}
