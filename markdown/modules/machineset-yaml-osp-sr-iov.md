...
# Sample YAML for a compute machine set custom resource that uses SR-IOV on {{ rh_openstack }} {id="machineset-yaml-osp-sr-iov_{{ context }}"}

To provision compute virtual machines (VMs) with single root I/O virtualization (SR-IOV) for high-performance networking, define the SR-IOV ports directly in the `providerSpec` and set `portSecurity` to `False`. {._abstract}
...
Only parameters that are specific to SR-IOV deployments are described in this sample. To review a more general sample, see "Sample YAML for a compute machine set custom resource on {{ rh_openstack }}".
