...
# Jenkins agent Gradle builds {id="images-other-jenkins-agent-gradle_{{ context }}"}

Hosting Gradle builds in the Jenkins agent on {{ product_title }} presents additional complications because in addition to the Jenkins JNLP agent and Gradle JVMs, Gradle spawns a third JVM to run tests if they are specified.

The following settings are suggested as a starting point for running Gradle builds in a memory constrained Jenkins agent on {{ product_title }}. You can modify these settings as required.
