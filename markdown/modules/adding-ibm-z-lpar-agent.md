...
# Adding {{ ibm_z_title }} agents in a Logical Partition (LPAR) {id="adding-ibm-z-lpar-agents_{{ context }}"}

You can manually add {{ ibm_z_name }} agents to your cluster that runs in an LPAR environment. {._abstract}

Use this procedure only for {{ ibm_z_name }} clusters running in an LPAR.
...
1.  Create a boot parameter file for the agents.
    ...
    *   To enable FIPS mode, specify `fips=1`. This entry is required in addition to setting the `fips` parameter to `true` in the `install-config.yaml` file.
        ...
        boot-artifacts
            ...
            └─ agent.s390x-rootfs.img
        ```
1.  Rename the `boot-artifacts` file present in the `generic.ins` parameter file to match the names of the `boot-artifacts` file generated by the installation program.
1.  Transfer the `initrd`, `kernel`, `generic.ins`, and `initrd.img.addrsize` parameter files to the file server. For more information, see [Booting Linux in LPAR mode](https://www.ibm.com/docs/en/linux-on-systems?topic=bl-booting-linux-in-lpar-mode) (IBM documentation).
1.  Start the machine.
