...
# Querying Prometheus metrics {id="ossm-tutorial-prometheus-querying-metrics_{{ context }}"}
...
1.  Verify that the `prometheus` Service is running in your cluster. In this example, `istio-system` is the name of the control plane project. Execute the following command:
    ```terminal
...
1.  Generate network traffic by accessing the Bookinfo application:
    ...
    ```
1.  A route to access the Prometheus user interface already exists. Query for details of the route:
    ```terminal
