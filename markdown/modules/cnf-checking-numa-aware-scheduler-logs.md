...
1.  Save the following YAML in the file `nro-scheduler-debug.yaml`. This example changes the log level to `Debug`:
    ...
    # ...
    ```
1.  Create the updated `Debug` logging `NUMAResourcesScheduler` resource by running the following command:
    ```terminal
...
1.  Check that the NUMA-aware scheduler was successfully deployed:
    ...
    1.  Check that the new custom scheduler is available by running the following command:
        ...
        ```
1.  Check that the logs for the scheduler shows the increased log level:
