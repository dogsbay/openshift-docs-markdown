...
# Sample YAML for SR-IOV deployments where port security is disabled {id="machineset-yaml-osp-sr-iov-port-security_{{ context }}"}

To create single-root I/O virtualization (SR-IOV) ports on a network that has port security disabled, define a compute machine set that includes the ports as items in the `spec.template.spec.providerSpec.value.ports` list. {._abstract}
...
Only parameters that are specific to SR-IOV deployments where port security is disabled are described in this sample. To review a more general sample, see "Sample YAML for a compute machine set custom resource that uses SR-IOV on {{ rh_openstack }}".
