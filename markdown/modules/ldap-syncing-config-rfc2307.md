...
# About the RFC 2307 configuration file {id="ldap-syncing-config-rfc2307_{{ context }}"}

Review the RFC 2307 LDAP sync configuration file so you can define user and group queries and the attributes used in {{ product_title }} group records. {._abstract}

The RFC 2307 schema requires you to provide an LDAP query definition for both user and group entries, as well as the attributes with which to represent them in the internal {{ product_title }} records.
...
:   Specifies the attribute that uniquely identifies a user on the LDAP server. You
