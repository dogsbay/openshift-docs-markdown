...
# Extracting and preparing credentials request resources {id="cco-ccoctl-upgrading-extracting_{{ context }}"}

Before updating a cluster that uses the Cloud Credential Operator (CCO) in manual mode, you must extract and prepare the `CredentialsRequest` custom resources (CRs) for the new release. {._abstract}

**Prerequisites**

*   Install the {{ oc_first }} that matches the version for your updated version.
