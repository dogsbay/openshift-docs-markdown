...
# Hosted cluster provisioning environment variables {id="nw-dpf-hcp-environment-variables_{{ context }}"}

The following environment variables are used throughout the hosted cluster provisioning procedures.
These environment variables must be set before you create secrets, the `DPUCluster` resource, or the `DPFHCPProvisioner` resource. {._abstract}
