...
# Creating a hosted cluster on non-bare-metal agent machines by using the web console {id="hcp-non-bm-hc-console_{{ context }}"}

You can create a hosted cluster on non-bare-metal agent machines by using the {{ product_title }} web console. {._abstract}
...
1.  In the console header, select **All Clusters**.
...
1.  On the **Create cluster** page, follow the prompts to enter details about the cluster, node pools, networking, and automation.
    ...
    *   On the **Cluster details** page, the pull secret is your {{ product_title }} pull secret that you use to access {{ product_title }} resources. If you selected a host inventory credential, the pull secret is automatically populated.
