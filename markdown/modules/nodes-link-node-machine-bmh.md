...
# Linking the node, bare metal host, and machine together {id="linking-node-machine-bmh_{{ context }}"}
...
1.  Link the new control plane node and `Machine` object to the `BareMetalHost` object by performing the following steps in a single bash shell session:
    ...
    1.  Define the `NEW_MACHINE_NAME` variable by running the following command:
        ```terminal
        $ NEW_MACHINE_NAME=<new_machine_name>
        ```

        Replace `<new_machine_name>` with the name of the new machine.
    1.  Define the `BMH_UID` by running the following commands to extract it from the new node’s `BareMetalHost` object:
        ```terminal
        ...
        $ echo $BMH_UID
        ```
    1.  Patch the `consumerRef` object into the bare metal host by running the following command:
        ```terminal
        ...
        ```
    1.  Patch the `providerID` value into the new node by running the following command:
        ```terminal
        $ oc patch node $NEW_NODE_NAME --type merge --patch '{"spec":{"providerID":"baremetalhost:///openshift-machine-api/'$NEW_NODE_NAME'/'$BMH_UID'"}}'
        ```
    1.  Review the `providerID` values by running the following command:
        ```terminal
