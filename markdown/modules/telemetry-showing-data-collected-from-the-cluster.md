...
# Showing data collected by Telemetry {id="showing-data-collected-from-the-cluster_{{ context }}"}

You can view the cluster and components time series data captured by Telemetry. {._abstract}
...
1.  Run the following command, which queries a cluster’s Prometheus service and returns the full set of time series data captured by Telemetry:

    :::note

    The following example contains some values that are specific to {{ product_title }} on AWS.
