...
# Preparing to access {{ product_title }} in AWS GovCloud using Keycloak {id="rosa-govcloud-keycloak-identity-management_{{ context }}"}

To access {{ product_title }} in AWS GovCloud using Keycloak for identity management.
...
1.  Change to the redhat-external realm.
1.  Under _Configure_, select **Identity providers**.
1.  Click on the IdP of the customer whose configuration was just merged into _keycloak-interface_.
1.  Under _OIDC_, click **Settings**.
1.  Expand the **Advanced** section and update the _Scopes_ field to _openid profile email_.
