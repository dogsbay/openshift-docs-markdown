...
*   Create and apply a `Gateway` CR file configured for your domain requirements by using one of the following examples:
    ...
    *   To use a `Secret` object in a different namespace, configure your listener and create a corresponding `ReferenceGrant` CR as shown in the following example:
        ...
        Unlike standard {{ product_title }} routes that use namespace-wide annotations for cross-namespace permissions, Gateway API uses the `ReferenceGrant` CR to authorize access. This `ReferenceGrant` CR gives access from the `Gateway` CR in one namespace to the `Secret` object in another namespace.
