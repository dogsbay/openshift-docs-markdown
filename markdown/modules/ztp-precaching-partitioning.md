...
# Partitioning the disk {id="ztp-partitioning_{{ context }}"}

To run the full pre-caching process, you have to boot from a live ISO and use the {{ factory_prestaging_tool }} from a container image to partition and pre-cache all the artifacts required. {._abstract}

A live ISO or {{ op_system }} live ISO is required because the disk must not be in use when the operating system ({{ op_system }}) is written to the device during the provisioning.
...
*   You have enough storage to install {{ product_title }} and pre-cache the required images.
...
1.  Verify that the disk is cleared:
    ...
    ```
1.  Erase any file system, RAID or partition table signatures from the device:
    ```terminal
    ...
    The `coreos-installer` requires the partition to be created at the end of the device and to be labelled as `data`. Both requirements are necessary to save the partition when writing the {{ op_system }} image to the disk.
...
1.  Run the container as `privileged` and partition the disk. In the following example, the size of the partition is 250 GiB to allow pre-caching the DU profile for Day 2 Operators:
    ...
    Where:
    *   `factory-precaching-cli partition` specifies the partitioning function of the {{ factory_prestaging_tool }}.
    *   `-d /dev/nvme0n1` defines the root directory on the disk.
    *   `-s 250` defines the size of the disk in GB.
1.  Check the storage information:
    ```terminal
    # lsblk
    ```
...
*   Check that the partition is mounted:
    ...
    The mount point is `/var/mnt` because the `/mnt` folder in {{ op_system }} is a link to `/var/mnt`.