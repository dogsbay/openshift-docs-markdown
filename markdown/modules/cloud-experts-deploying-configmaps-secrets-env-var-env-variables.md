...
# Configuration using environment variables {id="cloud-experts-deploying-configmaps-secrets-env-var-env-variables_{{ context }}"}

Using environment variables is an easy way to change application behavior without requiring code changes. It allows different deployments of the same application to potentially behave differently based on the environment variables. {{ product_title }} makes it simple to set, view, and update environment variables for pods or deployments. {._abstract}
