...
# Updating mirror registry for Red Hat OpenShift from a local host {id="mirror-registry-localhost-update_{{ context }}"}

You can update the _mirror registry for Red&#160;Hat OpenShift_ from a local host by using the `upgrade` command. Updating to the latest version ensures new features, bug fixes, and security vulnerability fixes. {._abstract}
...
*   You must not use the _mirror registry for Red&#160;Hat OpenShift_ user interface (UP).
...
*   To upgrade the _mirror registry for Red&#160;Hat OpenShift_ using the existing certificates, enter the following command:
    ```terminal
    ...
    *   Starting with _mirror registry for Red&#160;Hat OpenShift_ 2.0.11, the `upgrade` command preserves the existing `SERVER_HOSTNAME`, including custom ports, and the `quayRoot`, `quayStorage`, and `sqliteStorage` settings from the current installation. You do not need to pass `--quayHostname`, `--quayRoot`, `--quayStorage`, or `--sqliteStorage` for a standard upgrade. Pass these flags only when you want to override the preserved values.
    *   When upgrading from 1.3 to 2.y with the default installation directory at `/etc/quay-install`, _mirror registry for Red&#160;Hat OpenShift_ migrates Podman volumes for {{ quay }} storage, Postgres data, and `/etc/quay-install` data to the new `$HOME/quay-install` location.
...
*   Optional: To override preserved install or storage paths during upgrade, pass the `--quayRoot`, `--quayStorage`, or `--sqliteStorage` flags. For example:
    ...
    ```
*   Optional: To rotate SSL/TLS certificates during upgrade (_mirror registry for Red&#160;Hat OpenShift_ 2.0.11 and later), pass both the `--sslCert` and `--sslKey` flags. You must provide both flags together; if you pass only `--sslCert` or only `--sslKey`, the upgrade command fails with an error. For example:
    ```terminal
...
1.  Ensure that _mirror registry for Red&#160;Hat OpenShift_ has been updated by running the following command:
    ```terminal
    $ podman ps
    ```
