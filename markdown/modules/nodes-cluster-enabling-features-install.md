...
# Enabling feature sets at installation {id="nodes-cluster-enabling-features-install_{{ context }}"}

You can enable feature sets for all nodes in the cluster by editing the `install-config.yaml` file before you deploy the cluster. This allows you to use non-default features in your cluster. {._abstract}
