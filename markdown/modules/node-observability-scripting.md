...
# Configuring Node Observability Operator scripting {id="node-observability-scripting_{{ context }}"}

You can configure the Node Observability Operator to run pre-configured scripts for collecting metrics data from compute nodes. {._abstract}
...
1.  Trigger the scripting by creating the `NodeObservabilityRun` resource with the following command:
    ```terminal
    $ oc apply -f nodeobservabilityrun-script.yaml
    ```
1.  Review the status of the `NodeObservabilityRun` scripting by running the following command:
    ```terminal
...
1.  Retrieve the scripting data from the root path of the container by running the following bash script:
    ```bash
    #!/bin/bash
