...
# RBAC overview {id="security-rbac-overview_{{ context }}"}

Role-based access control (RBAC) objects determine whether a user is allowed to perform a given action within a project.  {._abstract}

Cluster administrators can use the cluster roles and bindings to control who has various access levels to the {{ product_title }} platform itself and all projects.
...
## Operational RBAC considerations {id="security-operational-rbac-considerations_{{ context }}"}
