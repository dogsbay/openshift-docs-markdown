...
# Creating a {{ product_title }} cluster using the CLI {id="rosa-hcp-sts-creating-a-cluster-cli_{{ context }}"}

You can create a {{ product_title }} cluster quickly by using the {{ rosa_cli_first }} with default options. {._abstract}
...
1.  Use one of the following commands to create your {{ product_title }} cluster:
    ...
    *   If you did not set environmental variables, run the following command:
        ...
        `--private`
        :   Optional. Use the `--private` argument to create private {{ product_title }} clusters. If you use this argument, ensure that you only use your private subnet ID for `--subnet-ids`.
