...
# Creating a compute machine set that deploys edge compute machines on an Outpost {id="aws-outposts-machine-set_{{ context }}"}

To deploy edge compute machines on {{ aws_first }} Outposts, you must create a compute machine set with a configuration compatible with the Outpost environment. {._abstract}

**Prerequisites**

*   You have an {{ aws_short }} Outposts site.
...
1.  List the compute machine sets in your cluster by running the following command:
    ...
    ```
1.  Record the names of the existing compute machine sets.
1.  Create a YAML file that contains the values for a new compute machine set custom resource (CR) by using one of the following methods:
    ...
    *   Create an empty YAML file named `<new_machine_set_name_1>.yaml` with your preferred text editor and include the required values for your new compute machine set.
        ...
        `metadata.name`
        :   Specifies a default node label. For {{ aws_short }} Outposts, you use the `outposts` role.
...
1.  Configure the new compute machine set to create edge compute machines in the Outpost by editing the `<new_machine_set_name_1>.yaml` file:
    ...
    :   Specifies the EBS volume type. {{ aws_short }} Outposts requires gp2 volumes.

    `spec.providerSpec.value.instanceType`
    :   Specifies the {{ aws_short }} instance type. You must use an instance type that is configured in your Outpost.

    `spec.providerSpec.value.placement.region`
    :   Specifies the {{ aws_short }} region in which the Outpost availability zone exists.
