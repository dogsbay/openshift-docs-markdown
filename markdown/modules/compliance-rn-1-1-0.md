...
# Release notes for OpenShift Compliance Operator 1.1.0 {id="compliance-operator-release-notes-1-1-0_{{ context }}"}

Release notes for OpenShift Compliance Operator 1.1.0. {._abstract}
...
## New features and enhancements {id="compliance-operator-1-1-0-new-features-and-enhancements_{{ context }}"}

*   A start and end timestamp is now available in the `ComplianceScan` custom resource definition (CRD) status.
*   The Compliance Operator can now be deployed on {{ hcp }} using the software catalog by creating a `Subscription` file. For more information, see [Installing the Compliance Operator on {{ hcp }}](/security/compliance_operator/co-management/compliance-operator-installation#installing-compliance-operator-hcp_compliance-operator-installation).

## Bug fixes {id="compliance-operator-1-1-0-bug-fixes_{{ context }}"}
...
*   Before this update, a new installation of {{ product_title }} with the latest Compliance Operator install failed on the `scheduler-no-bind-address` rule. With this update, the `scheduler-no-bind-address` rule has been disabled on newer versions of {{ product_title }} since the parameter was removed. ([**OCPBUGS-8347**](https://issues.redhat.com/browse/OCPBUGS-8347))