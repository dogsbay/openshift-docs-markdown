...
# Metrics for AdminNetworkPolicy {id="anp-banp-metrics_{{ context }}"}

You can use OVN-Kubernetes metrics to monitor `AdminNetworkPolicy` and `BaselineAdminNetworkPolicy` resources. These metrics report policy and rule counts by direction and action, and northbound database objects that the policies create. {._abstract}
