...
# Authorizing LokiStack rules RBAC permissions {id="loki-rbac-rules-permissions_{{ context }}"}
...
## Examples {id="loki-rbac-rules-permissions-examples_{{ context }}"}
...
```terminal title="Example cluster role binding command for alerting rule CRUD permissions in a specific namespace"
...
```terminal title="Example cluster role binding command for administrator permissions"
