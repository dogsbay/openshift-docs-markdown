...
# Creating a host device RDMA on RoCE {id="rdma-creating-host-device-rdma-roce_{{ context }}"}
...
1.  Generate a new host device `NicClusterPolicy` (CR), as shown below:
    ...
    EOF
    ```
1.  Create the `NicClusterPolicy` CR on the cluster by using the following command: 
    ```terminal
    ...
    ```
1.  Verify that the host device `NicClusterPolicy` CR by using the following command in the DOCA/MOFED container:
    ```terminal
    ...
    ```
1.  Confirm that the resources appear in the cluster `oc describe node` section by using the following command:
    ```terminal
    ...
    Allocatable:
      ...
      pods:                250
    ```
1.  Create a `HostDeviceNetwork` CR file:
    ```yaml
    ...
    EOF
    ```
1.  Create the `HostDeviceNetwork` resource on the cluster by using the following command:
    ```terminal
