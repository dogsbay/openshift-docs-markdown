...
# Configuring dynamic leap seconds handling for PTP grandmaster clocks {id="ptp-configuring-dynamic-leap-seconds-handling-for-tgm_{{ context }}"}

The PTP Operator container image includes the latest `leap-seconds.list` file  that is available at the time of release. {._abstract}
...
The {{ product_version }} version of the PTP Operator enables automatic leap second management by default.
