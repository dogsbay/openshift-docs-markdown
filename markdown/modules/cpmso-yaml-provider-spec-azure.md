...
# Sample {{ azure_short }} provider specification {id="cpmso-yaml-provider-spec-azure_{{ context }}"}
...
The following example YAML illustrates a valid configuration for a {{ azure_first }} cluster.
...
:   Specifies the public load balancer for the control plane.
    ...
    You can omit the `publicLoadBalancer` parameter on private {{ product_title }} clusters that have user-defined outbound routing.
