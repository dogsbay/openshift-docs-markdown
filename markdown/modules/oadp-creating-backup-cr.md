...
# Creating a Backup CR {id="oadp-creating-backup-cr_{{ context }}"}
...
1.  Create a `Backup` CR, as in the following example:
    ...
    1.  Optional: Specify an array of resources to include in the backup. Resources might be shortcuts (for example, 'po' for 'pods') or fully-qualified. If unspecified, all resources are included.
    1.  Optional: Specify an array of resources to exclude from the backup. Resources might be shortcuts (for example, 'po' for 'pods') or fully-qualified.
    1.  Specify the name of the `backupStorageLocations` CR.
    1.  Map of {key,value} pairs of backup resources that have **all** of the specified labels.
    1.  Map of {key,value} pairs of backup resources that have **one or more** of the specified labels.
