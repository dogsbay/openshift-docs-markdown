# Release notes for {{ pipelines_title }} General Availability 1.5 {id="op-release-notes-1-5_{{ context }}"}

{{ pipelines_title }} General Availability (GA) 1.5 is now available on {{ product_title }} 4.8.

## Compatibility and support matrix {id="compatibility-support-matrix-1-5_{{ context }}"}
...
## New features {id="new-features-1-5_{{ context }}"}

In addition to the fixes and stability improvements, the following sections highlight what is new in {{ pipelines_title }} 1.5.
...
*   Pipeline run and task runs will be automatically pruned by a cron job in the target namespace. The cron job uses the `IMAGE_JOB_PRUNER_TKN` environment variable to get the value of `tkn image`. With this enhancement, the following fields are introduced to the `TektonConfig` custom resource:
    ...
    ...
    ```
*   In {{ product_title }}, you can customize the installation of the Tekton Add-ons component by modifying the values of the new parameters `clusterTasks` and `pipelinesTemplates` in the `TektonConfig` custom resource:
    ```yaml
...
*   The `enableMetrics` parameter is added to the `TektonConfig` custom resource. You can use it to disable the service monitor, which is part of Tekton Pipelines for {{ product_title }}.
    ```yaml
    apiVersion: operator.tekton.dev/v1alpha1
    kind: TektonConfig
    metadata:
...
## Deprecated features {id="deprecated-features-1-5_{{ context }}"}
...
*   When you upgrade the {{ pipelines_title }} Operator to `v1.5`, the `openshift-client` and the `openshift-client-v-1-5-0` cluster tasks have the `SCRIPT` parameter. However, the `ARGS` parameter and the `git` resource are removed from the specification of the `openshift-client` cluster task. This is a breaking change, and only those cluster tasks that do not have a specific version in the `name` field of the `ClusterTask` resource upgrade seamlessly.
    ...
    ...
    ```
*   When you upgrade from {{ pipelines_title }} Operator `v1.4` to `v1.5`, the profile names in which the `TektonConfig` custom resource is installed now change.
...
*   The `disable-home-env-overwrite` and `disable-working-dir-overwrite` fields are now deprecated and will be removed in a future release. For this release, the default value of these flags is set to `true` for backward compatibility.
    ...
    In the next release ({{ pipelines_title }} 1.6), the `HOME` environment variable will not be automatically set to `/tekton/home`, and the default working directory will not be set to `/workspace` for task runs. These defaults collide with any value set by image Dockerfile of the step.
...
## Known issues {id="known-issues-1-5_{{ context }}"}

*   The [git-cli](https://github.com/tektoncd/catalog/tree/main/task/git-cli/0.1) cluster task is built off the [alpine/git](https://github.com/tektoncd/catalog/blob/68e44c629c9ee287393681030ed391d2c2e856cd/task/git-cli/0.1/git-cli.yaml#L32) base image, which expects `/root` as the user’s home directory. However, this is not explicitly set in the `git-cli` cluster task.
    ...
    This issue is expected to be fixed in the upcoming releases. For {{ pipelines_title }} 1.5 and earlier versions, you can _use any one of the following workarounds_ to avoid the failure of the `git-cli` cluster task:
    *   Set the `$HOME` environment variable in the steps, so that it is not overwritten.
        1.  [OPTIONAL] If you installed {{ pipelines_title }} using the Operator, then clone the `git-cli` cluster task into a separate task. This approach ensures that the Operator does not overwrite the changes made to the cluster task.
        1.  Execute the `oc edit clustertasks git-cli` command.
        1.  Add the expected `HOME` environment variable to the YAML of the step:
            ```yaml
            ...
            steps:
              - name: git
            ...
            For {{ pipelines_title }} installed by the Operator, if you do not clone the `git-cli` cluster task into a separate task before changing the `HOME` environment variable, then the changes are overwritten during Operator reconciliation.
    ...
    *   Disable overwriting the `HOME` environment variable in the `feature-flags` config map.
        ...
        1.  Set the value of the `disable-home-env-overwrite` flag to `true`.

            :::warning

            *   If you installed {{ pipelines_title }} using the Operator, then the changes are overwritten during Operator reconciliation.
    ...
    *   Use a different service account for the `git-cli` cluster task, as the overwriting of the `HOME` environment variable happens when the default service account for pipelines is used.
        ...
        1.  Link your Git secret to the service account you just created.
        1.  Use the service account while executing a task or a pipeline.
*   On {{ ibm_power_name }} Systems, {{ ibm_z_name }}, and {{ ibm_linuxone_name }}, the `s2i-dotnet` cluster task and the `tkn hub` command are unsupported.
...
## Fixed issues {id="fixed-issues-1-5_{{ context }}"}
