...
# Customizing the Jenkins image through source-to-image {id="images-other-jenkins-customize-s2i_{{ context }}"}

To customize the official {{ product_title }} Jenkins image, you can use the image as a source-to-image (S2I) builder.
...
```yaml title="Sample build configuration to customize the Jenkins image in {{ product_title }}"
...
1.  The `strategy` parameter defines the original Jenkins image to use as a source image for the build.
1.  The `output` parameter defines the resulting, customized Jenkins image that you can use in deployment configurations instead of the official Jenkins image.