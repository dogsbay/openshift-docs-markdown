...
# Sample YAML for a Windows MachineSet object on Nutanix {id="windows-machineset-nutanix_{{ context }}"}

You can define a Windows `MachineSet` object running on Nutanix by creating a YAML file similar to the following example that the Windows Machine Config Operator (WMCO) can react upon. {._abstract}
