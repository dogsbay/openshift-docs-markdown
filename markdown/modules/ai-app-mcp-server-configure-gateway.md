...
# Configure the MCP gateway {id="ai-app-mcp-server-configure-gateway_{{ context }}"}

Configure the Model Context Protocol (MCP) gateway so that it can route client traffic to your MCP server and discover the tools the server exposes. {._abstract}
...
1.  Verify the registration status by running the following commands:
    ```terminal
...
1.  Verify that your MCP server tools are available through the MCP gateway:
    ...
    1.  Extract the MCP session ID from response headers by running the following commands:
        ```terminal
        $ SESSION_ID=$(grep -i "mcp-session-id:" /tmp/mcp_headers | cut -d' ' -f2 | tr -d '\r')

        $ echo "MCP Session ID: $SESSION_ID"
        ```
    1.  List tools by using the session ID in the following command:
        ```terminal
        ...
        `<cluster-name>.<domain-name>` is the name and domain of your cluster.
    1.  Clean up by running the following command:
        ```terminal
