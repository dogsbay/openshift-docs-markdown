...
# Understanding horizontal pod autoscaler status conditions by using the CLI {id="nodes-pods-autoscaling-status-about_{{ context }}"}

You can review the horizontal pod autoscaler (HPA) status conditions set to determine if the HPA is able to scale or if it is currently restricted in any way. {._abstract}
