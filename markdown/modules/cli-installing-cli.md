...
To interact with a {{ product_title }} cluster from the terminal, install the {{ oc_first }}. Depending on your operating system, you can install `oc` by downloading the binary from the Customer Portal or {{ product_title }} web console, by using an RPM, or by using Homebrew.
{% endif %} {._abstract}
