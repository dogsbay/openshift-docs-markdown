...
# Image streams {id="cloud-experts-getting-started-openshift-concepts-imagestreams_{{ context }}"}

An image stream stores a mapping of tags to images, metadata overrides that are applied when images are tagged in a stream, and an optional reference to a Docker image repository on a registry. {._abstract}

## Image stream benefits {id="cloud-experts-getting-started-openshift-concepts-imagestreams-benefits_{{ context }}"}
