...
# SR-IOV Network Operator supported devices {id="supported-devices_{{ context }}"}

The `config.yaml` file lists the supported network devices for the SR-IOV Network Operator. {._abstract}

```yaml title="SR-IOV Network Operator configuration file"
apiVersion: v1
kind: ConfigMap
metadata:
...
For the most up-to-date list of supported cards and compatible {{ product_title }} versions available, see [Openshift Single Root I/O Virtualization (SR-IOV) and PTP hardware networks Support Matrix](https://access.redhat.com/articles/6954499).
