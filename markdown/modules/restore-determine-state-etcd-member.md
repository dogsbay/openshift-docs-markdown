...
Determine whether the unhealthy etcd member has a stopped machine, an unready node, or a crashlooping etcd pod. Knowing the failure state enables you to follow the correct replacement procedure. {._abstract}
...
1.  Determine if the status of the node is `NotReady`.
    ...
    1.  If the node is still reachable, then check whether the node is listed as `NotReady` by running the following command:
        ```terminal
...
1.  Determine if the etcd pod is crashlooping.
    ...
    1.  Verify that all control plane nodes are listed as `Ready` by running the following command:
        ...
        ```
    1.  Check whether the status of an etcd pod is either `Error` or `CrashloopBackoff` by running the following command:
        ```terminal
