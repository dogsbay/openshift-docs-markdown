...
# How vGPUs are assigned to nodes {id="how-vgpus-are-assigned-to-nodes_{{ context }}"}

{{ VirtProductName }} configures a single `mdev` type and the maximum number of instances of the selected `mdev` type for each physical device. The cluster architecture affects how devices are created and assigned to nodes. {._abstract}
