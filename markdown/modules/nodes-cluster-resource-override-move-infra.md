{% if context == "nodes-cluster-overcommit" %}
{%- set cro = true -%}
{% endif %}
...
1.  Move the Cluster Resource Override Operator pod by adding a node selector to the `Subscription` custom resource (CR) for the Cluster Resource Override Operator.
    1.  Edit the CR:
        ...
        ```
    1.  Add a node selector to match the node role label on the node where you want to install the Cluster Resource Override Operator pod:
        ```terminal
