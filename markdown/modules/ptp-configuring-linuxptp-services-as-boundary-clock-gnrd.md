...
# Configuring linuxptp services as a boundary clock without holdover on Intel Granite Rapids-D hardware {id="ptp-configuring-linuxptp-services-as-boundary-clock-gnrd_{{ context }}"}

Use this procedure to configure a Precision Time Protocol (PTP) boundary clock (BC) profile without holdover on Intel Granite Rapids-D (GNR-D) hardware that uses onboard Network Acceleration Complex (NAC) ports and optional Carter Flat expansion network interface cards (NICs). {._abstract}
...
1.  Verify that the `PtpConfig` profile is applied:
    ...
    ```
1.  Verify that the `linuxptp` daemon is running on the target node:
    ```terminal
    $ oc get pods -n openshift-ptp -o wide
    ```
1.  Review `linuxptp` daemon logs for BC status:
    ```terminal
