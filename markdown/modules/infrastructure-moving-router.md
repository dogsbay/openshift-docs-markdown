...
1.  View the `IngressController` custom resource for the router Operator by running the following command:
    ...
    ```
1.  Edit the `ingresscontroller` resource and change the `nodeSelector` to use the `infra` label by running the following command:
    ```terminal
...
*   Confirm that the router pod is running on the `infra` node.
    ...
    1.  View the node status of the running pod by running the following command:
        ```terminal
