...
# Boundary clocks without holdover on Intel Granite Rapids-D hardware {id="nw-ptp-granite-rapids-boundary-clock-overview_{{ context }}"}

Intel Granite Rapids-D (GNR-D) server platforms support Precision Time Protocol (PTP) boundary clock (BC) deployments without holdover for high-density Radio Access Network (RAN) sites that use onboard Network Acceleration Complex (NAC) ports and optional Carter Flat expansion network interface cards (NICs). {._abstract}
...
## Boundary clock without holdover behavior {id="nw-ptp-granite-rapids-boundary-clock-overview-without-holdover_{{ context }}"}
