...
# Using a NodePort to get traffic into the cluster {id="nw-using-nodeport_{{ context }}"}

Use a `NodePort`-type `Service` resource to expose a service on a specific port on all nodes in the cluster.  {._abstract}
