...
# Node placement rule examples {id="virt-node-placement-rule-examples_{{ context }}"}

Examples of node placement rules for different objects that determine where {{ VirtProductName }} components are deployed. {._abstract}

{% if not (openshift_rosa or openshift_dedicated) %}
## Subscription object node placement rule examples {id="subscription-object-node-placement-rules_{{ context }}"}
...
## HyperConverged object node placement rule example {id="hyperconverged-object-node-placement-rules_{{ context }}"}
...
## HostPathProvisioner object node placement rule example {id="hostpathprovisioner-object-node-placement-rules_{{ context }}"}
