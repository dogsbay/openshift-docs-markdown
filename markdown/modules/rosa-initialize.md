...
# Initializing {{ product_title }} {id="rosa-initialize_{{ context }}"}

Use the `init` command to initialize {{ product_title }} only if you are using non-STS. {._abstract}

## init {id="rosa-init_{{ context }}"}

Perform a series of checks to verify that you are ready to deploy a {{ product_title }} cluster.
...
Configure your AWS account to allow {{ product_title }} clusters:

```terminal title="Example"
$ rosa init
```

Configure a new AWS account using pre-existing {{ cluster_manager }} credentials:
