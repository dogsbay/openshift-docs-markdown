...
# Defining a custom DNS name {id="hcp-custom-dns_{{ context }}"}

As a cluster administrator, you can create a hosted cluster with an external API DNS name that differs from the internal endpoint that gets used for node bootstraps and control plane communication.  {._abstract}
