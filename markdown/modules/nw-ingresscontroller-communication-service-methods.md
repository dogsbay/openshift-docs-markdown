...
# Methods for communicating from outside the cluster {id="nw-ingresscontroller-communication-service-methods_{{ context }}"}

To enable communication between external networks and services in {{ product_title }}, configure the appropriate ingress method.  {._abstract}

{{ product_title }} provides the following methods for communicating from outside the cluster with services running in the cluster. Note that the methods are listed in order of preference.
