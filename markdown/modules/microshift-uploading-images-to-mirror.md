...
# Upload container images to a mirror registry {id="microshift-uploading-container-images-to-mirror_{{ context }}"}

To use your container images at an air-gapped site, upload them to the mirror registry by using the following procedure. {._abstract}
...
1.  Install the `skopeo` tool used for copying the container images by running the following command:
    ...
    ```
1.  Set the environment variables pointing to the pull secret file:
    ```terminal
    $ IMAGE_PULL_FILE=~/.pull-secret-mirror.json
    ```
1.  Set the environment variables pointing to the local container image directory:
    ```terminal
    $ IMAGE_LOCAL_DIR=~/microshift-containers
    ```
1.  Set the environment variables pointing to the mirror registry URL for uploading the container images:
    ```terminal
