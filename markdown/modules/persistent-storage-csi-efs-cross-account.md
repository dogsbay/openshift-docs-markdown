...
# AWS EFS CSI cross account support {id="persistent-storage-csi-efs-cross-account_{{ context }}"}

To mount AWS Elastic File System (EFS) volumes across AWS accounts, configure cross-account support that allows your 
{{ product_title }} cluster in one account to access file systems in another account. {._abstract}
...
*   Both the {{ product_title }} cluster and EFS file system must be located in the same AWS region.
...
*   {{ product_title }} AWS Account A: Contains a Red Hat {{ product_title }} cluster v4.16, or later, deployed within a VPC
...
1.  Set up the environment:
    ...
    1.  Create the working directory by running the following command:
        ```terminal
        mkdir -p $SCRATCH_DIR
        ```
    1.  Verify cluster connectivity by running the following command in the {{ product_title }} CLI:
        ```terminal
        $ oc whoami
        ```
    1.  Determine the {{ product_title }} cluster type and set node selector:
        ...
        not consistent for every {{ product_title }} type.
        *   If your cluster is deployed as a Hosted Control Plane (HyperShift), set the `NODE_SELECTOR` environment variable to hold the worker node label by running the following command:
            ...
            ```
        *   For all other {{ product_title }} types, set the `NODE_SELECTOR` environment variable to hold the master node label by running the following command:
            ```terminal
            export NODE_SELECTOR=node-role.kubernetes.io/master
            ```
    1.  Configure AWS CLI profiles as environment variables for account switching by running the following commands:
        ```terminal
        export AWS_ACCOUNT_A="<ACCOUNT_A_NAME>"
        export AWS_ACCOUNT_B="<ACCOUNT_B_NAME>"
        ```
    1.  Ensure that your AWS CLI is configured with JSON output format as the default for both accounts by running the following commands:
        ```terminal
    ...
    1.  Unset `AWS_PROFILE` in your shell to prevent conflicts with `AWS_DEFAULT_PROFILE` by running the following command:
        ```terminal
        unset AWS_PROFILE
        ```
1.  Configure the AWS Account B IAM roles and policies:
    1.  Switch to your Account B profile by running the following command:
        ...
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
        ```
    1.  Define the IAM role name for the EFS CSI Driver Operator by running the following command:
        ```terminal
        export ACCOUNT_B_ROLE_NAME=${CLUSTER_NAME}-cross-account-aws-efs-csi-operator
        ```
    1.  Create the IAM trust policy file by running the following command:
        ```terminal
        ...
        EOF
        ```
    1.  Create the IAM role for the EFS CSI Driver Operator by running the following command:
        ```terminal
        ...
        ```
    1.  Create the IAM policy file by running the following command:
        ```terminal
        ...
        EOF
        ```
    1.  Create the IAM policy by running the following command:
        ```terminal
...
1.  Configure the AWS Account A IAM roles and policies:
    1.  Switch to your Account A profile by running the following command:
        ...
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_A}
        ```
    1.  Create the IAM policy document by running the following command:
        ```terminal
        ...
        EOF
        ```
    1.  In AWS Account A, attach the AWS-managed policy "AmazonElasticFileSystemClientFullAccess" to the {{ product_title }} cluster master role by running the following command:
        ```terminal
...
1.  Attach policy to IAM User to allow role assumption
    1.  Identify the IAM User used by the EFS CSI Driver Operator by running the following command:
        ...
        ```
    1.  Attach the policy to the IAM user by running the following command:
        ```terminal
...
1.  Attach the policy to the IAM role to allow role assumption:
    1.  Identify the IAM role name currently used by the EFS CSI Driver Operator by running the following command:
        ...
        ```
    1.  Attach the policy to the IAM role used by the EFS CSI Driver Operator by running the following command:
        ```terminal
...
1.  Configure VPC peering:
    1.  Initiate a peering request from Account A to Account B by running the following command:
        ...
        ```
    1.  Accept the peering request from Account B by running the following command:
        ```terminal
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
        aws ec2 accept-vpc-peering-connection --vpc-peering-connection-id "${PEER_REQUEST_ID}"
        ```
    1.  Retrieve the route table IDs for Account A and add routes to the Account B VPC by running the following command:
        ```terminal
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_A}
        ...
        ```
    1.  Retrieve the route table IDs for Account B and add routes to the Account A VPC by running the following command:
        ```terminal
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
...
1.  Configure security groups in Account B to allow NFS traffic from Account A to EFS:
    1.  Switch to your Account B profile by running the following command:
        ...
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
        ```
    1.  Configure the VPC security groups for EFS access by running the following command:
        ```terminal
...
1.  Create a region-wide EFS filesystem in Account B:
    1.  Switch to your Account B profile by running the following command:
        ...
        export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
        ```
    1.  Create a region-wide EFS file system by running the following command:
        ```terminal
        ...
        ```
    1.  Configure region-wide mount targets for EFS by running the following command:
        ```terminal
...
1.  Configure the EFS Operator for cross-account access:
    1.  Define custom names for the secret and storage class that you will create in subsequent steps by running the following command:
        ...
        ```
    1.  Create a secret that references the role ARN in Account B by running the following command in the {{ product_title }} CLI:
        ```terminal
        oc create secret generic ${SECRET_NAME} -n ${CSI_DRIVER_NAMESPACE} --from-literal=awsRoleArn="${ACCOUNT_B_ROLE_ARN}"
        ```
    1.  Grant the CSI driver controller access to the newly created secret by running the following commands in the {{ product_title }} CLI:
        ```terminal
        ...
        ```
    1.  Create a new storage class that references the EFS ID from Account B and the secret created previously by running the following command in the {{ product_title }} CLI:
        ```terminal
        cat << EOF | oc apply -f -
        kind: StorageClass
