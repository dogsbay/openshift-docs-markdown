...
# Exposing template object fields {id="templates-exposing-object-fields_{{ context }}"}

To return connection details when users bind to your template service, add `template.openshift.io/expose-` or `template.openshift.io/base64-expose-` annotations to `ConfigMap`, `Secret`, `Service`, or `Route` objects. Binding clients then receive the needed credentials and endpoints directly. {._abstract}
