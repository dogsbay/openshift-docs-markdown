...
# Amazon Elastic File Storage troubleshooting {id="efs-troubleshooting_{{ context }}"}

To diagnose and resolve AWS Elastic File System (EFS) issues, use these troubleshooting steps including gathering logs, checking operator status, and verifying network connectivity. {._abstract}
...
*   If a volume cannot be mounted to a pod (as shown in the output of the following command):
    ...
    This error is frequently caused by AWS dropping packets between an {{ product_title }} node and Amazon EFS.
