...
# How huge pages are consumed by apps {id="how-huge-pages-are-consumed-by-apps_{{ context }}"}

You must ensure that nodes pre-allocate huge pages in order for the node to report its huge page capacity. A node can only pre-allocate huge pages for a single size. {._abstract}
...
## Huge page requirements {id="huge-pages-requirements_{{ context }}"}
