...
# Creating a hosted cluster with the KubeVirt platform by using external infrastructure {id="hcp-virt-create-hc-ext-infra_{{ context }}"}

By default, the HyperShift Operator hosts both the control plane pods of the hosted cluster and the KubeVirt worker VMs within the same cluster. With the external infrastructure feature, you can place the worker node VMs on a separate cluster from the control plane pods. {._abstract}

*   The _management cluster_ is the {{ product_title }} cluster that runs the HyperShift Operator and hosts the control plane pods for a hosted cluster.
*   The _infrastructure cluster_ is the {{ product_title }} cluster that runs the KubeVirt worker VMs for a hosted cluster.
