...
# Replication controllers {id="deployments-replicationcontrollers_{{ context }}"}

To keep a specified number of identical pods running in {{ product_title }}, you can use a replication controller. Create one through a `DeploymentConfig` object rather than directly, and use a replica set instead when you need set-based selectors or custom update orchestration. {._abstract}
