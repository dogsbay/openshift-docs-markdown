...
# Listing the OVN-Kubernetes northbound database contents {id="nw-ovn-kubernetes-list-database-contents_{{ context }}"}

To understand OVN-Kubernetes (OVN-K) logical networking entities, you must examine the northbound database running as a container inside the ovnkube-node pod. Each node is controlled by the ovnkube-controller container running within that local pod structure. {._abstract}
...
1.  List the pods in the network namespace by entering the following command:
    ...
    ```
1.  Optional: To list the pods with node information, enter the following command:
    ```terminal
    ...
    ```
1.  Open a remote shell into the `nbdb` or `sbdb` containers on the relevant node.
1.  Show all the objects in the northbound database by entering the following command:
    ```terminal
    $ ovn-nbctl show
    ```
    ...
    1.  View the list of logical switches by entering the following command:
        ```terminal
        $ oc exec -n openshift-ovn-kubernetes -it ovnkube-node-55xs2 \
    ...
    1.  View the list of load balancers by entering the following command:
        ```terminal
        $ oc exec -n openshift-ovn-kubernetes -it ovnkube-node-55xs2 \
...
1.  Display the options available with the `ovn-nbctl` command by entering the following command:
    ```terminal
    $ oc exec -n openshift-ovn-kubernetes -it ovnkube-node-55xs2 \
