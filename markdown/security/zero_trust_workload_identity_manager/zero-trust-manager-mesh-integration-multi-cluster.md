...
Configure {{ spire_full }} (SPIRE) federation across multiple {{ product_title }} clusters to enable cross-cluster mutual TLS (mTLS) authentication and zero trust workload identity in a multi-cluster service mesh deployment. {._abstract}
...
## Additional resources {id="_additional_resources" ._additional-resources-deploy-spire}
...
## Additional resources {id="additional-resources-spire-multicluster_{{ context }}" ._additional-resources}
