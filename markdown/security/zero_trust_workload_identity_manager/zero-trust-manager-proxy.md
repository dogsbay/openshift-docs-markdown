...
Operator Lifecycle Manager (OLM) automatically configures managed Operators with proxy settings when you use a cluster-wide egress proxy. To support proxying HTTPS connections, you can inject certificate authority (CA) certificates into the {{ zero_trust_full }}. {._abstract}

{% leveloffset +1 %}{% include "./modules/zero-trust-manager-proxy-support.md" %}{% endleveloffset %}

## Additional resources {id="zero-trust-manager-proxy_additional-resources" ._additional-resources}
