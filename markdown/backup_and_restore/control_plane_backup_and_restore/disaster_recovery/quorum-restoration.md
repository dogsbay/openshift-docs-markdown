...
You can restore etcd quorum on your {{ product_title }} cluster by running the `quorum-restore.sh` script on a recovery host when quorum loss leaves the API read-only. After quorum is restored, the API returns to read/write mode.

{% leveloffset +1 %}{% include "./modules/dr-restoring-etcd-quorum-ha.md" %}{% endleveloffset %}

## Additional resources {id="additional-resources_dr-quorum-restoration"}
