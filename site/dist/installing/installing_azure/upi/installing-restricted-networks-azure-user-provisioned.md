...
4. If you applied the `Contributor` role to your service principal, assign the `User Administrator Access` role by running the following command:
...
## Deploying the RHCOS cluster image for the Azure infrastructure {id="installation-azure-user-infra-deploying-rhcos_installing-restricted-networks-azure-user-provisioned"}

To provision cluster nodes on Microsoft Azure, you must use a valid Red&#160;Hat Enterprise Linux CoreOS (RHCOS) image for Microsoft Azure for your
...
### ARM template for image storage {id="installation-arm-image-storage_installing-restricted-networks-azure-user-provisioned"}
...
## Networking requirements for user-provisioned infrastructure {id="installation-network-user-infra_installing-restricted-networks-azure-user-provisioned"}
...
### Network connectivity requirements {id="installation-network-connectivity-user-infra_installing-restricted-networks-azure-user-provisioned"}
...
## Creating networking and load balancing components in Azure {id="installation-creating-azure-dns_installing-restricted-networks-azure-user-provisioned"}

To enable cluster communication on Microsoft Azure, you must deploy networking and load balancing components by using the Azure Resource Manager (ARM) template. {._abstract}
...
### ARM template for the network and load balancers {id="installation-arm-dns_installing-restricted-networks-azure-user-provisioned"}
...
## Creating the bootstrap machine in Azure {id="installation-creating-azure-bootstrap_installing-restricted-networks-azure-user-provisioned"}

To initialize your OpenShift Container Platform cluster on Microsoft Azure, you must deploy the bootstrap machine by using the `04_bootstrap.json` ARM template. {._abstract}
...
### ARM template for the bootstrap machine {id="installation-arm-bootstrap_installing-restricted-networks-azure-user-provisioned"}
...
## Creating the control plane machines in Azure {id="installation-creating-azure-control-plane_installing-restricted-networks-azure-user-provisioned"}

To form the control plane for your cluster on Microsoft Azure, you must deploy control plane machines by using the Azure
...
### ARM template for control plane machines {id="installation-arm-control-plane_installing-restricted-networks-azure-user-provisioned"}
...
## Wait for bootstrap completion and remove bootstrap resources in Azure {id="installation-azure-user-infra-wait-for-bootstrap_installing-restricted-networks-azure-user-provisioned"}

To complete cluster initialization on Microsoft Azure, you can wait for the bootstrap process to finish and then delete bootstrap resources. {._abstract}
...
## Creating additional worker machines in Azure {id="installation-creating-azure-worker_installing-restricted-networks-azure-user-provisioned"}

To add compute capacity on Microsoft Azure, you can create worker machines in Microsoft Azure for your cluster to use by launching individual instances discretely or by automated processes outside the cluster, such as auto scaling groups. {._abstract}
...
### ARM template for worker machines {id="installation-arm-worker_installing-restricted-networks-azure-user-provisioned"}
...
## Installing the OpenShift CLI on Linux {id="cli-installing-cli-linux_installing-restricted-networks-azure-user-provisioned"}
...
## Installing the OpenShift CLI on Windows {id="cli-installing-cli-windows_installing-restricted-networks-azure-user-provisioned"}
...
## Installing the OpenShift CLI on macOS {id="cli-installing-cli-macos_installing-restricted-networks-azure-user-provisioned"}
...
## Logging in to the cluster by using the CLI {id="cli-logging-in-kubeadmin_installing-restricted-networks-azure-user-provisioned"}
...
## Approving the certificate signing requests for your machines {id="installation-approve-csrs_installing-restricted-networks-azure-user-provisioned"}
...
## Adding the Ingress DNS records {id="installation-azure-create-ingress-dns-records_installing-restricted-networks-azure-user-provisioned"}
